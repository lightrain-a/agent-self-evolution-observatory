(() => {
  ...
  const frontierGroupsFor = row => Array.isArray(row.groups) && row.groups.length ? row.groups : (row.group ? [row.group] : []);
  const externalPapers = id => (landscape().papers || []).filter(row => frontierGroupsFor(row).includes(id));
  ...
  const coverageSummary = groups => {
    ...
    const external=groups.map(group=>{const formal=formalPapersForGroup(group.id).length,frontier=frontierPapersForGroup(group.id).length;return {group,formal,frontier,total:formal+frontier};});
    const externalDense=[...external].sort((a,b)=>b.total-a.total||b.formal-a.formal).slice(0,3);
    const externalThin=[...external].sort((a,b)=>a.total-b.total||a.formal-b.formal).slice(0,3);
    const externalRow=row=>`<span class="rpm-external-density" style="--group-color:${groupColor(row.group.id)}"><strong><i class="rpm-category-dot"></i>${esc(row.group.id)} · ${textOf(row.group.title)}</strong><em>${row.formal} ${mapPick("正式","formal")} + ${row.frontier} ${mapPick("前沿","frontier")}</em></span>`;
    return `<section class="panel rpm-gap-panel"><div class="rpm-section-head"><div><div class="eyebrow">${mapPick("内部积累 × 外部文献密度","INTERNAL ACCUMULATION × EXTERNAL LITERATURE DENSITY")}</div><h2 data-toc="false">${mapPick("把“我们做得多不多”和“外部已经拥不拥挤”分开看","Separate how much we have studied from how crowded the external literature is")}</h2><p>${mapPick("内部数量直接读取 canonical ResearchItem 总账；外部数量统计当前正式论文与前沿预印本在 A–G 中的映射。两者都只是覆盖信号，不表示真正生成过多少 Idea，也不自动等于研究优先级或新颖性判决。","Internal counts come from the canonical ResearchItem ledger; external counts map current formal papers and frontier preprints into A–G. Both are coverage signals only—not idea-generation counts, automatic priority, or novelty verdicts.")}</p></div></div><div class="rpm-gap-grid"><article><b>${mapPick("内部积累较多","More internal accumulation")}</b>${most.map(rowHtml).join("")}</article><article><b>${mapPick("内部目前覆盖较少","Thinner internal coverage")}</b>${least.map(rowHtml).join("")}</article></div><div class="rpm-density-divider"><b>${mapPick("外部文献密度","External literature density")}</b><span>${mapPick("多标签论文会同时计入相关 A–G，因此这里比较的是方向拥挤度，不是唯一论文总数。","Multi-label papers count in every relevant A–G category, so this compares directional density rather than unique-paper totals.")}</span></div><div class="rpm-gap-grid"><article><b>${mapPick("外部文献更密集","Denser external literature")}</b>${externalDense.map(externalRow).join("")}</article><article><b>${mapPick("当前外部文献相对较少","Relatively thinner external literature")}</b>${externalThin.map(externalRow).join("")}</article></div></section>`;
  };
  ...
  const currentControlBoard = () => {
    ...
    const handoffCount=Number(s.research_handoffs??rows.filter(row=>nextClass(row)==="PAPERSTATE_HANDOFF").length);
    ...
    return `<section class="panel rpm-control-board" id="current-control-board"><header class="rpm-control-head"><div><div class="eyebrow">${mapPick("当前控制面 · 跟踪不等于 active","CURRENT CONTROL PLANE · TRACKED ≠ ACTIVE")} · ${esc(d.as_of_date||"")}</div><h2 data-toc="false">${mapPick(`active ResearchItem=${activeCount}；另有 ${rows.length} 个可见跟踪对象：${handoffCount} 个 PaperState handoff，${waitingCount} 个等待重开条件，machine-actionable=${machineActionable}`,`Active ResearchItems=${activeCount}; ${rows.length} other objects remain visible: ${handoffCount} PaperState handoff, ${waitingCount} waiting on reopen conditions, machine-actionable=${machineActionable}`)}</h2><p>${mapPick("Research OS 允许 active=0。这里的 handoff 只保存研究→论文谱系，HOLD 只等待自己的重开条件；STOP/MERGED 只保留历史与证据，不会被回填成 active slot。","Research OS explicitly permits active=0. Handoff preserves research→paper lineage, HOLD waits on its reopen condition, and STOPPED/MERGED rows remain historical evidence rather than being backfilled into an active slot.")}</p></div><a class="link-btn" href="paper-ideas.html">${mapPick("打开完整研究组合 →","Open full portfolio →")}</a></header><div class="rpm-control-kpis"><span><b>${s.current_attention||rows.length}</b>${mapPick("控制面跟踪对象","tracked ResearchItems")}</span><span><b>${handoffCount}</b>${mapPick("PaperState handoff","PaperState handoff")}</span><span><b>${waitingCount}</b>${mapPick("等待重开条件","waiting reopen conditions")}</span><span><b>${machineActionable}</b>${mapPick("machine-actionable","machine-actionable")}</span></div><div class="rpm-control-grid"><section class="rpm-control-main"><article class="rpm-control-primary" data-dashboard-research="${esc(primary.code)}"><header><span>${esc(primary.code)} → ${esc(primary.paper_id||"PaperState")}</span><em>${esc(primaryPaper)}</em></header><h3 data-toc="false">${esc(valueText(primary.title)||primary.code)}</h3><p>${esc(brief(primary))}</p><div><b>${mapPick("唯一内部动作","Primary internal action")} · ${esc(nextClass(primary))}</b><span>${esc(next(primary))}</span></div><footer><a class="link-btn" href="${esc(primary.paper_href||"selected-paper.html")}">PaperState →</a><a class="link-btn" href="${esc(primary.timeline_href||"research-timeline.html")}">${mapPick("完整时间线 →","Full timeline →")}</a></footer></article><div class="rpm-control-list">${holds.map(rowCard).join("")}</div></section><aside class="rpm-control-week"><header><div><b>${mapPick("最近一周的关键变化","Key changes this week")}</b><span>${esc(week.start_date||"")} → ${esc(week.end_date||"")}</span></div><a href="${esc(week.timeline_href||"research-timeline.html")}">${mapPick("全部 →","All →")}</a></header><div class="rpm-control-week-kpis"><span><b>${week.research_days||0}</b>${mapPick("研究日","days")}</span><span><b>${week.substantive_events||0}</b>${mapPick("科研事件","research events")}</span><span><b>${week.key_changes||0}</b>${mapPick("关键变化","key changes")}</span></div><div class="rpm-control-highlights">${highlights}</div><p class="rpm-control-note">${mapPick("周摘要用于快速定位变化；具体科研结论与权限继续以对应 ResearchItem / PaperState 为准。","Weekly highlights locate change quickly; exact scientific decisions and authority remain in the linked ResearchItem / PaperState.")}</p></aside></div></section>`;
  };
  ...
  const itemActionClass = item => String((item.primary_next_action||{}).action_class||"--");
  ...
  const humanEvidence = item => {
    if(language==="zh"&&item.code==="E-7") {
      ...
      return paper.submission_ready
        ? "STRI 的 3/3 条核心主张已有对应证据，论文证据债为 0；最新 PaperRegistry 的内部门禁已经闭环。ResearchItem 不再新增科研或实验动作。"
        : "STRI 的 3/3 条核心主张已有对应证据，论文证据债为 0；科研实验线已闭环，后续内部动作只由 PaperState 控制面决定。";
    }
  ...
  const attentionCard = item => `<article class="rpm-attention-card state-${String(item.scientific_state||"").toLowerCase()}"><header><span>${esc(item.code||item.id||"--")}</span><div><b>${esc(valueText(item.title)||item.id||"--")}</b><em>${esc(stateLabel(item.scientific_state))}</em></div></header>${researchField(mapPick("研究问题","Research problem"),valueText(item.problem),300)}${researchField(mapPick("我们怎么做","Our approach"),valueText(item.mechanism)||valueText(item.method_logic),280)}${researchField(mapPick("最强对照","Strongest baseline"),valueText(item.strongest_baseline),260)}${researchField(mapPick("已经拿到的证据","Evidence so far"),humanEvidence(item),520)}<footer><b>${mapPick("唯一内部动作：","Primary internal action: ")}</b>${esc(itemActionClass(item))}${humanNext(item)?` · ${esc(humanNext(item))}`:""}</footer></article>`;
  const primaryLedger = (groupId,items) => `<div class="rpm-primary-ledger"><div class="rpm-subsection-title"><b>${mapPick("研究方向完整进展","Complete progress of research lines")}</b><span>${mapPick(`${items.length} 条可读研究方向全部列出；先看一句话结论，需要数字时再展开。`,`${items.length} reader-facing lines are all listed; scan the one-line decision first, then expand for methods and numbers.`)}</span></div>${items.map(item=>`<details class="rpm-primary-row"><summary><span>${esc(item.code||"--")}</span><b>${esc(valueText(item.title)||item.id||"--")}</b><em>${esc(stateLabel(item.scientific_state))}</em><small>${esc(itemActionClass(item))} · ${esc(humanDecision(item))}</small></summary><div class="rpm-primary-row-body">${researchField(mapPick("想解决什么","Problem"),valueText(item.problem),260)}${researchField(mapPick("核心做法","Approach"),valueText(item.mechanism)||valueText(item.method_logic),240)}${researchField(mapPick("最强简单/同信息对照","Strongest matched baseline"),valueText(item.strongest_baseline),240)}${researchField(mapPick("关键实验与当前结论","Evidence and current decision"),humanEvidence(item),520)}</div></details>`).join("")}</div>`;

  const frontierBoundary = groupId => {
    const note=landscape().frontier_boundaries?.[groupId];
    if(!note) return "";
    const verified=landscape().policy?.semantic_scholar_verified_at||landscape().verified_at||"";
    return `<aside class="rpm-frontier-boundary"><header><b>${mapPick("最新文献把边界推到哪里","What the latest literature changes")}</b><span>Semantic Scholar · ${esc(verified)}</span></header><p>${esc(mapPick(note.zh,note.en))}</p><small>${mapPick("这是公开文献 overlay 的边界解释，只影响 nearest-work 阅读，不自动改变 ResearchItem 的科学状态或实验权限。","This is a public-literature boundary overlay. It updates nearest-work interpretation only and never changes ResearchItem scientific state or experiment authority automatically.")}</small></aside>`;
  };
  const externalPaperCard = paper => { const groups=frontierGroupsFor(paper); return `<a class="rpm-external-paper" href="${esc(paper.url)}" target="_blank" rel="noopener"><header><b>${esc(paper.short||paper.title)}</b><span>${esc(mapPick(paper.status_zh,paper.status_en))}</span></header><p><strong>${mapPick("它做到：","What it establishes: ")}</strong>${esc(mapPick(paper.advance_zh,paper.advance_en))}</p>${groups.length>1?`<span class="rpm-formal-groups">${groups.map(id=>`<i style="--group-color:${groupColor(id)}">${esc(id)}</i>`).join("")}</span>`:""}</a>`; };
