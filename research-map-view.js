(() => {
  ...
  const knowledgeLane = group => {
    ...
    return `<section class="rpm-knowledge-lane" style="--group-color:${color}" id="research-knowledge-${esc(group.id.toLowerCase())}"><header class="rpm-knowledge-lane-head"><span>${esc(group.id)}</span><div><b>${textOf(group.title)}</b><small>${group.id==="G"?mapPick("当前安全 program overlay","current safety program overlay"):mapPick(`${projection.ideas.length} 个历史 Idea 节点 · ${projection.relationEdges.length} 条关联边`,`${projection.ideas.length} historical Idea nodes · ${projection.relationEdges.length} relation edges`)}</small></div><div class="rpm-public-paper-chips">${papers.map(p=>`<a href="${esc(p.url)}" target="_blank" rel="noopener"><strong>${esc(p.short)}</strong><small>${esc(mapPick(p.status_zh,p.status_en))}</small></a>`).join("")}</div></header><div class="rpm-knowledge-lane-body"><div class="rpm-canonical-projection"><div class="rpm-lane-label"><b>${mapPick("Canonical KG 投影","Canonical KG projection")}</b><span>${mapPick("真实节点/边类型；这里只展示高价值子图","real node/edge types; high-value subgraph only")}</span></div><div class="rpm-graph-chains">${graphBody}</div></div><div class="rpm-external-landscape"><div class="rpm-lane-label"><b>${mapPick("其他团队公开研究现状","Public research status from other teams")}</b><span>${mapPick(`公开状态核验：${landscape().verified_at||"2026-08-21"}`,`public status checked: ${landscape().verified_at||"2026-08-21"}`)}</span></div><div class="rpm-external-paper-list">${papers.map(p=>externalPaperCard(p,aliasTitles)).join("")}</div></div></div></section>`;
  };
  ...
  const knowledgeGraphPanel = groups => {
    const sg=scientificGraph().summary||{}, base=scientificGraph().base_graph||{}, eg=evidenceGraph().summary||{};
    return `<section class="panel rpm-knowledge-panel"><div class="rpm-section-head rpm-knowledge-head"><div><div class="eyebrow">${mapPick("Scientific Research Graph · 当前可读投影","SCIENTIFIC RESEARCH GRAPH · READABLE PROJECTION")}</div><h2 id="current-knowledge-graph">${mapPick("把我们的研究链和外部论文放在同一坐标系","Our research lineage and external papers in one coordinate system")}</h2><p>${mapPick(`底层已有 ${sg.nodes||0} 个节点 / ${sg.edges||0} 条边；其中 evidence graph=${base.nodes||eg.nodes||0}/${base.edges||eg.edges||0}，scientific overlay=${sg.overlay_nodes||0}/${sg.overlay_edges||0}。这里不把全部节点一次画成“毛线团”，而是按 A–G 投影 Track → Idea → Claim / nearest-work / Experiment 关系，并叠加已核验公开状态的外部论文。`,`The underlying graph has ${sg.nodes||0} nodes / ${sg.edges||0} edges; evidence graph=${base.nodes||eg.nodes||0}/${base.edges||eg.edges||0}, scientific overlay=${sg.overlay_nodes||0}/${sg.overlay_edges||0}. Instead of drawing every node into a hairball, this view projects Track → Idea → Claim / nearest-work / Experiment relations per A–G collection and overlays verified public papers.`)}</p></div><div class="rpm-graph-summary"><span><b>${sg.nodes||0}</b>${mapPick("节点","nodes")}</span><span><b>${sg.edges||0}</b>${mapPick("边","edges")}</span><span><b>${(landscape().papers||[]).length}</b>${mapPick("核验外部论文","verified external papers")}</span></div></div>${graphSchemaDetails()}<div class="rpm-graph-legend"><span>${graphNode("track","Track","")}${mapPick("研究轨道","research track")}</span><span>${graphNode("idea","Idea","")}${mapPick("研究问题","research problem")}</span><span>${graphNode("claim","Claim","")}${mapPick("问题/主张","problem / claim")}</span><span>${graphNode("paper","Paper","")}${mapPick("最近工作","nearest work")}</span><span>${graphNode("experiment","Experiment","")}${mapPick("实验节点","experiment node")}</span></div><div class="rpm-knowledge-lanes">${groups.map(knowledgeLane).join("")}</div><div class="rpm-knowledge-policy">${mapPick("外部论文的“正式发表 / 预印本”只是公开书目状态；canonical KG 和外部文献覆盖层都没有权限改变 ResearchItem 的 PASS / HOLD / STOP、实验授权或 PaperState。","Published/preprint is bibliographic status only. Neither the canonical KG nor the public-literature overlay can change ResearchItem decisions, experiment authority, or PaperState.")}</div></section>`;
  };
  ...
  window.renderCurrentResearchMap = function(config){
    ...
    return `${pageHeader(config)}${mapStats(inventory,terminal)}<section class="rpm-map-intro"><div><b>${mapPick("A–G 是当前正式坐标系；一个集合固定一种颜色","A–G is the current coordinate system; one collection uses one color")}</b><p>${mapPick("颜色只编码研究集合，不编码科学结论。PASS / HOLD / STOP、论文状态和实验权限继续用文字与独立标签表达，避免一张图里一套颜色承担两种语义。","Color encodes collection membership only, not scientific verdict. PASS / HOLD / STOP, paper status, and execution authority remain textual or independently labeled so one color never carries two meanings.")}</p></div><a class="link-btn" href="research-directions.html">${mapPick("查看历史领域方向 →","Open historical field directions →")}</a></section>${colorLegend(groups)}${knowledgeGraphPanel(groups)}<section class="rpm-section-head rpm-current-map-heading"><div><div class="eyebrow">${mapPick("当前内部研究组合","CURRENT INTERNAL PORTFOLIO")}</div><h2 id="current-a-g-portfolio">${mapPick("再看我们自己每个 A–G 集合当前做到哪里","Then inspect where our own A–G portfolio currently stands")}</h2><p>${mapPick("下面只汇总我们的 ResearchItem / Paper / 证据状态；上面的知识图谱负责解释它和外部研究之间是什么关系。","The cards below summarize only our ResearchItem / Paper / evidence state; the knowledge graph above explains how that state relates to the external field.")}</p></div></section><section class="rpm-map-grid">${groups.map(group=>groupCard(group,parents,inventory)).join("")}</section>${coverageSummary(groups,inventory)}${readingBridge()}`;
  };
})();
