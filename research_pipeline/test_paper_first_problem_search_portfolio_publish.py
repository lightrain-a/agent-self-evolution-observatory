...
class SearchPortfolioPublishTest(unittest.TestCase):
    ...
    def test_latest_terminal_run_is_appended_without_erasing_historical_shadow(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            ...
            (run/"shadow-run-qualification.json").write_text(json.dumps({"schema_version":"1.1","status":"READY_FOR_SHADOW_EXPANSION","stage_runner_required_schema":"1.4","control_snapshot_sha256":"c"*64,"main_commit":"d"*40,"scientific_authority":False}),encoding="utf-8")
            (run/"machine-audit.json").write_text(json.dumps({"summary":{"formulated":15,"reviewable":3,"reduction_pending":4,"blocked":8,"problem_falsifier_eligible":4},"blocked":[]}),encoding="utf-8")
