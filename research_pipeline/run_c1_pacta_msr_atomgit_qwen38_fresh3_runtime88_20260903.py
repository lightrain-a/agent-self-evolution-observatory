...
DEFAULT_ROOT = Path("/data/wyt/agent-self-evolution-observatory/runs/c1-pacta-msr-atomgit-qwen38-fresh3-runtime-20260903-v2")
...
def preflight(root: Path, blob_receipt_sha: str) -> dict[str, Any]:
    ...
    if root.exists():
        raise RuntimeError("runtime-v2 root exists; no overwrite")
    root.mkdir(parents=True)
    carrier = base.carrier_audit()
    if not carrier.get("pass"):
        raise RuntimeError("STOP_CARRIER_DRIFT")
    plan = load(IMAGE_ROOT / "blob-plan.json")
    rows = plan.get("rows") or []
    if len(rows) != 88 or plan.get("unique_blob_count") != 88:
        raise RuntimeError("STOP_FRESH3_RUNTIME88_PLAN_GEOMETRY")
    blobs = []
    for item in rows:
        path = base.CACHE / item["digest"][7:]
        passed = path.is_file() and path.stat().st_size == int(item["size"]) and sha256_file(path) == item["digest"][7:]
        blobs.append({"digest": item["digest"], "size": int(item["size"]), "path": str(path), "pass": passed})
    if not all(row["pass"] for row in blobs):
        raise RuntimeError("STOP_FRESH3_RUNTIME88_BLOB_CACHE_CORRUPTION")
    result = {
        "schema_version": 1,
        "created_at_utc": now(),
        "status": "FRESH3_RUNTIME88_PREFLIGHT_PASS",
        "docker": base.docker_metadata(),
        "carrier": carrier,
        "images": 20,
        "blobs": 88,
        "blob_bytes": sum(row["size"] for row in blobs),
        "fresh3_pool_sha256": POOL_SHA,
