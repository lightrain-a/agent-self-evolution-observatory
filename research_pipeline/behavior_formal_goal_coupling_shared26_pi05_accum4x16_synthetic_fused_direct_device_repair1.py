...
def main() -> int:
    ...
    if receipt_path.exists():
        raise RuntimeError(f"exactly-once synthetic 4x16 repair1 receipt exists: {receipt_path}")

    authority = json.loads(paths["authority"].read_text(encoding="utf-8"))
    if authority.get("status") != "AUTHORIZED_PI05_SYNTHETIC_FUSED_ACCUM4X16_DIRECT_DEVICE_REPAIR1_CLEAN_HOST":
    ...
    initial = {
        ...
        "status": "PI05_SYNTHETIC_FUSED_ACCUM4X16_DIRECT_DEVICE_REPAIR1_STARTED",
        "repair_host_role": "clean-host resource repair",
    ...
    lock_path = Path("/data/wyt/.formal-goal-pi05-synthetic-fused-accum4x16-direct-device-repair1.lock")
    with exclusive_lock(lock_path):
        write_receipt(receipt_path, initial)
        status = "PI05_SYNTHETIC_FUSED_ACCUM4X16_DIRECT_DEVICE_REPAIR1_HOLD"
        error = None
        state_ready = False
        done = 0
        accumulator = None
        ...
        try:
            ...
            for index in range(K):
                ...
                progress.update(
                    {
                        "status": "PI05_SYNTHETIC_FUSED_ACCUM4X16_DIRECT_DEVICE_REPAIR1_MICRO_GRADIENT_STARTED",
            ...
            status = "PI05_SYNTHETIC_FUSED_ACCUM4X16_DIRECT_DEVICE_REPAIR1_PASS"
            ...
            final.update(
                {
                    ...
                    "next_gate": (
                        ...
                        else "ACCUM4X16_REPAIR1_RESOURCE_FAILURE_ADJUDICATION_BEFORE_PRACTICAL_BATCH_CHILD"
                    ),
                }
            )
