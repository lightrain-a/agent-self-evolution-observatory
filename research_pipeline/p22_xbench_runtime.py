...
from .p22_search_backend import install_into_pinned_search_tools
...
def _install_third_party(memevolve_root: Path):
    ...
    install_into_pinned_search_tools()
    ...
    return OpenAIServerModel,MemoryItem,MemoryItemType,MemoryResponse,MemoryStatus,MemoryType,MMSearchAgent,grade_question
...
def execute_unit(*, phase: str, task_id: str, kval: int | str, memevolve_root: Path, xbench_root: Path, prediction_manifest: Path | None = None, agent_api_key: str | None = None, agent_api_base: str | None = None) -> dict[str, Any]:
    ...
    OpenAIServerModel,MemoryItem,MemoryItemType,MemoryResponse,MemoryStatus,MemoryType,MMSearchAgent,grade_question=third
    ...
    core={"schema_version":"1.0","status":"CAP_CENSORED" if cap_censored else "EXECUTION_ERROR" if error else "UNIT_COMPLETE","candidate_id":CANDIDATE_ID,"contract_sha256":CONTRACT_SHA256,"harness_plan_sha256":HARNESS_PLAN_SHA256,"phase":phase,"task_id":task_id,"k":kval,"k_int":_k_int(kval),"agent_model_requested":AGENT_MODEL,"judge_model_requested":JUDGE_MODEL,"prediction_manifest_sha256":prediction_sha,"selected_memory_ids":[x["memory_id"] for x in selected],"selected_memory_scores":[x["score"] for x in selected],"task_provider_call_attempts":task_model._p22_provider_counter["attempts"],"judge_provider_call_attempts":judge_model._p22_provider_counter["attempts"],"provider_call_attempts_total":task_model._p22_provider_counter["attempts"]+judge_model._p22_provider_counter["attempts"],"task_provider_receipts":task_model._p22_provider_receipts,"judge_provider_receipts":judge_model._p22_provider_receipts,"agent_result_sha256":result_sha,"agent_trajectory_sha256":trajectory_sha,"cap_censored":cap_censored,"downstream_success":int(score) if not error else None,"extracted_answer":extracted if not error else "","judge_explanation":explanation if not error else "","error":error,"memory_evolution_disabled":True,"scientific_authority":False,"belief_authority":False}
