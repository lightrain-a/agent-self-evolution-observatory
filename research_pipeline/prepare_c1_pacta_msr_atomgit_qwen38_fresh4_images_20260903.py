...
DEFAULT = Path("/data/wyt/agent-self-evolution-observatory/runs/c1-pacta-msr-atomgit-qwen38-fresh4-images-20260903-v1")


def bind() -> None:
    if not POOL.is_file() or sha256_file(POOL) != POOL_SHA:
        raise RuntimeError("STOP_FRESH4_IMAGE_POOL_HASH_DRIFT")
    base.POOL = POOL
...
def audit() -> dict[str, Any]:
    ...
    if len(rows) != 20 or len({row["instance_id"] for row in rows}) != 20:
        raise RuntimeError("STOP_FRESH4_IMAGE_GEOMETRY")
    return {
        "status": "FRESH4_IMAGE_RESOLVER_BINDING_PASS",
