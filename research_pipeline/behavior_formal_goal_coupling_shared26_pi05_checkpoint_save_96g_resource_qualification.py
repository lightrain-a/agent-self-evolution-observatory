...
def main() -> int:
    ...
    status_prefix = "PI05_CHECKPOINT_SAVE_96G_RESOURCE_CHILD"
    ...
    lock_path = Path("/data/wyt/.formal-goal-pi05-checkpoint-save-96g-resource-qualification.lock")
    ...
    with exclusive_lock(lock_path):
        ...
        if authority.get("status") != "AUTHORIZED_PI05_CHECKPOINT_SAVE_96G_RESOURCE_CHILD_QUALIFICATION":
        ...
        if authority.get("adjudication_sha256") != sha256_file(adjudication_path):
            raise RuntimeError("checkpoint failure adjudication SHA binding drift")
        if authority.get("independent_review_sha256") != sha256_file(review_path):
            raise RuntimeError("independent review SHA binding drift")
        adjudication = json.loads(adjudication_path.read_text())
        if adjudication.get("status") != "PI05_THROTTLED_SYNC_CHECKPOINT_SAVE_4GB_40G_REPRODUCED_CGROUP_OOM":
            raise RuntimeError("checkpoint repair-stop adjudication status drift")
        if adjudication.get("scientific_disposition", {}).get("formal_run3_authorized") is not False:
            raise RuntimeError("run3 authority unexpectedly open")
        review = json.loads(review_path.read_text())
        if review.get("verdict") != "APPROVE_ORDERED_A_THEN_B" or review.get("run3_authorized_now") is not False:
            raise RuntimeError("independent review verdict drift")
        if review.get("candidate_a", {}).get("memory_max_gib") != expected_memory_max_gib:
            raise RuntimeError("independent review MemoryMax drift")
        if int(authority.get("resource_scope", {}).get("memory_max_gib", -1)) != expected_memory_max_gib:
            raise RuntimeError("authority MemoryMax drift")
        ...
        initial = {
            "schema_version": "behavior-formal-goal-coupling-shared26-pi05-checkpoint-save-96g-resource-qualification-v1",
            "object_id": OBJECT_ID,
            "child_id": CHILD_ID,
            ...
            "pytree_save_concurrent_gb": 4,
            "pytree_restore_concurrent_gb": 4,
        ...
        try:
            ...
            progress.update({"generated_at": datetime.now(timezone.utc).isoformat(), "status": status_prefix + "_STATE_READY", "train_state_ready": True, "snapshots": snapshots})
            atomic_json(receipt_path, progress)

            import orbax.checkpoint as ocp
            output_root.mkdir(parents=True, exist_ok=False)
            train_state_handler = ocp.PyTreeCheckpointHandler(save_concurrent_gb=4, restore_concurrent_gb=4)
            params_handler = ocp.PyTreeCheckpointHandler(save_concurrent_gb=4, restore_concurrent_gb=4)
            if train_state_handler._save_concurrent_bytes != 4_000_000_000 or params_handler._save_concurrent_bytes != 4_000_000_000:
                raise RuntimeError("4GB save limiter resolution drift")
            if train_state_handler._restore_concurrent_bytes != 4_000_000_000 or params_handler._restore_concurrent_bytes != 4_000_000_000:
                raise RuntimeError("4GB restore limiter resolution drift")
            ...
            if isinstance(manager._checkpointer, ocp.AsyncCheckpointer):
                raise RuntimeError("sync checkpoint qualification resolved to AsyncCheckpointer")
            ...
            progress.update({
                "generated_at": datetime.now(timezone.utc).isoformat(),
                "status": status_prefix + "_SAVE_COMPLETE",
            ...
            state_signature_after = state_signature(restored)
            if state_signature_after != state_signature_before:
                raise RuntimeError("restored PyTree structure/shape/dtype signature drift")
            restore_completed = True
            snapshots["after_restore"] = {"cgroup": cgroup_snapshot(expected_memory_max_bytes), "host": host_snapshot(), "gpu": gpu_snapshot()}
            ...
            final.update({
                ...
                "state_signature_match": bool(state_signature_before and state_signature_before == state_signature_after),
                "snapshots": snapshots,
                "error": error,
                "dataset_accessed": False,
                "forward_pass_executed": False,
                ...
                "loss_values_read_or_reported": False,
                "formal_training_authorized": False,
                "next_gate": "FORMAL_RUN3_96G_RESOURCE_CHILD_AUTHORITY_DESIGN" if status.endswith("PASS") else "CANDIDATE_B_CHUNKED_D2H_SEPARATE_REVIEW_REQUIRED",
            })
