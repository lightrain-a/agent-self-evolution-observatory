...
from research_pipeline.agent_constraint_externality_codingplan_mimo25_live import (
    CATALOG_B1,
    CONTEXT_WINDOW,
    CONTRACT_OUTPUT,
    MODEL_ID,
...
class Mimo25LiveCapabilityB2Tests(unittest.TestCase):
    ...
    def test_b2_ladder_selects_mimo25(self):
        state = self._verified(SEARCH_STATE)
        self.assertEqual(state["status"], "CAPABILITY_BACKBONE_SEARCH_CONTINUE_MIMO25_NEXT")
        self.assertEqual(state["remaining_frozen_order"], ["mimo-v2.5", "mimo-v2.5-pro"])
    ...
    def test_q1_zero_request_real_mcp(self):
        ...
        self.assertEqual(q1["status"], "CODINGPLAN_MIMO25_LIVE_MCP_PREDISPATCH_PASS")
    ...
    def test_contract_freezes_same_gate_and_no_f0(self):
        ...
        state = self._verified(SEARCH_STATE)
        q1 = self._verified(Q1_OUTPUT)
        self.assertEqual(contract["status"], "CODINGPLAN_MIMO25_CAPABILITY_B2_AUTHORIZED")
    ...
    def test_eight_units_and_q1_binds_runner(self):
        ...
        q1 = self._verified(Q1_OUTPUT)
        from research_pipeline import agent_constraint_externality_codingplan_mimo25_live as runner
