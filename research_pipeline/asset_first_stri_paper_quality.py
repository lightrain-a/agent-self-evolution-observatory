...
def build_stri_quality_contract() -> dict[str, Any]:
    return {
        ...
        "claims": [
            {
                ...
                "why_better_or_why_matters": "Skill-SP shows the released controller-input distribution shift and exact quotient restoration; AutoSkill P19 independently shows a frozen identity split changing the semantic retrieval set and a mechanical downstream behavior across 18 fresh runs, with ID-placebo and quotient controls restoring the original pattern.",
        ...
        "analyses": [
            ...
            {"id": "AN-DYNAMIC-CONSEQUENCE", "analysis_type": "mechanism", "target_claim_ids": ["N1"], "purpose": "Test whether a semantics-preserving identity split propagates through a released retrieval budget into fresh executed behavior under a frozen task and executor.", "decision_rule": "Support only if original>=5/6, split<=1/6, ID-placebo=3/3, quotient-control=3/3, all runs are valid, and one-sided Fisher original>split is <=0.05; otherwise keep the result substrate-local or inconclusive."},
        ],
        "planned_outputs": [
            ...
            {"id": "O-DYNAMIC", "output_type": "mechanism_result", "purpose": "Compact four-arm AutoSkill P19 result establishing the bounded representation-to-retrieval-to-behavior consequence and its ID/quotient controls."},
        ],
        "visualizations": [
...
def build_stri_completion(project_root: Path = PROJECT_ROOT) -> dict[str, Any]:
    ...
    dynamic_refs = [DYNAMIC_QUALIFICATION, DYNAMIC_CONTRACT, DYNAMIC_PLAN, DYNAMIC_RESULT, DYNAMIC_RUN_MANIFEST]
    ...
    manuscript_dynamic = "AutoSkill: dynamic behavioral propagation" in body and "6/6 original" in body and "0/6 split" in body and "p=0.00108" in body
