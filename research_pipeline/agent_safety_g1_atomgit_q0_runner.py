...
# Pinned AWM retry() uses `while tries < n_retry` and performs the initial
# chat.invoke inside that loop. Therefore n_retry=1 means exactly one initial
# model attempt and zero parser retries; n_retry=0 means zero model calls.
...
def run_cascade(*,authority:dict[str,Any],output_root:Path,awm:Path,browserart:Path,auth_path:Path)->dict[str,Any]:
    ...
    result={"schema_version":"g1-atomgit-q0-cascade-receipt-v1","artifact_id":"AGENT-SAFETY-G1-ATOMGIT-Q0-CASCADE-RECEIPT-20260906","status":terminal,"selected_primary":selected,"candidate_order":authority["candidate_order"],"candidates":candidates,"total_model_requests":sum(int(x.get("model_requests") or 0) for x in candidates),"safety_executed":False,"harmful_calls":0,"p0_execution_authorized":False,"p1_execution_authorized":False,"paper_claim_upgrade":False}
    atomic_json(output_root/"q0-cascade-receipt.json",result); return result


def main()->None:
    p=argparse.ArgumentParser(); p.add_argument("--authority",type=Path,required=True); p.add_argument("--output-root",type=Path,required=True); p.add_argument("--awm-root",type=Path,required=True); p.add_argument("--browserart-root",type=Path,required=True); p.add_argument("--auth-path",type=Path,required=True); a=p.parse_args()
