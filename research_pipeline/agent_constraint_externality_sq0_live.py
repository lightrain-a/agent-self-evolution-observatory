...
from research_pipeline.agent_constraint_externality_sq0_build import (
    ...
    TOOL_CALL_CAP, evaluate_case_from_state, load_cases,
)
...
UPTAKE_CLOSEOUT = GENERATED / "agent-constraint-externality-f0-source-closeout-mimo25pro-20260903.json"
...
MODEL_ROUND_CAP = 32
CASE_COUNT = 12
EXECUTION_ID = "CODINGPLAN-MIMO25PRO-SQ0-TARGET-FAILURE-V1"
AUTH_OUTPUT = GENERATED / "agent-constraint-externality-sq0-human-authorization-20260903.json"
Q1_OUTPUT = GENERATED / "agent-constraint-externality-sq0-mimo25pro-mcp-q1-predispatch-20260903.json"
EXEC_CONTRACT = GENERATED / "agent-constraint-externality-sq0-mimo25pro-execution-contract-v1-20260903.json"
RESULT_OUTPUT = GENERATED / "agent-constraint-externality-sq0-mimo25pro-result-v1-20260903.json"
...
def _mcp(case_id: str, root: Path, progress: Path, task_id: str) -> dict[str, Any]:
    args=["-m","research_pipeline.agent_constraint_externality_sq0_mcp_bridge","--bundle",str(OUTPUT_BUNDLE),"--case-id",case_id,"--runtime-root",str(root/"appworld"),"--task-id",task_id,"--experiment-name","ace-sq0-mimo25pro-v1","--progress",str(progress),"--tool-call-cap",str(TOOL_CALL_CAP)]
...
def _unit_id(case_id: str) -> str: return f"sq0:{MODEL_ID}|{case_id}|1"

def _freeze_inputs() -> tuple[dict[str,Any],dict[str,Any],dict[str,Any]]:
    static=_verified(STATIC_CONTRACT,"SQ0_TARGET_CHALLENGE_V1_STATIC_DESIGN_READY")
    qual=_verified(STATIC_QUAL,"SQ0_TARGET_CHALLENGE_V1_PUBLIC_REACHABILITY_PASS")
    selected=_verified(SELECTION,"CAPABILITY_BACKBONE_SELECTED_MIMO25PRO_PASS")
    stopped=_verified(UPTAKE_CLOSEOUT,"F0_UPDATE_UPTAKE_FAIL_SOURCE_CLOSEOUT")
    if selected.get("selected_backbone",{}).get("model_id") != MODEL_ID: raise SQ0Stop("Selected backbone drifted.")
    if stopped.get("verdict") != "F0_UPDATE_UPTAKE_FAIL": raise SQ0Stop("Current F0 is not at frozen uptake stop.")
...
def qualify_q1() -> dict[str,Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="ace-sq0-q1-") as d:
        ...
        try:
            ...
            state=_read(progress); payload={"schema_version":"ace-sq0-mimo25pro-q1-v1","object_id":OBJECT_ID,"status":"SQ0_MIMO25PRO_MCP_PREDISPATCH_PASS","case_id":case["case_id"],"model_profile":MODEL_PROFILE,"model_id":MODEL_ID,"session_mcp_progress_status":state.get("status"),"session_mcp_tool_count":int(state.get("tool_count",0)),"codingplan_window_used_before":used_before,"codingplan_window_used_after":used_after,"codingplan_model_requests":used_after-used_before,"scientific_dispatch_sent":False,"scientific_outcomes_observed":0,"runner_source_sha256":sha256_file(Path(__file__)),"bridge_source_sha256":sha256_file(ROOT/"research_pipeline/agent_constraint_externality_sq0_mcp_bridge.py"),"bundle_sha256":sha256_file(OUTPUT_BUNDLE)}
...
def freeze() -> dict[str,Any]:
    ...
    auth={"schema_version":"ace-sq0-human-authorization-v1","object_id":OBJECT_ID,"status":"USER_AUTHORIZED_SQ0_TARGET_FAILURE_QUALIFICATION_AFTER_F0_UPTAKE_FAIL","authorization_basis":"USER_CONTINUE_AFTER_PROSPECTIVE_SQ0_PLAN_AND_STATIC_REACHABILITY_AUDIT","selected_backbone_content_sha256":selected["content_sha256"],"static_contract_content_sha256":static["content_sha256"],"static_qualification_content_sha256":qual["content_sha256"],"authority":{"sq0_execution":True,"f0_r1":False,"probe":False,"p1":False,"toolsandbox":False,"appworld_ul":False,"paper_claim":False},"scientific_outcomes_observed":0}
    auth["content_sha256"]=sha256_value(auth); _write(AUTH_OUTPUT,auth)
    contract={"schema_version":"ace-sq0-mimo25pro-execution-contract-v1","object_id":OBJECT_ID,"execution_id":EXECUTION_ID,"status":"SQ0_MIMO25PRO_V1_EXECUTION_AUTHORIZED","human_authorization_content_sha256":auth["content_sha256"],"q1_content_sha256":q1["content_sha256"],"static_contract_content_sha256":static["content_sha256"],"static_qualification_content_sha256":qual["content_sha256"],"selected_backbone_content_sha256":selected["content_sha256"],"model":{"profile":MODEL_PROFILE,"id":MODEL_ID,"context_window":CONTEXT_WINDOW,"retry_max_attempts":RETRY_MAX_ATTEMPTS},"harness":{"id":"ATOMCODE_CODINGPLAN_MCP_V1","sq0_bridge":True,"tool_call_cap":TOOL_CALL_CAP,"model_round_cap_per_case":MODEL_ROUND_CAP,"retry_allowed":False,"replacement_allowed":False,"ai_session_naming":False,"subagents":False},"panel":{"case_count":CASE_COUNT,"case_ids":[c["case_id"] for c in load_cases()],"one_episode_per_case":True,"confirmatory_reuse":False},"gate":{"usable_failure_rate_min":0.75,"usable_failure_rate_max":0.90,"non_semantic_failure_invalidates_qualification":True},"execution_policy":{"durable_dispatch_before_model_request":True,"unknown_after_dispatch_replay":False,"partial_outcome_redesign":False,"aggregate_read_only_after_all_terminal":True},"authority":{"sq0_execution":True,"f0_r1":False,"probe":False,"p1":False,"toolsandbox":False,"appworld_ul":False,"paper_claim":False},"scientific_outcomes_observed":0}
...
def adjudicate(ledger_path:Path)->dict[str,Any]:
    ...
    if invalid: status="SQ0_QUALIFICATION_INVALID_NON_SEMANTIC_FAILURE_STOP"
    elif 0.75<=rate<=0.90: status="SQ0_TARGET_FAILURE_QUALIFICATION_PASS"
    elif rate<0.75: status="SQ0_TARGET_CHALLENGE_TOO_EASY_STOP"
    else: status="SQ0_TARGET_CHALLENGE_TOO_HARD_STOP"
    result={"schema_version":"ace-sq0-mimo25pro-result-v1","object_id":OBJECT_ID,"execution_id":EXECUTION_ID,"status":status,"provider":PROVIDER,"model_profile":MODEL_PROFILE,"model_id":MODEL_ID,"harness":"ATOMCODE_CODINGPLAN_MCP_V1","case_count":CASE_COUNT,"usable_target_failure_count":failures,"usable_target_failure_rate":rate,"target_success_count":sum(bool(r["target_success"]) for r in completions),"non_semantic_failure_units":invalid,"scientific_model_round_count":sum(int(r["model_round_count"]) for r in completions),"appworld_tool_call_total":sum(int(r["appworld_tool_call_count"]) for r in completions),"prompt_tokens_total":sum(int(r["prompt_tokens_total"]) for r in completions),"completion_tokens_total":sum(int(r["completion_tokens_total"]) for r in completions),"ledger_sha256":sha256_file(ledger_path),"development_only":True,"confirmatory_reuse":False,"scientific_effects_observed":0,"authority":{"sq0_complete":True,"f0_r1":False,"probe":False,"p1":False,"toolsandbox":False,"appworld_ul":False,"paper_claim":False}}
