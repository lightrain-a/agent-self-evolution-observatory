...
def main() -> int:
    ...
    if authority.get("status") != "AUTHORIZED_PI05_B1_SERIAL_ITEM_CHECKPOINT_QUALIFICATION":
        raise RuntimeError("B1 authority inactive")
    ...
    if saved.get("status") != "PI05_B1_SERIAL_ITEM_CHECKPOINT_SAVE_STAGE_PASS":
        raise RuntimeError(f"B1 save stage not PASS: {saved.get('status')}")
    if saved.get("manager_steps") != [SAVE_LABEL] or saved.get("checkpoint_save_completed") is not True:
        raise RuntimeError("B1 save-stage completion drift")
    expected = saved.get("fingerprints") or {}
    initial = {
        "schema_version": "behavior-formal-goal-coupling-shared26-pi05-b1-serial-item-checkpoint-qualification-v1",
        "generated_at": datetime.now(timezone.utc).isoformat(),
        "status": "PI05_B1_SERIAL_ITEM_CHECKPOINT_RESTORE_VERIFY_STARTED",
    ...
    status = "PI05_B1_SERIAL_ITEM_CHECKPOINT_QUALIFICATION_HOLD"
    error = None
    actual = {}
    comparisons = {}
    try:
        ...
        status = "PI05_B1_SERIAL_ITEM_CHECKPOINT_QUALIFICATION_PASS"
    ...
    final.update({
        ...
        "error": error,
        "next_gate": "SEPARATE_FORMAL_RUN3_B1_AUTHORITY_DESIGN" if status.endswith("PASS") else "B1_FAILURE_ADJUDICATION_B2_REVIEW_ONLY",
    })
