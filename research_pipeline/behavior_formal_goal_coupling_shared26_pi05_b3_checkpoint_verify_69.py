...
def main() -> int:
    ...
    if Path("/etc/machine-id").read_text().strip() != EXPECTED_MACHINE_ID:
        raise RuntimeError("B3 verification may run only on host69")
    ...
    if outputs.get("save_result") != str(p["save_result"]) or outputs.get("checkpoint_root") != str(p["output_root"]):
        raise RuntimeError("B3 verification path drift")
    expected_result = outputs.get("disk_result") if p["mode"] == "disk" else outputs.get("restore_result")
    if expected_result != str(p["result"]):
        raise RuntimeError("B3 verification result path drift")
    if p["mode"] == "restore" and outputs.get("disk_result") != str(p["disk_result"]):
        raise RuntimeError("B3 disk-result path drift")

    save_result = load_json(p["save_result"])
    if save_result.get("status") != "PI05_B3_CHECKPOINT_SAVE_QUALIFICATION_PASS" or save_result.get("checkpoint_save_completed") is not True:
        raise RuntimeError("B3 save result not PASS")
    if save_result.get("manager_steps") != [SAVE_LABEL]:
        raise RuntimeError("B3 manager step drift")
    if save_result.get("policy_outcomes_read") is not False or save_result.get("optimizer_update_executed") is not False:
        raise RuntimeError("B3 save scientific boundary drift")

    if p["result"].exists():
        raise RuntimeError(f"B3 verification result already exists: {p['result']}")
    step_dir = p["output_root"] / str(SAVE_LABEL)
    if not step_dir.is_dir():
        raise RuntimeError("B3 completed checkpoint step missing")

    scope_before = cgroup_snapshot(EXPECTED_MEMORY_MAX_BYTES)
    initial = {
        "schema_version": "behavior-formal-goal-coupling-shared26-pi05-b3-checkpoint-verify-v1",
        "object_id": OBJECT_ID,
        "child_id": CHILD_ID,
        "generated_at": datetime.now(timezone.utc).isoformat(),
        "status": f"PI05_B3_{p['mode'].upper()}_VERIFY_STARTED",
        "mode": p["mode"],
    ...
    status = f"PI05_B3_{p['mode'].upper()}_VERIFY_HOLD"
    error = None
    ...
    try:
        if p["mode"] == "disk":
            ...
            status = "PI05_B3_DISK_VERIFY_PASS"
        else:
            disk = load_json(p["disk_result"])
            if disk.get("status") != "PI05_B3_DISK_VERIFY_PASS" or disk.get("evidence", {}).get("exact_per_leaf_sha256_match") is not True:
                raise RuntimeError("B3 disk verification not PASS")
            gpu = gpu_snapshot()
            if len(gpu["gpus"]) != 1:
            ...
            status = "PI05_B3_STANDARD_RESTORE_PASS"
    ...
    final.update({
        ...
        "policy_outcomes_read": False,
        "formal_run3_authorized": False,
        "next_gate": "PI05_B3_STANDARD_RESTORE" if status == "PI05_B3_DISK_VERIFY_PASS" else ("SEPARATE_FORMAL_RUN3_AUTHORITY_DESIGN_ONLY" if status == "PI05_B3_STANDARD_RESTORE_PASS" else "STOP_B3_NO_AUTOMATIC_RETRY"),
    })
