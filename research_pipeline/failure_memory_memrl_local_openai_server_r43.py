"""Loopback-only OpenAI-compatible server for the frozen B1 MemRL runtime.

The pinned MemRL and MemoryOS code both expect OpenAI-compatible chat and
embedding endpoints. This server keeps those interfaces while serving only the
content-addressed local Qwen2.5-7B-Instruct and MPNet-isometric-3072 models.
It binds to 127.0.0.1 by default and contains no benchmark logic.
"""
...
class Runtime:
    def __init__(self) -> None:
        self.llm = LocalQwenProvider(device="cuda:0", max_new_tokens=512)
        self.embedder = LocalMPNetEmbedder(device="cuda:1", output_dimension=3072)
...
class Handler(BaseHTTPRequestHandler):
    runtime: Runtime
    server_version = "B1R43LocalOpenAI/1.0"
...
def main() -> None:
    ...
    if args.host not in {"127.0.0.1", "localhost"}:
        raise SystemExit("B1 R43 server is loopback-only")
