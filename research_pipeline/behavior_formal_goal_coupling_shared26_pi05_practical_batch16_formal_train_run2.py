...
EXP_NAME = "shared26-seed42-practical-b16-run2-fdlimit"
...
RUN1_FAILURE_ADJ_SHA = "7f100fa01223b3db365d48a79fb8b94af1443b5a9e6044475851d5d64da54e4c"
FD_QUAL_CANONICAL_SHA = "d24b7076a90a4d3c5e6dbc34403accb332f46a00ff7bf508d615bd4996a66dfd"
FD_QUAL_SOURCE_SHA = "fa2cd4d9c94c3c82d93b40905aa7a5efe8eb4ad762f12c03951177efb7d174a5"
RUN2_NOFILE_SOFT = 65536
BATCH = 16
SEED = 42
ACTION_HORIZON = 32
EPISODES = 5200
...
def main() -> int:
    ...
    for name in [
        ...
        "ffmpeg_runtime_qualification", "run1_failure_adjudication", "fd_qualification", "openpi_child_root", "params_root", "projection_root", "norm_stats", "tokenizer", "progress", "result",
    ]:
    ...
    if auth.get("status") != "AUTHORIZED_PI05_PRACTICAL_BATCH16_FORMAL_RUN2_FDLIMIT":
        raise RuntimeError("formal run2 authority not active")
    ...
    require(p["run1_failure_adjudication"], RUN1_FAILURE_ADJ_SHA, "run1 FD failure adjudication")
    run1_adj = json.loads(p["run1_failure_adjudication"].read_text())
    if run1_adj.get("status") != "PI05_PRACTICAL_BATCH16_FORMAL_RUN1_PRETERMINAL_INFRASTRUCTURE_FAILURE_EMFILE" or not run1_adj.get("adjudication", {}).get("new_prospective_run2_permitted"):
        raise RuntimeError("run1 failure adjudication does not permit run2")
    require(p["fd_qualification"], FD_QUAL_CANONICAL_SHA, "FD cache qualification canonical copy")
    fdq = json.loads(p["fd_qualification"].read_text())
    if fdq.get("status") != "VIDEO_DECODER_FD_CACHE_QUALIFICATION_PASS" or fdq.get("max_open_fds") != 1150 or not fdq.get("crossed_1024_fds"):
        raise RuntimeError("FD cache qualification drift")
    if run1_adj.get("mechanism_evidence", {}).get("fd_growth_qualification", {}).get("source_receipt_sha256") != FD_QUAL_SOURCE_SHA:
        raise RuntimeError("FD qualification source SHA lineage drift")
    ...
    if nofile_soft != RUN2_NOFILE_SOFT or nofile_hard < RUN2_NOFILE_SOFT:
        raise RuntimeError(f"run2 RLIMIT_NOFILE drift: soft={nofile_soft} hard={nofile_hard}")
    ...
    with lock(Path("/data/wyt/.formal-goal-pi05-practical-batch16-formal-run2-fdlimit.lock")):
        started = {
            "schema_version": "behavior-formal-goal-coupling-shared26-pi05-practical-batch16-formal-run2-progress-v1",
            "object_id": OBJECT_ID, "child_id": CHILD_ID, "generated_at": datetime.now(timezone.utc).isoformat(),
            "status": "PI05_PRACTICAL_BATCH16_FORMAL_RUN2_FDLIMIT_STARTED", "exp_name": EXP_NAME,
            ...
            "run1_failure_adjudication_sha256": RUN1_FAILURE_ADJ_SHA,
            "fd_qualification_canonical_sha256": FD_QUAL_CANONICAL_SHA,
            "fd_qualification_source_sha256": FD_QUAL_SOURCE_SHA,
            "rlimit_nofile_soft": nofile_soft, "rlimit_nofile_hard": nofile_hard,
        ...
        status = "PI05_PRACTICAL_BATCH16_FORMAL_RUN2_FDLIMIT_HOLD"
        ...
        try:
            ...
            status = "PI05_PRACTICAL_BATCH16_FORMAL_RUN2_FDLIMIT_COMPLETE"
        except Exception as exc:
            error = f"{type(exc).__name__}: {exc}"
        finally:
            result = {
                "schema_version": "behavior-formal-goal-coupling-shared26-pi05-practical-batch16-formal-run2-result-v1",
                "object_id": OBJECT_ID, "child_id": CHILD_ID, "generated_at": datetime.now(timezone.utc).isoformat(),
