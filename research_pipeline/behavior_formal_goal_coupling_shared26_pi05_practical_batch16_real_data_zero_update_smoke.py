...
def main() -> int:
    ...
    if p["receipt"].exists():
        raise RuntimeError(f"exactly-once real-data batch16 smoke receipt exists: {p['receipt']}")
    auth = json.loads(p["authority"].read_text())
    if auth.get("status") != "AUTHORIZED_PI05_PRACTICAL_BATCH16_REAL_DATA_ZERO_UPDATE_SMOKE":
        raise RuntimeError("real-data smoke authority not active")
    ...
    initial = {
        "schema_version": "behavior-formal-goal-coupling-shared26-pi05-practical-batch16-real-data-zero-update-smoke-v1",
        "object_id": OBJECT_ID, "child_id": CHILD_ID, "generated_at": datetime.now(timezone.utc).isoformat(),
        "status": "PI05_PRACTICAL_BATCH16_REAL_DATA_ZERO_UPDATE_STARTED", "batch_size": BATCH,
    ...
    with lock(Path("/data/wyt/.formal-goal-pi05-practical-batch16-real-data-zero-update-smoke.lock")):
        write_receipt(p["receipt"], initial)
        status = "PI05_PRACTICAL_BATCH16_REAL_DATA_ZERO_UPDATE_HOLD"; error = None
        ...
        try:
            ...
            status = "PI05_PRACTICAL_BATCH16_REAL_DATA_ZERO_UPDATE_PASS"
            ...
            final = dict(initial); final.update({
                ...
                "next_gate": "FREEZE_PRACTICAL_BATCH16_FORMAL_TRAINING_AUTHORITY" if status.endswith("PASS") else "REAL_DATA_BATCH16_PRETRAINING_REVIEW",
