...
from .paper_first_evidence_acquisition import build_provisional_evidence_plan,build_substrate_preflight_request,compile_evidence_designs,compile_evidence_reviews,compile_operationalization_recompiles,compile_substrate_preflight,compile_harness_implementation_receipts,evidence_design_prompt,evidence_review_prompt,operationalization_recompile_prompt
...
def compile_legacy_reduction_migration(*,source_run:Path,project_root:Path=PROJECT_ROOT)->dict[str,Any]:
 ...
 machine={"schema_version":"1.0-evidence-migration","scientific_authority":False,"authority":{"paper_design":False,"method":False,"experiment":False,"p0":False,"gpu":False},"problem_falsifier_queue":[{"candidate_id":r["candidate_id"],"title":r["candidate"].get("title"),"discovery_lane":r["candidate"].get("discovery_lane"),"source_branch_id":r["candidate"].get("source_branch_id"),"blockers":list((r["audit"] or {}).get("blockers") or []),"irreducible_object":r["candidate"].get("irreducible_object"),"endpoint_headroom_requirement":r["candidate"].get("endpoint_headroom_requirement"),"exact_prediction":r["candidate"].get("exact_prediction"),"strongest_same_information_baseline":r["candidate"].get("strongest_same_information_baseline"),"cheapest_problem_falsifier":r["candidate"].get("cheapest_problem_falsifier"),"scientific_authority":False} for r in pending]}
...
def recompile_legacy_operationalization(*,private_state_path:Path,part:int=1,model:str="ark-code-latest",public_json:Path=DEFAULT_JSON,public_js:Path=DEFAULT_JS)->dict[str,Any]:
 ...
 if str(state.get("current_control_snapshot_sha256") or "")!=current_control:raise ValueError("evidence migration control snapshot drift; re-prepare continuation before operationalization recompile")
 plan=state.get("evidence_plan") or {};root=private_state_path.parent;stem=f"evidence-recompile-p{part}";prompt,candidate_ids=operationalization_recompile_prompt(plan,part=part,batch_size=2);res=_ark_with_provider_receipt(run_root=root,stem=stem,requested_model=model,context={"part":part,"candidate_ids":candidate_ids,"control_snapshot_sha256":current_control},prompt=prompt,max_output_tokens=5600,temperature=0.0)
 raw=str(res.get("text") or "");resolved=str(res.get("resolved_model") or model);payload,raw_sha=_parse_archived_evidence_design_json(root,stem,raw,resolved);compiled=compile_operationalization_recompiles(plan,payload,part=part,recompiler_model=resolved);state["evidence_plan"]=compiled;state=_sync_evidence_summary(state);private_state_path.write_text(json.dumps(state,ensure_ascii=False,indent=2)+"\n",encoding="utf-8")
...
def replay_legacy_operationalization(*,private_state_path:Path,raw_path:Path,part:int=1,resolved_model:str="archived-provider-response",public_json:Path=DEFAULT_JSON,public_js:Path=DEFAULT_JS)->dict[str,Any]:
 state=_load(private_state_path);current_control=compute_control_snapshot(project_root=PROJECT_ROOT)["control_snapshot_sha256"]
 if str(state.get("current_control_snapshot_sha256") or "")!=current_control:raise ValueError("evidence migration control snapshot drift; re-prepare continuation before recompile replay")
 plan=state.get("evidence_plan") or {};root=private_state_path.parent;stem=f"evidence-recompile-p{part}";raw=raw_path.read_text(encoding="utf-8");payload,raw_sha=_parse_archived_evidence_design_json(root,stem,raw,resolved_model);compiled=compile_operationalization_recompiles(plan,payload,part=part,recompiler_model=resolved_model);state["evidence_plan"]=compiled;state=_sync_evidence_summary(state);private_state_path.write_text(json.dumps(state,ensure_ascii=False,indent=2)+"\n",encoding="utf-8")
 candidate_ids=[str(row.get("candidate_id") or "") for row in payload.get("recompiles") or [] if isinstance(row,dict)];artifact={"schema_version":"1.0","generated_at":_now(),"migration_id":state.get("migration_id"),"source_run_id":state.get("source_run_id"),"part":part,"candidate_ids":candidate_ids,"response_origin":"ARCHIVED_PROVIDER_RESPONSE_REPLAY","resolved_model":resolved_model,"raw_sha256":raw_sha,"plan_summary":compiled.get("summary") or {},"provider_called":False,"scientific_authority":False,"authority":dict(AUTHORITY)};(root/f"{stem}.json").write_text(json.dumps(artifact,ensure_ascii=False,indent=2)+"\n",encoding="utf-8")
...
def validate_public_migration(state:dict[str,Any])->list[str]:
 ...
 if state.get("scientific_authority") is not False or p.get("legacy_terminal_run_is_immutable") is not True or p.get("migration_cannot_rewrite_parent_terminal_artifacts") is not True or p.get("migrated_candidates_enter_bounded_evidence_design_only") is not True or p.get("independent_evidence_contract_review_required_before_execution") is not True or p.get("evidence_designer_cannot_self_review") is not True or p.get("bounded_substrate_preflight_required_after_contract_review") is not True or p.get("contract_review_clear_does_not_authorize_execution") is not True or p.get("source_asset_dependency_may_receive_one_operationalization_recompile") is not True or p.get("operationalization_recompile_cannot_change_frozen_prediction_or_baseline") is not True or p.get("operationalization_recompile_requires_independent_equivalence_review") is not True or p.get("frozen_scientific_fields_are_compiler_owned") is not True or p.get("outcome_labels_are_compiler_owned") is not True or p.get("execution_adapter_is_compiler_owned") is not True:errors.append("legacy evidence migration must remain immutable-parent zero-authority reviewed control")
...
def main()->None:
 ...
 recompile=sub.add_parser("recompile");recompile.add_argument("--private-state",type=Path,required=True);recompile.add_argument("--part",type=int,default=1);recompile.add_argument("--model",default="ark-code-latest")
 recompile_replay=sub.add_parser("recompile-replay");recompile_replay.add_argument("--private-state",type=Path,required=True);recompile_replay.add_argument("--raw",type=Path,required=True);recompile_replay.add_argument("--part",type=int,default=1);recompile_replay.add_argument("--resolved-model",default="archived-provider-response")
 ...
 elif a.command=="recompile":result=recompile_legacy_operationalization(private_state_path=a.private_state,part=a.part,model=a.model)
 elif a.command=="recompile-replay":result=replay_legacy_operationalization(private_state_path=a.private_state,raw_path=a.raw,part=a.part,resolved_model=a.resolved_model)
