...
Q5_QUERY = (
    "Create a directory named /tmp/b1_migration_q5, write the literal SUPPORT_OK "
    "to /tmp/b1_migration_q5/result.txt, and verify that the file contains it."
)
Q5B_FORMAT_INSTRUCTION = """Your response MUST have exactly this structure:
...
The first characters of the response must be exactly: Act: bash
Do not output text before Act: bash or explanations after the code block.
"""


def _processed_memories(retrieved: list[dict[str, Any]]) -> dict[str, list[dict[str, Any]]]:
    result: dict[str, list[dict[str, Any]]] = {}
    for row in retrieved:
        metadata = row.get("metadata")
        key = "successed" if getattr(metadata, "success", False) else "failed"
        result.setdefault(key, []).append(row)
    return result


def q5_gate(client: Any, service: Any, parse: Any, image: str,
            *, explicit_diagnostic: bool) -> dict[str, Any]:
    ...
    from memrl.lifelongbench_eval.prompts import (
        DEFAULT_SYSTEM_PROMPT, build_llb_prompt_with_memory,
    )
    from src.tasks.instance.os_interaction.container import OSInteractionContainer

    rows: list[dict[str, Any]] = []
    for _ in range(REPEATS):
        retrieved = service.retrieve(Q5_QUERY, k=3, threshold=0.0)
        memory_context = format_llb_memory_context(
            _processed_memories(retrieved), task="os"
        )
        native_system = build_llb_prompt_with_memory(
            task="os", base_prompt=DEFAULT_SYSTEM_PROMPT,
            memory_context=memory_context,
        )
        user_prompt = (
            Q5B_FORMAT_INSTRUCTION + "\nInstruction: " + Q5_QUERY
            if explicit_diagnostic else Q5_QUERY
        )
        ...
        parsed = parse(text)
        row: dict[str, Any] = {
            "retrieval_sha256": digest(ids(retrieved)),
            "native_system_prompt_sha256": hashlib.sha256(native_system.encode()).hexdigest(),
            "user_prompt_sha256": hashlib.sha256(user_prompt.encode()).hexdigest(),
            "response_sha256": hashlib.sha256(text.encode()).hexdigest(),
            "literal_act_bash": "Act: bash" in text,
            "begins_act_bash": text.startswith("Act: bash"),
            "parsed_action": str(parsed.action),
            "parsed_command_nonempty": bool(parsed.content),
            "command_executed": False,
            "evaluator_called": False,
        }
        if str(parsed.action) == "execute" and parsed.content:
            box = OSInteractionContainer(10, image=image)
            try:
                action = box.execute_independent(cmd(
                    {"command_name": "bash", "script": parsed.content}
                ))
                verdict = box.execute_independent(cmd({
                    "command_name": "bash",
                    "script": "test -d /tmp/b1_migration_q5 && "
                              "test \"$(cat /tmp/b1_migration_q5/result.txt)\" = SUPPORT_OK",
                }))
            finally:
                box.terminate()
            row.update({
                ...
                "action_exit": action.exit_code,
                "action_timeout": action.timeout_flag,
                "evaluator_called": True,
                "verdict_exit": verdict.exit_code,
                "verdict_timeout": verdict.timeout_flag,
            })
        rows.append(row)

    required = all(
        row["literal_act_bash"]
        and row["parsed_action"] == "execute"
        and row["parsed_command_nonempty"]
        and row["command_executed"]
        and row.get("action_exit") == 0
        and row.get("action_timeout") is False
        and row["evaluator_called"]
        and row.get("verdict_exit") == 0
        and row.get("verdict_timeout") is False
        for row in rows
    )
    if explicit_diagnostic:
        required = required and all(row["begins_act_bash"] for row in rows)
    stable_fields = (
        "retrieval_sha256", "native_system_prompt_sha256", "user_prompt_sha256",
        "response_sha256", "parsed_action", "action_sha256", "action_exit",
        "action_timeout", "verdict_exit", "verdict_timeout",
    )
    stable = all(
        all(row.get(key) == rows[0].get(key) for key in stable_fields)
        for row in rows[1:]
    )
    return {
        "pass": bool(required and stable),
        "diagnostic_only": explicit_diagnostic,
        "repetitions": REPEATS,
        "native_frozen_prompt_builder": "build_llb_prompt_with_memory(task=os)",
        "native_parser_used": True,
        "parser_or_postprocessing_repair": False,
        "temperature": 0,
        "max_tokens": 128,
        "stable": stable,
        "validation_unit_used": False,
        "rows": rows,
    }


def receipt(script: pathlib.Path, source: pathlib.Path, image: str, base_url: str,
            gates: dict[str, Any], contract: pathlib.Path, prompt_diff: pathlib.Path,
            wall: float) -> dict[str, Any]:
    import docker
    release_keys = ("Q1", "Q2", "Q3", "Q4", "Q5a")
    release = all(gates[key].get("pass") is True for key in release_keys)
    payload = {
        "schema_version": "1.0", "paper_id": PAPER_ID,
        "receipt_id": "D2-FAILURE-MEMORY-PROVENANCE-R45M1-Q5V2-HOST-MIGRATION-EQUIVALENCE",
        "recorded_date": "2026-09-01",
        "status": "HOST_MIGRATION_EQUIVALENCE_PASS" if release
                  else "HOLD_NATIVE_FROZEN_INTERFACE_NOT_QUALIFIED",
        "role": "ZERO_CONFIRMATORY_OUTCOME_SUPPORT_INTERFACE_QUALIFICATION",
        ...
        "contract": {"path": str(contract), "sha256": file_sha(contract)},
        "prompt_contract_diff": {"path": str(prompt_diff), "sha256": file_sha(prompt_diff)},
        ...
        "qualification": gates,
        "release_gate_keys": list(release_keys),
        "all_q1_q5a_pass": release,
        "q5b_diagnostic_only": True,
        "access_accounting": {"training_support_units_initialized": REPEATS,
            ...
            "synthetic_memory_builds": 3, "synthetic_retrieval_calls": REPEATS * 3,
            ...
            "scientific_source_units_executed": 0, "external_provider_calls": 0},
        "scientific_contract": {"parser_changed": False,
            "scientific_source_prompt_changed": False,
            "source_build_runner_changed": False,
            "model_changed": False, "temperature_changed": False,
            "validation_exposure": 0, "scientific_source_exposure": 0},
        "scientific_authority": False, "experiment_authority": False,
        "support_only": True,
        "failure_route": "HOLD_NATIVE_FROZEN_INTERFACE_NOT_QUALIFIED",
        "wall_seconds": wall}
...
def main() -> None:
    ...
    parser.add_argument("--prompt-contract-diff", type=pathlib.Path, required=True)
    ...
    gates["Q5a"] = q5_gate(
        client, service, OSInteraction._parse_agent_response, args.image,
    ...
    result = receipt(
        script, source, args.image, args.base_url, gates,
        args.contract.resolve(), args.prompt_contract_diff.resolve(),
        time.time() - start,
    )
    (outdir / "host-migration-equivalence-q5v2.json").write_text(
