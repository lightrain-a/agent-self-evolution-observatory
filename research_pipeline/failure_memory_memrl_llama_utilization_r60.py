#!/usr/bin/env python3
"""R60: Llama executor utilization replication on the frozen R54 memory surfaces."""
...
AUTH_STATUS = "R60_LLAMA_FRESH_UTILIZATION_AUTHORITY_FROZEN_PRE_PROBE"
...
def preflight(manifest_path: pathlib.Path, authority_path: pathlib.Path, r54_path: pathlib.Path,
              selection_path: pathlib.Path, frozen_path: pathlib.Path, source_receipt_path: pathlib.Path, parser_receipt_path: pathlib.Path):
    m, a, q, s, f, src, parser = map(load, [manifest_path, authority_path, r54_path, selection_path, frozen_path, source_receipt_path, parser_receipt_path])
    if any(x.get("paper_id") != PAPER_ID for x in [m, a, q, s, f, src, parser]): raise RuntimeError("R60-paper-id-drift")
    if m.get("status") != MANIFEST_STATUS or a.get("status") != AUTH_STATUS or q.get("status") != R54_PASS: raise RuntimeError("R60-status-drift")
    if not all(valid(x) for x in [m, a, q, s, f, src, parser]): raise RuntimeError("R60-receipt-hash-drift")
    b = a.get("bindings") or {}
    checks = {
    ...
    for k, observed in checks.items():
        if b.get(k) != observed: raise RuntimeError(f"R60-binding-drift:{k}")
    if int(q.get("validation_treatment_outcomes_observed") or 0) != 0: raise RuntimeError("R60-validation-already-opened")
    if parser.get("status") != "LLAMA_NATIVE_PARSER_QUALIFICATION_PASS_PRIMARY_STILL_SEALED" or parser.get("pass") is not True or int(parser.get("native_execute_nonempty_count") or 0) != 3:
        raise RuntimeError("R60-parser-qualification-not-pass")
    if int(parser.get("validation_treatment_outcomes_observed") or 0) != 0 or int(parser.get("terminal_evaluator_calls") or 0) != 0:
        raise RuntimeError("R60-parser-opened-forbidden-outcome")
    if src.get("status") != "SOURCE_BUILD_COMPLETE" or int(src.get("completed_count") or 0) != 350: raise RuntimeError("R60-source-not-full350")
    ...
    if len(ids) != 8 or ids_hash(ids) != u.get("representative_ids_sha256") or list(u.get("arms") or []) != ARMS: raise RuntimeError("R60-utilization-id-or-arm-drift")
    if len(primary_ids) != 32 or ids_hash(primary_ids) != c.get("representative_ids_sha256") or set(ids) & set(primary_ids): raise RuntimeError("R60-primary-id-drift")
    if ids != [str(x) for x in s.get("utilization_representative_ids") or []] or primary_ids != [str(x) for x in s.get("primary_representative_ids") or []]: raise RuntimeError("R60-selection-id-drift")
    records = list(s.get("utilization_records") or []); by = {str(x.get("validation_task_id")): x for x in records}
    if set(by) != set(ids) or len(records) != 8: raise RuntimeError("R60-utilization-record-drift")
    for tid in ids:
        selected = list(by[tid].get("selected") or [])
        if not selected or any(x.get("eligible") is not True for x in selected): raise RuntimeError(f"R60-selected-retrieval-ineligible:{tid}")
        if any(type(x.get("source_outcome_success")) is not bool or not x.get("content") for x in selected): raise RuntimeError(f"R60-selected-retrieval-field-invalid:{tid}")
    ...
    if socket.gethostname()!=h["logical_name"] or pathlib.Path(sys.executable).resolve()!=pathlib.Path(h["python"]).resolve() or os.environ.get("PYTHONDONTWRITEBYTECODE")!="1": raise RuntimeError("R60-host-python-drift")
    ...
    if head!=source["revision"] or dirty: raise RuntimeError("R60-source-checkout-drift")
    split=root/c["split"]
    if r47.sha(split)!=c["split_sha256"]: raise RuntimeError("R60-validation-split-drift")
    image=subprocess.check_output(["docker","image","inspect",e["runtime_image"]["execution_tag"],"--format","{{.Id}}"],text=True).strip()
    if image!=e["runtime_image"]["id"]: raise RuntimeError("R60-image-drift")
    ...
    if {e["external_runtime_adapter"]["llm_model_id"],e["external_runtime_adapter"]["embedding_model_id"]}-models: raise RuntimeError("R60-loopback-route-drift")
...
def build_plan(m: dict[str, Any], by: dict[str, dict[str, Any]]) -> dict[str, Any]:
    ...
    p={"schema_version":"1.0","paper_id":PAPER_ID,"role":"R60_FRESH_UTILIZATION_SCHEDULE_FROZEN_PRE_TREATMENT","randomization_seed":seed,
...
def resume_guard(out: pathlib.Path, schedule: list[tuple[int,str,str,str]], resume: bool) -> tuple[list[dict[str,Any]],list[dict[str,Any]]]:
    ...
    if sk!=schedule[:len(sk)] or ck!=schedule[:len(ck)]: raise RuntimeError("R60-ledger-not-schedule-prefix")
    if len(started)!=len(complete): raise RuntimeError("R60_EXPOSED_INCOMPLETE_ARM_NO_RETRY")
    if complete and len(complete)<len(schedule) and not resume: raise RuntimeError("R60-partial-requires-explicit-resume")
    return started,complete


def main() -> None:
    p=argparse.ArgumentParser()
    for x in ["manifest","authorization","r54v2-qualification","selection","frozen-retrieval","source-receipt","parser-receipt","output-dir"]: p.add_argument("--"+x,type=pathlib.Path,required=True)
    p.add_argument("--resume",action="store_true"); a=p.parse_args(); out=a.output_dir.resolve(); out.mkdir(parents=True,exist_ok=True)
    m,auth,q,sel,frozen,src,by=preflight(a.manifest.resolve(),a.authorization.resolve(),a.r54v2_qualification.resolve(),a.selection.resolve(),a.frozen_retrieval.resolve(),a.source_receipt.resolve(),a.parser_receipt.resolve())
    ...
    if plan.get("plan_sha256")!=r47.digest({k:v for k,v in plan.items() if k!="plan_sha256"}): raise RuntimeError("R60-plan-hash-drift")
    ...
    result=r47.analyze(plan,complete); result.update({"role":"R60_LLAMA_FRESH_UTILIZATION_QUALIFICATION_ADJUDICATION","manifest_receipt_sha256":m["receipt_sha256"],
