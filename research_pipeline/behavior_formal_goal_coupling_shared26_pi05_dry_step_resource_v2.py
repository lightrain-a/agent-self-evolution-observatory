...
def validate_resource_scope() -> dict:
    ...
    if len(unified) != 1:
        raise RuntimeError(f"resource-v2 requires one cgroup-v2 path, got {unified}")
    ...
    if memory_max_text == "max":
        raise RuntimeError("resource-v2 must run inside a finite MemoryMax scope")
    memory_max = int(memory_max_text)
    if memory_max > MAX_SCOPE_MEMORY_BYTES:
        raise RuntimeError(f"resource-v2 MemoryMax too large: {memory_max} > {MAX_SCOPE_MEMORY_BYTES}")
    if swap_max_text != "0":
        raise RuntimeError(f"resource-v2 requires MemorySwapMax=0, observed {swap_max_text}")
...
def main() -> int:
    ...
    lock_path = Path("/data/wyt/.formal-goal-pi05-dry-step-resource-v2.lock")

    if receipt_path.exists():
        raise RuntimeError(f"resource-v2 dry-step receipt already exists; exactly-once child attempt cannot be relaunched: {receipt_path}")

    authority = json.loads(authority_path.read_text(encoding="utf-8"))
    if authority.get("status") != "AUTHORIZED_SINGLE_A100_NO_UPDATE_DRY_TRAIN_STEP_RESOURCE_V2":
        raise RuntimeError("resource-v2 dry-step authority is not active")
    if authority.get("object_id") != OBJECT_ID:
        raise RuntimeError("resource-v2 authority object drift")
    failure_sha = sha256_file(failure_adjudication_path)
    if authority.get("parent_failure_adjudication_sha256") != failure_sha:
        raise RuntimeError("resource-v2 authority does not bind the supplied failure adjudication")
    ...
    if authority.get("resource_adapter_qualification_sha256") != adapter_qualification_sha:
        raise RuntimeError("resource-v2 authority does not bind the supplied adapter qualification")
    ...
    if Path(runner_binding.get("path", "")).name != Path(__file__).name or runner_binding.get("sha256") != sha256_file(Path(__file__).resolve()):
        raise RuntimeError("resource-v2 runner content-address binding drift")
    ...
    if adapter.get("source_num_workers") != EXPECTED_SOURCE_NUM_WORKERS or adapter.get("num_workers") != RESOURCE_REPAIR_NUM_WORKERS:
        raise RuntimeError("resource-v2 authority DataLoader adapter drift")
    ...
    for key, expected in REQUIRED_THREAD_ENV.items():
        if os.environ.get(key) != expected:
            raise RuntimeError(f"resource-v2 requires {key}={expected}, observed {os.environ.get(key)!r}")
    ...
    initial = {
        "schema_version": "behavior-formal-goal-coupling-shared26-pi05-dry-step-resource-v2-result-v1",
        "object_id": OBJECT_ID,
        "generated_at": datetime.now(timezone.utc).isoformat(),
        "status": "PI05_NO_UPDATE_DRY_STEP_RESOURCE_V2_STARTED",
        "attempt_count_under_resource_v2_authority": 1,
        "attempt_is_exactly_once": True,
        "parent_v1_attempt_consumed": True,
    ...
    with exclusive_lock(lock_path):
        ...
        status = "PI05_NO_UPDATE_DRY_STEP_RESOURCE_V2_HOLD"
        error = None
        ...
        try:
            ...
            started_payload.update(
                {
                    "status": "PI05_NO_UPDATE_DRY_STEP_RESOURCE_V2_GRADIENT_STARTED",
                    "batch_ready": True,
                    "train_state_ready": True,
            ...
            status = "PI05_NO_UPDATE_DRY_STEP_RESOURCE_V2_PASS"
            ...
            final.update(
                {
                    ...
                    "next_gate": (
                        ...
                        if status == "PI05_NO_UPDATE_DRY_STEP_RESOURCE_V2_PASS"
                        else "RESOURCE_V2_DRY_STEP_FAILURE_ADJUDICATION_NO_AUTOMATIC_RETRY"
                    ),
                }
            )
        ...
        return 0 if status == "PI05_NO_UPDATE_DRY_STEP_RESOURCE_V2_PASS" else 2
