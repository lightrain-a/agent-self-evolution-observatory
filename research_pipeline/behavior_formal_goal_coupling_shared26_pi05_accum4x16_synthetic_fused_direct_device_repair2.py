...
PREV_RESOURCE_ADJ_SHA = "d9306c6f8731bbd875e513a7584a1c8d463a508a731ba572ff64ae086bef55cd"
DATA_ORDER_SHA = "34da62085824d5725fbd2044f3f12fccc42971a137eb498999b02391a769f101"
CONSUMED_ATTEMPT_SHA = "9dd2f352c84dadbbea8725dc09ee5da28960f44169251dd95550265823c20f43"
ATTEMPT1_ADJ_SHA = "b05ae019ca0ab2ce8b697277f398a2720e6c36c922a7521a2c44fed9f5c1fa09"
CONSUMED_REPAIR1_SHA = "2d128a77c340d9e822e0381f3f8bbc0c0cb7d4d682faf8153bfb0a1011730ab9"
REPAIR1_ADJ_SHA = "9aa65c21014333fb277d8b7d063986bb58282e85ddcc0f4070c4f0e0a9f68281"
HOST_MEMORY_LADDER_SHA = "6fb67f1e1131b8a08778f53ead8f59c635755432bf487e3b1c5ec2ebe7a752c4"
SOURCE_BATCH = 64
MICRO = 4
K = 16
EFFECTIVE = 64
...
def resource_scope() -> dict:
    ...
    if memory_max_text == "max" or int(memory_max_text) != MAX_HOST_MEM:
        raise RuntimeError(f"synthetic repair2 requires MemoryMax exactly 32G, got {memory_max_text}")
    if swap_max_text != "0":
...
def main() -> int:
    ...
    if receipt_path.exists():
        raise RuntimeError(f"exactly-once synthetic 4x16 repair2 receipt exists: {receipt_path}")

    authority = json.loads(paths["authority"].read_text(encoding="utf-8"))
    if authority.get("status") != "AUTHORIZED_PI05_SYNTHETIC_FUSED_ACCUM4X16_DIRECT_DEVICE_REPAIR2_MEMORYMAX32G":
        raise RuntimeError("authority is not active")
    ...
    require(paths["consumed_attempt"], CONSUMED_ATTEMPT_SHA, "consumed 4x16 attempt1 receipt")
    require(paths["attempt1_adjudication"], ATTEMPT1_ADJ_SHA, "4x16 attempt1 host-OOM adjudication")
    consumed = json.loads(paths["consumed_attempt"].read_text(encoding="utf-8"))
    if consumed.get("status") != "PI05_SYNTHETIC_FUSED_ACCUM4X16_DIRECT_DEVICE_STARTED" or consumed.get("micro_gradients_completed") != 0 or consumed.get("optimizer_update"):
        raise RuntimeError("consumed attempt1 lineage drift")
    adjud = json.loads(paths["attempt1_adjudication"].read_text(encoding="utf-8"))
    if adjud.get("status") != "PI05_SYNTHETIC_FUSED_ACCUM4X16_ATTEMPT1_PRE_GRADIENT_HOST_CGROUP_OOM":
        raise RuntimeError("attempt1 adjudication status drift")

    require(paths["consumed_repair1"], CONSUMED_REPAIR1_SHA, "consumed 4x16 repair1 receipt")
    require(paths["repair1_adjudication"], REPAIR1_ADJ_SHA, "4x16 repair1 host-OOM adjudication")
    require(paths["host_memory_ladder"], HOST_MEMORY_LADDER_SHA, "4x16 host-memory ladder")
    repair1 = json.loads(paths["consumed_repair1"].read_text(encoding="utf-8"))
    if repair1.get("status") != "PI05_SYNTHETIC_FUSED_ACCUM4X16_DIRECT_DEVICE_REPAIR1_STARTED" or repair1.get("micro_gradients_completed") != 0 or repair1.get("optimizer_update"):
        raise RuntimeError("consumed repair1 lineage drift")
    repair1_adj = json.loads(paths["repair1_adjudication"].read_text(encoding="utf-8"))
    if repair1_adj.get("status") != "PI05_SYNTHETIC_FUSED_ACCUM4X16_REPAIR1_PRE_GRADIENT_HOST_CGROUP_OOM":
        raise RuntimeError("repair1 adjudication status drift")
    ladder = json.loads(paths["host_memory_ladder"].read_text(encoding="utf-8"))
    if ladder.get("status") != "PREREGISTERED_PRE_GRADIENT_HOST_MEMORY_REPAIR_LADDER" or ladder.get("memory_max_ladder_gib") != [32, 40]:
        raise RuntimeError("host-memory ladder drift")

    require(paths["preregistration"], PREREG_SHA, "accumulation preregistration")
    require(paths["prev_resource_adjudication"], PREV_RESOURCE_ADJ_SHA, "8x8 GPU-resource adjudication")
    require(paths["data_order_qualification"], DATA_ORDER_SHA, "4x16 data-order qualification")
    ...
    candidate = prereg["microbatch_resource_ladder"][2]
    if candidate != {"priority": 3, "physical_micro_batch": 4, "accumulation_steps": 16, "effective_batch": 64}:
        raise RuntimeError(f"4x16 ladder drift: {candidate}")
    order = json.loads(paths["data_order_qualification"].read_text(encoding="utf-8"))
    if order.get("status") != "PI05_ACCUM_4X16_DATA_ORDER_QUALIFICATION_PASS" or not order.get("sampler_groups_exactly_equal"):
        raise RuntimeError("4x16 data-order gate not PASS")
    ...
    initial = {
        "schema_version": "behavior-formal-goal-coupling-shared26-pi05-synthetic-fused-accum4x16-direct-device-result-v1",
        "object_id": OBJECT_ID,
        "child_id": CHILD_ID,
        "generated_at": datetime.now(timezone.utc).isoformat(),
        "status": "PI05_SYNTHETIC_FUSED_ACCUM4X16_DIRECT_DEVICE_REPAIR2_STARTED",
        "repair_host_role": "host-memory ladder repair2 at 32GiB",
        "consumed_attempt_sha256": CONSUMED_ATTEMPT_SHA,
        "attempt1_adjudication_sha256": ATTEMPT1_ADJ_SHA,
        "consumed_repair1_sha256": CONSUMED_REPAIR1_SHA,
        "repair1_adjudication_sha256": REPAIR1_ADJ_SHA,
        "host_memory_ladder_sha256": HOST_MEMORY_LADDER_SHA,
    ...
    lock_path = Path("/data/wyt/.formal-goal-pi05-synthetic-fused-accum4x16-direct-device-repair2.lock")
    with exclusive_lock(lock_path):
        write_receipt(receipt_path, initial)
        status = "PI05_SYNTHETIC_FUSED_ACCUM4X16_DIRECT_DEVICE_REPAIR2_HOLD"
        error = None
        state_ready = False
        done = 0
        accumulator = None
        ...
        try:
            ...
            for index in range(K):
                ...
                progress.update(
                    {
                        "status": "PI05_SYNTHETIC_FUSED_ACCUM4X16_DIRECT_DEVICE_REPAIR2_MICRO_GRADIENT_STARTED",
            ...
            status = "PI05_SYNTHETIC_FUSED_ACCUM4X16_DIRECT_DEVICE_REPAIR2_PASS"
            ...
            final.update(
                {
                    ...
                    "formal_training_authorized": False,
                    "next_gate": (
                        "REAL_DATA_STREAMING_ACCUM4X16_IMPLEMENTATION_AUTHORITY"
                        if status.endswith("PASS")
                        else "ACCUM4X16_REPAIR2_FAILURE_ADJUDICATION_PER_HOST_MEMORY_LADDER"
                    ),
                }
            )
