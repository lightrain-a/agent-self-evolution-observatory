...
SEARCH_STATE = GENERATED / "agent-constraint-externality-capability-backbone-search-state-b3-20260903.json"
CATALOG_B1 = GENERATED / "agent-constraint-externality-codingplan-catalog-b1-20260903.json"
MIMO25_CLOSEOUT = GENERATED / "agent-constraint-externality-codingplan-mimo25-capability-b2-closeout-20260903.json"
...
REPEATS = (1, 2)
EXECUTION_ID = "CODINGPLAN-MIMO25PRO-CAPABILITY-B3"
STAGE = "CODINGPLAN_MIMO25PRO_CAPABILITY_B3"
LEDGER_SCHEMA = "ace-codingplan-mimo25pro-live-capability-b3-ledger-v1"
SELECTION_OUTPUT = SEARCH_STATE
Q1_OUTPUT = GENERATED / "agent-constraint-externality-codingplan-mimo25pro-live-mcp-q1-predispatch-20260903.json"
CONTRACT_OUTPUT = GENERATED / "agent-constraint-externality-codingplan-mimo25pro-capability-b3-contract-20260903.json"
RESULT_OUTPUT = GENERATED / "agent-constraint-externality-codingplan-mimo25pro-capability-b3-result-20260903.json"

class MIMO25PROLiveStop(RunnerError):
    pass
...
def _verified(path: Path, status: str | None = None) -> dict[str, Any]:
    ...
    if payload.get("object_id") != OBJECT_ID:
        raise MIMO25PROLiveStop(f"Object mismatch: {path}")
    if status is not None and payload.get("status") != status:
        raise MIMO25PROLiveStop(f"Unexpected status in {path}: {payload.get('status')}")
    claimed = payload.get("content_sha256")
    if claimed is not None:
        unsigned = dict(payload); unsigned.pop("content_sha256", None)
        if claimed != sha256_value(unsigned):
            raise MIMO25PROLiveStop(f"Content hash mismatch: {path}")
    return payload
...
def units() -> list[EpisodeUnit]:
    ...
    if len(rows) != 8 or len({row.unit_id for row in rows}) != 8:
        raise MIMO25PROLiveStop("Expected exactly eight unique mimo-v2.5-pro units.")
    return rows
...
def mcp_config(*, unit: EpisodeUnit, unit_root: Path, progress: Path, task_id: str) -> dict[str, Any]:
    args = ["-m", "research_pipeline.agent_constraint_externality_codingplan_mcp_bridge", "--appworld-root", str(APPWORLD_ROOT), "--bundle", str(V4_BUNDLE), "--family-id", unit.family_id, "--repeat", str(unit.repeat), "--runtime-root", str(unit_root / "appworld"), "--task-id", task_id, "--experiment-name", "ace-codingplan-mimo25pro-live-capability-b3", "--progress", str(progress), "--tool-call-cap", str(TOOL_CALL_CAP)]
...
def prepare_unit_runtime(*, unit: EpisodeUnit, unit_root: Path) -> tuple[Path, Path, Path, dict[str, Any]]:
    ...
    if not auth_source.is_file(): raise MIMO25PROLiveStop("AtomCode auth.toml missing.")
    ...
    task_id = "acemimoprob3" + unit.family_id.lower().replace("-", "") + f"r{unit.repeat}_1"
...
def build_selection() -> dict[str, Any]:
    state = _verified(SEARCH_STATE, "CAPABILITY_BACKBONE_SEARCH_CONTINUE_MIMO25PRO_NEXT")
    catalog = _verified(CATALOG_B1, "CODINGPLAN_ACCOUNT_CATALOG_REFRESH_PASS_ZERO_MODEL_REQUESTS")
    closeout = _verified(MIMO25_CLOSEOUT, "CODINGPLAN_MIMO25_B2_CEILING_CLOSEOUT")
    if state.get("next_candidate") != {"model_id": MODEL_ID, "profile": MODEL_PROFILE}:
        raise MIMO25PROLiveStop("Frozen backbone ladder does not select mimo-v2.5-pro next.")
    if state.get("remaining_frozen_order") != ["mimo-v2.5-pro"]:
        raise MIMO25PROLiveStop("Frozen final backbone order drifted.")
    row = next((r for r in catalog.get("models", []) if r.get("model_id") == MODEL_ID), None)
    if row is None or row.get("profile") != MODEL_PROFILE:
        raise MIMO25PROLiveStop("mimo-v2.5-pro is absent from the frozen account catalog.")
    if row.get("context_window") != CONTEXT_WINDOW or row.get("max_tokens") is not None:
        raise MIMO25PROLiveStop("mimo-v2.5-pro catalog runtime limits drifted.")
    if closeout.get("verdict") != "CAPABILITY_CALIBRATION_FAIL_CEILING_STOP":
        raise MIMO25PROLiveStop("mimo-v2.5 predecessor is not frozen at ceiling.")
    return state

def qualify_q1() -> dict[str, Any]:
    _patch_live_constants(); unit = units()[0]
    with tempfile.TemporaryDirectory(prefix="ace-mimo25-live-q1-") as directory:
        ...
        try:
            ...
            while time.time() < deadline:
                if stream_error: raise MIMO25PROLiveStop(stream_error[-1])
                if progress.is_file() and _read(progress).get("status") == "TOOLS_LISTED": break
                time.sleep(0.1)
            else: raise MIMO25PROLiveStop("mimo-v2.5-pro Q1 did not list AppWorld MCP tools.")
            ...
            payload: dict[str, Any] = {
                "schema_version": "ace-codingplan-mimo25pro-live-mcp-q1-v1", "object_id": OBJECT_ID,
                "status": "CODINGPLAN_MIMO25PRO_LIVE_MCP_PREDISPATCH_PASS", "model_profile": MODEL_PROFILE, "model_id": MODEL_ID,
            ...
            if payload["codingplan_model_requests"] != 0 or payload["session_mcp_progress_status"] != "TOOLS_LISTED" or payload["session_mcp_tool_count"] <= 0:
                raise MIMO25PROLiveStop("mimo-v2.5-pro Q1 predispatch did not pass fail-closed checks.")
...
def build_contract(selection: dict[str, Any], q1: dict[str, Any]) -> dict[str, Any]:
    ...
    payload: dict[str, Any] = {
        "schema_version": "ace-codingplan-mimo25pro-capability-b3-contract-v1", "object_id": OBJECT_ID, "execution_id": EXECUTION_ID,
        "status": "CODINGPLAN_MIMO25PRO_CAPABILITY_B3_AUTHORIZED", "backbone_search_state_sha256": selection["content_sha256"], "q1_predispatch_sha256": q1["content_sha256"],
        ...
        "panel": {"family_ids": list(CAPABILITY_FAMILIES), "repeats": list(REPEATS), "episodes": 8, "arm": "LOW", "reuse_prior_mimo25pro_measurements": False},
        ...
        "authority": {"capability_b3": True, "f0": False, "p1": False, "toolsandbox": False, "appworld_ul": False, "paper_claim": False}, "scientific_outcomes_observed": 0,
    }
    payload["content_sha256"] = sha256_value(payload); return payload
...
def execute_panel(*, runtime_root: Path, ledger_path: Path) -> None:
    ...
    _verified(SELECTION_OUTPUT, "CAPABILITY_BACKBONE_SEARCH_CONTINUE_MIMO25PRO_NEXT")
    _verified(Q1_OUTPUT, "CODINGPLAN_MIMO25PRO_LIVE_MCP_PREDISPATCH_PASS")
    _verified(CONTRACT_OUTPUT, "CODINGPLAN_MIMO25PRO_CAPABILITY_B3_AUTHORIZED")
    _verified(V4_QUAL, "CAPABILITY_SUBSTRATE_V4_PUBLIC_REACHABILITY_WITH_HEADROOM_PASS")
    ...
    for unit in units():
        ...
        if state is not None: raise MIMO25PROLiveStop(f"Refusing replay of dispatched unit {unit.unit_id}: {state}")
        family = families[unit.family_id]; arm = next(row for row in family["arms"] if row["coupling_level"] == "LOW")
        if int(arm["matching"]["tool_budget"]) != TOOL_CALL_CAP: raise MIMO25PROLiveStop("V4 tool budget drifted from 16.")
        unit_root = runtime_root / unit.unit_id.replace(":", "_").replace("|", "_")
        if unit_root.exists(): raise MIMO25PROLiveStop(f"Refusing overwrite: {unit_root}")
        ...
        try:
            ...
            def before_submit() -> dict[str, Any]:
                ...
                if progress.get("status") != "TOOLS_LISTED": raise MIMO25PROLiveStop(f"Session MCP catalog not frozen: {progress.get('status')}")
                usage = live.codingplan_usage(base, token)
                if int(usage["remaining"]) < MODEL_ROUND_CAP + 5: raise MIMO25PROLiveStop("Insufficient CodingPlan rolling-window headroom.")
            ...
            if live_result["prohibited_tool"]:
                ...
                raise MIMO25PROLiveStop(f"Non-AppWorld tool attempted: {live_result['prohibited_tool']}")
            if live_result["error_message"] or live_result["stop_reason"] in {"provider_error", "timeout", "rate_limited", "prompt_rejected", "policy_denied", "runtime_stopped"}:
                ...
                raise MIMO25PROLiveStop(message)
        finally:
            if process is not None: live.terminate_process(process)
        if live_result is None: raise MIMO25PROLiveStop("No live result after dispatch.")
...
def adjudicate(*, ledger_path: Path) -> dict[str, Any]:
    ...
    if missing: raise MIMO25PROLiveStop("Incomplete mimo-v2.5-pro panel: " + ", ".join(missing))
    completions = [row for row in rows if row["event"] == "COMPLETION"]
    if len(completions) != 8: raise MIMO25PROLiveStop("Expected exactly eight completion rows.")
    ...
    result: dict[str, Any] = {
        "schema_version": "ace-codingplan-mimo25pro-capability-b3-result-v1", "object_id": OBJECT_ID, "execution_id": EXECUTION_ID, "status": gate["verdict"],
