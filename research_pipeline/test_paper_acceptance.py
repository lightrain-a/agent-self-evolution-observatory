...
class PaperAcceptanceTest(unittest.TestCase):
    ...
    def test_active_unrefuted_claim_is_valid_manuscript_claim_without_becoming_supported(self) -> None:
        contract = PaperContract(
            paper_id="ACTIVE-PAPER",
            title="Active hypothesis paper",
            central_question="Does the mechanism reach downstream behavior?",
            supported_claims={"C1": "The upstream write channel is directly observed."},
            active_unrefuted_claims={"C2": "The state perturbation reaches later behavior."},
            active_claim_experiment_debt={"C2": ("paired downstream injection",)},
            unsupported_claims={"U1": "The effect is universal across all agents."},
            evidence_refs=("artifact:f0",),
            scientific_status=ScientificPaperStatus.READY,
        )
        active_story = StoryCandidate("story-active", "Mechanism chain", "Lead with the observed channel and active downstream hypothesis.", ("C1", "C2"), ("C1", "C2"))
        overclaim_story = StoryCandidate("story-overclaim", "Universal claim", "Expand beyond the frozen contract.", ("C1", "U1"), ("U1",))
        self.assertEqual(active_story.validate(contract), ())
        self.assertTrue(overclaim_story.validate(contract))
        result = evaluate_story_search(contract, [active_story, overclaim_story])
        self.assertEqual(result["selected_story_id"], "story-active")
        self.assertEqual(result["candidates"][0]["active_unrefuted_claim_coverage"], 1.0)

    def test_claim_audit_allows_registered_active_unrefuted_claim_when_evidence_bound(self) -> None:
        contract = PaperContract(
            paper_id="ACTIVE-PAPER",
            title="Active hypothesis paper",
            central_question="Does the mechanism reach downstream behavior?",
        ...
        result = build_claim_audit_receipt(
            contract,
