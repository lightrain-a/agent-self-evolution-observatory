...
def _closure(r:dict,hold=False,source_artifact="shadow_search_memory")->dict:
 ...
 explicit_mid=_t(r.get("memory_id"),160) if source_artifact!="shadow_search_memory" else "";mid=explicit_mid or _id(kind,cid,cl,strong,reopen)
 return {"memory_id":mid,"kind":kind,"title":_t(r.get("title") or r.get("problem_text") or cid,420),"summary":strong or _t(r.get("problem_text"),1000),"candidate_id":cid,"scope":_t(c.get("scope") or r.get("scope") or r.get("basin"),700),"affected_layer":fl or cl,"memory_class":_t(r.get("memory_class"),100),"durability_class":"scientific","prompt_eligible":True,"search_closure_certified":r.get("search_closure_certified") is True,"scientific_dead_end_certified":dead,"principle_update_allowed":r.get("principle_update_allowed") is True,"reopen_condition":reopen,"opposite_search_seed":_t(c.get("opposite_search_seed"),900),"reusable_precheck":"","source_refs":_refs(r),"source_artifact":source_artifact,"reuse_effectiveness":{},"scientific_authority":False}
...
def build_research_memory_wiki(*,search_design_state:dict,failure_asset_library:dict,scientific_meta_trace:dict,candidate_portfolio:dict,experiment_iteration:dict,generator_state:dict,claim_ledger:list[dict]|None=None,generated_at:str|None=None,principle_closure_registry:list[dict]|None=None)->dict:
 ...
 if principle_closure_registry is not None:
  entries += [_closure(r,False,"p0_principle_closure_registry") for r in principle_closure_registry if isinstance(r,dict)]
 ...
 source_manifest={"search_memory_closed_objects":len(mem.get("closed_objects") or []),"search_memory_hold_objects":len(mem.get("hold_objects") or []),"failure_assets":int((failure_asset_library.get("summary") or {}).get("assets") or 0),"candidate_portfolio_visible":int((candidate_portfolio.get("summary") or {}).get("visible_candidates") or 0),"unresolved_principles":int((scientific_meta_trace.get("summary") or {}).get("unresolved_principles") or 0),"generator_blocked_attempts":int((((generator_state.get("saturation_memory") or {}).get("blocked_problem_memory") or {}).get("blocked_candidate_attempts") or 0))}
 if principle_closure_registry is not None:source_manifest["p0_principle_closures"]=sum(isinstance(r,dict) for r in principle_closure_registry)
 out={"schema_version":SCHEMA_VERSION,"generated_at":generated_at or _now(),"status":"MEMORY_COMPILED","policy":dict(POLICY),"source_manifest":source_manifest,"summary":{"entries":len(entries),"search_closures":kc.get("SEARCH_CLOSURE",0),"scientific_closures":kc.get("SCIENTIFIC_CLOSURE",0),"holds":kc.get("HOLD",0),"failure_assets":kc.get("FAILURE_ASSET",0),"success_assets":kc.get("SUCCESS_ASSET",0),"open_questions":kc.get("OPEN_QUESTION",0),"repeated_review_blocks":kc.get("REPEATED_REVIEW_BLOCK",0),"prompt_eligible":sum(r.get("prompt_eligible") is True for r in entries),"transient":dc.get("transient",0),"recurring_systemic":dc.get("recurring-systemic",0),"scientific":dc.get("scientific",0)},"entries":entries,"scientific_authority":False,"authority":{"problem_gate":False,"paper_design":False,"method":False,"experiment":False,"p0":False,"gpu":False}}
