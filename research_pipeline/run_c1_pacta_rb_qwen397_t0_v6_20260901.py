#!/usr/bin/env python3
"""T0-v6: acquire the ten untouched ReasoningBank source trajectories only."""
...
from research_pipeline.c1_pacta_rb_qwen397_t0_runtime_v6 import Container,ROOTFUL_DOCKER_HOST,SOURCE_MAX_COMPLETION_TOKENS,PACTA_FIRST_DECISION_BUDGET,execute_trajectory
...
DEFAULT=Path("/data/wyt/agent-self-evolution-observatory/runs/c1-pacta-rb-qwen397-t0-rootful-source-20260901-v6")
MODEL="qwen3.5-397b-a17b"
CONSUMED_SOURCE="pydata__xarray-4966"
FUTURES={
...
def prepare(root:Path)->dict[str,Any]:
    if root.exists():raise RuntimeError(f"T0-v6 root exists; no overwrite/retry: {root}")
    verify_frozen();units=pool_units();digests=digest_map();root.mkdir(parents=True)
    if CONSUMED_SOURCE in FUTURES:raise AssertionError("consumed source leaked into v6")
    schedule=[]
    ...
    contract={"schema_version":1,"created_at_utc":now(),"experiment":"C1-PACTA-RB-QWEN397-T0-v6-20260901","status":"FROZEN_BEFORE_SOURCE_POLICY","model":MODEL,"source_max_completion_tokens":SOURCE_MAX_COMPLETION_TOKENS,"pacta_first_decision_budget_unchanged":PACTA_FIRST_DECISION_BUDGET,"docker_host":ROOTFUL_DOCKER_HOST,"official_commit":OFFICIAL_COMMIT,"official_config_sha256":CONFIG_SHA256,"source_budget_q0_sha256":SOURCE_BUDGET_Q0_SHA256,"scheduled_source_units":10,"consumed_excluded_source":CONSUMED_SOURCE,"replacement":False,"top_up":False,"logical_attempts_per_source":1,"rate_limit_transport_recovery":{"max_retries":2,"backoff_seconds":[60,120],"only_when_no_model_content":True},"writer_calls":0,"binder_calls":0,"shadow_calls":0,"final_measurement_calls":0,"future_task_executions":0,"forbidden":["rerun consumed source","replacement source","writer","binder","shadow","gate","random_gate","final","future_policy","future_evaluator","model_switch"]}
    atomic_json(root/"contract.json",contract);atomic_json(root/"acquisition-schedule.json",{"schema_version":1,"created_at_utc":now(),"status":"FROZEN","schedule":schedule,"scheduled_count":10,"selection":"all ten untouched fixed units","consumed_source_excluded":CONSUMED_SOURCE,"replacement":False,"top_up":False})
    return {"scheduled":10,"contract_sha256":sha256_file(root/"contract.json"),"schedule_sha256":sha256_file(root/"acquisition-schedule.json")}
...
def prelaunch(root:Path)->dict[str,Any]:
    ...
    result={"schema_version":1,"created_at_utc":now(),"status":"T0_V6_PRELAUNCH_PASS" if all(r["pass"] for r in rows) else "STOP_T0_V6_PRELAUNCH","qualified":sum(r["pass"] for r in rows),"total":10,"provider_calls":0,"rows":rows};atomic_json(root/"prelaunch-qualification.json",result);return result
...
def acquire(root:Path)->dict[str,Any]:
    ...
    if pre.get("status")!="T0_V6_PRELAUNCH_PASS" or pre.get("qualified")!=10:raise RuntimeError("prelaunch not qualified")
    ...
    if len(rows)<10:decision="STOP_T0_V6_PROVIDER_OR_IMPLEMENTATION_FAILURE"
    elif len(valid)==10:decision="SOURCE_TRAJECTORY_POOL_PARTIAL_10_STOP_BEFORE_PACTA"
    elif len(valid)>=6:decision="SOURCE_TRAJECTORY_POOL_PARTIAL_STOP_BEFORE_PACTA"
    else:decision="HOLD_FRESH_SUPPORT_INSUFFICIENT_AFTER_ACQUISITION"
    result={"schema_version":1,"created_at_utc":now(),"decision":decision,"rows":rows,"source_logical_attempts":len(rows),"N_valid_trajectory":len(valid),"N_valid_repository":len({units[r["source_task_id"]]["task_family"] for r in valid}),"maximum_possible_pool_after_consumed_exclusion":10,"full_6_plus_5_design_recovered":False,"stop_reason":stop_reason,"source_budget":SOURCE_MAX_COMPLETION_TOKENS,"pacta_first_decision_budget":PACTA_FIRST_DECISION_BUDGET,"writer_calls":0,"binder_calls":0,"shadow_calls":0,"final_measurement_calls":0,"future_task_executions":0,"claim_authority":"NO_NEW_PACTA_EFFECT_EVIDENCE","active_manuscript":"R9"};atomic_json(root/"support-audit.json",result);return result
