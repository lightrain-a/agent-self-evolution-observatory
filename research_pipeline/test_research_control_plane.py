...
class ResearchControlPlaneTest(unittest.TestCase):
    ...
    def test_public_projection_is_selective_and_does_not_mutate_source_snapshots(self) -> None:
        ...
        self.assertEqual(state["component_snapshots"]["research_skill_registry"]["skill_packs_catalogued_not_installed"], 0)
        self.assertEqual(state["component_snapshots"]["research_skill_registry"]["external_skill_packs_distilled"], 8)
        self.assertEqual(state["component_snapshots"]["research_skill_registry"]["canonical_internal_skills"], 7)
        self.assertEqual(len(state["component_snapshots"]["research_skill_registry"]["internal_skill_ids"]), 7)
        self.assertEqual(len(state["component_snapshots"]["research_skill_registry"]["external_distillation"]), 8)
        self.assertTrue(all(row["kept"] and row["discarded"] for row in state["component_snapshots"]["research_skill_registry"]["external_distillation"]))
        self.assertEqual(state["component_snapshots"]["manuscript_integrity_layer"]["audit_surfaces"], 7)
        self.assertEqual(state["summary"]["catalogued_skill_packs"], 0)
