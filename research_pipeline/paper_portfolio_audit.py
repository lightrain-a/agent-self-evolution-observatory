...
from .reopened_p0_pre_experiment_adapter import public_p0_pre_experiment
from .reopened_p0_experiment_lease_request import public_p0_lease_request
...
def scientific_reopen_state(root: Path, paper_id: str, attempt: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if handoff_sha:
        ...
        if contract_summary.get('status') == 'NEW_SCIENTIFIC_CONTRACT_CREATED_PROBLEM_GATE_REQUIRED':
            ...
            p0_pre_summary = public_p0_pre_experiment(root / 'scientific-contract-p0-pre-experiment', str(contract_summary.get('contract_id') or '')) if p0_plan_summary.get('status') == 'P0_CONFIRMATORY_PLAN_FROZEN_PRE_EXPERIMENT_REQUIRED' else public_p0_pre_experiment(Path('/nonexistent'), '')
            p0_req_summary = public_p0_lease_request(root / 'scientific-contract-p0-lease-requests', str(contract_summary.get('contract_id') or '')) if p0_pre_summary.get('status') == 'P0_PRE_EXPERIMENT_COMPILER_PASS_FRESH_LEASE_REQUIRED' else public_p0_lease_request(Path('/nonexistent'), '')
    ...
    if state == 'LEARN':
        if learning['status']!='LEARN_COMPLETE':
            ...
            if scientific_reopen['status']=='SCIENTIFIC_REOPEN_PROPOSAL_REQUIRED':
                ...
                actions=['the local-F0 run-start receipt is historical but its experiment or GPU resource lease is no longer active. Do not continue execution under the stale run; reconcile/close the run before any new execution authority is acquired']
            elif scientific_reopen['status']=='REOPEN_LOCAL_F0_RUN_LEDGER_INVALID':
                actions=['the local-F0 run-start ledger is invalid. Stop execution immediately and repair the content-addressed run/resource lineage before interpreting any produced artifact']
...
def build(root: Path = DEFAULT_ROOT) -> dict[str, Any]:
    ...
    return {
        ...
        'summary': {
            ...
            'reopen_local_f0_run_start_required': sum(p['scientific_reopen_status'] == 'REOPEN_LOCAL_F0_RUN_START_REQUIRED' for p in papers),
            ...
            'reopen_p0_pre_experiment_blocked': sum(p['scientific_reopen_status'] == 'P0_PRE_EXPERIMENT_COMPILER_BLOCKED' for p in papers),
            'reopen_p0_pre_experiment_pass': sum(p['scientific_reopen_status'] == 'P0_PRE_EXPERIMENT_COMPILER_PASS_FRESH_LEASE_REQUIRED' for p in papers),
            'reopen_p0_pre_experiment_invalid': sum(p['scientific_reopen_status'] == 'P0_PRE_EXPERIMENT_LEDGER_INVALID' for p in papers),
            'reopen_p0_lease_request_required': sum(p['scientific_reopen_status'] == 'P0_EXPERIMENT_LEASE_REQUEST_REQUIRED' for p in papers),
            'reopen_p0_lease_request_ready': sum(p['scientific_reopen_status'] == 'P0_EXPERIMENT_LEASE_REQUEST_READY_EXPLICIT_ACQUIRE_REQUIRED' for p in papers),
            'reopen_p0_lease_request_invalid': sum(p['scientific_reopen_status'] == 'P0_EXPERIMENT_LEASE_REQUEST_LEDGER_INVALID' for p in papers),
