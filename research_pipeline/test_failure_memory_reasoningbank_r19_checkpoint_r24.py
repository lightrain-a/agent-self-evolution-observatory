...
try:
    from research_pipeline.failure_memory_reasoningbank_r19_partial_checkpoint_r24 import RECEIPT_ID, build
    from research_pipeline.failure_memory_reasoningbank_r19_csv_checkpoint_r24 import RECEIPT_ID as CSV_RECEIPT_ID
except ModuleNotFoundError:
    from failure_memory_reasoningbank_r19_partial_checkpoint_r24 import RECEIPT_ID, build
    from failure_memory_reasoningbank_r19_csv_checkpoint_r24 import RECEIPT_ID as CSV_RECEIPT_ID


class TestR19CheckpointR24(unittest.TestCase):
    ...
    def test_r24_identity_and_no_interim_boundary(self) -> None:
        out = build(self._root())
        self.assertEqual(out["receipt_id"], RECEIPT_ID)
        self.assertEqual(CSV_RECEIPT_ID, "D2-FAILURE-MEMORY-PROVENANCE-L2B-R19-CSV-DURABILITY-R24")
