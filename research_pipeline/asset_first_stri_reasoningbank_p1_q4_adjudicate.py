"""Outcome-blind adjudication for the fresh P1 Q4 implementation qualification."""
...
OUTPUT = ROOT / "generated/asset-first-stri-reasoningbank-p1-q4-adjudication-20260830.json"
...
def summarize_run(receipt: dict[str, Any], run: dict[str, Any]) -> dict[str, Any]:
    ...
    checks = {
        ...
        "fresh_exact_base_normalization_receipt_valid": (
            base_receipt.get("returncode") == 0
            and base_receipt.get("rule")
            == "exact_base_after_preregistered_hard_reset"
            and base_receipt.get("expected_base_commit")
            == base_receipt.get("observed_head")
            and (base_receipt.get("pre_normalization") or {}).get("returncode") == 0
            and (base_receipt.get("normalization") or {}).get("returncode") == 0
            and base_receipt.get("normalization_action")
            == "git reset --hard <frozen expected base commit>"
            and base_receipt.get("git_clean_invoked") is False
        ),
...
def adjudicate(
    *, index_path: Path = INDEX, output: Path = OUTPUT,
    ...
    if output.exists():
        raise RuntimeError(f"refusing to overwrite immutable Q4 adjudication: {output}")
    ...
    for receipt in completed:
        ...
        if not path.is_relative_to(ROOT.resolve()):
            raise RuntimeError("Q4 run path escapes repository")
    ...
    qualification_checks = {
        "contract_and_fixture_bindings_exact": (
            ...
            and index["q3_runtime_hold_sha256"] == sha256_file(Q3_RUNTIME_HOLD)
        ...
        "runtime_qualification_exact_and_passed": (
            ...
            and runtime["decision"] == "P1_Q4_RUNTIME_QUALIFIED"
            and runtime["scientific_boundary"]["q4_task_outcome_observed"] is False
            and runtime["checks"]["all_base_states_exact_after_preregistered_reset"]
            is True
            and runtime["frozen_input_verification"]["q3_runtime_hold_preserved"]["pass"]
            is True
    ...
    decision = (
        "P1_Q4_IMPLEMENTATION_QUALIFIED_FULL_P1_PLANNING_GATE_OPEN_EXECUTION_UNAUTHORIZED"
        if implementation_qualified
        else "P1_Q4_IMPLEMENTATION_UNQUALIFIED_FULL_P1_HOLD"
    )
    payload = {
        "schema_version": 1,
        "experiment_id": "E1-STRI-REASONINGBANK-P1-Q4-ADJUDICATION-20260830",
        ...
        "scientific_boundary": {
            ...
            "q3_model_run_count_before_q4": 0,
            "prior_negative_evidence_preserved": True,
            "q4_is_prospective_implementation_qualification_only": True,
