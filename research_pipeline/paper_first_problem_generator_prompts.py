...
def generator_prompt(records: list[dict[str, Any]], dead_end_memory: dict[str, Any] | None = None) -> str:
        ...
        "candidates": [
            {
                ...
                "lane_evidence": {"lane-specific-required-field": "...", "CONTRADICTION additionally requires source_a_intervention/source_b_intervention": "...", "intervention_surface_match/executor_state_match/comparator_match/endpoint_match/timing_match": True, "treatment_equivalence_argument": "..."},
                ...
                "reduction_falsifiability_contract": {"same_observable_information_checked": True, "ex_ante_exact_prediction_checked": True, "distinguishing_prediction_checked": True, "scope_boundary_checked": True, "all_exact_reduction_tests_resolved": False},
    ...
    return (
        ...
        "CONTRADICTION TREATMENT-ALIGNMENT GATE (BEFORE comparing signs): explicitly state source_a_intervention and source_b_intervention, then independently verify five dimensions: intervention_surface_match, executor_state_match, comparator_match, endpoint_match, timing_match. All five booleans must be true for a CONTRADICTION candidate. 'Both are prompts/skills/procedural text' is NOT treatment equivalence when one is inference-time conditioning and the other includes parameter updates, filtering/training, different comparator construction, a different endpoint, or a different intervention time. If any dimension differs or cannot be established from supplied evidence, lane_search MUST use status=REDUCIBLE for that pair with a cross-treatment reason and MUST NOT emit a candidate. treatment_equivalence_argument must explain causal-surface equivalence rather than merely repeat shared_operationalization. "
        "GENERATOR/REVIEWER REDUCTION SPLIT: the Generator may emit a lane-valid candidate with reduction_class=NEEDS_EXACT_REDUCTION_TEST so the independent Reviewer can adjudicate that exact reduction; in that case all_exact_reduction_tests_resolved MUST be false. If no pending exact test remains, it may be true. A candidate with reduction_class=VALID_HARD_VETO, or any exact known hard reduction in matched_patterns, is NOT emit-eligible and must instead make the lane REDUCIBLE. Never mark all_exact_reduction_tests_resolved=true while any baseline or saturation entry is pending. "
        "If either mature theory already expresses the exact ex-ante prediction under the same information with a proven hard reduction, discard the candidate. If the anomaly is real but an exact reduction is unresolved, the candidate may proceed only to independent semantic review with the cheapest distinguishing test explicit; this is not scientific clearance. Domain transfer, mathematical renaming, another benchmark/metric/taxonomy/test-generator, or combining occupied atoms is not novelty. Every mature_theory_baselines item MUST include name, same_information_projection, ex_ante_prediction, distinguishing_prediction, cannot_express, reduction_class, and exact_reduction_test. Every emitted candidate MUST include reduction_falsifiability_contract with the four audit booleans true; all_exact_reduction_tests_resolved must be false whenever any exact reduction remains pending and true only when none remains.\n\n"
        ...
        + "\n\nLAYER-TYPED CLOSED-BASIN MEMORY (search-control only; zero scientific authority):\n"
        + json.dumps(dead_end_memory, ensure_ascii=False, separators=(",", ":"))
        + "\nThese are exact candidate formulations already closed by independent review or scoped readjudication. search_closure_certified means the exact search basin is blocked absent its reopen evidence; dead_end_certified is reserved for the core_principle scientific-dead-end subset and does NOT imply broader benchmark/phenomenon death. closure_layer=problem_novelty is an upstream literature/theory collision and is not an experimental failure layer. All scientific closures must use the canonical failure_layer values execution, experiment_identifiability, optimization, operationalization, method_realization, assumption_scope, or core_principle. Only core_principle may update the scoped scientific principle; even then, broader benchmark/phenomenon death requires a separate broader_core_principle_falsified or benchmark-level dead-end flag. Do not regenerate a semantically equivalent object by swapping sources, renaming the object, or rephrasing the same reduction escape. Reopening requires NEW evidence capable of satisfying the recorded reopen_condition. In particular, if repeated_reduction_basin=true, actively search outside the top basin. A prior closed basin may be reconsidered only when CURRENT evidence contains a concrete new observable that directly defeats the recorded strongest reduction; mere new wording or a new domain is insufficient. research_memory_query_pack is the unified bounded Wiki context: read every selected item before proposing candidates. FAILURE_ASSET is a precheck, not a veto; SUCCESS_ASSET is scoped evidence, not permission to generalize; HOLD remains reopenable; transient operational noise is excluded by the compiler.\n"
...
def reviewer_prompt(candidates: list[dict[str, Any]], evidence_by_ref: dict[str, dict[str, Any]], *, shadow_mode: bool = False) -> str:
    ...
    return (
        ...
        "(2) lane contract: the two grounded evidence-item claims, evidence roles, relation, and lane_evidence must genuinely satisfy the selected discovery lane. For CONTRADICTION, verify shared intervention semantics and adaptation stage and independently verify that source_a_intervention and source_b_intervention use the same causal treatment surface with matched executor/parameter state, comparator, endpoint, and intervention timing. Inference-time context/retrieval, offline filtering, optimizer/weight updates, parameter-efficient tuning, and full-parameter training are different treatment surfaces unless explicitly conditioned rather than erased; in particular, inference-time conditioning versus parameter-updated/SFT training is a cross-treatment contrast and MUST be BLOCK, not a contradiction; "
        ...
        "If source grounding or the lane contract fails, verdict=BLOCK. For CONTRADICTION, lane_contract_verified=true additionally requires all five declared treatment-alignment booleans to be true AND supported by the primary evidence rather than generator assertion. CLEAR only means both evidence items are grounded, the lane-specific distinct-source minimum is satisfied, lane_contract_verified=true, and no proven/pending exact mature reduction remains; it never means scientific approval.\n\n"
        "LANE CONTRACTS:\n"
        + json.dumps(lane_contracts, ensure_ascii=False, separators=(",", ":"))
        + "\n\nLEDGER:\n"
        + json.dumps(reductions, ensure_ascii=False, separators=(",", ":"))
