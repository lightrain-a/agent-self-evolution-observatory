...
def generator_prompt(records: list[dict[str, Any]], dead_end_memory: dict[str, Any] | None = None) -> str:
    ...
    return (
        ...
        + "\nThese are exact candidate formulations already closed by independent review or scoped readjudication. dead_end_certified means the exact search basin is blocked absent its reopen evidence; it does NOT by itself mean the broader benchmark/phenomenon was falsified. Read failure_layer/memory_class: PROBLEM_NOVELTY stops are upstream literature/theory collisions; OPERATIONALIZATION_IDENTIFIABILITY stops mean the proposed observable/contrast/causal path cannot identify the mechanism; METHOD_FORMULATION stops mean a same-information simpler explanation or supported counter-mechanism absorbs the standalone claim; ASSUMPTION_SCOPE stops mean a necessary condition does not hold; PRINCIPLE_STOP means the scoped scientific principle itself is closed after a valid counter-explanation/reduction, but still does not imply benchmark-level or broader-phenomenon death unless a separate broader_core_principle_falsified/benchmark_level_dead_end flag says so. Do not regenerate a semantically equivalent object by swapping sources, renaming the object, or rephrasing the same reduction escape. Reopening requires NEW evidence capable of satisfying the recorded reopen_condition. In particular, if repeated_reduction_basin=true, actively search outside the top basin. A prior closed basin may be reconsidered only when CURRENT evidence contains a concrete new observable that directly defeats the recorded strongest reduction; mere new wording or a new domain is insufficient.\n"
