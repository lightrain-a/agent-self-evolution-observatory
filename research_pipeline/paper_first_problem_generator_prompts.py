...
def generator_prompt(records: list[dict[str, Any]], dead_end_memory: dict[str, Any] | None = None) -> str:
    ...
    return (
        ...
        + "\nThese are prior candidates already BLOCKED by independent review or principle adjudication. Do not regenerate a semantically equivalent object by swapping sources, renaming the object, or rephrasing the same reduction escape. Dead ends whose source_refs overlap the current Primary are prioritized. Reusing the exact same primary source set in the same live discovery lane is machine-blocked as principle-dead-end re-entry; reopening requires NEW evidence capable of satisfying the recorded reopen_condition. In particular, if repeated_reduction_basin=true, actively search outside the top basin. A prior dead end may be reconsidered only when the CURRENT evidence contains a concrete new observable that directly defeats the recorded strongest reduction; mere new wording or a new domain is insufficient.\n"
