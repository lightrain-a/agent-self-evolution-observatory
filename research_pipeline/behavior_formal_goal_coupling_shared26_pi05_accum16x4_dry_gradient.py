...
V4_ADJ_SHA='b2671ad042fd9bd5bfe44d9cba3d2beb0421c504712a5ad8128f5dfec691ea29'
DATA_ORDER_SHA='0d2778edcfed2203cca93e8aa3e6c4733a24ba3b8dadb3e808a0fa2f795bcfec'
...
SOURCE_BATCH=64; MICRO=16; K=4; EFFECTIVE=64; ACTION_HORIZON=32; FSDP=1; SEED=42
...
def main():
    ...
    for name in ['authority','preregistration','v4_adjudication','data_order_qualification','resource_admission','model_load_result','dataloader_smoke','tokenizer_result','base_receipt','openpi_child_root','params_root','tokenizer_source','openpi_data_home','receipt']:
    ...
    if R.exists(): raise RuntimeError(f'exactly-once accum16x4 receipt exists: {R}')
    auth=json.loads(P['authority'].read_text())
    if auth.get('status')!='AUTHORIZED_PI05_SINGLE_GPU_ACCUM16X4_NO_UPDATE_DRY_GRADIENT' or auth.get('object_id')!=OBJECT_ID or auth.get('child_id')!=CHILD_ID: raise RuntimeError('authority drift')
    if auth.get('runner_sha256')!=sha(Path(__file__).resolve()): raise RuntimeError('runner SHA binding drift')
    require(P['preregistration'],PREREG_SHA,'prereg'); require(P['v4_adjudication'],V4_ADJ_SHA,'v4 adjudication'); require(P['data_order_qualification'],DATA_ORDER_SHA,'data order')
    require(P['resource_admission'],RESOURCE_SHA,'resource admission'); require(P['model_load_result'],MODEL_LOAD_SHA,'model load'); require(P['dataloader_smoke'],LOADER_SHA,'loader smoke'); require(P['tokenizer_result'],TOKEN_RESULT_SHA,'tokenizer result'); require(P['tokenizer_source'],TOKEN_SHA,'tokenizer')
    pre=json.loads(P['preregistration'].read_text()); first=pre['microbatch_resource_ladder'][0]
    if first!={'priority':1,'physical_micro_batch':16,'accumulation_steps':4,'effective_batch':64}: raise RuntimeError(f'ladder drift {first}')
    order=json.loads(P['data_order_qualification'].read_text())
    if order.get('status')!='PI05_ACCUM_16X4_DATA_ORDER_QUALIFICATION_PASS' or not order.get('sampler_groups_exactly_equal'): raise RuntimeError('data-order gate not PASS')
    ...
    initial={'schema_version':'behavior-formal-goal-coupling-shared26-pi05-accum16x4-dry-gradient-result-v1','object_id':OBJECT_ID,'child_id':CHILD_ID,'generated_at':datetime.now(timezone.utc).isoformat(),'status':'PI05_ACCUM16X4_NO_UPDATE_DRY_GRADIENT_STARTED','candidate_priority':1,'physical_micro_batch':MICRO,'accumulation_steps':K,'effective_batch':EFFECTIVE,'attempt_count_under_candidate_authority':1,'attempt_is_exactly_once':True,'authority_sha256':sha(P['authority']),'preregistration_sha256':PREREG_SHA,'v4_adjudication_sha256':V4_ADJ_SHA,'data_order_qualification_sha256':DATA_ORDER_SHA,'openpi_child_commit':child,'base_object_count_rehashed':bc,'base_bytes_rehashed':bb,'resource_scope':scope,'gpu_before':g0,'micro_gradients_completed':0,'accumulated_gradient_complete':False,'loss_value_retained_or_reported':False,'gradient_numerical_values_read':False,'optimizer_update':False,'parameter_update':False,'checkpoint_written':False,'scientific_training_started':False,'policy_rollouts_started':False,'policy_outcomes_read':False,'formal_training_authorized':False}
    with lock(Path('/data/wyt/.formal-goal-pi05-accum16x4-dry-gradient-v1.lock')):
        write_receipt(R,initial); status='PI05_ACCUM16X4_NO_UPDATE_DRY_GRADIENT_HOLD'; err=None; batch_ready=False; state_ready=False; done=0; acc=None; leaf_count=None; elem_count=None; gpu_state=None; mem_state=None; gpu_micro=[]; mem_micro=[]
        try:
            os.chdir(P['openpi_child_root']); sys.path.insert(0,str(P['openpi_child_root']/ 'src'))
            ...
            cfg=dataclasses.replace(src,exp_name=EXP_NAME,weight_loader=weight_loaders.CheckpointWeightLoader(str(P['params_root'])),batch_size=MICRO,num_workers=WORKERS,wandb_enabled=False,resume=False,overwrite=True)
            ...
            loader=data_loader.create_b1k_data_loader(cfg,sharding=ds,shuffle=True,num_batches=K,skip_norm_stats=False); batches=list(iter(loader))
            if len(batches)!=K or any(b[1].shape[0]!=MICRO for b in batches): raise RuntimeError('microbatch shape/count drift')
            batch_ready=True; rng=jax.random.key(cfg.seed); train_rng,init_rng=jax.random.split(rng); state,state_shard=train_b1k.init_train_state(cfg,init_rng,mesh,resume=False); jax.block_until_ready(state)
