#!/usr/bin/env python3
"""Rootful exact-base T0 smoke, bridge, and fixed source acquisition.

This entry point cannot reach writer, binder, shadow, gate, final measurement, or
future-task execution. It consumes the sealed Q0 binding and the 11/11 T0.5-R
...
T05R=Path("/data/wyt/agent-self-evolution-observatory/runs/c1-pacta-rb-qwen397-t05r-images-rootful-20260901-v1")
DEFAULT=Path("/data/wyt/agent-self-evolution-observatory/runs/c1-pacta-rb-qwen397-t0-rootful-source-20260901-v1")
...
def frozen()->tuple[dict[str,Any],dict[str,Any],dict[str,Any],dict[str,Any]]:
 ...
 qualification_path=T05R/"normalization-qualification.json"
...
def smoke()->dict[str,Any]:
 ...
 smoke_root=T05R/"rootful-multistep-smoke"
 ...
 atomic_json(T05R/"rootful-synthetic-smoke.json",result)
...
def prepare(root:Path)->dict[str,Any]:
 ...
 contract={"schema_version":1,"created_at_utc":now(),"status":"FROZEN_BEFORE_SOURCE_POLICY",
  ...
  "runtime_qualification_path":str(T05R/"normalization-qualification.json"),
  "runtime_qualification_sha256":sha256_file(T05R/"normalization-qualification.json"),
...
def bridge(root:Path)->dict[str,Any]:
 key=require_key()
 if not (T05R/"rootful-synthetic-smoke.json").is_file():raise RuntimeError("synthetic smoke not qualified")
 smoke=json.loads((T05R/"rootful-synthetic-smoke.json").read_text())
