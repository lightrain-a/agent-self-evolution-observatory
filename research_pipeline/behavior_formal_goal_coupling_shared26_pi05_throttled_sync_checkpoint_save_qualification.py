...
def main() -> int:
    ...
    expected_memory_max_gib = 40
    expected_memory_max_bytes = 40 * 1024**3
    status_prefix = "PI05_THROTTLED_SYNC_CHECKPOINT_SAVE_4GB_40G"
    ...
    lock_path = Path("/data/wyt/.formal-goal-pi05-throttled-sync-checkpoint-save-4gb-40g-qualification.lock")
    ...
    with exclusive_lock(lock_path):
        ...
        if authority.get("status") != "AUTHORIZED_PI05_THROTTLED_SYNC_CHECKPOINT_SAVE_4GB_40G_QUALIFICATION":
        ...
        if adjudication.get("status") != "PI05_SYNC64_CHECKPOINT_OOM_LIMITER_REPAIR_DESIGN_READY":
            raise RuntimeError("limiter repair adjudication status drift")
        if adjudication.get("prospective_repair", {}).get("formal_run3_authorized") is not False:
        ...
        initial = {
            "schema_version": "behavior-formal-goal-coupling-shared26-pi05-throttled-sync-checkpoint-save-qualification-v1",
            "object_id": OBJECT_ID,
            "child_id": CHILD_ID,
        ...
        try:
            ...
            final.update({
                ...
                "loss_values_read_or_reported": False,
                "formal_training_authorized": False,
                "next_gate": "FORMAL_RUN3_THROTTLED_SYNC_CHECKPOINT_AUTHORITY_DESIGN" if status.endswith("PASS") else "THROTTLED_SYNC_CHECKPOINT_RESOURCE_STOP",
            })
