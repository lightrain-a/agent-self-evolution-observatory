...
PID=core.PAPER_ID;DATA=core.DATA_ROOT;SRC=DATA/'paper-acceptance'/'source-native-replay'/PID/'20260824-extension-bls-cpi-crossdomain';BASE=DATA/'paper-acceptance'/'source-native-replay'/PID/'20260824-extension-bls-cpi-planning-base-repeat-r1';PLAN=BASE/'plan.json';RESULTS=BASE/'results.csv';OWNER=PID+':EXT:BLS-PLANNING-REPEAT'
...
def prep():
 eps=json.load(open(SRC/'endpoints.json'))['endpoints'];plan={'schema_version':'1.0','paper_id':PID,'experiment':'E2_EXTENSION_BLS_PLANNING_BASE_REPEAT','endpoint_ids':[e['endpoint_id'] for e in eps],'model_identity':{'requested_model':MODEL,'required_resolved_model':RES,'required_plan_base_url':URL},'planned_model_calls':8,'selection_rule':'Repeat robustness for BLS planning-only baseline on all four frozen CPI endpoints; no helper, no endpoint selection, and repeat is not an independent unit.'};plan['plan_body_sha256']=csha({k:v for k,v in plan.items() if k!='plan_body_sha256'});atomic(PLAN,plan);return plan
def run():
