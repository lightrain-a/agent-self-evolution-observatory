#!/usr/bin/env python3
"""Freeze the fresh2 source acquisition schedule before any source outcome."""
...
SPLIT_SHA = "b44f69d211d50a75c71b295cbe698ca3286d2305a841045150b55ff7252def26"
PROBES = ROOT / "paper_drafts/c1-manuscript-strengthening-20260825/c1-pacta-msr-atomgit-qwen38-fresh2-probe-specs-20260903.json"
PROBES_SHA = "3b98bd26f31824168196cda6fcf60ba448ae2bbd7579456f413b51f1bcdf9bd6"
...
OUT = ROOT / "paper_drafts/c1-manuscript-strengthening-20260825/c1-pacta-msr-atomgit-qwen38-fresh2-source-schedule-20260903.json"
ORDER_SALT = "C1-PACTA-MSR-ATOMGIT-QWEN38-FRESH2-SOURCE-v1"
...
def prepare() -> dict[str, Any]:
    for path, expected, label in (
        ...
        if not path.is_file() or sha256_file(path) != expected:
            raise RuntimeError(f"STOP_FRESH2_SOURCE_SCHEDULE_INPUT_DRIFT:{label}")
    ...
    if q02.get("selected_source_budget") != 32768 or q02.get("invocation_timeout_seconds") != 900:
        raise RuntimeError("STOP_FRESH2_SOURCE_Q02_PARAMETER_DRIFT")
    units = sorted(
        pool["units"],
    ...
    for sequence, unit in enumerate(units, 1):
        rows.append({
            ...
            "runtime_binding": "DEFERRED_UNTIL_FRESH2_20_RUNTIME_READY",
        })
    if len(rows) != 10 or len({row["source_task_id"] for row in rows}) != 10:
        raise RuntimeError("STOP_FRESH2_SOURCE_SCHEDULE_GEOMETRY")
    return {
        "schema_version": 1,
        "experiment": "C1-PACTA-MSR-ATOMGIT-QWEN38-FRESH2-SOURCE-T0-20260903",
        "status": "FRESH2_SOURCE_SCHEDULE_FROZEN_PRE_SOURCE_OUTCOME",
        ...
        "source_gate": "all 10 provenance-valid; any consumed-invalid source retires the entire fresh2 pool; no replacement or top-up",
        "rows": rows,
        "provider_calls": 0,
...
def main() -> None:
    if OUT.exists():
        raise RuntimeError("fresh2 source schedule exists; no overwrite")
    result = prepare()
