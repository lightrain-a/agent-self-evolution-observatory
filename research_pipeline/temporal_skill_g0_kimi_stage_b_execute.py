...
OWNER_ID = PAPER_ID + ":TEMP-O4-G0-KIMI-STAGE-B"
PLAN = PROJECT_ROOT / "generated/temporal-skill-g0-kimi-stage-b-plan-20260824.json"
STAGE = PROJECT_ROOT / "generated/temporal-skill-g0-kimi-stage-b-stage-contract-20260824.json"
AUTH = PROJECT_ROOT / "generated/temporal-skill-g0-kimi-stage-b-human-authorization-20260824.json"
OUTPUT = core.REPLAY_ROOT / "20260824-g0-kimi-stage-b" / "results.json"
...
def validate_auth(auth: dict[str, Any], plan: dict[str, Any], stage: dict[str, Any]) -> list[str]:
    ...
    checks = [
        ...
        (auth.get("bound_family_scorer_sha256") == sha_file(PROJECT_ROOT / "research_pipeline/temporal_skill_g0_family_score.py"), "scorer-hash"),
...
def run_one(client: ArkResponsesClient, assets: dict[str, Any], plan_row: dict[str, Any]) -> dict[str, Any]:
    ...
    row["family_score_contract_sha256"] = sha_file(PROJECT_ROOT / "research_pipeline/temporal_skill_g0_family_score.py")
    return row
...
def pilot_gate(output: Path, stage: dict[str, Any]) -> dict[str, Any]:
    ...
    result = {
        "schema_version": "1.0", "gate": "TEMP-O4-G0-KIMI-STAGE-B-PILOT-RUNTIME-INTEGRITY",
        ...
        "promotion_rule": "runtime/protocol/checkpoint integrity only",
    }
...
def build_results(output: Path, plan: dict[str, Any], auth: dict[str, Any], assets: dict[str, Any]) -> dict[str, Any]:
    ...
    result = {
        "schema_version": "1.0", "paper_id": PAPER_ID, "run_id": "TEMP-O4-G0-KIMI-STAGE-B-20260824",
        ...
        "family_scorer_sha256": sha_file(PROJECT_ROOT / "research_pipeline/temporal_skill_g0_family_score.py"),
...
def execute(stage_name: str) -> dict[str, Any]:
    ...
    if {r["endpoint_id"] for r in plan["rows"]} != {e for e in assets["endpoints"] if assets["source"][e] == "R3"}:
        raise RuntimeError("Kimi endpoint inventory mismatch")
    ...
    run_id = f"TEMP-O4-G0-KIMI-{stage_name.upper()}-20260824"
    authority = acquire_authority(DATA_ROOT, OWNER_ID, plan["plan_body_sha256"], "temporal-skill-g0-kimi-executor", f"KIMI-{stage_name}", run_id)
...
def main() -> None:
    ...
    plan = read_json(PLAN); stage = read_json(STAGE); auth = read_json(AUTH) if AUTH.exists() else {}
