...
def main() -> int:
    ...
    ap.add_argument("--expected-memory-max-gib", type=int, required=True)
    args = ap.parse_args()

    paths = {k: Path(v).resolve() for k, v in vars(args).items() if k != "expected_memory_max_gib"}
    expected_memory_max_gib = int(args.expected_memory_max_gib)
    if expected_memory_max_gib not in (40, 64):
        raise RuntimeError(f"unregistered sync-checkpoint MemoryMax: {expected_memory_max_gib}")
    expected_memory_max_bytes = expected_memory_max_gib * 1024**3
    status_prefix = f"PI05_SYNC_CHECKPOINT_SAVE_{expected_memory_max_gib}G"
    ...
    lock_path = Path(f"/data/wyt/.formal-goal-pi05-sync-checkpoint-save-{expected_memory_max_gib}g-qualification.lock")
    ...
    with exclusive_lock(lock_path):
        ...
        if authority.get("status") != "AUTHORIZED_PI05_SYNC_CHECKPOINT_SAVE_QUALIFICATION":
        ...
        if adjudication.get("status") != "PI05_ASYNC_CHECKPOINT_RESOURCE_PATH_CLOSED":
            raise RuntimeError("async checkpoint resource adjudication status drift")
        if adjudication.get("next_repair", {}).get("formal_run3_authorized") is not False:
        ...
        if int(authority.get("resource_scope", {}).get("memory_max_gib", -1)) != expected_memory_max_gib:
            raise RuntimeError("authority/CLI MemoryMax drift")
        ...
        initial = {
            "schema_version": "behavior-formal-goal-coupling-shared26-pi05-sync-checkpoint-save-qualification-v1",
            "object_id": OBJECT_ID,
            "child_id": CHILD_ID,
        ...
        try:
            ...
            final.update({
                ...
                "loss_values_read_or_reported": False,
                "formal_training_authorized": False,
                "next_gate": "FORMAL_RUN3_SYNCHRONOUS_CHECKPOINT_BARRIER_AUTHORITY_DESIGN" if status.endswith("PASS") else "SYNC_CHECKPOINT_SAVE_FAILURE_ADJUDICATION_NO_FORMAL_RESTART",
            })
