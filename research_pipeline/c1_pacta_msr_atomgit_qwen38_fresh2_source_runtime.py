...
PROVIDER_ID = "ATOMGIT_CODINGPLAN_ATOMCODE_QWEN38_HEADLESS_FRESH2_SOURCE_V1"
...
def _nonzero_jsonl_diagnostics(stdout: str) -> dict[str, Any]:
    """Audit model consumption even when AtomCode exits nonzero.

    The retired T0 showed that an output-truncation exit can still contain a full
    provider usage row and substantial model reasoning.  Fresh2 never retries a
    nonzero invocation, but the receipt must record whether model content was
    scientifically consumed rather than misclassifying the event as transport-only.
    """
    started_model = ""
    usage: dict[str, Any] = {}
    ...
    for raw in stdout.splitlines():
        ...
        if kind == "run.started":
            started_model = str(row.get("model") or "")
        elif kind == "usage":
            usage = row
            if int(row.get("completion_tokens") or 0) > 0:
                model_content_observed = True
        elif kind in {"message.delta", "reasoning.delta"} and str(row.get("text") or ""):
            model_content_observed = True
        elif kind == "retry" and str(row.get("kind") or "") == "output_truncation":
            output_truncation = True
        elif kind == "error":
            error_messages.append(str(row.get("message") or ""))
    return {
        "started_model": started_model,
        "usage": usage,
        "output_truncation": output_truncation,
        "model_content_observed": model_content_observed,
...
class AtomCodeSourceProvider:
    ...
    def call(self, messages: list[dict[str, str]], label: str) -> dict[str, Any]:
        ...
        if completed.returncode != 0:
            diagnostics = _nonzero_jsonl_diagnostics(completed.stdout)
            ...
            receipt = {
                **base,
                ...
                "started_model": diagnostics["started_model"],
                "usage": usage,
                "output_truncation": diagnostics["output_truncation"],
                "error_messages": diagnostics["error_messages"],
                "model_content_observed": diagnostics["model_content_observed"],
            }
            atomic_json(self.root / "calls" / f"{logical:04d}.json", receipt)
