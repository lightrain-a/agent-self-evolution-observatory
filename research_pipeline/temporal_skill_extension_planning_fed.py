...
PID=core.PAPER_ID;DATA=core.DATA_ROOT;SRC=DATA/'paper-acceptance'/'source-native-replay'/PID/'20260824-extension-fed-fomc-planning-prospective';BASE=SRC/'planning-base';PLAN=BASE/'plan.json';RESULTS=BASE/'results.csv';OWNER=PID+':EXT:FED-FOMC-PLANNING'
...
def prep():
 eps=json.load(open(SRC/'endpoints.json'))['endpoints'];plan={'schema_version':'1.0','paper_id':PID,'experiment':'E2_EXTENSION_FED_FOMC_PLANNING_BASE_PROSPECTIVE','endpoint_ids':[e['endpoint_id'] for e in eps],'model_identity':{'requested_model':MODEL,'required_resolved_model':RES,'required_plan_base_url':URL},'planned_model_calls':8,'selection_rule':'Prospective Fed FOMC planning-only arm on all four endpoints whose single-turn-vs-planning comparison was frozen before any Fed model outcomes; no helper and no endpoint selection.'};plan['plan_body_sha256']=csha({k:v for k,v in plan.items() if k!='plan_body_sha256'});atomic(PLAN,plan);return plan
def run():
