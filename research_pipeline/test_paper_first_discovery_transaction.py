...
class PaperFirstDiscoveryTransactionTest(unittest.TestCase):
    ...
    def test_success_commits_all_public_roles_with_one_transaction_id(self) -> None:
        ...
        self.assertEqual(payloads[2]["primary_evidence_pool"],"private-data://paper-first-problem-discovery/primary-evidence-pool.json")
        self.assertIn("private-data://paper-first-problem-discovery/auto-candidate-inbox.json",payloads[2]["source_inboxes"])
        self.assertNotIn(".paper-first-",json.dumps(payloads[2]))
    ...
    def test_typed_evidence_recompile_replays_archived_generator_with_zero_provider_calls(self) -> None:
        ...
        self.assertIn("private-data://paper-first-problem-discovery/auto-candidate-inbox.json",queue["source_inboxes"])
        self.assertNotIn(".paper-first-typed-recompile-",json.dumps(queue))
    ...
    def test_operator_recompile_reuses_exact_primary_without_source_scheduler(self) -> None:
        ...
        self.assertEqual(queue["primary_evidence_pool"],"private-data://paper-first-problem-discovery/primary-evidence-pool.json")
        self.assertIn("private-data://paper-first-problem-discovery/auto-candidate-inbox.json",queue["source_inboxes"])
        self.assertNotIn(".paper-first-recompile-",json.dumps(queue))
