...
from research_pipeline import run_c1_pacta_rb_qwen397_t05r3_t0_20260901 as runner
...
def test_new_epoch_preserves_both_prior_smokes_and_targets_new_t0_root():
 source=Path(runner.__file__).read_text()
 assert runner.ORIGINAL_SMOKE == runner.T05R_RUNTIME/"rootful-synthetic-smoke.json"
 assert "c1-pacta-rb-qwen397-t05r2-smoke-20260901-v1" in str(runner.PRIOR_SMOKE)
 assert "c1-pacta-rb-qwen397-t05r3-smoke-20260901-v1" in str(runner.SMOKE_EPOCH)
 assert "c1-pacta-rb-qwen397-t0-rootful-source-20260901-v3" in str(runner.DEFAULT)
 assert "prior_smoke_stop_preserved" in source


def test_final_termination_smoke_changes_only_ceiling_and_is_bounded():
 source=Path(runner.__file__).read_text()
 assert runner.SMOKE_STEP_CEILING == 16
 ...
 assert "increase only the synthetic smoke step ceiling from 8 to 16" in source
 assert "if 16 steps fail, stop without further ceiling expansion" in source
