...
class AppWorldConstraintF0PreflightTest(unittest.TestCase):
    ...
    def test_provider_summary_has_no_secret_and_readiness_is_honest(self) -> None:
        ...
        if readiness.get("capability_model_selection_state") == "SELECTED_MIMO25PRO_SQ0_V5_FINAL_INVALID_STOP_NO_V6":
            self.assertEqual(readiness["status"], "SQ0_V5_FINAL_CALIBRATION_INVALID_STOP_NO_V6")
            self.assertEqual(readiness["next_authorized_action"], "STOP_SQ0_DEVELOPMENT_REVIEW_TRANSPORT_ISOLATION_OR_RESEARCH_OBJECT")
            self.assertTrue(readiness["sq0_v5_closed"])
            self.assertEqual(readiness["sq0_v5_result_status"], "SQ0_V5_QUALIFICATION_INVALID_NON_SEMANTIC_FAILURE_STOP")
            self.assertEqual(readiness["sq0_v5_completed_case_count"], 6)
            self.assertEqual(readiness["sq0_v5_usable_target_failure_count"], 6)
            self.assertEqual(readiness["sq0_v5_target_success_count"], 0)
            self.assertEqual(readiness["sq0_v5_non_semantic_failure_units"], ["sq0v5:mimo-v2.5-pro|SQ0V5-TNF-01|1"])
            self.assertEqual(readiness["sq0_v5_closeout_status"], "SQ0_V5_FINAL_CALIBRATION_INVALID_STOP_NO_V6")
            self.assertEqual(readiness["sq0_v5_root_cause_status"], "SQ0_V5_RECURRENT_ATOMCODE_NATIVE_READ_FILE_CONTAMINATION_FINAL_STOP")
            self.assertEqual(readiness["sq0_v5_true_never_dispatched_case_count"], 5)
            self.assertEqual(readiness["sq0_v5_attempted_native_tool"], "read_file")
            self.assertFalse(readiness["sq0_v5_execution_authorized"])
            self.assertFalse(readiness["sq0_v6_execution_authorized"])
            ...
            self.assertFalse(readiness["p1_authorized"])
        elif readiness.get("capability_model_selection_state") == "SELECTED_MIMO25PRO_SQ0_V5_FINAL_CALIBRATION_AUTHORIZED_AFTER_V4_TOO_HARD":
