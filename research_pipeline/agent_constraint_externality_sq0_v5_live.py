...
from research_pipeline.agent_constraint_externality_sq0_v5_build import (
    CONTRACT_OUTPUT as STATIC_CONTRACT, OUTPUT_BUNDLE, QUAL_OUTPUT as STATIC_QUAL,
    TOOL_CALL_CAP, load_cases,
)
from research_pipeline.agent_constraint_externality_sq0_v5_cases import evaluate_case_from_state
...
MODEL_ROUND_CAP = 56
CASE_COUNT = 12
EXECUTION_ID = "CODINGPLAN-MIMO25PRO-SQ0-SEMANTIC-FAILURE-V5"
AUTH_OUTPUT = GENERATED / "agent-constraint-externality-sq0-v5-human-authorization-20260903.json"
Q1_OUTPUT = GENERATED / "agent-constraint-externality-sq0-v5-mimo25pro-mcp-q1-predispatch-20260903.json"
EXEC_CONTRACT = GENERATED / "agent-constraint-externality-sq0-v5-mimo25pro-execution-contract-20260903.json"
RESULT_OUTPUT = GENERATED / "agent-constraint-externality-sq0-v5-mimo25pro-result-20260903.json"
...
system_prompt = "This is an AppWorld simulation. Complete the target-local SQ0-V5 task using only tools whose names begin mcp__appworld__. Any path beginning ~/ is virtual AppWorld state and MUST be accessed through mcp__appworld__file_system__ tools; never call native read_file, write_file, edit_file, bash, web, skill, task, agent, or other host tools. Preserve unrelated AppWorld state. Batch independent AppWorld tool calls when safe."
[loop_config]
...
def _unit_id(case_id: str) -> str: return f"sq0v5:{MODEL_ID}|{case_id}|1"

def _freeze_inputs() -> tuple[dict[str,Any],dict[str,Any],dict[str,Any]]:
    static=_verified(STATIC_CONTRACT,"SQ0_V5_STATIC_DESIGN_READY_FINAL_CALIBRATION")
    qual=_verified(STATIC_QUAL,"SQ0_V5_PUBLIC_REACHABILITY_AND_FRESHNESS_PASS_FINAL_CALIBRATION")
    ...
    v4_closeout=_verified(V4_CLOSEOUT,"SQ0_V4_TOO_HARD_CLOSEOUT")
    v4_root=_verified(V4_ROOT_CAUSE,"SQ0_V4_TOO_HARD_WITH_SERIALIZATION_AND_SEMANTIC_FAILURE_MIX")
    ...
    if v4_closeout.get("authority",{}).get("sq0_v5_execution") is not False or v4_root.get("authority",{}).get("sq0_v5_execution") is not False:
        raise SQ0Stop("V4 closeout must not pre-authorize V5 execution.")
    if static.get("v4_closeout_content_sha256") != v4_closeout.get("content_sha256") or static.get("v4_root_cause_content_sha256") != v4_root.get("content_sha256"):
        raise SQ0Stop("V5 static design is not bound to the frozen V4 closeout/root cause.")
    if static.get("final_sq0_calibration_iteration") is not True or static.get("failure_to_pass_disposition") != "STOP_SQ0_DEVELOPMENT_NO_V6":
        raise SQ0Stop("V5 final-calibration stop rule drifted.")
...
def qualify_q1() -> dict[str,Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="ace-sq0-q1-") as d:
        ...
        try:
            ...
            state=_read(progress); payload={"schema_version":"ace-sq0-v5-mimo25pro-q1-v1","object_id":OBJECT_ID,"status":"SQ0_V5_MIMO25PRO_MCP_PREDISPATCH_PASS","case_id":case["case_id"],"model_profile":MODEL_PROFILE,"model_id":MODEL_ID,"session_mcp_progress_status":state.get("status"),"session_mcp_tool_count":int(state.get("tool_count",0)),"codingplan_window_used_before":used_before,"codingplan_window_used_after":used_after,"codingplan_model_requests":used_after-used_before,"scientific_dispatch_sent":False,"scientific_outcomes_observed":0,"runner_source_sha256":sha256_file(Path(__file__)),"bridge_source_sha256":sha256_file(ROOT/"research_pipeline/agent_constraint_externality_sq0_v5_mcp_bridge.py"),"bundle_sha256":sha256_file(OUTPUT_BUNDLE)}
...
def freeze() -> dict[str,Any]:
    ...
    auth={"schema_version":"ace-sq0-human-authorization-v1","object_id":OBJECT_ID,"status":"USER_AUTHORIZED_SQ0_V5_AFTER_V4_TOO_HARD_CLOSEOUT_AND_STATIC_PASS","authorization_basis":"USER_CONTINUATION_AFTER_SQ0_V4_TOO_HARD_CLOSEOUT_AND_SQ0_V5_STATIC_PASS","selected_backbone_content_sha256":selected["content_sha256"],"static_contract_content_sha256":static["content_sha256"],"static_qualification_content_sha256":qual["content_sha256"],"authority":{"sq0_v5_execution":True,"f0_r1":False,"probe":False,"p1":False,"toolsandbox":False,"appworld_ul":False,"paper_claim":False},"scientific_outcomes_observed":0}
    ...
    contract={"schema_version":"ace-sq0-v5-mimo25pro-execution-contract-v1","object_id":OBJECT_ID,"execution_id":EXECUTION_ID,"status":"SQ0_V5_MIMO25PRO_EXECUTION_AUTHORIZED","human_authorization_content_sha256":auth["content_sha256"],"q1_content_sha256":q1["content_sha256"],"static_contract_content_sha256":static["content_sha256"],"static_qualification_content_sha256":qual["content_sha256"],"selected_backbone_content_sha256":selected["content_sha256"],"inherited_transport_result_content_sha256":inherited_transport["content_sha256"],"transport_inherited_without_new_model_request":True,"model":{"profile":MODEL_PROFILE,"id":MODEL_ID,"context_window":CONTEXT_WINDOW,"retry_max_attempts":RETRY_MAX_ATTEMPTS},"harness":{"id":"ATOMCODE_CODINGPLAN_MCP_V1","sq0_bridge":True,"tool_call_cap":TOOL_CALL_CAP,"model_round_cap_per_case":MODEL_ROUND_CAP,"retry_allowed":False,"replacement_allowed":False,"ai_session_naming":False,"subagents":False},"panel":{"development_iteration":5,"case_count":CASE_COUNT,"case_ids":[c["case_id"] for c in load_cases()],"one_episode_per_case":True,"confirmatory_reuse":False},"gate":{"usable_failure_rate_min":0.75,"usable_failure_rate_max":0.90,"non_semantic_failure_invalidates_qualification":True},"execution_policy":{"durable_dispatch_before_model_request":True,"unknown_after_dispatch_replay":False,"partial_outcome_redesign":False,"aggregate_read_only_after_all_terminal_or_futility_proven":True,"failure_to_pass_disposition":"STOP_SQ0_DEVELOPMENT_NO_V6","futility_early_stop":{"acceptable_final_failure_counts":[9,10],"stop_too_easy_if_target_success_count_exceeds":3,"stop_too_hard_if_usable_failure_count_exceeds":10,"undispatched_cases_are_never_replaced":True}},"authority":{"sq0_v5_execution":True,"f0_r1":False,"probe":False,"p1":False,"toolsandbox":False,"appworld_ul":False,"paper_claim":False},"scientific_outcomes_observed":0}
...
def _futility_status(completions:list[dict[str,Any]])->str|None:
    ...
    if successes>3:
        return "SQ0_V5_FUTILITY_TOO_EASY_STOP"
    if failures>10:
        return "SQ0_V5_FUTILITY_TOO_HARD_STOP"
    return None

def execute(*,runtime_root:Path,ledger_path:Path)->None:
    _patch_live(); _verified(EXEC_CONTRACT,"SQ0_V5_MIMO25PRO_EXECUTION_AUTHORIZED"); cases=load_cases(); states=live.ledger_states(ledger_path); runtime_root=runtime_root.resolve(); ledger_path=ledger_path.resolve()
    for case in cases:
...
def adjudicate(ledger_path:Path)->dict[str,Any]:
    ...
    if failures_rows:
        status="SQ0_V5_QUALIFICATION_INVALID_NON_SEMANTIC_FAILURE_STOP"
    else:
        ...
        if set(states)==expected and len(completions)==CASE_COUNT:
            ...
            if invalid: status="SQ0_V5_QUALIFICATION_INVALID_NON_SEMANTIC_FAILURE_STOP"
            elif 0.75<=rate<=0.90: status="SQ0_V5_TARGET_FAILURE_QUALIFICATION_PASS"
            elif rate<0.75: status="SQ0_V5_TARGET_CHALLENGE_TOO_EASY_STOP"
            else: status="SQ0_V5_TARGET_CHALLENGE_TOO_HARD_STOP"
        elif futility in {"SQ0_V5_FUTILITY_TOO_EASY_STOP","SQ0_V5_FUTILITY_TOO_HARD_STOP","SQ0_V5_QUALIFICATION_INVALID_NON_SEMANTIC_FAILURE_STOP"}:
            status=futility
        else:
            raise SQ0Stop("Incomplete SQ0-V5 panel without preregistered futility proof.")
    ...
    result={"schema_version":"ace-sq0-v5-mimo25pro-result-v1","object_id":OBJECT_ID,"execution_id":EXECUTION_ID,"status":status,"provider":PROVIDER,"model_profile":MODEL_PROFILE,"model_id":MODEL_ID,"harness":"ATOMCODE_CODINGPLAN_MCP_V1","planned_case_count":CASE_COUNT,"completed_case_count":completed,"remaining_undispatched_case_count":remaining,"usable_target_failure_count":failure_count,"observed_usable_target_failure_rate":observed_rate,"target_success_count":success_count,"possible_final_failure_count_interval":[min_final_failures,max_final_failures],"acceptable_final_failure_counts":[9,10],"futility_proven":status.startswith("SQ0_V5_FUTILITY_") or status=="SQ0_V5_QUALIFICATION_INVALID_NON_SEMANTIC_FAILURE_STOP","non_semantic_failure_units":invalid+[r["unit_id"] for r in failures_rows],"scientific_model_round_count":sum(int(r.get("model_round_count",0)) for r in completions),"appworld_tool_call_total":sum(int(r.get("appworld_tool_call_count",0)) for r in completions),"prompt_tokens_total":sum(int(r.get("prompt_tokens_total",0)) for r in completions),"completion_tokens_total":sum(int(r.get("completion_tokens_total",0)) for r in completions),"ledger_sha256":sha256_file(ledger_path),"development_only":True,"development_iteration":5,"confirmatory_reuse":False,"scientific_effects_observed":0,"authority":{"sq0_v5_closed":True,"f0_r1":False,"probe":False,"p1":False,"toolsandbox":False,"appworld_ul":False,"paper_claim":False}}
