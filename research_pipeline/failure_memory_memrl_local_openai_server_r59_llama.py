"""Loopback-only OpenAI-compatible server for the R59 Llama executor replication on host 231.

The server changes only the executor backbone from the frozen Qwen run to a content-addressed Meta-Llama-3.1-8B-Instruct model. The MPNet embedding route remains available for interface compatibility. This module contains no benchmark logic.
"""
...
LLM_MODEL_ID = "B1-Meta-Llama-3.1-8B-Instruct-r59"
EMBED_MODEL_ID = "B1-all-mpnet-base-v2-isometric3072-r43"
LLM_ROOT = Path("/data/lry/models/Meta-Llama-3.1-8B-Instruct")
...
class Handler(BaseHTTPRequestHandler):
    runtime: Runtime
    server_version = "B1R59LlamaLocalOpenAI/1.0"
    ...
    def do_GET(self) -> None:
        if self.path.rstrip("/") in {"/v1/models", "/models"}:
            self._json(200, {"object": "list", "data": [
                {"id": LLM_MODEL_ID, "object": "model", "owned_by": "local-b1-r59"},
                {"id": EMBED_MODEL_ID, "object": "model", "owned_by": "local-b1-r59"},
            ]})
            return
...
def main() -> None:
    ...
    if args.host not in {"127.0.0.1", "localhost"}:
        raise SystemExit("B1 R59 Llama server is loopback-only")
