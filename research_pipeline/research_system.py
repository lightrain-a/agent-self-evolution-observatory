...
def validate_state(state: dict[str, Any]) -> list[str]:
    ...
    if architecture_summary.get("reader_chapters") != 7 or architecture_summary.get("reader_stage_coverage") != 11 or architecture_summary.get("reader_stage_missing") != 0 or architecture_summary.get("reader_stage_duplicates") != 0 or architecture_summary.get("reader_stage_extra") != 0: errors.append("system-overview reading groups must cover every canonical temporal stage exactly once")
    ...
    if api_memory.get("status")=="API_RESEARCH_MEMORY_READY" and (api_memory.get("scientific_authority") is not False or api_memory_policy.get("append_only_event_log") is not True or api_memory_policy.get("raw_output_archived_before_parse") is not True or api_memory_policy.get("successful_parse_is_persisted_immediately_before_completed_run_import") is not True or api_memory_policy.get("api_output_cannot_authorize_problem_gate_paper_method_experiment_p0_or_gpu") is not True or api_memory_policy.get("retrieval_is_search_context_not_scientific_verdict") is not True or api_memory_policy.get("memory_consumption_is_receipted_before_effectiveness_attribution") is not True or api_memory_policy.get("cross_run_identity_is_deterministic_exact_contract_not_semantic_authority") is not True or api_memory_policy.get("canonical_run_memory_missing_fails_closed") is not True): errors.append("API research memory must remain append-only, archive-before-parse, immediate-parse-persistent, fail-closed on canonical absence, and zero scientific authority")
    if api_memory.get("status")=="API_RESEARCH_MEMORY_READY" and int(api_memory_summary.get("scientific_identities") or 0)!=int(api_memory_summary.get("research_objects") or 0): errors.append("API research memory must identity-index every research object without semantic promotion")
