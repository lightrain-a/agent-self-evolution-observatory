...
from .asset_first_stri_paper_quality import build_asset_first_stri_paper_quality, write_asset_first_stri_paper_quality
...
from .research_control_plane import build_research_control_plane_state
...
EXTERNAL_FAILURE_ASSETS_JSON = PROJECT_ROOT / "research_pipeline" / "external_failure_assets.json"
...
def _load_external_failure_assets(path: Path = EXTERNAL_FAILURE_ASSETS_JSON) -> list[dict[str, Any]]:
    try:
        index = json.loads(path.read_text(encoding="utf-8"))
    except FileNotFoundError:
        return []
    except (OSError, json.JSONDecodeError, TypeError, ValueError) as error:
        raise ValueError(f"external failure asset index is invalid: {path}") from error
    if not isinstance(index, dict) or index.get("scientific_authority") is not False:
        raise ValueError("external failure asset index must be a zero-authority JSON object")
    rows: list[dict[str, Any]] = []
    root = PROJECT_ROOT.resolve()
    for binding in index.get("assets") or []:
        if not isinstance(binding, dict) or not str(binding.get("source_path") or "").strip():
            raise ValueError("external failure asset binding requires source_path")
        source = (PROJECT_ROOT / str(binding["source_path"])).resolve()
        if source != root and root not in source.parents:
            raise ValueError("external failure asset path escapes PROJECT_ROOT")
        try:
            payload = json.loads(source.read_text(encoding="utf-8"))
        except (OSError, json.JSONDecodeError, TypeError, ValueError) as error:
            raise ValueError(f"external failure asset source is invalid: {source}") from error
        key = str(binding.get("source_key") or "asset")
...
def _build_pre_experiment_state(storage: StorageSettings) -> dict[str, Any]:
    ...
    return {
        ...
        "summary": {
            ...
            "internal_skill_routes": sum(bool((row.get("research_execution_plan") or {}).get("internal_skill_route")) for row in cards),
            "internal_skill_routes_ready": sum(((row.get("research_execution_plan") or {}).get("internal_skill_route") or {}).get("status") == "INTERNAL_SKILL_ROUTE_READY" for row in cards),
...
def _component_manifest(state: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    manuscript_integrity = state["manuscript_integrity_layer"]["summary"]
    ...
    feynman_socratic = state["feynman_socratic_gate"]["summary"]
    reproduction_gate = state["reproduction_gate"]["summary"]
    reasoning_layer = state["research_reasoning_layer"]["summary"]
    figure_claim = state["figure_claim_graph"]["summary"]
    review_control = state["review_control"]["summary"]
    failure_differential = state["failure_differential_registry"]["summary"]
    research_control = state["research_control_plane"]["summary"]
    ...
    return [
        ...
        {"source":"Biomni / BioMedAgent / PaperQA2", "component":{"en":"Declarative research capability registry","zh":"声明式科研能力注册表"}, "status":"running", "evidence":{"en":f"{capabilities['capabilities']} typed capabilities / {capabilities['high_risk']} high-risk / {capabilities.get('external_skill_packs_distilled',0)} reviewed external packs distilled into {capabilities.get('canonical_internal_skills',0)} canonical local skills / runtime external dependencies={capabilities.get('external_skill_runtime_dependencies',0)} / zero skill authority","zh":f"{capabilities['capabilities']} 类 typed capability / {capabilities['high_risk']} 类高风险 / {capabilities.get('external_skill_packs_distilled',0)} 个外部 pack 已蒸馏为 {capabilities.get('canonical_internal_skills',0)} 个本地 canonical skill / 外部运行依赖={capabilities.get('external_skill_runtime_dependencies',0)} / skill 权限为 0"}},
        {"source":"AutoResearchBench / PaperQA2 / SciNetBench / ScientistOne / verifier calibration", "component":{"en":"Literature retrieval + Evidence Integrity layer","zh":"文献检索 + Evidence Integrity 层"}, "status":"running", "evidence":{"en":f"{literature_audit['retrieval_modes']} retrieval modes / {evidence_integrity['claim_types']} claim types / verifier {evidence_integrity['verifier_calibration_status']} / {reasoning_layer['contracts']} zero-authority reasoning contracts including literature depth + proximity + ambiguity routing","zh":f"{literature_audit['retrieval_modes']} 类检索模式 / {evidence_integrity['claim_types']} 类 claim / verifier {evidence_integrity['verifier_calibration_status']} / {reasoning_layer['contracts']} 个零权限 reasoning contract（含文献深浅、Proximity 与分析歧义路由）"}},
        ...
        {"source":"Advisor paper-first research contract", "component":{"en":"Paper novelty → method → experiment blueprint contract","zh":"论文 Novelty → 方法 → 实验蓝图合同"}, "status":"running", "evidence":{"en":f"{paper_first['paper_design_passed']}/{paper_first['cards']} live cards satisfy paper-first / Feynman replay {feynman_socratic['problem_novelty_reductions_detected']}/{feynman_socratic['problem_novelty_reductions']} typed novelty reductions with {feynman_socratic['false_reduction_alerts']} false alerts / reproduction falsifier {reproduction_gate['matched_cases']}/{reproduction_gate['local_cases']} / post-C2: {paper_post_c2['decision']}","zh":f"当前 {paper_first['paper_design_passed']}/{paper_first['cards']} 份 live 卡满足 paper-first / Feynman 历史回放识别 {feynman_socratic['problem_novelty_reductions_detected']}/{feynman_socratic['problem_novelty_reductions']} 个 typed novelty reduction、误报 {feynman_socratic['false_reduction_alerts']} / Reproduction 本地 falsifier {reproduction_gate['matched_cases']}/{reproduction_gate['local_cases']} / post-C2：{paper_post_c2['decision']}"}},
        ...
        {"source":"ARIS research wiki pattern + Notes2Skills + local typed closure", "component":{"en":"Typed Scientific Research Graph","zh":"类型化科学研究图谱"}, "status":"running", "evidence":{"en":f"{research_graph['nodes']} graph nodes / {research_graph['edges']} edges; memory wiki {research_memory['entries']} entries / certainty typed {research_memory.get('certainty_confirmed',0)+research_memory.get('certainty_supported',0)+research_memory.get('certainty_tentative',0)+research_memory.get('certainty_speculative',0)+research_memory.get('certainty_refuted',0)+research_memory.get('certainty_not_applicable',0)} / {research_memory.get('skill_candidates',0)} zero-authority skill candidates / {research_memory['failure_assets']} failure / {research_memory['success_assets']} success / lint warnings {research_memory_lint['warnings']}","zh":f"研究图谱 {research_graph['nodes']} 个节点 / {research_graph['edges']} 条边；Memory Wiki {research_memory['entries']} 条 / certainty typed {research_memory.get('certainty_confirmed',0)+research_memory.get('certainty_supported',0)+research_memory.get('certainty_tentative',0)+research_memory.get('certainty_speculative',0)+research_memory.get('certainty_refuted',0)+research_memory.get('certainty_not_applicable',0)} 条 / {research_memory.get('skill_candidates',0)} 个零权限 SkillCandidate / 失败 {research_memory['failure_assets']} / 成功 {research_memory['success_assets']} / lint warning {research_memory_lint['warnings']}"}},
        ...
        {"source":"ARIS / ResearchArena / AI-Scientist-v2 / reviewer-decision workflow", "component":{"en":"Paper acceptance closure and manuscript integrity","zh":"论文验收闭环与成稿完整性"}, "status":"running", "evidence":{"en":f"{paper_acceptance['paper_states']} paper states / {paper_acceptance['registered_papers']} ledgers / post-draft integrity {manuscript_integrity['audit_surfaces']} surfaces before independent review / FigureClaim {figure_claim['claims']} claims + {figure_claim['edges']} edges with {figure_claim['blockers']} blockers / review control {review_control['review_receipts']} receipts, {review_control['decision_critical_objections']} decision-critical objections, {review_control['targeted_experiment_proposals']} experiment proposals with zero execution authority","zh":f"{paper_acceptance['paper_states']} 个论文状态 / {paper_acceptance['registered_papers']} 个 ledger / 独立 review 前 {manuscript_integrity['audit_surfaces']} 类 post-draft integrity 检查 / FigureClaim {figure_claim['claims']} 个 claim + {figure_claim['edges']} 条边、blocker={figure_claim['blockers']} / review control {review_control['review_receipts']} 份 receipt、{review_control['decision_critical_objections']} 个 decision-critical objection、{review_control['targeted_experiment_proposals']} 个零执行权限实验建议"}},
        ...
        {"source":"Web GPT + domestic-model independent consultation", "component":{"en":"Five-checkpoint AI consultation clinic","zh":"五节点 AI 会诊诊断层"}, "status":"running", "evidence":{"en":f"{ai_clinic['checkpoints']} checkpoints / {ai_clinic['pre_gpu_checkpoints']} before GPU / differential shadow labels {failure_differential['historical_terminalized_labels']} / prospective {failure_differential['prospective_scored_cases']}/{failure_differential['minimum_prospective_replay_cases']} / zero AI-authoritative checkpoints","zh":f"{ai_clinic['checkpoints']} 个会诊节点 / {ai_clinic['pre_gpu_checkpoints']} 个位于 GPU 前 / 差分诊断 shadow 历史终态 {failure_differential['historical_terminalized_labels']} 条 / prospective {failure_differential['prospective_scored_cases']}/{failure_differential['minimum_prospective_replay_cases']} / AI 直接授权节点为 0"}},
        ...
        {"source":"AIDE / AI-Scientist-v2 / R&D-Agent / SCION", "component":{"en":"Updater prerequisite + derived Research Execution Plan + eight-gate Pre-Experiment Compiler","zh":"Updater 前置 + 派生 Research Execution Plan + 八门实验启动前编译器"}, "status":"running", "evidence":{"en":f"REP {pre_experiment['research_execution_plans']}/{pre_experiment['compiled_cards']} / internal skill routes {pre_experiment.get('internal_skill_routes_ready',0)}/{pre_experiment.get('internal_skill_routes',0)} / updater prerequisite {pre_experiment['updater_prerequisite_pass']}/{pre_experiment['compiled_cards']} / launch-ready {pre_experiment['execution_ready']}/{pre_experiment['compiled_cards']}","zh":f"REP {pre_experiment['research_execution_plans']}/{pre_experiment['compiled_cards']} / 本地 skill 路由 {pre_experiment.get('internal_skill_routes_ready',0)}/{pre_experiment.get('internal_skill_routes',0)} / Updater prerequisite {pre_experiment['updater_prerequisite_pass']}/{pre_experiment['compiled_cards']} / 可启动 {pre_experiment['execution_ready']}/{pre_experiment['compiled_cards']}"}},
        ...
        {"source":"ScienceFlow / AutoResearchEval / EurekAgent / TeLLAgent / Claw AI Lab", "component":{"en":"Recoverable research execution kernel","zh":"可恢复科研执行内核"}, "status":"running", "evidence":{"en":f"{execution_kernel['contracts']} shared contracts / control-plane checks {research_control['component_checks_passed']}/{research_control['component_checks_total']} / {research_control['experiment_tree_nodes']} visible experiment-tree nodes / {execution_kernel['legacy_atomic_configs_detected']} legacy atomic configs / zero automatic scientific, experiment, or GPU authority","zh":f"{execution_kernel['contracts']} 个公共合同 / Control Plane {research_control['component_checks_passed']}/{research_control['component_checks_total']} 检查通过 / {research_control['experiment_tree_nodes']} 个可见 ExperimentTree 节点 / {execution_kernel['legacy_atomic_configs_detected']} 份既有 atomic config / 自动科学、实验、GPU 权限均为 0"}},
...
def build_research_system_state() -> dict[str, Any]:
    ...
    manuscript_integrity_layer = build_manuscript_integrity_layer_state()
    ...
    asset_first_stri_paper_quality_state = build_asset_first_stri_paper_quality(PROJECT_ROOT)
    figure_claim_graph = build_figure_claim_graph(asset_first_stri_paper_quality_state)
    ...
    feynman_socratic_gate = build_feynman_socratic_gate_state(PROJECT_ROOT)
    reproduction_gate = build_reproduction_gate_state(PROJECT_ROOT)
    research_reasoning_layer = build_research_reasoning_layer_state()
    failure_differential_registry = build_sage_mhfa_shadow_state(PROJECT_ROOT)
    ...
    review_control = build_review_control_state_from_registry(paper_ledger_index)
    research_control_plane = build_research_control_plane_state(
        research_execution_kernel=research_execution_kernel,
        research_reasoning_layer=research_reasoning_layer,
        feynman_socratic_gate=feynman_socratic_gate,
        reproduction_gate=reproduction_gate,
        review_control=review_control,
        figure_claim_graph=figure_claim_graph,
        failure_differential_registry=failure_differential_registry,
        research_skill_registry=research_capability_registry,
        manuscript_integrity_layer=manuscript_integrity_layer,
        experiment_nodes=experiment_iteration.get("nodes") or [],
        research_states=[],
        governance_state=research_governance_v2,
        failure_asset_library=failure_asset_library,
        paper_registry_summary=paper_ledger_index.get("summary") or {},
    )
    ...
    state = {
        ...
        "summary":{
            ...
            "research_skill_packs_catalogued":research_capability_registry["summary"]["skill_packs_catalogued_not_installed"],
            "research_external_skill_packs_distilled":research_capability_registry["summary"].get("external_skill_packs_distilled",0),
            "research_canonical_internal_skills":research_capability_registry["summary"].get("canonical_internal_skills",0),
            "research_external_skill_runtime_dependencies":research_capability_registry["summary"].get("external_skill_runtime_dependencies",0),
            "research_skill_admission_contract_installed":research_capability_registry["summary"]["skill_admission_contract_installed"],
            "research_skill_router_contract_installed":research_capability_registry["summary"]["skill_router_contract_installed"],
            "manuscript_integrity_audit_surfaces":manuscript_integrity_layer["summary"]["audit_surfaces"],
            "manuscript_integrity_internal_skill_routes":manuscript_integrity_layer["summary"].get("internal_skill_routes",0),
            "manuscript_integrity_internal_skill_routes_ready":manuscript_integrity_layer["summary"].get("internal_skill_routes_ready",0),
            "manuscript_integrity_automatic_experiment_authority":manuscript_integrity_layer["summary"]["automatic_experiment_authority"],
            ...
            "feynman_socratic_replay_cases":feynman_socratic_gate["summary"]["replay_cases"],
            "feynman_socratic_false_reduction_alerts":feynman_socratic_gate["summary"]["false_reduction_alerts"],
            "reproduction_gate_local_cases":reproduction_gate["summary"]["local_cases"],
            "reproduction_gate_matched_cases":reproduction_gate["summary"]["matched_cases"],
            "research_reasoning_contracts":research_reasoning_layer["summary"]["contracts"],
            "research_reasoning_extensions":research_reasoning_layer["summary"].get("extensions",0),
            "contribution_attribution_replay_cases":research_reasoning_layer["summary"].get("contribution_replay_cases",0),
            "contribution_attribution_wrong_whole_paper_stops":research_reasoning_layer["summary"].get("contribution_replay_wrong_whole_paper_stops",0),
            "contribution_attribution_object_reduction_misses":research_reasoning_layer["summary"].get("contribution_replay_object_reduction_misses",0),
            "problem_first_shadow_generation_target":research_reasoning_layer["summary"].get("problem_first_shadow_generation_target",0),
            "figure_claim_graph_claims":figure_claim_graph["summary"]["claims"],
            "figure_claim_graph_blockers":figure_claim_graph["summary"]["blockers"],
            "review_control_papers":review_control["summary"]["papers"],
            "review_control_decision_critical_objections":review_control["summary"]["decision_critical_objections"],
            "failure_differential_historical_labels":failure_differential_registry["summary"]["historical_terminalized_labels"],
            "failure_differential_prospective_scored":failure_differential_registry["summary"]["prospective_scored_cases"],
            "failure_differential_gap_test_ready":1 if failure_differential_registry["summary"]["adoption_gap_test_ready"] else 0,
            "research_control_plane_status":research_control_plane["status"],
            "research_control_plane_checks_passed":research_control_plane["summary"]["component_checks_passed"],
            "research_control_plane_checks_total":research_control_plane["summary"]["component_checks_total"],
            ...
            "research_memory_certainty_typed":sum(int(research_memory_wiki["summary"].get(key) or 0) for key in ("certainty_confirmed","certainty_supported","certainty_tentative","certainty_speculative","certainty_refuted","certainty_not_applicable")),
            "research_memory_skill_candidates":int(research_memory_wiki["summary"].get("skill_candidates") or 0),
        ...
        "review_control":review_control,
        "failure_differential_registry":failure_differential_registry,
        "research_control_plane":research_control_plane,
...
def _health(state: dict[str, Any], corpus: dict[str, Any]) -> dict[str, Any]:
    ...
    checks = [
        ...
        {"key":"capability-and-literature-audit", "pass":state["research_capability_registry"]["policy"]["capabilities_are_declared_not_prompt_inferred"] and state["research_capability_registry"]["policy"]["model_or_tool_routing_cannot_escalate_scientific_authority"] and state["research_capability_registry"]["summary"]["capabilities"] >= 9 and ("canonical_internal_skills" not in state["research_capability_registry"]["summary"] or (state["research_capability_registry"]["summary"].get("canonical_internal_skills")==7 and state["research_capability_registry"]["summary"].get("external_skill_packs_distilled")==8 and state["research_capability_registry"]["summary"].get("external_skill_runtime_dependencies")==0 and state["research_capability_registry"]["summary"].get("skill_packs_catalogued_not_installed")==0)) and state["literature_retrieval_audit"]["policy"]["deep_and_wide_retrieval_are_distinct_capabilities"] and state["literature_retrieval_audit"]["policy"]["citation_verifier_must_be_named_versioned_and_calibrated"] and state["literature_retrieval_audit"]["summary"]["benchmark_status"] == "spec-ready-not-yet-scored" and state["evidence_integrity"]["policy"]["every_publishable_claim_requires_evidence_chain"] and state["evidence_integrity"]["policy"]["uncalibrated_verifier_cannot_be_treated_as_ground_truth"], "detail":{"capabilities":state["research_capability_registry"]["summary"],"literature":state["literature_retrieval_audit"]["summary"],"evidence_integrity":state["evidence_integrity"]["summary"]}},
        {"key":"research-skill-and-manuscript-integrity", "pass":state["research_capability_registry"]["policy"].get("third_party_skill_requires_admission_certificate_before_routing") is True and state["research_capability_registry"]["policy"].get("skill_router_selects_least_privilege_sufficient_skill") is True and int(state["research_capability_registry"]["summary"].get("automatic_skill_authority") or 0)==0 and state["manuscript_integrity_layer"].get("status")=="MANUSCRIPT_INTEGRITY_CONTRACTS_INSTALLED" and int(state["manuscript_integrity_layer"]["summary"].get("audit_surfaces") or 0)==7 and all(int(state["manuscript_integrity_layer"]["summary"].get(key) or 0)==0 for key in ("automatic_scientific_authority","automatic_experiment_authority","automatic_gpu_authority","automatic_submission_authority")), "detail":{"skills":state["research_capability_registry"]["summary"],"manuscript_integrity":state["manuscript_integrity_layer"]["summary"]}},
        {"key":"cross-cutting-methodology-controls", "pass":state["methodology_controls"]["summary"]["controls"] == 3 and state["methodology_controls"]["summary"]["primary_components_added"] == 0 and state["methodology_controls"]["summary"]["functional_layers_added"] == 0 and state["methodology_controls"]["summary"].get("registered_reviewer_gates") == 7 and state["methodology_controls"]["summary"].get("c1_reviewer_gate_loaded") is True and state["methodology_controls"]["summary"].get("c1_reviewer_gate_historical_d0_design_eligible") is True and state["methodology_controls"]["summary"].get("c1_reviewer_gate_d0_design_eligible") is False and state["methodology_controls"]["summary"].get("c1_reviewer_gate_downstream_authority") is False and state["methodology_controls"]["summary"].get("c1_d0b_structural_observation_loaded") is True and state["methodology_controls"]["summary"].get("c1_d0b_structurally_feasible") is True and state["methodology_controls"]["summary"].get("c1_d0b_semantic_authority") is False and state["methodology_controls"]["summary"].get("c1_d0b_downstream_authority") is False and state["methodology_controls"]["summary"].get("c1_d0b_claim_binding_observation_loaded") is True and state["methodology_controls"]["summary"].get("c1_d0b_claim_binding_envelope_feasible") is True and state["methodology_controls"]["summary"].get("c1_d0b_claim_binding_ready") is False and state["methodology_controls"]["summary"].get("c1_d0b_claim_binding_semantic_authority") is False and state["methodology_controls"]["summary"].get("c1_d0b_claim_binding_downstream_authority") is False and state["methodology_controls"]["summary"].get("c1_d0b1_identifiability_observation_loaded") is True and state["methodology_controls"]["summary"].get("c1_d0b1_operational_contrast_identifiable") is True and state["methodology_controls"]["summary"].get("c1_d0b1_causal_atom_purity_certified") is False and state["methodology_controls"]["summary"].get("c1_d0b1_semantic_authority") is False and state["methodology_controls"]["summary"].get("c1_d0b1_downstream_authority") is False and state["methodology_controls"]["summary"].get("c1_d0b1c_locator_observation_loaded") is True and state["methodology_controls"]["summary"].get("c1_d0b1c_compiler_ready") is True and state["methodology_controls"]["summary"].get("c1_d0b1c_locator_partial_fail_closed") is True and state["methodology_controls"]["summary"].get("c1_d0b1c_semantic_authority") is False and state["methodology_controls"]["summary"].get("c1_d0b1c_downstream_authority") is False and state["methodology_controls"]["summary"].get("c1_d0b2_readiness_observation_loaded") is True and state["methodology_controls"]["summary"].get("c1_d0b2_readiness_hold_valid") is True and state["methodology_controls"]["summary"].get("c1_d0b2_semantic_adjudicator_ready") is False and state["methodology_controls"]["summary"].get("c1_d0b2_semantic_authority") is False and state["methodology_controls"]["summary"].get("c1_d0b2_downstream_authority") is False and state["methodology_controls"]["summary"].get("c1_d0b2_closure_observation_loaded") is True and state["methodology_controls"]["summary"].get("c1_d0b2_terminal_closure_valid") is True and state["methodology_controls"]["summary"].get("c1_d0b2_current_extension_stopped") is True and state["methodology_controls"]["summary"].get("c1_d0b2_measurement_paper_retained") is True and state["methodology_controls"]["summary"].get("c1_d0b2_closure_scientific_failure") is False and state["methodology_controls"]["summary"].get("c1_d0b2_closure_automatic_reopen") is False and state["methodology_controls"]["summary"].get("c1_d0b2_closure_downstream_authority") is False and state["methodology_controls"]["policy"]["post_outcome_protocol_changes_require_a_new_registered_contract"] and state["methodology_controls"]["policy"]["search_or_tool_access_must_not_leak_hidden_evaluation_answers"] and state["methodology_controls"]["policy"]["reproducibility_requires_reexecution_not_only_trace_presence"] and state["methodology_controls"]["policy"].get("baseline_demoted_components_cannot_reenter_method_novelty_without_new_collision_evidence") is True and state["methodology_controls"]["policy"].get("c1_d0_design_gate_cannot_authorize_fresh_execution") is True and state["methodology_controls"]["policy"].get("packet_level_evidence_hash_is_not_claim_level_evidence_binding") is True and state["methodology_controls"]["policy"].get("candidate_memory_atom_cannot_be_called_residual_claim_without_branch_residual_identity") is True and state["methodology_controls"]["policy"].get("claim_specific_evidence_refs_required_before_semantic_validity") is True and state["methodology_controls"]["policy"].get("semantic_validity_required_before_nonzero_branch_authority") is True and state["methodology_controls"]["policy"].get("operational_branch_contrast_is_not_atom_level_causal_purity") is True and state["methodology_controls"]["policy"].get("causal_residual_language_requires_seed_or_same_condition_noise_floor_control") is True and state["methodology_controls"]["policy"].get("evidence_locator_similarity_cannot_grant_semantic_support") is True and state["methodology_controls"]["policy"].get("unlocated_claims_cannot_receive_imputed_evidence_authority") is True and state["methodology_controls"]["policy"].get("semantic_validity_requires_a_qualified_content_addressed_adjudicator") is True and state["methodology_controls"]["policy"].get("generic_nli_model_existence_without_task_specific_qualification_is_not_validity_authority") is True and state["methodology_controls"]["policy"].get("absence_of_qualified_semantic_adjudicator_is_readiness_hold_not_scientific_failure") is True and state["methodology_controls"]["policy"].get("c1_current_cbrg_extension_stop_merge_does_not_invalidate_measurement_paper") is True and state["methodology_controls"]["policy"].get("c1_stopped_extension_cannot_reopen_on_renamed_baseline_or_generic_nli") is True and state["methodology_controls"]["policy"].get("c1_extension_reopen_requires_new_qualified_content_addressed_evidence") is True and state["methodology_controls"]["policy"].get("c1_terminal_closure_overrides_historical_d0_design_eligibility") is True, "detail":state["methodology_controls"]["summary"]},
        ...
        {"key":"paper-first-research-contract", "pass":state["paper_first_workflow"]["policy"]["paper_novelty_precedes_method_design"] and state["paper_first_workflow"]["policy"]["method_design_precedes_experiment_blueprint"] and state["paper_first_workflow"]["policy"]["local_validation_is_for_falsification_not_method_discovery"] and state["paper_first_workflow"]["policy"]["full_experiment_requires_frozen_method_and_blueprint"] and state["paper_first_workflow"]["policy"]["paper_development_quality_v1_distinguishes_scientific_closure_from_manuscript_maturity"] and state["paper_first_workflow"]["policy"]["paper_development_quality_v1_required_for_schema_2_4_plus_or_explicit_contract"] and state["paper_first_workflow"]["policy"]["current_initial_drafts_are_not_retroactively_demoted_by_new_development_guidance"], "detail":state["paper_first_workflow"]["summary"]},
        ...
        {"key":"research-vnext-reasoning-and-control", "pass":state["feynman_socratic_gate"].get("status")=="Feynman_Socratic_GATE_INSTALLED" and state["feynman_socratic_gate"].get("scientific_authority") is False and state["feynman_socratic_gate"]["summary"].get("replay_cases")==20 and state["feynman_socratic_gate"]["summary"].get("false_reduction_alerts")==0 and state["reproduction_gate"].get("status")=="REPRODUCTION_GATE_INSTALLED" and state["reproduction_gate"]["summary"].get("matched_cases")==3 and state["research_reasoning_layer"].get("status")=="REASONING_CONTRACTS_INSTALLED" and state["research_reasoning_layer"]["summary"].get("automatic_scientific_authority")==0 and state["figure_claim_graph"].get("status")=="PASS_FIGURE_CLAIM_GRAPH" and state["figure_claim_graph"]["summary"].get("blockers")==0 and state["review_control"].get("status")=="REVIEW_CONTROL_STATE_COMPILED" and state["review_control"]["summary"].get("automatic_memory_promotions")==0 and state["failure_differential_registry"].get("status") in {"SHADOW_REGISTRY_READY_PROSPECTIVE_REPLAY_PENDING","READY_FOR_SAGE_MHFA_GAP_TEST"} and state["failure_differential_registry"]["summary"].get("historical_terminalized_labels",0)>=15 and state["failure_differential_registry"]["summary"].get("retrospective_label_leakage_allowed")==0 and state["failure_differential_registry"].get("scientific_authority") is False and state["research_control_plane"].get("status")=="CONTROL_PLANE_READY" and state["research_control_plane"]["summary"].get("component_checks_passed")==state["research_control_plane"]["summary"].get("component_checks_total") and all(state["research_control_plane"]["summary"].get(key)==0 for key in ("automatic_scientific_authority","automatic_experiment_authority","automatic_gpu_authority")), "detail":{"feynman":state["feynman_socratic_gate"]["summary"],"reproduction":state["reproduction_gate"]["summary"],"reasoning":state["research_reasoning_layer"]["summary"],"figure_claim":state["figure_claim_graph"]["summary"],"review":state["review_control"]["summary"],"failure_differential":state["failure_differential_registry"]["summary"],"control":state["research_control_plane"]["summary"]}},
        {"key":"research-vnext1-contribution-aware", "pass":state["research_reasoning_layer"].get("status")=="REASONING_CONTRACTS_INSTALLED" and int((state["research_reasoning_layer"].get("summary") or {}).get("extensions") or 0)==4 and int((state["research_reasoning_layer"].get("summary") or {}).get("contribution_replay_cases") or 0)==40 and int((state["research_reasoning_layer"].get("summary") or {}).get("contribution_replay_wrong_whole_paper_stops") or 0)==0 and int((state["research_reasoning_layer"].get("summary") or {}).get("contribution_replay_object_reduction_misses") or 0)==0 and int((state["research_reasoning_layer"].get("summary") or {}).get("problem_first_shadow_generation_target") or 0)==120 and int((state["research_reasoning_layer"].get("summary") or {}).get("prospective_contribution_shadow_minimum") or 0)==20 and int((state["research_reasoning_layer"].get("summary") or {}).get("automatic_live_gate_migrations") or 0)==0 and ((state["research_reasoning_layer"].get("prospective_contribution_shadow_protocol") or {}).get("automatic_migration") is False) and ((state["paper_first_problem_discovery_contract"].get("problem_first_shadow") or {}).get("live_discovery_operator_changed") is False) and int((state["paper_first_problem_discovery_contract"].get("problem_first_shadow") or {}).get("provider_calls_authorized") or 0)==0 and int((state["research_memory_wiki"].get("summary") or {}).get("contribution_aware_lesson_templates") or 0)==2 and all((row or {}).get("scientific_authority") is False for row in (state["research_memory_wiki"].get("lesson_templates") or {}).values()), "detail":{"reasoning":state["research_reasoning_layer"].get("summary"),"problem_first":state["paper_first_problem_discovery_contract"].get("problem_first_shadow"),"memory_templates":list((state["research_memory_wiki"].get("lesson_templates") or {}).keys())}},
...
def validate_state(state: dict[str, Any]) -> list[str]:
    ...
    skill_policy=state["research_capability_registry"].get("policy") or {};skill_summary=state["research_capability_registry"].get("summary") or {}
    skill_integrity_required=bool(state.get("manuscript_integrity_layer")) or "research_skill_packs_catalogued" in (state.get("summary") or {})
    if skill_integrity_required:
        if skill_policy.get("third_party_skill_requires_admission_certificate_before_routing") is not True or skill_policy.get("skill_router_selects_least_privilege_sufficient_skill") is not True or skill_policy.get("skill_pack_orchestrator_never_becomes_a_second_research_os") is not True: errors.append("research skill supply chain must require admission and least-privilege routing without a second Research OS")
        internalized_skill_contract="canonical_internal_skills" in skill_summary or bool(state["research_capability_registry"].get("internal_skill_library"))
        if internalized_skill_contract:
            internal_library=state["research_capability_registry"].get("internal_skill_library") or {};internal_summary=internal_library.get("summary") or {};internal_policy=internal_library.get("policy") or {}
            if internal_library.get("status")!="INTERNAL_RESEARCH_SKILLS_READY" or int(skill_summary.get("external_skill_packs_distilled") or 0)!=8 or int(skill_summary.get("canonical_internal_skills") or 0)!=7 or int(skill_summary.get("skill_packs_catalogued_not_installed") or 0)!=0 or int(skill_summary.get("external_skill_runtime_dependencies") or 0)!=0: errors.append("the eight reviewed external skill packs must be fully distilled into seven canonical local skills with zero runtime dependency")
            if int(internal_summary.get("external_skill_packs_left_catalogued") or 0)!=0 or int(internal_summary.get("external_runtime_dependencies") or 0)!=0 or internal_policy.get("external_skill_packs_are_design_inputs_not_runtime_dependencies") is not True or internal_policy.get("internal_skill_cannot_become_a_second_research_orchestration_authority") is not True: errors.append("distilled internal skills must replace the external catalog without creating a second Research OS")
        elif int(skill_summary.get("skill_packs_catalogued_not_installed") or 0)<1:
            errors.append("legacy external research skill catalog must not disappear before the internal distillation contract exists")
        if int(skill_summary.get("automatic_skill_authority") or 0)!=0: errors.append("research skills must have zero automatic authority")
        manuscript_integrity=state.get("manuscript_integrity_layer") or {};mi_policy=manuscript_integrity.get("policy") or {};mi_summary=manuscript_integrity.get("summary") or {}
        if manuscript_integrity.get("status")!="MANUSCRIPT_INTEGRITY_CONTRACTS_INSTALLED" or manuscript_integrity.get("scientific_authority") is not False or int(mi_summary.get("audit_surfaces") or 0)!=7: errors.append("post-draft manuscript integrity layer must expose seven zero-authority audit surfaces")
        if "internal_skill_routes" in mi_summary and (int(mi_summary.get("internal_skill_routes") or 0)!=3 or int(mi_summary.get("internal_skill_routes_ready") or 0)!=3 or int(mi_summary.get("external_skill_runtime_dependencies") or 0)!=0): errors.append("manuscript integrity must route drafting, citation integrity, and theory audit through canonical internal skills")
        if mi_policy.get("post_draft_integrity_precedes_independent_model_review") is not True or mi_policy.get("every_publishable_number_requires_content_addressed_source_binding") is not True or mi_policy.get("every_table_cell_requires_source_field_provenance") is not True or mi_policy.get("machine_like_prose_lint_is_editorial_not_ai_detector_evasion") is not True: errors.append("post-draft integrity must verify artifact-backed content before reviewer prose repair")
        if any(int(mi_summary.get(key) or 0)!=0 for key in ("automatic_scientific_authority","automatic_experiment_authority","automatic_gpu_authority","automatic_submission_authority")): errors.append("manuscript integrity audit must not grant automatic authority")
    ...
    if methodology_summary.get("registered_reviewer_gates") != 7 or methodology_summary.get("c1_reviewer_gate_loaded") is not True: errors.append("C1 executable-closure reviewer gates must be registered and bound to the versioned revision program")
    if methodology_summary.get("c1_reviewer_gate_historical_d0_design_eligible") is not True: errors.append("C1 historical executable-closure gate must preserve the earlier D0 design qualification")
    if methodology_summary.get("c1_reviewer_gate_d0_design_eligible") is not False: errors.append("C1 terminal closure must override current D0 design eligibility")
    if methodology_summary.get("c1_reviewer_gate_downstream_authority") is not False: errors.append("C1 D0 reviewer gate must not grant scientific/provider/GPU/claim/submission authority")
    if methodology_summary.get("c1_d0b_structural_observation_loaded") is not True or methodology_summary.get("c1_d0b_structurally_feasible") is not True: errors.append("C1 D0-B structural receipt observation must be loaded and pass its content-addressed audit")
    if methodology_summary.get("c1_d0b_semantic_authority") is not False or methodology_summary.get("c1_d0b_downstream_authority") is not False: errors.append("C1 D0-B structural GO must preserve semantic-authority HOLD and zero downstream authority")
    if methodology_summary.get("c1_d0b_claim_binding_observation_loaded") is not True or methodology_summary.get("c1_d0b_claim_binding_envelope_feasible") is not True: errors.append("C1 D0-B claim-binding correction must be loaded and preserve receipt-envelope integrity")
    if methodology_summary.get("c1_d0b_claim_binding_ready") is not False or methodology_summary.get("c1_d0b_claim_binding_semantic_authority") is not False or methodology_summary.get("c1_d0b_claim_binding_downstream_authority") is not False: errors.append("C1 D0-B claim binding must remain HOLD with zero semantic/downstream authority")
    if methodology_summary.get("c1_d0b1_identifiability_observation_loaded") is not True or methodology_summary.get("c1_d0b1_operational_contrast_identifiable") is not True: errors.append("C1 D0-B1 must preserve the operational same-trajectory branch contrast")
    if methodology_summary.get("c1_d0b1_causal_atom_purity_certified") is not False or methodology_summary.get("c1_d0b1_semantic_authority") is not False or methodology_summary.get("c1_d0b1_downstream_authority") is not False: errors.append("C1 D0-B1 atom-level causal purity and semantic/downstream authority must remain HOLD")
    if methodology_summary.get("c1_d0b1c_locator_observation_loaded") is not True or methodology_summary.get("c1_d0b1c_compiler_ready") is not True or methodology_summary.get("c1_d0b1c_locator_partial_fail_closed") is not True: errors.append("C1 D0-B1c operational compiler/partial locator must remain bound and fail-closed")
    if methodology_summary.get("c1_d0b1c_semantic_authority") is not False or methodology_summary.get("c1_d0b1c_downstream_authority") is not False: errors.append("C1 D0-B1c locator must not grant semantic or downstream authority")
    if methodology_summary.get("c1_d0b2_readiness_observation_loaded") is not True or methodology_summary.get("c1_d0b2_readiness_hold_valid") is not True: errors.append("C1 D0-B2 semantic-readiness HOLD must remain content-addressed and valid")
    if methodology_summary.get("c1_d0b2_semantic_adjudicator_ready") is not False or methodology_summary.get("c1_d0b2_semantic_authority") is not False or methodology_summary.get("c1_d0b2_downstream_authority") is not False: errors.append("C1 D0-B2 must not advance semantic/downstream authority without a qualified adjudicator")
    if methodology_summary.get("c1_d0b2_closure_observation_loaded") is not True or methodology_summary.get("c1_d0b2_terminal_closure_valid") is not True or methodology_summary.get("c1_d0b2_current_extension_stopped") is not True: errors.append("C1 current CBRG extension must remain terminally STOP/MERGE after bounded adjudicator exhaustion")
    if methodology_summary.get("c1_d0b2_measurement_paper_retained") is not True or methodology_summary.get("c1_d0b2_closure_scientific_failure") is not False: errors.append("C1 method-extension closure must retain the measurement paper without declaring scientific failure")
    if methodology_summary.get("c1_d0b2_closure_automatic_reopen") is not False or methodology_summary.get("c1_d0b2_closure_downstream_authority") is not False: errors.append("C1 terminal closure must not auto-reopen or grant downstream authority")
    if methodology_policy.get("packet_level_evidence_hash_is_not_claim_level_evidence_binding") is not True: errors.append("packet-level evidence hashes must not be treated as claim-level evidence binding")
    if methodology_policy.get("candidate_memory_atom_cannot_be_called_residual_claim_without_branch_residual_identity") is not True: errors.append("candidate memory atoms require certified branch-residual identity before claim status")
    if methodology_policy.get("claim_specific_evidence_refs_required_before_semantic_validity") is not True: errors.append("claim-specific evidence refs must precede semantic validity")
    if methodology_policy.get("semantic_validity_required_before_nonzero_branch_authority") is not True: errors.append("semantic validity must precede nonzero branch authority")
    if methodology_policy.get("operational_branch_contrast_is_not_atom_level_causal_purity") is not True: errors.append("operational branch contrast must remain distinct from atom-level causal residual purity")
    if methodology_policy.get("causal_residual_language_requires_seed_or_same_condition_noise_floor_control") is not True: errors.append("atom-level causal residual language must require seed binding or same-condition noise-floor control")
    if methodology_policy.get("evidence_locator_similarity_cannot_grant_semantic_support") is not True: errors.append("evidence locator similarity must not grant semantic support")
    if methodology_policy.get("unlocated_claims_cannot_receive_imputed_evidence_authority") is not True: errors.append("unlocated claims must remain fail-closed without imputed evidence authority")
    if methodology_policy.get("semantic_validity_requires_a_qualified_content_addressed_adjudicator") is not True: errors.append("semantic validity must require a qualified content-addressed adjudicator")
    if methodology_policy.get("generic_nli_model_existence_without_task_specific_qualification_is_not_validity_authority") is not True: errors.append("generic NLI model existence must not grant validity authority without task-specific qualification")
    if methodology_policy.get("absence_of_qualified_semantic_adjudicator_is_readiness_hold_not_scientific_failure") is not True: errors.append("missing semantic adjudicator must be represented as readiness HOLD rather than scientific failure")
    if methodology_policy.get("c1_current_cbrg_extension_stop_merge_does_not_invalidate_measurement_paper") is not True: errors.append("C1 CBRG STOP/MERGE must not invalidate the stage-resolved measurement paper")
    if methodology_policy.get("c1_stopped_extension_cannot_reopen_on_renamed_baseline_or_generic_nli") is not True: errors.append("C1 stopped extension must not reopen from renamed baselines or generic NLI")
    if methodology_policy.get("c1_extension_reopen_requires_new_qualified_content_addressed_evidence") is not True: errors.append("C1 extension reopen must require new qualified content-addressed evidence")
    if methodology_policy.get("c1_terminal_closure_overrides_historical_d0_design_eligibility") is not True: errors.append("C1 terminal closure must override historical D0 design eligibility")
    if methodology_policy.get("baseline_demoted_components_cannot_reenter_method_novelty_without_new_collision_evidence") is not True: errors.append("demoted baseline components must not re-enter novelty without new collision evidence")
    if methodology_policy.get("c1_d0_design_gate_cannot_authorize_fresh_execution") is not True: errors.append("C1 D0 design gate must not authorize fresh execution")
    ...
    workflow_schema=str(state["paper_first_workflow"].get("schema_version") or "1.0")
    if workflow_schema!="1.0":
        workflow_policy=state["paper_first_workflow"].get("policy") or {}
        if workflow_policy.get("paper_development_quality_v1_distinguishes_scientific_closure_from_manuscript_maturity") is not True: errors.append("paper development must distinguish scientific closure from manuscript maturity")
        if workflow_policy.get("paper_development_quality_v1_required_for_schema_2_4_plus_or_explicit_contract") is not True: errors.append("schema 2.4+ Paper Design must require Paper Development Quality v1")
        if workflow_policy.get("current_initial_drafts_are_not_retroactively_demoted_by_new_development_guidance") is not True: errors.append("new paper-development guidance must not retroactively demote existing scientific PaperStates")
    ...
    if paper_backlog.get("status")!="NOT_RUN":
        ...
        if backlog_entries and (backlog_policy.get("paper_design_memory_precheck_required_for_pending_entries") is not True or backlog_policy.get("paper_design_memory_precheck_is_zero_authority") is not True or backlog_policy.get("paper_review_memory_is_context_not_scientific_evidence") is not True): errors.append("Paper-Design backlog must bind a zero-authority Research Memory precheck before human review")
        ...
        if int(backlog_summary.get("memory_prechecks") or 0)!=sum(bool(row.get("paper_design_memory_precheck")) for row in backlog_entries): errors.append("Paper-Design backlog memory-precheck accounting mismatch")
        if any((row.get("paper_design_memory_precheck") or {}).get("purpose")!="PAPER_DESIGN" or (row.get("paper_design_memory_precheck") or {}).get("scientific_authority") is not False or not re.fullmatch(r"[0-9a-f]{64}",str((row.get("paper_design_memory_precheck") or {}).get("query_pack_sha256") or "")) for row in pending_backlog): errors.append("every pending Paper-Design entry must carry a content-addressed zero-authority PAPER_DESIGN memory precheck")
    ...
    if pa_policy.get("new_story_search_receipts_bind_paper_design_memory_precheck") is not True or pa_policy.get("paper_design_memory_precheck_is_context_not_authority") is not True: errors.append("new Story Search receipts must bind zero-authority PAPER_DESIGN Research Memory context")
    ...
    post_draft_contract_required="post_draft_integrity_contract_installed" in pa_summary
    if post_draft_contract_required:
        if pa_policy.get("post_draft_integrity_audit_precedes_independent_model_review_for_new_workflows") is not True or pa_policy.get("post_draft_integrity_is_receipt_backed_not_reviewer_self_attestation") is not True or pa_policy.get("legacy_paper_ledgers_are_not_retroactively_invalidated_by_new_post_draft_integrity_contract") is not True: errors.append("new manuscript workflows must add receipt-backed post-draft integrity without retroactively invalidating legacy ledgers")
        if int(pa_summary.get("post_draft_integrity_contract_installed") or 0)!=1 or int(pa_summary.get("post_draft_integrity_automatic_experiment_authority") or 0)!=0: errors.append("Paper Acceptance must expose one zero-experiment-authority post-draft integrity contract")
    ...
    skill_routed_rep=state["pre_experiment_compiler"]["policy"].get("internal_research_skill_route_is_frozen_inside_execution_plan") is True
    if skill_routed_rep:
        if state["pre_experiment_compiler"]["policy"].get("internal_skill_route_cannot_authorize_execution") is not True or state["pre_experiment_compiler"]["policy"].get("missing_declared_internal_skill_is_capability_hold_not_scientific_negative") is not True: errors.append("Research Execution Plan internal skill routing must remain zero-authority capability control")
        if int(state["pre_experiment_compiler"]["summary"].get("internal_skill_routes") or 0)!=expected_pre_experiment_cards or int(state["pre_experiment_compiler"]["summary"].get("internal_skill_routes_ready") or 0)!=expected_pre_experiment_cards or int(state["pre_experiment_compiler"]["summary"].get("external_skill_runtime_dependencies") or 0)!=0: errors.append("all current Research Execution Plans must resolve canonical internal skills with zero external runtime dependency")
    ...
    failure_differential=state.get("failure_differential_registry") or {}; fd_policy=failure_differential.get("policy") or {}; fd_summary=failure_differential.get("summary") or {}
    fd_required=bool(failure_differential) or "failure_differential_historical_labels" in (state.get("summary") or {})
    if fd_required:
        if failure_differential.get("status") not in {"SHADOW_REGISTRY_READY_PROSPECTIVE_REPLAY_PENDING","READY_FOR_SAGE_MHFA_GAP_TEST"} or failure_differential.get("scientific_authority") is not False: errors.append("failure differential registry must remain a zero-authority prospective shadow registry")
        if int(fd_summary.get("historical_terminalized_labels") or 0)<15 or int(fd_summary.get("retrospective_label_leakage_allowed") or 0)!=0: errors.append("failure differential registry must inventory enough typed terminal labels without allowing hindsight backfill")
        if fd_policy.get("hypotheses_must_be_frozen_before_final_failure_layer_is_visible") is not True or fd_policy.get("historical_final_labels_cannot_be_used_to_retroactively_generate_hypotheses") is not True: errors.append("failure differential hypotheses must be prospective and label-blind")
    ...
    memory_certainty_required=str(memory_wiki.get("schema_version") or "1.0")>="1.2" or str(state.get("schema_version") or "1.0")>="1.1"
    if memory_certainty_required and ((memory_wiki.get("certainty_audit") or {}).get("status")!="PASS" or memory_policy.get("certainty_is_preserved_before_memory_can_be_reused_as_instruction") is not True or memory_policy.get("automatic_memory_to_skill_promotion_is_forbidden") is not True): errors.append("research memory must preserve certainty and forbid automatic memory-to-skill promotion")
    reasoning=state.get("research_reasoning_layer") or {};reasoning_policy=reasoning.get("policy") or {};reasoning_summary=reasoning.get("summary") or {};reasoning_replay=reasoning.get("contribution_aware_replay") or {};prospective_contribution=reasoning.get("prospective_contribution_shadow_protocol") or {}
    vnext1_required=int(reasoning_summary.get("extensions") or 0)>0 or "research_reasoning_extensions" in (state.get("summary") or {})
    if vnext1_required:
        if reasoning.get("status")!="REASONING_CONTRACTS_INSTALLED" or int(reasoning_summary.get("extensions") or 0)!=4: errors.append("vNext.1 contribution-aware reasoning must install four zero-authority extensions without adding a top-level component")
        if reasoning_policy.get("method_complexity_is_not_paper_contribution") is not True or reasoning_policy.get("simple_baseline_dominance_only_reduces_claim_layers_it_reproduces") is not True or reasoning_policy.get("method_reduction_does_not_imply_scientific_object_reduction") is not True: errors.append("vNext.1 must distinguish method complexity/reduction from paper and scientific-object contribution")
        if reasoning_replay.get("status")!="PASS" or int(reasoning_summary.get("contribution_replay_cases") or 0)!=40 or int(reasoning_summary.get("contribution_replay_wrong_whole_paper_stops") or 0)!=0 or int(reasoning_summary.get("contribution_replay_object_reduction_misses") or 0)!=0: errors.append("vNext.1 contribution attribution must pass the frozen 40-case retrospective regression with zero wrong whole-paper stops and zero object-reduction misses")
        if int(reasoning_summary.get("problem_first_shadow_generation_target") or 0)!=120 or int(reasoning_summary.get("prospective_contribution_shadow_minimum") or 0)!=20 or int(reasoning_summary.get("automatic_live_gate_migrations") or 0)!=0 or prospective_contribution.get("automatic_migration") is not False or prospective_contribution.get("live_problem_gate_mutation_before_review") is not False: errors.append("vNext.1 problem-first and contribution-aware routing must remain shadow-only until at least 20 independently adjudicated prospective cases")
        discovery_shadow=(state.get("paper_first_problem_discovery_contract") or {}).get("problem_first_shadow") or {}
        if len(discovery_shadow.get("primitives") or [])!=4 or sum(int(v or 0) for v in (discovery_shadow.get("generation_prior") or {}).values())!=120 or discovery_shadow.get("live_discovery_operator_changed") is not False or int(discovery_shadow.get("provider_calls_authorized") or 0)!=0 or discovery_shadow.get("scientific_authority") is not False: errors.append("problem-first search must remain a four-primitive 120-target shadow prior with zero provider/scientific authority")
        templates=memory_wiki.get("lesson_templates") or {}
        if set(templates)!={"COMPLEXITY_FOR_NOVELTY_FAILURE","METHOD_REDUCTION_DID_NOT_KILL_SCIENTIFIC_OBJECT"} or int(memory_summary.get("contribution_aware_lesson_templates") or 0)!=2 or any((row or {}).get("scientific_authority") is not False for row in templates.values()): errors.append("contribution-aware Research Memory must expose exactly two zero-authority lesson templates without inventing historical evidence")
        workflow_policy=(state.get("paper_first_workflow") or {}).get("policy") or {}
        if workflow_policy.get("contribution_archetype_is_explicit_for_schema_2_5_plus") is not True or workflow_policy.get("insight_dominant_paper_may_use_simple_method_but_requires_problem_insight_and_evidence_closure") is not True or workflow_policy.get("legacy_contracts_are_not_retroactively_forced_into_contribution_archetypes") is not True: errors.append("paper design must support contribution archetypes and Insight Evidence Ladder without retroactively rewriting legacy contracts")
    ...
    material_child_race=state.get("longitudinal_safety_material_child_race") or {};material_child_summary=material_child_race.get("summary") or {}
    material_child_errors=validate_material_child_race(material_child_race)
    if material_child_errors: errors.append("longitudinal material-child race invalid:"+";".join(material_child_errors))
    ...
    ai4ai_reopen=state.get("ai4ai_strategy_reopen_contradiction") or {};ai4ai_reopen_summary=ai4ai_reopen.get("summary") or {}
    ai4ai_reopen_errors=validate_ai4ai_strategy_reopen_contradiction(ai4ai_reopen)
    if ai4ai_reopen_errors: errors.append("AI4AI strategy reopen contradiction invalid:"+";".join(ai4ai_reopen_errors))
