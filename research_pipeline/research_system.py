...
def _component_manifest(state: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    return [
        ...
        {"source":"Biomni / BioMedAgent / PaperQA2", "component":{"en":"Declarative research capability registry","zh":"声明式科研能力注册表"}, "status":"running", "evidence":{"en":f"{capabilities['capabilities']} typed capabilities / {capabilities['high_risk']} high-risk / {capabilities.get('skill_packs_catalogued_not_installed',0)} external skill packs catalogued-not-installed / admission+least-privilege routing / zero skill authority","zh":f"{capabilities['capabilities']} 类 typed capability / {capabilities['high_risk']} 类高风险 / {capabilities.get('skill_packs_catalogued_not_installed',0)} 个外部 skill pack 仅登记未安装 / 准入+最小权限路由 / skill 权限为 0"}},
...
def validate_state(state: dict[str, Any]) -> list[str]:
    ...
    if skill_integrity_required:
        ...
        if int(skill_summary.get("skill_packs_catalogued_not_installed") or 0)<1 or int(skill_summary.get("automatic_skill_authority") or 0)!=0: errors.append("external research skills must remain catalogued/qualified capabilities with zero automatic authority")
