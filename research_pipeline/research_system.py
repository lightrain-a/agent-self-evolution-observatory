...
def validate_state(state: dict[str, Any]) -> list[str]:
    ...
    if fresh_summary.get("survivors") != 0 or fresh_summary.get("drafts_reviewed") != fresh_summary.get("stopped") or fresh_summary.get("drafts_reviewed") != len(fresh_sat.get("drafts") or []): errors.append("fresh contradiction-first discovery saturation accounting mismatch")
    ...
    if generator.get("status") in {"GENERATED_ZERO_CANDIDATES","GENERATED_AWAIT_PROBLEM_GATE"} and generator_policy.get("independent_reviewer_must_ground_both_source_claims_to_exact_abstract_excerpts") is not True: errors.append("generated problem candidates require independent exact-abstract source-claim grounding")
    ...
    if problem_queue_policy.get("all_candidates_require_problem_gate") is not True or problem_queue_policy.get("problem_gate_pass_only_grants_human_paper_design_eligibility") is not True or problem_queue_policy.get("verified_primary_evidence_registry_required_for_submitted_candidates") is not True or problem_queue_policy.get("independent_semantic_reduction_review_required") is not True or problem_queue_policy.get("semantic_reviewer_is_block_only") is not True or problem_queue_summary.get("method_authorized") != 0 or problem_queue_summary.get("experiment_authorized") != 0 or problem_queue_summary.get("p0_authorized") != 0: errors.append("paper-first problem queue must require primary provenance + block-only semantic review and grant only paper-design eligibility")
