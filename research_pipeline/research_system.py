...
def validate_state(state: dict[str, Any]) -> list[str]:
    ...
    if shadow_latest:
        ...
        if latest_policy.get("control_snapshot_bound_run") is True:
            ...
            if str(shadow_latest.get("stage_runner_required_schema") or "")!="1.5" or len(control_sha)!=64 or any(ch not in "0123456789abcdef" for ch in control_sha) or len(qualification_commit)!=40 or any(ch not in "0123456789abcdef" for ch in qualification_commit) or latest_policy.get("control_snapshot_provenance_is_bounded_sha_only") is not True or latest_policy.get("control_snapshot_terminal_provenance_verified") is not True: errors.append("qualified shadow run must expose bounded schema-1.5 control snapshot provenance through terminal gate")
