...
def validate_state(state: dict[str, Any]) -> list[str]:
    ...
    if primary_evidence.get("status") == "READY" and (primary_policy.get("source_coverage_scheduler_is_discovery_only") is not True or primary_policy.get("source_review_exposure_has_zero_scientific_authority") is not True or primary_policy.get("source_exposure_cannot_skip_generation_or_problem_gate") is not True or primary_policy.get("source_exposure_does_not_relax_relevance_or_freshness") is not True or primary_policy.get("source_coverage_exploration_prefers_preregistered_lanes") is not True): errors.append("READY primary evidence must keep source-exposure scheduling retrieval-only, zero-authority, lane-grounded, and inside the same freshness/relevance gates")
