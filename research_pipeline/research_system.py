...
from .paper_first_primary_evidence import TYPED_EVIDENCE_EXTRACTION_VERSION, load_primary_evidence_state
...
def validate_state(state: dict[str, Any]) -> list[str]:
    ...
    if primary_evidence.get("status") == "READY" and (primary_policy.get("typed_evidence_extraction_version") != TYPED_EVIDENCE_EXTRACTION_VERSION or primary_policy.get("derived_typed_evidence_reused_only_when_extractor_version_matches") is not True or primary_policy.get("typed_evidence_requires_first_party_ownership_or_nonliterature_attribution") is not True): errors.append("READY primary evidence must use first-party-owned versioned typed evidence and forbid cross-version typed-evidence reuse")
    ...
    if "one_generator_call_max" in generator_policy and generator_policy.get("one_generator_call_max") is not True: errors.append("canonical problem generator must allow at most one generator call per content-addressed pool")
