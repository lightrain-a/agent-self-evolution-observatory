...
class StoryCandidate:
    ...
    def validate(self, contract: PaperContract) -> tuple[str, ...]:
        ...
        unknown = sorted(referenced - supported)
        errors: list[str] = []
        if unknown:
...
def evaluate_story_search(contract: PaperContract, candidates: Sequence[StoryCandidate]) -> dict[str, Any]:
    ...
    for candidate in candidates:
        ...
        emphasized = set(candidate.emphasized_claim_ids) & supported
        contributions = set(candidate.contribution_order) & supported
        ...
        rows.append({
            ...
            "valid": not errors,
            "errors": list(errors),
            "supported_claim_coverage": round(len(emphasized) / denom, 4),
...
def compile_review_action(objection: ReviewerObjection, contract: PaperContract) -> ReviewAction:
    unknown_claims = sorted(set(objection.claim_ids) - set(contract.supported_claims))
...
def evaluate_claim_audit(
    ...
    *,
    ...
    unknown = sorted(claimed - supported)
    if unknown:
    ...
    unbound = sorted((claimed & supported) - evidence_bound)
    if unbound:
