...
def finalize(*, root:Path, study:Path)->dict[str,Any]:
    ...
    try:
        ...
        metrics={}
        for arm in ARMS:
            rows=per[arm];ideas=gens[arm]['ideas'];clear=sum((not r['hard']['history_near_duplicate']) and r['hard']['cheapest_falsifier_complete'] and r['agent']['agent_specificity']=='AGENT_SPECIFIC' and r['reduction']['reduction_verdict']=='RESIDUAL_PLAUSIBLE' for r in rows);other=gens['relevant_escape' if arm=='relevant' else 'relevant']['ideas']
        ...
        primary={'escape_vs_plain_panel_clear_delta':metrics['relevant_escape']['criterion_panel_clear_rate']-metrics['relevant']['criterion_panel_clear_rate'],'escape_vs_plain_agent_specific_delta':metrics['relevant_escape']['agent_specific_rate']-metrics['relevant']['agent_specific_rate'],'escape_vs_plain_exact_reduction_delta':metrics['relevant_escape']['exact_reduction_rate']-metrics['relevant']['exact_reduction_rate'],'same_selected_memory_ids':prep['packs']['relevant']['selected_memory_ids']==prep['packs']['relevant_escape']['selected_memory_ids'],'same_memory_characters':metrics['relevant']['selected_memory_characters']==metrics['relevant_escape']['selected_memory_characters'],'interpretation':'framing-only search-policy smoke; not Problem Gate, scientific truth, or publication-success evidence'}
        report={'schema_version':'2.4','status':'API_MEMORY_ESCAPE_SMOKE_COMPLETE','study':'API_MEMORY_ESCAPE_SMOKE_V24','memory_instance_id':prep['packs']['relevant']['memory_instance_id'],'frozen_ablation_plan_sha256':prep['plan']['plan_sha256'],'history_pool_available_objects':prep['packs']['relevant']['summary']['available'],'generator_model':GENERATOR_MODEL,'reviewers':{'hard':{'requested':HARD_REVIEWER_MODEL,'resolved':hard['resolved_model'],'usage':hard['usage'],'attempts':ha},'agent':{'requested':AGENT_REVIEWER_MODEL,'resolved':agent['resolved_model'],'usage':agent['usage'],'attempts':aa},'reduction':{'requested':REDUCTION_REVIEWER_MODEL,'resolved':reduction['resolved_model'],'usage':reduction['usage'],'attempts':ra}},'metrics':metrics,'primary_comparison':primary,'generated_outputs_promoted_to_research_objects':False,'scientific_authority':False,'belief_authority':False};report['report_sha256']=_sha_text(_canonical(report));_write(output,report);return report
    finally:
