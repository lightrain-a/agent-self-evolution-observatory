...
class PaperFirstIdeaIncubationTest(unittest.TestCase):
 ...
 def test_premortem_is_valid_and_only_explicit_promotions_enter_p0(self):
  ...
  self.assertEqual((s['p0_authorized'],s['gpu_authorized']),(4,0))
 ...
 def test_every_candidate_is_paper_first_and_collision_aware(self):
  ...
  for row in self.state['candidates']:
   ...
   self.assertEqual(bool(row['p0_authorized']),row['id'] in {'PF-1','PF-2','PF-4','PF-6'}); self.assertFalse(row['gpu_authorized'])
