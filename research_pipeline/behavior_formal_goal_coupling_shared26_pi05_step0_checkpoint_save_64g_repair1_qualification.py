...
def main() -> int:
    ...
    lock_path = Path("/data/wyt/.formal-goal-pi05-step0-checkpoint-save-64g-repair1-qualification.lock")
    ...
    with exclusive_lock(lock_path):
        ...
        if authority.get("status") != "AUTHORIZED_PI05_STEP0_CHECKPOINT_SAVE_64G_REPAIR1_QUALIFICATION":
        ...
        if adjudication.get("status") != "PI05_STEP0_CHECKPOINT_SAVE_40G_REPRODUCED_CGROUP_OOM":
            raise RuntimeError("40G checkpoint-save adjudication status drift")
        if adjudication.get("next_gate", {}).get("formal_run3_authorized") is not False:
        ...
        initial = {
            "schema_version": "behavior-formal-goal-coupling-shared26-pi05-step0-checkpoint-save-64g-repair1-qualification-v1",
            "object_id": OBJECT_ID,
            "child_id": CHILD_ID,
            "generated_at": datetime.now(timezone.utc).isoformat(),
            "status": "PI05_STEP0_CHECKPOINT_SAVE_64G_REPAIR1_STARTED",
        ...
        status = "PI05_STEP0_CHECKPOINT_SAVE_64G_REPAIR1_HOLD"
        error = None
        state_ready = False
        save_started = False
        ...
        try:
            ...
            progress.update({"generated_at": datetime.now(timezone.utc).isoformat(), "status": "PI05_STEP0_CHECKPOINT_SAVE_64G_REPAIR1_STATE_READY", "train_state_ready": True, "snapshots": snapshots})
            ...
            save_started = True
            progress.update({"generated_at": datetime.now(timezone.utc).isoformat(), "status": "PI05_STEP0_CHECKPOINT_SAVE_64G_REPAIR1_SAVE_STARTED", "checkpoint_save_started": True})
            atomic_json(receipt_path, progress)
            ...
            progress.update({
                "generated_at": datetime.now(timezone.utc).isoformat(),
                "status": "PI05_STEP0_CHECKPOINT_SAVE_64G_REPAIR1_SAVE_COMPLETE",
            ...
            status = "PI05_STEP0_CHECKPOINT_SAVE_64G_REPAIR1_PASS"
            ...
            final.update({
                ...
                "loss_values_read_or_reported": False,
                "formal_training_authorized": False,
                "next_gate": "FORMAL_RUN3_SYNCHRONOUS_CHECKPOINT_BARRIER_AUTHORITY_DESIGN" if status.endswith("PASS") else "CHECKPOINT_SAVE_64G_REPAIR1_FAILURE_ADJUDICATION_NO_FORMAL_RESTART",
            })
