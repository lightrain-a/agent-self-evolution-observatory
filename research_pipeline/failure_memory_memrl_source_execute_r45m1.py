#!/usr/bin/env python3
"""Execute only the frozen B1 MemRL R45-M1 replacement source-build phase.

R45-M1 is an infrastructure-only replacement wrapper, not a new scientific design. It preserves the
...
EXPECTED_AUTH_STATUS = "HUMAN_BOUNDED_R45M1_REPLACEMENT_EXECUTION_AUTHORITY_RECORDED"
...
def _preflight(
    ...
    bound = (auth.get("bindings") or {}).get("migration_manifest") or {}
    if bound.get("sha256") != _sha(manifest_path) or bound.get("receipt_sha256") != manifest.get("receipt_sha256"):
        raise RuntimeError("authorization-migration-manifest-binding-drift")
    ...
    if lineage.get("replacement") != "R45-M1" or lineage.get("resume_of_old_r45") is not False:
        raise RuntimeError("replacement-lineage-drift")
    ...
    if source_scope.get("authorized") is not True or int(source_scope.get("count") or 0) != 1 or int(source_scope.get("exact_selected_source_tasks") or 0) != 128:
        raise RuntimeError("replacement-source-scope-drift")
    ...
    preflight = {
        "schema_version": "1.0",
        "paper_id": PAPER_ID,
        "role": "R45M1_SOURCE_EXECUTION_PREFLIGHT_NO_NEW_BENCHMARK_OUTCOME",
        "recorded_at": _now(),
        "migration_manifest_receipt_sha256": manifest.get("receipt_sha256"),
...
def _execute_source(manifest: dict[str, Any], outdir: pathlib.Path, *, resume: bool) -> dict[str, Any]:
    ...
    receipt: dict[str, Any] = {
        "schema_version": "1.0",
        "paper_id": PAPER_ID,
        "role": "R45M1_FROZEN_MEMRL_SOURCE_BUILD_EXECUTION",
        "recorded_at": _now(),
