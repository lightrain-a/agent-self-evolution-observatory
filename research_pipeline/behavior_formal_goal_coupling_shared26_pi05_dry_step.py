...
def main() -> int:
    ...
    lock_path = Path("/data/wyt/.formal-goal-pi05-dry-step-v1.lock")

    if receipt_path.exists():
        raise RuntimeError(f"dry-step receipt already exists; exactly-once attempt cannot be relaunched: {receipt_path}")

    authority = json.loads(authority_path.read_text(encoding="utf-8"))
    if authority.get("status") != "AUTHORIZED_SINGLE_A100_NO_UPDATE_DRY_TRAIN_STEP":
        raise RuntimeError("dry-step authority is not active")
    ...
    initial = {
        "schema_version": "behavior-formal-goal-coupling-shared26-pi05-dry-step-result-v1",
        "object_id": OBJECT_ID,
        "generated_at": datetime.now(timezone.utc).isoformat(),
        "status": "PI05_NO_UPDATE_DRY_STEP_STARTED",
        "attempt_count": 1,
    ...
    with exclusive_lock(lock_path):
        ...
        status = "PI05_NO_UPDATE_DRY_STEP_HOLD"
        error = None
        ...
        try:
            ...
            started_payload.update(
                {
                    "status": "PI05_NO_UPDATE_DRY_STEP_GRADIENT_STARTED",
                    "batch_ready": True,
                    "train_state_ready": True,
            ...
            status = "PI05_NO_UPDATE_DRY_STEP_PASS"
            ...
            final.update(
                {
                    ...
                    "formal_training_authorized": False,
                    "next_gate": (
                        "EXACTLY_ONCE_PI05_SHARED26_TRAINING_LAUNCH_AUTHORITY"
                        if status == "PI05_NO_UPDATE_DRY_STEP_PASS"
                        else "DRY_STEP_FAILURE_ADJUDICATION_NO_AUTOMATIC_RETRY"
                    ),
                }
            )
        ...
        return 0 if status == "PI05_NO_UPDATE_DRY_STEP_PASS" else 2
