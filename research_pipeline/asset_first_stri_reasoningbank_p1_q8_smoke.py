"""Q8 non-scientific smoke and authority for the unchanged Q5 replay."""
...
from research_pipeline.asset_first_stri_reasoningbank_p1_q8_prepare import (
    CONTRACT as Q8_CONTRACT, DIFFERENTIAL as Q7_SMOKE_DIFFERENTIAL,
)
from research_pipeline.asset_first_stri_reasoningbank_p1_q8_runtime import (
    Q8_CONTRACT_SHA256, StartGraceDockerRun,
)

SMOKE = ROOT / "generated/asset-first-stri-reasoningbank-p1-q8-evaluator-verbosity-smoke-20260831.json"
AUTHORITY = ROOT / "generated/asset-first-stri-reasoningbank-p1-q8-q5-replay-execution-authority-20260831.json"
...
def run_smoke(output: Path = SMOKE) -> dict[str, Any]:
    if output.exists():
        raise RuntimeError(f"refusing to overwrite immutable Q8 smoke: {output}")
    verification = verify_q5_contract()
    if sha256_file(Q8_CONTRACT) != Q8_CONTRACT_SHA256:
        raise RuntimeError("Q8 contract SHA drift")
    fixture = fixture_by_id()["sphinx-doc__sphinx-9230"]
    container = StartGraceDockerRun(
        fixture["image_pull_reference"],
        fixture["model_visible"]["base_commit"],
        "q8-nonscientific-evaluator-verbosity-smoke",
        exact_base=True,
    )
    try:
        ...
        checks = {
            ...
            "S7_container_create_receipt_qualified": (
                ack["contract_sha256"] == Q8_CONTRACT_SHA256
                and (
        ...
        payload = {
            "schema_version": 1,
            "experiment_id": "NON_SCIENTIFIC_Q8_EVALUATOR_VERBOSITY_SMOKE-20260831",
            "created_at_utc": utcnow(),
            "q5_contract_sha256": CONTRACT_SHA256,
            "q8_contract_sha256": Q8_CONTRACT_SHA256,
            "q7_smoke_failure_differential_sha256": sha256_file(Q7_SMOKE_DIFFERENTIAL),
            ...
            "pass": passed,
            "decision": (
                "Q8_RUNTIME_AND_EVALUATOR_SMOKE_PASS"
                if passed else "Q8_RUNTIME_AND_EVALUATOR_SMOKE_HOLD"
            ),
            "model_calls": 0,
            "provider_calls": 0,
        ...
        payload = {
            ...
            "q8_contract_sha256": Q8_CONTRACT_SHA256,
            "pass": False,
            "decision": "Q8_RUNTIME_AND_EVALUATOR_SMOKE_HOLD",
...
def generate_authority(
    ...
    if output.exists():
        raise RuntimeError(f"refusing to overwrite immutable Q8 authority: {output}")
    verification = verify_q5_contract()
    ...
    checks = {
        ...
        "q8_contract_exact": sha256_file(Q8_CONTRACT) == Q8_CONTRACT_SHA256,
        "q5_failed_smoke_and_differential_preserved": (
            sha256_file(Q7_SMOKE_DIFFERENTIAL)
            == load_payload(Q8_CONTRACT)["bindings"]["q7_failure_differential_sha256"]
        ),
        "q4_sources_exact": all(r["pass"] for r in verification["source_checks"]),
        "official_parser_exact": True,
        "q8_smoke_pass": (
            smoke["decision"] == "Q8_RUNTIME_AND_EVALUATOR_SMOKE_PASS"
            and smoke["pass"] is True
            and all(smoke["checks"].values())
        ),
        "q8_single_runtime_variable_exact": (
    ...
    payload = {
        "schema_version": 1,
        "experiment_id": "E1-STRI-REASONINGBANK-P1-Q8-Q5-REPLAY-AUTHORITY-20260831",
        "created_at_utc": utcnow(),
        "q5_contract_sha256": CONTRACT_SHA256,
        "q8_contract_sha256": Q8_CONTRACT_SHA256,
        "q8_smoke_sha256": sha256_file(smoke_path),
        "checks": checks,
        "decision": (
            "P1_Q8_RUNTIME_AND_EVALUATOR_QUALIFIED_Q5_REPLAY_AUTHORIZED"
            if authorized else "P1_Q8_RUNTIME_OR_EVALUATOR_HOLD_Q5_REPLAY_UNAUTHORIZED"
        ),
