...
REPAIR1_RESULT_SHA = "4db9b8f86c5bdc93ca062c7dbbb4e2f0426f2e77ffa760fcf8ae2b2f8bc87bbc"
REPAIR1_ADJUDICATION_SHA = "7beee1431014a2287a1ec3b1840fde68e42233751d211fbdfd114a728a0dbf36"
CHECKPOINT_SEAL_SHA = "64fcbd5023247561d59cd6d5ffb9b34c3f91604094904ae7f75959d73fd8295f"
CHECKPOINT_HYGIENE_SHA = "6d64a3ec6fe80f3fc6164283ec03401af4ba28f0b958e0f09b400403b410fa91"
REPAIR2_RESULT_SHA = "4e678226f8b7176d50421d7da231560430f431466f6c4c523f7d36b22496d068"
REPAIR2_ADJUDICATION_SHA = "3986978ca4445e29d7276c64e326f12f74d8dea8ea9baea838e42998da09dd02"
DURABILITY_PREFLIGHT_SHA = "98f709804d22a277d98963b042cead6d26de39935b99d25cc3cd87dc33bab847"
SOURCE_BATCH = 64
MICRO = 8
K = 8
EFFECTIVE = 64
...
def validate_keepalive_durability() -> dict:
    completed = subprocess.run(
        ["pgrep", "-af", r"^formal-goal-keepalive67-[AB] 21600$"],
        check=False,
        capture_output=True,
        text=True,
    )
    lines = [line.strip() for line in completed.stdout.splitlines() if line.strip()]
    has_a = any("formal-goal-keepalive67-A 21600" in line for line in lines)
    has_b = any("formal-goal-keepalive67-B 21600" in line for line in lines)
    manager = subprocess.run(
        ["pgrep", "-u", str(os.getuid()), "-x", "systemd"], check=False, capture_output=True, text=True
    ).stdout.splitlines()
    loginctl = subprocess.run(
        ["loginctl", "show-user", "wyt", "-p", "State", "-p", "Linger"],
        check=False,
        capture_output=True,
        text=True,
    ).stdout
    if not (has_a and has_b) or not manager or "State=active" not in loginctl:
        raise RuntimeError(
            f"host67 keepalive durability gate failed: has_a={has_a} has_b={has_b} manager={manager} loginctl={loginctl!r}"
        )
    return {
        "keepers": lines,
        "keeper_count": len(lines),
        "user_manager_pid": int(manager[0]),
        "state_active": True,
        "linger": "Linger=yes" in loginctl,
    }
...
def main() -> int:
    ...
    for name in [
        ...
        "repair1_result",
        "repair1_adjudication",
        "checkpoint_seal",
        "checkpoint_hygiene",
        "repair2_result",
        "repair2_adjudication",
    ...
    if authority.get("status") != "AUTHORIZED_PI05_SYNTHETIC_FUSED_ACCUM8X8_DIRECT_DEVICE_REPAIR3_HOST67_DURABLE":
        raise RuntimeError("repair3 authority is not active")
    ...
    require(paths["repair1_result"], REPAIR1_RESULT_SHA, "repair1 result")
    require(paths["repair1_adjudication"], REPAIR1_ADJUDICATION_SHA, "repair1 adjudication")
    require(paths["checkpoint_seal"], CHECKPOINT_SEAL_SHA, "host67 checkpoint exact seal")
    require(paths["checkpoint_hygiene"], CHECKPOINT_HYGIENE_SHA, "host67 checkpoint hygiene")
    repair1 = json.loads(paths["repair1_result"].read_text(encoding="utf-8"))
    repair1_adj = json.loads(paths["repair1_adjudication"].read_text(encoding="utf-8"))
    seal = json.loads(paths["checkpoint_seal"].read_text(encoding="utf-8"))
    hygiene = json.loads(paths["checkpoint_hygiene"].read_text(encoding="utf-8"))
    if repair1.get("status") != "PI05_SYNTHETIC_FUSED_ACCUM8X8_DIRECT_DEVICE_REPAIR1_HOLD" or repair1.get("train_state_ready") is not False or repair1.get("micro_gradients_completed") != 0 or repair1.get("optimizer_update") is not False:
        raise RuntimeError("repair1 pre-model HOLD boundary drift")
    if repair1_adj.get("status") != "PI05_SYNTHETIC_FUSED_ACCUM8X8_REPAIR1_PRE_MODEL_CHECKPOINT_RESIDUE_HOLD":
        raise RuntimeError("repair1 checkpoint-residue adjudication drift")
    if seal.get("status") != "PI05_BASE_HOST67_EXACT_SEAL_PASS" or seal.get("verified_object_count") != 20 or seal.get("verified_bytes") != 12_441_721_931:
        raise RuntimeError("host67 checkpoint exact seal drift")
    if hygiene.get("status") != "PI05_BASE_HOST67_CHECKPOINT_HYGIENE_PASS" or hygiene.get("formal_object_count") != 20 or hygiene.get("part_files_quarantined") != 15:
        raise RuntimeError("host67 checkpoint hygiene drift")
    require(paths["repair2_result"], REPAIR2_RESULT_SHA, "repair2 result")
    require(paths["repair2_adjudication"], REPAIR2_ADJUDICATION_SHA, "repair2 adjudication")
    require(paths["durability_preflight"], DURABILITY_PREFLIGHT_SHA, "host67 durability preflight")
    repair2 = json.loads(paths["repair2_result"].read_text(encoding="utf-8"))
    repair2_adj = json.loads(paths["repair2_adjudication"].read_text(encoding="utf-8"))
    durability = json.loads(paths["durability_preflight"].read_text(encoding="utf-8"))
    if (
        repair2.get("status") != "PI05_SYNTHETIC_FUSED_ACCUM8X8_DIRECT_DEVICE_REPAIR2_MICRO_GRADIENT_STARTED"
        or repair2.get("train_state_ready") is not True
        or repair2.get("micro_gradients_completed") != 0
        or repair2.get("optimizer_update") is not False
        or repair2.get("parameter_update") is not False
        or repair2.get("checkpoint_written") is not False
    ):
    ...
    initial = {
        ...
        "status": "PI05_SYNTHETIC_FUSED_ACCUM8X8_DIRECT_DEVICE_REPAIR3_STARTED",
        ...
        "keepalive_durability": keepalive,
    ...
    lock_path = Path("/data/wyt/.formal-goal-pi05-synthetic-fused-accum8x8-direct-device-repair3.lock")
    with exclusive_lock(lock_path):
        write_receipt(receipt_path, initial)
        status = "PI05_SYNTHETIC_FUSED_ACCUM8X8_DIRECT_DEVICE_REPAIR3_HOLD"
        error = None
        state_ready = False
        done = 0
        accumulator = None
        ...
        try:
            ...
            for index in range(K):
                live_keepalive = validate_keepalive_durability()
                progress = dict(initial)
                progress.update(
                    {
                        "status": "PI05_SYNTHETIC_FUSED_ACCUM8X8_DIRECT_DEVICE_REPAIR3_MICRO_GRADIENT_STARTED",
                        ...
                        "keepalive_durability": live_keepalive,
                    }
                )
            ...
            status = "PI05_SYNTHETIC_FUSED_ACCUM8X8_DIRECT_DEVICE_REPAIR3_PASS"
