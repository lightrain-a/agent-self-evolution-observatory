...
class FreshPhenomenonPortfolioTest(unittest.TestCase):
    ...
    def test_pace_invalid_child_is_separated_from_scoped_principle_stop(self) -> None:
        state = self.build()
        self.assertEqual([], validate_fresh_phenomenon_portfolio(state))
        pace = next(row for row in state["candidates"] if row["candidate_id"] == "PA-06-PACE-MECHANISM-REDESIGN-IDENTIFIABILITY")
        self.assertEqual("STOP_REDUCTION", pace["status"])
        self.assertEqual("PRINCIPLE_CLOSED_GENERIC_PROGRAM_SYNTHESIS_REDUCTION", pace["support_status"])
        self.assertTrue(pace["evidence"]["principle_dead_end_certified"])
        self.assertEqual("PRINCIPLE_STOP", pace["evidence"]["stop_class"])
        self.assertFalse(pace["evidence"]["benchmark_level_dead_end_certified"])
        child = pace["evidence"]["historical_child"]
        ...
        self.assertEqual(4, pace["evidence"]["parameter_only_oracle_pairs"])
        self.assertEqual(140, pace["evidence"]["structural_oracle_pairs"])
        self.assertEqual(17, pace["evidence"]["reference_censored_cells"])
        self.assertEqual(6, pace["evidence"]["run_attempt_budget"])
        self.assertEqual(20, pace["evidence"]["paper_attempt_budget"])
        self.assertEqual(0, state["summary"]["archived"])
        self.assertFalse(pace["evidence"]["revised_f0_authorized"])
    ...
    def test_pace_binding_tamper_fails_closed(self) -> None:
        ...
        self.assertIn("PA-06 historical PACE transport F0 binding is stale", validate_fresh_phenomenon_portfolio(state))
    ...
    def test_authorized_negative_f0_releases_active_slot_and_stops_scout(self) -> None:
        ...
        self.assertEqual([], validate_fresh_phenomenon_portfolio(state))
        self.assertEqual(0, state["summary"]["active_f0"])
        self.assertEqual(6, state["summary"]["stop_reduction"])
        echo = next(row for row in state["candidates"] if row["candidate_id"] == "PA-01-EVIDENCE-ECHO")
    ...
    def test_defense_restrictiveness_is_closed_by_principle_reduction(self) -> None:
        state = build_fresh_phenomenon_portfolio(
            evidence_echo=self.echo_receipt(),
            primary_state={"summary": {}},
            dead_end_memory=self.memory(),
        )
        row = next(row for row in state["candidates"] if row["candidate_id"] == "PA-02-DEFENSE-RESTRICTIVENESS")
        self.assertEqual("STOP_REDUCTION", row["status"])
        self.assertEqual("PRINCIPLE_CLOSED_SAME_INFORMATION_REDUCTION", row["support_status"])
