...
def _a1(rows,t):
 ...
 return {'schema_version':'1.0','generated_at':_now(),'idea_id':'update-trust-region','code':'A-1','repair':'soft audit allocation from early branch divergence','scientific_role':'retrospective repair F0 only; no method PASS/FAIL authority for the original governance claim','split':{'development':len(dev),'future_eval':len(future),'development_nonzero':sum(int(r['controlled_delta'])!=0 for r in dev),'future_nonzero':pos,'future_harm':harms},'selection_rule':'choose h maximizing development nonzero recall subject to <=35% paired-action cost; tie lower cost then h','development_curve':curve,'selected_h':chosen['h'],'future_branch_policy':test,'matched_baselines':{'target_family_prior':target,'source_family_prior':source,'random_full_audit':_random(future,t,budget),'simple_h1_navigate_trigger':trigger},'decision':'STOP_REPAIR_SOFT_AUDIT_SIMPLE_TRIAGE_DOMINATES' if dominated else 'REPAIR_F0_SIGNAL_NEEDS_FRESH_VALIDATION','standalone_method_authorized':False,'hard_gate_authorized':False,'formal_p0_authorized':False,'harm_claim_authorized':harms>=3,'interpretation':'Early branch divergence remains an operational audit-priority signal, but a simpler target-family prior has higher held-out impact recall at lower cost and an h=1 navigate trigger matches recall at lower cost. Only one future harmful unit exists, so harm prioritization is not identifiable.','next_action':'Merge branch soft-audit into research-system scheduling; stop standalone A-1 repair and do not spend GPU unless a materially new observable/substrate is proposed.'}
def _family_policy(rows,t,policy):
 pos=[r for r in rows if int(r['controlled_delta'])!=0]; flagged=[]; cost=0
 for r in rows:
  h=policy[r['target_family']]
  if int(r['first_divergence_index'])<h: flagged.append(r); cost+=_full(r,t)
  else: cost+=_screen(r,h,t)
...
def _a2(rows,t):
 dev=[r for r in rows if r['evaluation_role']=='probe_development']; future=[r for r in rows if r['evaluation_role']=='future_eval']; fam=sorted({r['target_family'] for r in dev}); cand=[]
 for hs in itertools.product(HORIZONS,repeat=len(fam)):
  z=_family_policy(dev,t,dict(zip(fam,hs)))
  if z['nonzero_recall']>=.8: cand.append(z)
 selected=min(cand,key=lambda z:(z['cost_fraction'],-z['nonzero_recall'],sum(z['policy'].values()))); test=_family_policy(future,t,selected['policy'])
