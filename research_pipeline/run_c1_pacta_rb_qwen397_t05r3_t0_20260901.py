#!/usr/bin/env python3
"""Prospective T0.5-R3 smoke, bridge, and fixed source acquisition.

The repaired T0.5-R2 smoke remains immutable: it correctly created, executed,
inspected, and staged the synthetic marker within eight model actions but had no
remaining slot for the completion action. This final non-scientific termination
qualification changes only the synthetic smoke step ceiling from 8 to 16 while
preserving the task, sealed Q0 model binding, 512-token response budget, parser,
rootful exact-base runtime, and all scientific locks. If it still does not submit,
this line stops rather than widening the ceiling again.
"""
...
ORIGINAL_SMOKE=T05R_RUNTIME/"rootful-synthetic-smoke.json"
PRIOR_SMOKE=Path("/data/wyt/agent-self-evolution-observatory/runs/c1-pacta-rb-qwen397-t05r2-smoke-20260901-v1/rootful-synthetic-smoke.json")
SMOKE_EPOCH=Path("/data/wyt/agent-self-evolution-observatory/runs/c1-pacta-rb-qwen397-t05r3-smoke-20260901-v1")
DEFAULT=Path("/data/wyt/agent-self-evolution-observatory/runs/c1-pacta-rb-qwen397-t0-rootful-source-20260901-v3")
SMOKE_STEP_CEILING=16
...
def smoke()->dict[str,Any]:
 ...
 if not ORIGINAL_SMOKE.is_file() or not PRIOR_SMOKE.is_file():
  raise RuntimeError("prior smoke artifact missing; historical STOP cannot be audited")
 original=json.loads(ORIGINAL_SMOKE.read_text());prior=json.loads(PRIOR_SMOKE.read_text())
 if original.get("pass") is not False or int(original.get("provider_calls") or 0)!=8:
  raise RuntimeError("original smoke history drift")
 if prior.get("pass") is not False or int(prior.get("provider_calls") or 0)!=8:
  raise RuntimeError("repaired smoke history drift")
 if prior.get("task_contract") != ("Synthetic non-scientific runtime qualification. This task is intentionally self-contained: "
  ...
  raise RuntimeError("repaired smoke task drift")
 ...
 result={"schema_version":1,"created_at_utc":now(),"epoch":"C1-PACTA-RB-QWEN397-T05R3-SMOKE",
  "non_scientific":True,"multi_step":True,"prior_smoke_stop_preserved":True,
  "original_smoke_path":str(ORIGINAL_SMOKE),"original_smoke_sha256":sha256_file(ORIGINAL_SMOKE),
  "prior_smoke_path":str(PRIOR_SMOKE),"prior_smoke_sha256":sha256_file(PRIOR_SMOKE),
  "repair_basis":"single-variable qualification repair: increase only the synthetic smoke step ceiling from 8 to 16; keep the repaired task, Q0 binding, response budget, parser, provider packet, and runtime unchanged; no source task or PACTA outcome was observed; if 16 steps fail, stop without further ceiling expansion",
