...
from research_pipeline.temporal_skill_extension_multiturn import SRC,MODEL,RES,URL,planning_prompt,history,post,text,safe_receipt,sh,csha
PID=core.PAPER_ID;DATA=core.DATA_ROOT;BASE=DATA/'paper-acceptance'/'source-native-replay'/PID/'20260824-extension-multiturn-base-repeat-r1';PLAN=BASE/'plan.json';RESULTS=BASE/'results.csv';OWNER=PID+':EXT:PLANNING-REPEAT'
...
def prep():
 eps=json.load(open(SRC/'endpoints_clean.json'))['endpoints'];plan={'schema_version':'1.0','paper_id':PID,'experiment':'E2_EXTENSION_MULTITURN_BASE_REPEAT','endpoint_ids':[e['endpoint_id'] for e in eps],'model_identity':{'requested_model':MODEL,'required_resolved_model':RES,'required_plan_base_url':URL},'planned_model_calls':8,'selection_rule':'Repeat BASE planning+final on all four fixed EIA clean endpoints after the exploratory multi-turn result; no endpoint selection; repeat is robustness only.'};plan['plan_body_sha256']=csha({k:v for k,v in plan.items() if k!='plan_body_sha256'});atomic(PLAN,plan);return plan
def run():
 plan=json.load(open(PLAN));eps={e['endpoint_id']:e for e in json.load(open(SRC/'endpoints_clean.json'))['endpoints']};a=core.load_assets()
