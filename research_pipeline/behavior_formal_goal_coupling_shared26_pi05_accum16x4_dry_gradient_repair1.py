...
def main():
    ...
    if auth.get('status')!='AUTHORIZED_PI05_SINGLE_GPU_ACCUM16X4_NO_UPDATE_DRY_GRADIENT_REPAIR1' or auth.get('object_id')!=OBJECT_ID or auth.get('child_id')!=CHILD_ID: raise RuntimeError('authority drift')
    ...
    with lock(Path('/data/wyt/.formal-goal-pi05-accum16x4-dry-gradient-repair1.lock')):
