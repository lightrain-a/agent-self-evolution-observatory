...
PF4_CORRECTED_QUALIFICATION_JSON=PROJECT_ROOT/'generated'/'pf4-functional-diagnostic-equivalence-qualification-readjudication-20260816.json'
...
def from_shared(iid,result,key,*,readjudication:dict[str,Any]|None=None,source_analysis_sha256:str=""):
 ...
 return card
def _pf4_corrected_falsifier():
 receipt=load(PF4_CORRECTED_QUALIFICATION_JSON)
 if not isinstance(receipt,dict) or receipt.get('scientific_authority') is not False:return {}
 if receipt.get('fault_phase_executed') is not False:return {}
 if str(receipt.get('terminal_status') or '')!='INCONCLUSIVE_FUNCTIONAL_EQUIVALENCE_QUALIFICATION_FAILED':return {}
 source_sha=str(receipt.get('source_terminal_sha256') or '')
 if len(source_sha)!=64:return {}
 q=receipt.get('qualification') or {}
 return {"status":receipt.get('terminal_status'),"diagnosis_layer":receipt.get('diagnosis_layer'),"qualification_passed":bool(q.get('passed')),"checks":dict(q.get('checks') or {}),"conditions":dict(q.get('conditions') or {}),"fault_phase_executed":False,"scientific_result_available":False,"broader_principle_falsified":False,"scientific_update":receipt.get('scientific_update'),"receipt":str(PF4_CORRECTED_QUALIFICATION_JSON.relative_to(PROJECT_ROOT)),"source_terminal_sha256":source_sha,"contract_sha256":receipt.get('contract_sha256'),"scientific_authority":False}
def quarantine(card):
...
def build_paper_first_p0_f0_state(data_root:Path|None=None):
 ...
 corrected=_pf4_corrected_falsifier()
 if corrected:
  pf4=next((c for c in cards if c.get('idea_id')=='diagnosability-preserving-self-evolution'),None)
  if isinstance(pf4,dict):pf4['corrected_bounded_falsifier']=corrected
 observed_pass=sum(c['decision']=='F0_SUPPORT_PASS' for c in cards); observed_hold=sum(c['decision']=='HOLD_F0_SUPPORT_INSUFFICIENT' for c in cards); observed_running=sum(c['decision']=='F0_RUNNING' for c in cards);readjudicated_invalid=sum((c.get('scientific_readjudication') or {}).get('status')=='HISTORICAL_SUPPORT_PASS_INVALIDATED' for c in cards);corrected_inconclusive=sum((c.get('corrected_bounded_falsifier') or {}).get('status')=='INCONCLUSIVE_FUNCTIONAL_EQUIVALENCE_QUALIFICATION_FAILED' for c in cards)
 cards=[quarantine(c) for c in cards]
 return {"schema_version":"1.3","generated_at":now(),"run_root":str(run),"authority":HISTORICAL_RUN_AUTHORITY,"policy":{"f0_cannot_emit_method_fail":True,"shared_collection_for_pf2_pf4_pf6":True,"p0_method_requires_support_pass_and_pre_experiment_authority":True,"unauthorized_execution_is_preserved_as_diagnostic_not_scientific_authority":True,"diagnostic_f0_cannot_create_p0_lifecycle_or_method_admission":True,"later_authority_cannot_retroactively_validate_this_run":True,"post_selected_outcome_cohorts_cannot_be_compared_to_full_baseline_for_causal_drop":True,"paired_estimand_readjudication_dominates_historical_support_interpretation":True,"corrected_falsifier_qualification_failure_cannot_update_scientific_belief":True,"failed_qualification_forbids_future_fault_phase":True},"summary":{"ideas":4,"running":0,"support_pass":0,"support_hold":0,"observed_running":observed_running,"observed_support_pass":observed_pass,"observed_support_hold":observed_hold,"readjudicated_historical_support_invalidated":readjudicated_invalid,"corrected_bounded_falsifier_inconclusive":corrected_inconclusive,"quarantined":4,"scientifically_authorized":0,"method_fail_authorized":0},"cards":cards}
