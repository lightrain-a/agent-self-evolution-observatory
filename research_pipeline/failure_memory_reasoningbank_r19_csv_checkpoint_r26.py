#!/usr/bin/env python3
"""Bind private CSV durability mirrors to the R26 no-interim R19 checkpoint."""
...
RECEIPT_ID = "D2-FAILURE-MEMORY-PROVENANCE-L2B-R19-CSV-DURABILITY-R26"
...
def main() -> None:
    ...
    ap.add_argument("--output", type=Path, default=Path("generated/d2-failure-memory-provenance-l2b-r19-csv-durability-r26.json"))
    a = ap.parse_args()
    ck = json.loads(a.checkpoint.read_text(encoding="utf-8"))
    if ck.get("receipt_id") != "D2-FAILURE-MEMORY-PROVENANCE-L2B-R19-PARTIAL-CHECKPOINT-R26":
