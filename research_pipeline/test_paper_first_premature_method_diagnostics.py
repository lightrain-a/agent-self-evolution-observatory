...
class PrematurePaperFirstMethodDiagnosticsWriterTest(unittest.TestCase):
    def test_missing_local_source_preserves_valid_frozen_snapshot(self) -> None:
        frozen={
            "schema_version":"1.0",
        ...
        incomplete={
            "schema_version":"1.0",
            "summary":{"directions":2,"completed_diagnostics":0,"design_holds":0,"same_information_reducibility_findings":0,"hidden_executions":0,"scientifically_authorized":0,"p0_lifecycle_mutations":0,"full_experiment_authorized":0},
            "cards":[{"status":"diagnostic-artifact-incomplete"},{"status":"diagnostic-artifact-incomplete"}],
        }
        with tempfile.TemporaryDirectory() as td:
            root=Path(td); json_path=root/"state.json"; js_path=root/"state.js"
            json_path.write_text(json.dumps(frozen),encoding="utf-8")
            js_path.write_text("frozen-js",encoding="utf-8")
