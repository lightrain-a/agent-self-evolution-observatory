"""Q7 non-scientific smoke and authority for the unchanged Q5 replay."""
...
from research_pipeline.asset_first_stri_reasoningbank_p1_q7_prepare import (
    CONTRACT as Q7_CONTRACT, DIFFERENTIAL as Q6_SMOKE_DIFFERENTIAL,
)
from research_pipeline.asset_first_stri_reasoningbank_p1_q7_runtime import (
    Q7_CONTRACT_SHA256, GracefulReconciledDockerRun,
)

SMOKE = ROOT / "generated/asset-first-stri-reasoningbank-p1-q7-evaluator-verbosity-smoke-20260831.json"
AUTHORITY = ROOT / "generated/asset-first-stri-reasoningbank-p1-q7-q5-replay-execution-authority-20260831.json"
...
def run_smoke(output: Path = SMOKE) -> dict[str, Any]:
    if output.exists():
        raise RuntimeError(f"refusing to overwrite immutable Q7 smoke: {output}")
    verification = verify_q5_contract()
    if sha256_file(Q7_CONTRACT) != Q7_CONTRACT_SHA256:
        raise RuntimeError("Q7 contract SHA drift")
    ...
    container = GracefulReconciledDockerRun(
        ...
        "q7-nonscientific-evaluator-verbosity-smoke",
        exact_base=True,
    )
    try:
        ...
        checks = {
            ...
            "S7_container_create_receipt_qualified": (
                ack["contract_sha256"] == Q7_CONTRACT_SHA256
                and (
        ...
        payload = {
            "schema_version": 1,
            "experiment_id": "NON_SCIENTIFIC_Q7_EVALUATOR_VERBOSITY_SMOKE-20260831",
            "created_at_utc": utcnow(),
            "q5_contract_sha256": CONTRACT_SHA256,
            "q7_contract_sha256": Q7_CONTRACT_SHA256,
            "q6_smoke_failure_differential_sha256": sha256_file(Q6_SMOKE_DIFFERENTIAL),
            ...
            "pass": passed,
            "decision": (
                "Q7_RUNTIME_AND_EVALUATOR_SMOKE_PASS"
                if passed else "Q7_RUNTIME_AND_EVALUATOR_SMOKE_HOLD"
            ),
            "model_calls": 0,
            "provider_calls": 0,
        ...
        payload = {
            ...
            "q7_contract_sha256": Q7_CONTRACT_SHA256,
            "pass": False,
            "decision": "Q7_RUNTIME_AND_EVALUATOR_SMOKE_HOLD",
...
def generate_authority(
    ...
    if output.exists():
        raise RuntimeError(f"refusing to overwrite immutable Q7 authority: {output}")
    verification = verify_q5_contract()
    ...
    checks = {
        ...
        "q7_contract_exact": sha256_file(Q7_CONTRACT) == Q7_CONTRACT_SHA256,
        "q5_failed_smoke_and_differential_preserved": (
            sha256_file(Q6_SMOKE_DIFFERENTIAL)
            == load_payload(Q7_CONTRACT)["bindings"]["q6_failure_differential_sha256"]
        ),
        "q4_sources_exact": all(r["pass"] for r in verification["source_checks"]),
        "official_parser_exact": True,
        "q7_smoke_pass": (
            smoke["decision"] == "Q7_RUNTIME_AND_EVALUATOR_SMOKE_PASS"
            and smoke["pass"] is True
            and all(smoke["checks"].values())
        ),
        "q7_single_runtime_variable_exact": (
    ...
    payload = {
        "schema_version": 1,
        "experiment_id": "E1-STRI-REASONINGBANK-P1-Q7-Q5-REPLAY-AUTHORITY-20260831",
        "created_at_utc": utcnow(),
        "q5_contract_sha256": CONTRACT_SHA256,
        "q7_contract_sha256": Q7_CONTRACT_SHA256,
        "q7_smoke_sha256": sha256_file(smoke_path),
        "checks": checks,
        "decision": (
            "P1_Q7_RUNTIME_AND_EVALUATOR_QUALIFIED_Q5_REPLAY_AUTHORIZED"
            if authorized else "P1_Q7_RUNTIME_OR_EVALUATOR_HOLD_Q5_REPLAY_UNAUTHORIZED"
        ),
