...
SCHEMA_VERSION = "behavior-formal-goal-coupling-structure-scan-v1"
...
def _assert_frozen_contract(qualification: dict[str, Any], project_root: Path) -> dict[str, str]:
    metric_path = project_root / "research_pipeline" / "behavior_formal_goal_coupling.py"
    distribution_path = project_root / "research_pipeline" / "behavior_formal_goal_coupling_distribution.py"
    expected_metric = str(qualification["synthetic_gate"]["metric_code_sha256"])
    actual_metric = _sha(metric_path)
    if actual_metric != expected_metric:
        raise ValueError(f"frozen metric implementation drift:{actual_metric}!={expected_metric}")
    frozen = qualification["frozen_distribution_gate_before_policy_outcomes"]
    expected_thresholds = {
...
def scan_activity_definitions(
    *,
    ...
    if qualification.get("task_distribution_inspected") is not False:
        raise ValueError("source qualification is not the pre-distribution frozen state")
    ...
    for path in files:
        ...
        try:
            ...
            _, _, _, goal_state = parse_problem(
                path.parent.name,
                0,
                "behavior-1k",
                predefined_problem=text,
            )
            metrics = analyze_goal_state(goal_state)
            rows.append({
    ...
    receipt = {
        ...
        "frozen_qualification_sha256": qualification["qualification_sha256"],
        "frozen_code": frozen_code,
        "distribution_summary": summary,
        "parser_errors": parser_errors,
        "task_structure_rows": rows,
        "next_gate": (
            "independent review of the exact structure-scan receipt before any policy-result access"
            if summary["gates"]["pass"]
            else "HOLD/STOP fresh formal-goal-coupling lane; do not access policy results"
        ),
    }
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Scan BEHAVIOR BDDL formal goal structure without policy results")
