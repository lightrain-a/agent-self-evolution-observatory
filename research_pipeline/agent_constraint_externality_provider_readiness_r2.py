...
AUTH_OUTPUT = GENERATED / "agent-constraint-externality-provider-readiness-human-authorization-r2-20260905.json"
RESULT_OUTPUT = GENERATED / "agent-constraint-externality-provider-readiness-r2-20260905.json"

BASE_URL = "https://api.aa.com.cn/api/v1"
MODEL_ID = "qwen3.7-flash"
AUTHORIZED_AT = "2026-09-05T16:02:00+08:00"
AUTHORIZATION_SOURCE = "CURRENT_SESSION_USER_MESSAGE_CONTINUE_AFTER_R1_CREDENTIAL_BLOCKER_AND_SECURE_SECRET_RESTORATION"
...
def build_authorization() -> dict[str, Any]:
    ...
    payload: dict[str, Any] = {
        "schema_version": "ace-provider-readiness-human-authorization-r2-v1",
        "object_id": OBJECT_ID,
        "status": "USER_AUTHORIZED_NON_SCIENTIFIC_PROVIDER_READINESS_R2_ONLY",
...
def verify_authorization(payload: dict[str, Any]) -> None:
    ...
    if payload.get("status") != "USER_AUTHORIZED_NON_SCIENTIFIC_PROVIDER_READINESS_R2_ONLY":
...
def _base_result(auth: dict[str, Any]) -> dict[str, Any]:
    body = _request_body()
    return {
        "schema_version": "ace-provider-readiness-r2-v1",
        "object_id": OBJECT_ID,
        "execution_id": "ACE-PROVIDER-READINESS-R2-20260905",
...
def run_readiness(
    ...
    *,
    ...
    if not api_key:
        result.update({
            "status": "PROVIDER_READINESS_R2_NOT_DISPATCHED_CREDENTIAL_UNAVAILABLE_STOP",
    ...
    try:
        ...
        result.update({
            "status": "PROVIDER_READINESS_R2_TRANSPORT_ERROR_STOP",
    ...
    if pass_check:
        status = "PROVIDER_READINESS_R2_PASS_GATE0_AUTHORITY_STILL_CLOSED"
        ...
        status = "PROVIDER_READINESS_R2_INTERFACE_OR_MODEL_BINDING_FAIL_STOP"
...
def verify_result(payload: dict[str, Any]) -> None:
    ...
    if payload.get("readiness_pass"):
        if count != 1 or payload.get("status") != "PROVIDER_READINESS_R2_PASS_GATE0_AUTHORITY_STILL_CLOSED":
