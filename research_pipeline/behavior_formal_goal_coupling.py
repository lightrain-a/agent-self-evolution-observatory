...
SCHEMA_VERSION = "behavior-formal-goal-coupling-v1"
...
def _normalize_argument(argument: Any, scope: dict[str, str]) -> tuple[str, int]:
...
def _walk(expr: Any, scope: dict[str, str], next_scope_id: list[int], negated: bool = False) -> _WalkResult:
    ...
    if head not in OPERATORS:
        ...
        for raw in expr[1:]:
            ...
            normalized, missing = _normalize_argument(raw, scope)
    ...
    if head == "not":
        ...
        child = _walk(expr[1], scope, next_scope_id, not negated)
    ...
    if head in {"and", "or", "imply"}:
        ...
        walked = [_walk(child, scope, next_scope_id, negated) for child in children]
        return _WalkResult(
    ...
    child = _walk(body, child_scope, next_scope_id, negated)
    child.logical_depth += 1
...
def analyze_goal_state(goal_state: list[Any]) -> dict[str, Any]:
    ...
    scope_counter = [0]
    results = [_walk(clause, {}, scope_counter, False) for clause in goal_state]
