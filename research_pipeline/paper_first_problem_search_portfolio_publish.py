...
def _model_identity_receipts(root:Path)->dict:
    ...
    Portfolio generator identity covers successful expand/evolve/formulate calls.
    Semantic-review independence is evaluated only against the formulation receipts
    attached to each review batch, because those are the generators of the reviewed
    candidates. Missing receipts remain visibly missing and can never imply independence.
    """
    generator_requested=[];generator_resolved=[];generator_receipts=[]
    for pattern in ("expand-*.json","evolve-*.json","formulate-p*.json"):
        for path in sorted(root.glob(pattern)):
            payload=load(path);requested=str(payload.get("requested_model") or "").strip();resolved=str(payload.get("resolved_model") or "").strip();raw_sha=str(payload.get("raw_sha256") or "").strip()
    ...
    for path in sorted(root.glob("review-p*.json")):
        ...
        if requested:reviewer_requested.append(requested)
        if resolved:reviewer_resolved.append(resolved)
        batch_generators=[]
        for row in payload.get("generator_receipts") or []:
            if not isinstance(row,dict):continue
            value=str(row.get("resolved_model") or "").strip()
            if value:batch_generators.append(value);review_generator_resolved.append(value)
        if not batch_generators:
            batch_generators=[value for value in str(payload.get("generator_resolved_model") or "").split("|") if value]
            review_generator_resolved.extend(batch_generators)
        if not batch_generators:
            for candidate in payload.get("candidates") or []:
                if not isinstance(candidate,dict):continue
                semantic=candidate.get("semantic_reduction_review") or {}
                batch_generators.extend(value for value in str(semantic.get("generator_resolved_model") or "").split("|") if value)
            review_generator_resolved.extend(batch_generators)
        independent=bool(resolved and batch_generators and resolved not in set(batch_generators))
        independence.append(independent)
        review_receipts.append({"source_artifact":path.name,"requested_model":requested,"resolved_model":resolved,"generator_resolved_model":_joined(batch_generators),"independent_resolved_model":independent})
    review_batches=len(review_receipts)
    return {
        "generator_requested_model":_joined(generator_requested),
        "generator_resolved_model":_joined(generator_resolved),
        "reviewer_requested_model":_joined(reviewer_requested),
        "reviewer_resolved_model":_joined(reviewer_resolved),
        "review_generator_resolved_model":_joined(review_generator_resolved),
        "review_batches":review_batches,
        "all_review_batches_independent_resolved_model":bool(review_batches and all(independence)),
        "identity_complete_for_review_independence":bool(review_batches and all(row["resolved_model"] and row["generator_resolved_model"] for row in review_receipts)),
...
def _latest_shadow_run(root:Path)->dict:
    ...
    model_identity=_model_identity_receipts(root)
...
def publish(root:Path):
    ...
    shadow_policy=_base_policy(portfolio=True);shadow_policy.update({'scientific_authority':False,'shadow_only':True,'canonical_primary_generator_queue_untouched':True,'cannot_create_live_portable_review_receipt':True,'cannot_grant_live_paper_design_eligibility':True,'model_identity_from_provider_receipts':True,'missing_model_identity_never_implies_independence':True})
    state={'schema_version':'3.2-shadow','generated_at':now(),'run_id':run_id,'status':'SHADOW_PORTFOLIO_COMPLETE','scientific_authority':False,'generator_model':model_identity['generator_requested_model'] or model_identity['generator_resolved_model'],'reviewer_model':model_identity['reviewer_requested_model'] or model_identity['reviewer_resolved_model'],'policy':shadow_policy,'summary':summary,'search_diagnostics':search_diagnostics,'search_portfolio':portfolio,'model_identity':model_identity,'generation_notes':f"Shadow Search Portfolio: raw={bs.get('raw_seeds',0)}, unique={bs.get('semantic_unique',0)}, evolved={len(evolved)}, formulated={live}, machine-reviewable={(machine.get('summary') or {}).get('reviewable',0)}, counterfactual Problem-Gate PASS={queue['summary']['passed_problem_gate']}. No live Paper/Method/Experiment/P0 authority.",'raw_artifacts':{'generator':{'sha256':portfolio['stage_manifest_sha256'],'requested_model':model_identity['generator_requested_model'],'resolved_model':model_identity['generator_resolved_model'],'provider_receipts':model_identity['generator_receipts'],'portfolio':True,'calls':summary['portfolio_calls']},'semantic_reviewer':{'requested_model':model_identity['reviewer_requested_model'],'resolved_model':model_identity['reviewer_resolved_model'],'generator_resolved_model':model_identity['review_generator_resolved_model'],'identity_complete_for_review_independence':model_identity['identity_complete_for_review_independence'],'all_batches_independent_resolved_model':model_identity['all_review_batches_independent_resolved_model'],'provider_receipts':model_identity['review_receipts'],'calls':len(list(root.glob('review-p*.json')))}},'shadow_memory':{'live_source_coverage_effect':False,'current_shadow_receipt':receipt,'scientific_authority':False},'candidates':[{'candidate_id':c['candidate_id'],'title':c['title'],'search_primitive':c['discovery_lane'],'semantic_verdict':(c.get('semantic_reduction_review') or {}).get('verdict'),'counterfactual_problem_gate_pass':c['candidate_id'] in passed_ids,'paper_design_eligible':False,'scientific_authority':False,'authority':{'method':False,'experiment':False,'p0':False,'gpu':False}} for c in reviewed]}
    GEN_JSON.write_text(json.dumps(state,ensure_ascii=False,indent=2)+'\n',encoding='utf-8'); GEN_JS.write_text('window.PAPER_FIRST_PROBLEM_SEARCH_PORTFOLIO = '+json.dumps(state,ensure_ascii=False,separators=(',',':'))+';\n',encoding='utf-8')
