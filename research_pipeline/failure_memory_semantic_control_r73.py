#!/usr/bin/env python3
"""R73 fail-closed executor/analyzer for the frozen R72 semantic-control R3.
...
The checked-in R72 HOLD has zero execution authority. --validate-only is safe.
"""
...
try:
 ...
 import failure_memory_semantic_control_r72 as r72  # type: ignore
 from failure_memory_provenance_r66_sparse_discordance_stats import clopper_pearson  # type: ignore

PAPER_ID=r72.PAPER_ID
MODEL_KEYS={"qwen":r72.QWEN,"llama":r72.LLAMA}
...
def static_preflight(protocol:dict[str,Any],panel:dict[str,Any],hold:dict[str,Any],token_audit:dict[str,Any],review:dict[str,Any],r2_review:dict[str,Any],qwen_manifest:dict[str,Any],llama_manifest:dict[str,Any],r54:dict[str,Any],r54_path:pathlib.Path)->dict[str,dict[str,Any]]:
 objs=[protocol,panel,hold,token_audit,review,r2_review,qwen_manifest,llama_manifest,r54]
 if not all(valid(x) for x in objs):raise RuntimeError("receipt-hash-invalid")
 if protocol.get("status")!=r72.STATUS:raise RuntimeError("R72-status-drift")
 if sha(r54_path)!=r72.R54_FROZEN_SHA:raise RuntimeError("R54-file-drift")
 b=protocol.get("bindings") or {}
 if b.get("r73_execute_runner_sha256")!=sha(pathlib.Path(__file__).resolve()):raise RuntimeError("R73-runner-binding-drift")
 if b.get("r68_panel_receipt_sha256")!=panel.get("receipt_sha256") or b.get("r70_tokenizer_audit_receipt_sha256")!=token_audit.get("receipt_sha256") or b.get("r70_independent_review_receipt_sha256")!=review.get("receipt_sha256") or b.get("r72_r2_review_receipt_sha256")!=r2_review.get("receipt_sha256"):raise RuntimeError("review-binding-drift")
 if review.get("verdict")!="REDUCE_OR_REDIRECT" or r2_review.get("verdict")!="REVISE_R70_BEFORE_EXECUTION":raise RuntimeError("review-verdict-drift")
 ...
 sf=protocol.get("shuffle_freeze") or {};assign=list(sf.get("assignments") or [])
 if sf.get("seed")!=r72.SHUFFLE_SEED or len(assign)!=57 or sf.get("assignments_sha256")!=r72.digest(assign):raise RuntimeError("shuffle-freeze-drift")
 if hold.get("status")!="HOLD_R72_REQUIRES_INDEPENDENT_R3_REVIEW_BEFORE_EXECUTION":raise RuntimeError("R72-hold-status-drift")
 return {"qwen":patched_manifest(qwen_manifest,panel),"llama":patched_manifest(llama_manifest,panel)}

def runtime_records(protocol:dict[str,Any],panel:dict[str,Any],r54:dict[str,Any])->dict[str,dict[str,Any]]:
 rr=r72.r54_suffix(r54,panel);out={}
 audit={str(x["validation_task_id"]):x for x in panel.get("records") or []};frozen={str(x["task_id"]):x for x in (protocol.get("shuffle_freeze") or {}).get("assignments") or []}
 if len(frozen)!=57:raise RuntimeError("shuffle-freeze-cardinality-drift")
 for row in rr:
  tid=str(row["validation_task_id"]);sel=r72.selected(row);ref=audit[tid]
  content_sha=r72.digest([hashlib.sha256(str(s["content"]).encode()).hexdigest() for s in sel]);outcome_sha=r72.digest([bool(s["source_outcome_success"]) for s in sel])
  ...
  truth=r72.truth_codes(sel);shuf=r72.shuffled_codes(tid,truth);fr=frozen.get(tid)
  if shuf is None:
   if fr is not None:raise RuntimeError(f"homogeneous-task-has-shuffle-freeze:{tid}")
  else:
   if not fr or fr.get("truthful_code_sequence")!=truth or fr.get("shuffled_code_sequence")!=shuf or fr.get("truthful_code_sequence_sha256")!=r72.digest(truth) or fr.get("shuffled_code_sequence_sha256")!=r72.digest(shuf):raise RuntimeError(f"runtime-shuffle-realization-drift:{tid}")
  out[tid]=row
 return out
...
def execute_stage(model_key:str,protocol:dict[str,Any],panel:dict[str,Any],manifest:dict[str,Any],records:dict[str,dict[str,Any]],outdir:pathlib.Path,resume:bool=False)->None:
 ...
 for item in sched[len(terminal):]:
  ordinal=int(item["stage_ordinal"]);tid=str(item["task_id"]);arm=str(item["arm"]);ctx=r72.render_contexts(records[tid])[arm];prompt=prompt_for(manifest,ctx);pre_fail=[]
...
def worst_case_missing_signflip_p(left_only:int,right_only:int,missing:int)->dict[str,Any]:
 """Maximize the exact paired sign-test p-value over all missing-pair completions.

 Each technical-missing planned pair may become left-only, right-only, or
 concordant under the unobserved full-data table.  Concordant completions do
 not enter the conditional sign test, so allowing a+b<=missing enumerates all
 admissible discordant completions while the remainder is concordant.  The
 resulting maximum is a prospective worst-case inferential sensitivity, not an
 imputation or estimator.
 """
 best_p=-1.0;best={"left_only_added":0,"right_only_added":0,"concordant_added":missing}
 for add_left in range(missing+1):
  for add_right in range(missing-add_left+1):
   p=float(r48.exact_two_sided_signflip(left_only+add_left,right_only+add_right))
   if p>best_p:
    best_p=p;best={"left_only_added":add_left,"right_only_added":add_right,"concordant_added":missing-add_left-add_right}
 return {"p":best_p,"completion":best}
...
def exact_stats(effect_rows:list[tuple[str,bool,bool]],planned_n:int)->dict[str,Any]:
 ...
 if n:
  lo,hi=r48.percentile_ci(effects,r72.BOOTSTRAP_SEED,100000);p=r48.exact_two_sided_signflip(left_only,right_only);rd=obs_sum/n
 ...
 worst_sign=worst_case_missing_signflip_p(left_only,right_only,missing)
 rd_robust=bool(direction and ((direction>0 and sens[0]>0) or (direction<0 and sens[1]<0)))
 sign_robust=bool(n and p<0.05 and worst_sign["p"]<0.05)
 detected=bool(rd_robust and sign_robust)
 return {"planned_pairs":planned_n,"complete_pairs":n,"technical_missing_pairs":missing,"paired_risk_difference_complete_pairs":rd,"left_only_success":left_only,"right_only_success":right_only,"discordant_pairs":left_only+right_only,"exact_two_sided_signflip_p":p,"paired_bootstrap_ci95_complete_pairs":[lo,hi],"conservative_sparse_rd_ci95_complete_pairs":conservative,"technical_missing_worst_best_rd_bounds":sens,"technical_missing_worst_case_signflip_p":worst_sign["p"],"technical_missing_worst_case_signflip_completion":worst_sign["completion"],"technical_missing_rd_direction_robust":rd_robust,"technical_missing_significance_robust":sign_robust,"direction":direction,"effect_detected":detected}
...
def main():
 ...
 authority_check(auth,protocol,a.model);execute_stage(a.model,protocol,panel,manifests[a.model],runtime_records(protocol,panel,r54),a.output_dir.resolve(),a.resume);print(json.dumps({"status":"R73_STAGE_TERMINAL","model":a.model},sort_keys=True))
