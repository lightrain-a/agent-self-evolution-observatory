...
SCHEMA_VERSION = "behavior-formal-goal-coupling-demo-horizon-result-v1.1"
OBJECT_ID = "SUCC-C-BEHAVIOR2026-DEMO-HORIZON"
EXPOSED_TASK_INDICES = {0, 1}
EXPECTED_EPISODES_PER_TASK = 200
EXPECTED_FRESH_TASK_COUNT = 98
...
def _read_episode_lengths(path: Path, expected_task_index: int) -> list[int]:
    ...
    if len(set(episode_indices)) != EXPECTED_EPISODES_PER_TASK:
        raise ValueError(f"{path}: episode_index values are not unique")
    lo = min(episode_indices)
    hi = max(episode_indices)
    if sorted(episode_indices) != list(range(lo, hi + 1)):
        raise ValueError(f"{path}: episode_index is not a contiguous global range")
...
def load_task_outcomes(metadata_root: Path, structure_rows: list[dict]) -> list[dict]:
    ...
    if len(outcomes) != EXPECTED_FRESH_TASK_COUNT:
        raise ValueError(f"expected {EXPECTED_FRESH_TASK_COUNT} fresh task outcomes, got {len(outcomes)}")
    return outcomes
...
def analyze(
    ...
    *,
    ...
    if prereg.get("status") != "PREREGISTERED_V1_1_BEFORE_TASK_2_99_VALUE_ACCESS":
    ...
    return {
        ...
        "scientific_scope": "fresh 98-task (task 2..99) human-demonstration-horizon construct/mechanism lane only; never a rescue of BEHAVIOR policy-Q confirmatory objects",
