...
class ResearchSystemTest(unittest.TestCase):
    ...
    def test_reference_components_are_explicit(self) -> None:
        ...
        self.assertEqual((queue["summary"]["submitted"],queue["summary"]["passed_problem_gate"],queue["summary"]["blocked_problem_gate"],queue["summary"]["paper_design_eligible"]),(2,0,2,0))
        self.assertEqual(queue["summary"]["submitted_by_lane"]["ASSUMPTION_BREAK"],1)
        self.assertEqual(queue["summary"]["submitted_by_lane"]["UNEXPLAINED_BOUNDARY"],1)
        self.assertEqual(queue["summary"]["blocked_by_lane"]["ASSUMPTION_BREAK"],1)
        self.assertEqual(queue["summary"]["blocked_by_lane"]["UNEXPLAINED_BOUNDARY"],1)
        self.assertEqual({row["discovery_lane"] for row in queue["blocked"]},{"ASSUMPTION_BREAK","UNEXPLAINED_BOUNDARY"})
        self.assertIn("semantic-reduction-review-block",queue["blocked"][0]["blockers"])
