...
class ResearchSystemTest(unittest.TestCase):
    ...
    def test_fresh_phenomenon_portfolio_separates_f0_from_problem_authority(self) -> None:
        ...
        schedule=next(row for row in portfolio["candidates"] if row.get("candidate_id")=="PA-07-SCHEDULE-CONDITIONAL-EVOLUTION")
        self.assertEqual(schedule["status"],"HOLD_SUPPORT")
        self.assertEqual(schedule["paper_state"],"INCUBATION_EVIDENCE_BOUND")
        self.assertFalse(schedule["f0_design_ready"])
        self.assertEqual((schedule["evidence"]["novelty_review"] or {}).get("nearest_collision"),"arXiv:2606.02461")
    ...
    def test_asset_first_stri_quality_ready_is_separate_from_canonical_problem_gate(self) -> None:
        ...
        self.assertEqual((stri.get("summary") or {}).get("paper_quality_content_addressed_files"),29)
    ...
    def test_live_problem_discovery_rejects_shadow_portfolio_authority_leak(self) -> None:
        ...
        if generator_policy.get("search_portfolio_enabled") is True:
            ...
            # A saturated/legacy-shaped zero-call Generator keeps the historical Search Portfolio
            # outside the live transaction. Flip that actual boundary rather than assigning its
            # already-valid shadow-only value again.
        ...
        if multi_policy.get("search_portfolio_enabled") is True:
            multi_policy["one_generator_call_max"]=True
            self.assertTrue(any("one bounded discovery transaction" in error for error in validate_state(multi)))
        else:
            multi_policy["one_generator_call_max"]=False
