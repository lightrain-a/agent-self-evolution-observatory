...
class ResearchSystemTest(unittest.TestCase):
    ...
    def test_fresh_phenomenon_portfolio_separates_f0_from_problem_authority(self) -> None:
        ...
        self.assertEqual(summary["stop_reduction"],5)
        ...
        pace=next(row for row in portfolio["candidates"] if row.get("candidate_id")=="PA-06-PACE-SEARCH-CONTROL-TRANSPORT")
        self.assertEqual(pace["status"],"ARCHIVED")
        self.assertEqual(pace["support_status"],"REGISTERED_F0_STOP_NO_BIDIRECTIONAL_RANK_REVERSAL")
        self.assertTrue((pace.get("evidence") or {}).get("registered_prediction_rejected"))
        self.assertFalse((pace.get("evidence") or {}).get("principle_dead_end_certified"))
        self.assertEqual(self.state["summary"]["paper_first_fresh_phenomenon_archived"],1)
