...
class ResearchSystemTest(unittest.TestCase):
    ...
    def test_fresh_phenomenon_portfolio_separates_f0_from_problem_authority(self) -> None:
        ...
        self.assertEqual(summary["hold_support"],0)
        self.assertEqual(summary["stop_reduction"],6)
        ...
        harness=next(row for row in portfolio["candidates"] if row.get("candidate_id")=="PA-03-HARNESS-SELECTION-INVERSION")
        self.assertEqual(harness["status"],"STOP_REDUCTION")
        self.assertEqual(harness["support_status"],"PRINCIPLE_CLOSED_SELECTION_BIAS_REDUCTION")
        self.assertIn("selection-aware/winner's-curse baseline",harness["reopen_only_if"])
        pace=next(row for row in portfolio["candidates"] if row.get("candidate_id")=="PA-06-PACE-MECHANISM-REDESIGN-IDENTIFIABILITY")
        self.assertEqual(pace["status"],"STOP_REDUCTION")
        self.assertEqual(pace["support_status"],"PRINCIPLE_CLOSED_GENERIC_PROGRAM_SYNTHESIS_REDUCTION")
        self.assertTrue((pace.get("evidence") or {}).get("principle_dead_end_certified"))
        self.assertEqual((pace.get("evidence") or {}).get("stop_class"),"PRINCIPLE_STOP")
        self.assertFalse((pace.get("evidence") or {}).get("benchmark_level_dead_end_certified"))
        self.assertEqual(((pace.get("evidence") or {}).get("historical_child") or {}).get("status"),"ARCHIVED_INVALID_OPERATIONALIZATION")
        self.assertEqual(self.state["summary"]["paper_first_fresh_phenomenon_archived"],0)
        ...
        self.assertTrue(any("Fresh phenomenon portfolio" in error for error in validate_state(broken)))
    ...
    def test_reference_components_are_explicit(self) -> None:
        ...
        self.assertEqual(len(self.state["research_governance_v2"]["paper_first_macro_stages"]), 21)
        architecture = self.state["system_architecture"]
        self.assertEqual(architecture["summary"]["temporal_stages"], 21)
        ...
        self.assertTrue(self.state["research_governance_v2"]["policy"]["paper_acceptance_begins_only_after_paper_evidence_closure"])
        self.assertTrue(self.state["research_governance_v2"]["policy"]["paper_optimization_cannot_expand_scientific_claims"])
        self.assertEqual(self.state["paper_acceptance"]["summary"]["automatic_scientific_authority"], 0)
        self.assertEqual(self.state["paper_acceptance"]["summary"]["mandatory_manuscript_ci_checks"], 9)
        self.assertFalse(self.state["paper_acceptance"]["ledger_index"]["scientific_authority"])
        self.assertEqual(self.state["paper_acceptance"]["ledger_index"]["summary"]["invalid_ledgers"], 0)
        self.assertTrue(self.state["paper_acceptance"]["ledger_index"]["policy"]["public_projection_excludes_raw_reviewer_prose"])
        self.assertEqual(self.state["paper_acceptance"]["summary"]["registered_papers"], self.state["paper_acceptance"]["ledger_index"]["summary"]["papers"])
        ...
        paper_acceptance_health = next(row for row in self.state["health"]["checks"] if row["key"] == "paper-acceptance-closure")
        self.assertTrue(paper_acceptance_health["pass"])
        ...
        self.assertEqual(self.state["summary"]["p0_failure_diagnosis_required"], 27)
        self.assertEqual(self.state["summary"]["p0_failure_diagnosis_complete"], 27)
        self.assertEqual(self.state["summary"]["p0_failure_diagnosis_incomplete"], 0)
        self.assertEqual(sum(self.state["summary"]["p0_failure_layer_counts"].values()), 27)
        ...
        if primary["status"]=="READY":
            ...
            self.assertIn(primary["policy"]["typed_evidence_extraction_version"],SUPPORTED_TYPED_EVIDENCE_SNAPSHOT_VERSIONS)
        ...
        self.assertTrue(discovery["policy"]["fresh_phenomenon_recent_window_source_coverage_required"])
        self.assertTrue(discovery["policy"]["fresh_phenomenon_target_is_evidence_level_not_source_level"])
        self.assertTrue(discovery["policy"]["fresh_phenomenon_principle_closure_is_exact_evidence_sha_only"])
        self.assertTrue(discovery["policy"]["fresh_phenomenon_closure_does_not_blacklist_source"])
        self.assertTrue(discovery["policy"]["fresh_phenomenon_measured_failure_requires_failure_cue"])
        self.assertTrue(discovery["policy"]["fresh_phenomenon_shard_has_deterministic_target_ref"])
        self.assertTrue(discovery["policy"]["fresh_phenomenon_shard_has_deterministic_phenomenon_id"])
        self.assertTrue(discovery["policy"]["fresh_phenomenon_seed1_must_match_target_ref"])
        self.assertTrue(discovery["policy"]["fresh_phenomenon_seed1_must_match_target_phenomenon"])
        ...
        self.assertTrue(discovery["policy"]["temporal_exposure_relabeling_after_longitudinal_reduction_forbidden"])
        self.assertTrue(discovery["policy"]["treatment_semantics_seed_requires_executable_version_change"])
        self.assertTrue(discovery["policy"]["treatment_semantics_seed_requires_versioned_treatment_reduction_first"])
    ...
    def test_asset_first_stri_r16_ready_remains_zero_authority(self) -> None:
        ...
        self.assertGreaterEqual((stri.get("summary") or {}).get("paper_quality_content_addressed_files"),20)
        ...
        acceptance=self.state["paper_acceptance"]["summary"]
        ledger_summary=(self.state["paper_acceptance"].get("ledger_index") or {}).get("summary") or {}
        self.assertEqual(acceptance["registered_papers"],int(ledger_summary.get("papers") or 0))
        self.assertEqual(acceptance["scientific_holds"],int(ledger_summary.get("scientific_holds") or 0))
        self.assertEqual(acceptance["submission_ready_papers"],int(ledger_summary.get("submission_ready") or 0))
        self.assertEqual(acceptance["gate_clean_submission_ready_papers"],int(ledger_summary.get("gate_clean_submission_ready") or 0))
        self.assertEqual(acceptance["internal_action_required_papers"],int(ledger_summary.get("internal_action_required") or 0))
        self.assertEqual(acceptance["no_internal_action_papers"],int(ledger_summary.get("no_internal_action") or 0))
        ...
        self.assertEqual(self.state["summary"]["asset_first_stri_paper_quality_content_addressed_files"],73)
    ...
    def test_latest_shadow_terminal_is_fail_closed_and_zero_authority(self) -> None:
        ...
        self.assertTrue(any("schema-1.4/1.5 control snapshot" in error for error in validate_state(bad_control)))
        ...
        self.assertTrue(any("schema-1.4/1.5 control snapshot" in error for error in validate_state(bad_control_schema)))
    ...
    def test_legacy_typed_v2_primary_snapshot_is_supported_but_unknown_version_is_rejected(self) -> None:
        state=copy.deepcopy(self.state)
        state["paper_first_primary_evidence"]["policy"]["typed_evidence_extraction_version"]="typed-v2"
        self.assertEqual(validate_state(state),[])
        broken=copy.deepcopy(state)
        broken["paper_first_primary_evidence"]["policy"]["typed_evidence_extraction_version"]="typed-v1"
        self.assertTrue(any("supported first-party-owned typed-evidence snapshot" in error for error in validate_state(broken)))
