...
def main() -> int:
    ...
    lock_path = Path("/data/wyt/.formal-goal-pi05-no-update-model-load.lock")

    with exclusive_lock(lock_path):
        authority = json.loads(authority_path.read_text(encoding="utf-8"))
        if authority.get("status") != "AUTHORIZED_PI05_NO_UPDATE_MODEL_LOAD_QUALIFICATION":
            raise RuntimeError("model-load authority is not active")
        ...
        status = "PI05_NO_UPDATE_MODEL_LOAD_HOLD"
        error = None
        ...
        try:
            ...
            status = "PI05_NO_UPDATE_MODEL_LOAD_PASS"
        ...
        receipt = {
            "schema_version": "behavior-formal-goal-coupling-pi05-no-update-model-load-result-v1",
            "object_id": OBJECT_ID,
            ...
            "next_gate": "FROZEN_DISTRIBUTED_TRAINING_RESOURCE_ADMISSION" if train_state_ready else "MODEL_LOAD_FAILURE_REVIEW",
        }
