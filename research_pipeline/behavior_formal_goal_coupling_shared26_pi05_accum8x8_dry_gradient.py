...
V4_ADJ_SHA='1a87183e3c48bfca43fe32dab518b76498f337044eb4db506be99ad74404714f'
...
def main():
    ...
    if auth.get('status')!='AUTHORIZED_PI05_SINGLE_GPU_ACCUM8X8_NO_UPDATE_DRY_GRADIENT' or auth.get('object_id')!=OBJECT_ID or auth.get('child_id')!=CHILD_ID: raise RuntimeError('authority drift')
    ...
    initial={'schema_version':'behavior-formal-goal-coupling-shared26-pi05-accum8x8-dry-gradient-result-v1','object_id':OBJECT_ID,'child_id':CHILD_ID,'generated_at':datetime.now(timezone.utc).isoformat(),'status':'PI05_ACCUM8X8_NO_UPDATE_DRY_GRADIENT_STARTED','candidate_priority':2,'physical_micro_batch':MICRO,'accumulation_steps':K,'effective_batch':EFFECTIVE,'attempt_count_under_candidate_authority':1,'attempt_is_exactly_once':True,'authority_sha256':sha(P['authority']),'preregistration_sha256':PREREG_SHA,'v4_adjudication_sha256':V4_ADJ_SHA,'data_order_qualification_sha256':DATA_ORDER_SHA,'openpi_child_commit':child,'base_object_count_rehashed':bc,'base_bytes_rehashed':bb,'resource_scope':scope,'gpu_before':g0,'micro_gradients_completed':0,'accumulated_gradient_complete':False,'loss_value_retained_or_reported':False,'gradient_numerical_values_read':False,'optimizer_update':False,'parameter_update':False,'checkpoint_written':False,'scientific_training_started':False,'policy_rollouts_started':False,'policy_outcomes_read':False,'formal_training_authorized':False}
    with lock(Path('/data/wyt/.formal-goal-pi05-accum8x8-dry-gradient-v1.lock')):
        write_receipt(R,initial); status='PI05_ACCUM8X8_NO_UPDATE_DRY_GRADIENT_HOLD'; err=None; batch_ready=False; state_ready=False; done=0; acc=None; leaf_count=None; elem_count=None; gpu_state=None; mem_state=None; gpu_micro=[]; mem_micro=[]
        try:
