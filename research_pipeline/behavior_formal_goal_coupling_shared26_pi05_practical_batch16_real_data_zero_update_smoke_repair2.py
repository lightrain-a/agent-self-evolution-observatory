...
CONSUMED_REPAIR1_SHA = "c5f8e01aa0a657af5111f062fe39f8dd210126c5dec67b17f111ff2ad7fc49a0"
CONSUMED_REPAIR1_AUTH_SHA = "570a374a9f3e65ab9734bc7563d2937ac1a3555cd19236d767a07faa6391da93"
DATA_LOADER_TEST_SHA = "a0cf1bc293e8b0eb9bc5516bc05d30d9940da06c94fcfa1addca7267f8358815"
TRANSFORMS_SHA = "c59480647d70e2721fb7b864934a57546ae966bdefa8c89d210a2f9b99da11cd"
BATCH = 16
SEED = 42
ACTION_HORIZON = 32
EPISODES = 5200
...
def main() -> int:
    ...
    for name in [
        ...
        "consumed_repair1", "consumed_repair1_authority",
    ...
    if p["receipt"].exists():
        raise RuntimeError(f"exactly-once real-data batch16 smoke repair2 receipt exists: {p['receipt']}")
    auth = json.loads(p["authority"].read_text())
    if auth.get("status") != "AUTHORIZED_PI05_PRACTICAL_BATCH16_REAL_DATA_ZERO_UPDATE_SMOKE_REPAIR2_FINAL_ACTION_DIM":
        raise RuntimeError("real-data smoke repair2 authority not active")
    ...
    require(p["consumed_repair1"], CONSUMED_REPAIR1_SHA, "consumed real-data smoke repair1 HOLD")
    require(p["consumed_repair1_authority"], CONSUMED_REPAIR1_AUTH_SHA, "consumed real-data smoke repair1 authority")
    repair1 = json.loads(p["consumed_repair1"].read_text())
    if repair1.get("status") != "PI05_PRACTICAL_BATCH16_REAL_DATA_ZERO_UPDATE_REPAIR1_HOLD" or not repair1.get("state_ready") or repair1.get("batch_ready") or repair1.get("optimizer_update"):
        raise RuntimeError("consumed repair1 lineage drift")
    if "action shape drift: (16, 32, 32)" not in str(repair1.get("error") or ""):
        raise RuntimeError("consumed repair1 is not the frozen stale-action-shape assertion")
    ...
    require(p["openpi_child_root"] / "src/openpi/training/data_loader_test.py", DATA_LOADER_TEST_SHA, "OpenPI data_loader_test source")
    require(p["openpi_child_root"] / "src/openpi/transforms.py", TRANSFORMS_SHA, "OpenPI transforms source")
    ...
    initial = {
        "schema_version": "behavior-formal-goal-coupling-shared26-pi05-practical-batch16-real-data-zero-update-smoke-repair2-v1",
        "object_id": OBJECT_ID, "child_id": CHILD_ID, "generated_at": datetime.now(timezone.utc).isoformat(),
        "status": "PI05_PRACTICAL_BATCH16_REAL_DATA_ZERO_UPDATE_REPAIR2_STARTED", "batch_size": BATCH,
        ...
        "consumed_repair1_sha256": CONSUMED_REPAIR1_SHA, "consumed_repair1_authority_sha256": CONSUMED_REPAIR1_AUTH_SHA,
        "source_shape_contract": {"model_action_dim": 32, "data_loader_test_sha256": DATA_LOADER_TEST_SHA, "transforms_sha256": TRANSFORMS_SHA},
    ...
    with lock(Path("/data/wyt/.formal-goal-pi05-practical-batch16-real-data-zero-update-smoke-repair2.lock")):
        write_receipt(p["receipt"], initial)
        status = "PI05_PRACTICAL_BATCH16_REAL_DATA_ZERO_UPDATE_REPAIR2_HOLD"; error = None
        ...
        try:
            ...
            expected_action_shape = (BATCH, ACTION_HORIZON, cfg.model.action_dim)
            expected_state_shape = (BATCH, cfg.model.action_dim)
            if tuple(batch[1].shape) != expected_action_shape:
                raise RuntimeError(f"final training action shape drift: {batch[1].shape}/{expected_action_shape}")
            if tuple(batch[0].state.shape) != expected_state_shape:
                raise RuntimeError(f"final training state shape drift: {batch[0].state.shape}/{expected_state_shape}")
            batch_ready = True; gpu_batch = gpu_snapshot(); host_batch = host_snapshot(); jm_batch = jax_mem(device); metadata = tree_metadata(batch)
            status = "PI05_PRACTICAL_BATCH16_REAL_DATA_ZERO_UPDATE_REPAIR2_PASS"
            ...
            final = dict(initial); final.update({
                ...
                "next_gate": "FREEZE_PRACTICAL_BATCH16_FORMAL_TRAINING_AUTHORITY" if status.endswith("PASS") else "REAL_DATA_BATCH16_REPAIR2_PRETRAINING_REVIEW",
