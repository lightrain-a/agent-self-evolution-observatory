...
def host_memory_snapshot() -> dict:
    ...
    user_slice = Path('/sys/fs/cgroup/user.slice/user-1006.slice/memory.current')
...
def main() -> int:
    ...
    lock_path = Path("/data/wyt/.formal-goal-pi05-direct-device-no-update-model-load.lock")
    ...
    with exclusive_lock(lock_path):
        ...
        if authority.get("status") != "AUTHORIZED_PI05_DIRECT_DEVICE_NO_UPDATE_MODEL_LOAD_QUALIFICATION":
            raise RuntimeError("direct-device model-load authority is not active")
        ...
        status = "PI05_DIRECT_DEVICE_NO_UPDATE_MODEL_LOAD_HOLD"
        error = None
        ...
        try:
            ...
            status = "PI05_DIRECT_DEVICE_NO_UPDATE_MODEL_LOAD_PASS"
        ...
        receipt = {
            "schema_version": "behavior-formal-goal-coupling-pi05-direct-device-no-update-model-load-result-v1",
            "object_id": OBJECT_ID,
            ...
            "next_gate": "STREAMING_ACCUM8X8_DIRECT_DEVICE_REPAIR_AUTHORITY" if train_state_ready else "DIRECT_DEVICE_MODEL_LOAD_FAILURE_REVIEW",
        }
