...
The smoke artifact is `/data/wyt/agent-self-evolution-observatory/p0-runtime-smoke.json`. Readiness smoke is deliberately lightweight: it validates the real Qwen tokenizer/chat template, reads one small tensor from every indexed safetensors shard, then executes an actual ALFWorld OOD `reset → admissible-command parse → env.step`. It does **not** load the full 7B model or claim that generation works. Full model loading and generation are validated by the first formal P0 transaction; any failure leaves the run `failed` and cannot create a Pilot Registry result. The smoke artifact is bound to a runtime contract hash covering the adapter source, ALFWorld config, selected Python/model paths, and detected torch/transformers/alfworld/textworld versions, so relevant code or dependency changes automatically invalidate stale smoke results. Smoke uses its own runtime lock. Formal executions use **per-idea locks**, so A-1 and A-2 may run concurrently while duplicate launches of the same idea remain blocked.

When formal collection starts, the runner writes one isolated state file under `/data/wyt/agent-self-evolution-observatory/p0-executions/<idea-id>.json` and moves it through `running` → `collected` → `registered`, or records `failed`. Runtime readiness exposes all active states through `execution_states` and keeps `execution_state` only as a backward-compatible latest-state view. These markers are operational state only and are never treated as scientific P0 results; only validated Pilot Registry results populate measured effects.

## Execution safety policy for every P0 and later experiment

- **No end-only persistence.** Raw traces are appended after every completed environment episode. Candidate/sequence-level derived rows are appended as soon as that candidate/sequence completes. `progress.json` and the per-idea execution state are refreshed throughout the run.
- **Crash salvage is mandatory.** A killed or budget-stopped process must leave completed traces on disk; future runners should follow the same incremental/checkpoint pattern rather than accumulating an entire experiment only in RAM.
- **Budgets are enforced online.** GPU-hour, wall-hour, and episode caps are checked during collection, not only during result registration. A cap hit stops collection before an over-budget result can be registered; partial artifacts remain for diagnosis.
- **Writable paths are per run.** ALFWorld/model/data/package files are shared read-only, but every run gets its own output directory and ALFWorld `save_path`. Copying the full environment is unnecessary on one host unless dependencies conflict.
- **Concurrency is per idea/run, not global.** Distinct authorized ideas may execute in parallel on separate GPUs or servers. The scheduler must still respect real GPU memory/SM contention; logical permission to run concurrently does not imply co-locating multiple 7B rollouts on an already saturated GPU.
- **Use staged compute.** New pilots should begin with a cheap phenomenon/signal gate before expanding candidates, probes, hidden tasks, seeds, or models. Do not treat a large nominal episode cap as a reason to immediately consume the full factorial rollout budget.
