...
class BehaviorFormalGoalCouplingTest(unittest.TestCase):
    ...
    def test_declared_question_prefixed_instance_is_grounded(self) -> None:
        object_map = {
            "hot_tub.n.02": ["hot_tub.n.02_1"],
            "chlorine.n.01": ["chlorine.n.01_1"],
        }
        result = analyze_goal_state(
            [["contains", "?hot_tub.n.02_1", "?chlorine.n.01_1"]],
            object_map=object_map,
        )
        self.assertEqual(result["unbound_variable_count"], 0)
        self.assertEqual(result["atomic_goal_count"], 1)

    def test_bound_variable_precedes_declared_instance_resolution(self) -> None:
        object_map = {
            "desk_leg.n.01": ["desk_leg.n.01_1", "desk_leg.n.01_2"],
            "desk_bracket.n.01": ["desk_bracket.n.01_1"],
        }
        goal = [[
            "forall", ["?desk_leg.n.01", "-", "desk_leg.n.01"],
            ["and",
                ["attached", "?desk_leg.n.01", "?desk_bracket.n.01_1"],
                ["clean", "?desk_leg.n.01"],
            ],
        ]]
        result = analyze_goal_state(goal, object_map=object_map)
        self.assertEqual(result["unbound_variable_count"], 0)
        self.assertEqual(result["shared_argument_edge_count"], 1)

    def test_mixed_ground_bound_and_truly_unresolved_tokens(self) -> None:
        object_map = {"cup.n.01": ["cup.n.01_1"]}
        goal = [[
            "forall", ["?x", "-", "cup.n.01"],
            ["and",
                ["nextto", "?x", "?cup.n.01_1"],
                ["inside", "?unknown", "?cup.n.01_1"],
            ],
        ]]
        result = analyze_goal_state(goal, object_map=object_map)
        self.assertEqual(result["unbound_variable_count"], 1)
