...
def main() -> int:
    ...
    if authority.get("status") != "AUTHORIZED_PI05_B2_LEAF_BATCHED_CHECKPOINT_QUALIFICATION_69":
    ...
    if review.get("verdict") != "APPROVE" or review.get("required_host") != "69" or review.get("host67_forbidden") is not True:
    ...
    if Path("/etc/machine-id").read_text().strip() != EXPECTED_MACHINE_ID:
        raise RuntimeError("B2 may run only on host69 machine-id")
