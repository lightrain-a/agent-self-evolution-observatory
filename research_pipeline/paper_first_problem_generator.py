...
def _private_dead_end_prompt_memory(storage:StorageSettings,public_memory:dict[str,Any],current_refs:set[str]|None=None)->dict[str,Any]:
    ...
    wiki=load_research_memory_wiki();query_pack=compile_research_memory_query_pack(wiki,purpose="IDEA_SEARCH",context={"source_refs":sorted(current_refs or set()),"blocked_by_lane":blocked_by_lane,"top_reduction_basin":public_memory.get("top_reduction_basin") or {}})
    return {
        ...
        "research_memory_query_pack":query_pack,
        "research_memory_query_pack_sha256":query_pack.get("query_pack_sha256",""),
        "scientific_authority":False,
    }
...
def _is_current_operator_receipt(row:dict[str,Any],pool_sha:str,negative_space_sha:str,execution_mode:str)->bool:
    return bool(
        row.get("status") in {"GENERATED_ZERO_CANDIDATES","GENERATED_PRE_F0_EVIDENCE_ACQUISITION","GENERATED_AWAIT_PROBLEM_GATE"}
...
def _record_saturation_run(storage:StorageSettings,state:dict[str,Any],pool_sha:str,registry:dict[str,dict[str,Any]],saturation_ledger_path:Path|None=None)->None:
    if state.get("status") not in {"GENERATED_ZERO_CANDIDATES","GENERATED_PRE_F0_EVIDENCE_ACQUISITION","GENERATED_AWAIT_PROBLEM_GATE"}: return
...
def _pre_f0_candidate_row(normalized:dict[str,Any],route:dict[str,Any])->dict[str,Any]:
    ...
    row={
        ...
        "irreducible_object":str(normalized.get("irreducible_object") or "").strip(),
...
def installed_problem_generator_policy(*, portfolio: bool = True) -> dict[str, Any]:
    """Return the currently installed generator harness policy without relabeling old receipts."""
...
def _merge_portable_review_receipts(state:dict[str,Any],previous:dict[str,Any])->dict[str,Any]:
    ...
    for row in rows:
        ...
        if not run_id or len(refs)<4 or status not in {"GENERATED_ZERO_CANDIDATES","GENERATED_PRE_F0_EVIDENCE_ACQUISITION","GENERATED_AWAIT_PROBLEM_GATE","EXTERNAL_FRESH_INTAKE_REVIEWED"} or row.get("scientific_authority") is not False:
            continue
