...
SPLIT_SHA = "21fe79b83b33ea79f3d82bda9a3770b40e87c8685a63a573c44877987976677e"
PROBES = ROOT / "paper_drafts/c1-manuscript-strengthening-20260825/c1-pacta-msr-atomgit-qwen38-fresh4-probe-specs-20260903.json"
PROBES_SHA = "fa94289f89f7cda882cfc4f00ae967fb377001dd370e6b7f9c7a9db66886dc17"
...
Q03 = ROOT / "paper_drafts/c1-manuscript-strengthening-20260825/c1-pacta-msr-atomgit-qwen38-q03-output-mcp-closure-20260903.json"
Q03_SHA = "af311a6a2785bff2d06cc12febf5288de5f4759a156d3ca4ac0407cd550837ea"
OUT = ROOT / "paper_drafts/c1-manuscript-strengthening-20260825/c1-pacta-msr-atomgit-qwen38-fresh4-source-schedule-20260903.json"
ORDER_SALT = "C1-PACTA-MSR-ATOMGIT-QWEN38-FRESH4-SOURCE-v1"
...
def prepare() -> dict[str, Any]:
    for path, expected, label in (
        ...
        (Q02, Q02_SHA, "q02"),
        (Q03, Q03_SHA, "q03-controlled-output"),
    ):
        if not path.is_file() or sha256_file(path) != expected:
            raise RuntimeError(f"STOP_FRESH4_SOURCE_SCHEDULE_INPUT_DRIFT:{label}")
    ...
    if q02.get("selected_source_budget") != 32768 or q02.get("invocation_timeout_seconds") != 900:
        raise RuntimeError("STOP_FRESH4_SOURCE_Q02_PARAMETER_DRIFT")
    ...
    units = sorted(pool["units"], key=lambda unit: (sha(ORDER_SALT + "|" + unit["unit_id"]), unit["unit_id"]))
    rows = []
    for sequence, unit in enumerate(units, 1):
        rows.append({
            ...
            "sampling_stratum": unit.get("sampling_stratum"),
            ...
            "runtime_binding": "DEFERRED_UNTIL_FRESH4_20_RUNTIME_READY",
        })
    if len(rows) != 10 or len({row["source_task_id"] for row in rows}) != 10:
        raise RuntimeError("STOP_FRESH4_SOURCE_SCHEDULE_GEOMETRY")
    return {
        "schema_version": 1,
        "experiment": "C1-PACTA-MSR-ATOMGIT-QWEN38-FRESH4-SOURCE-T0-20260903",
        "status": "FRESH4_SOURCE_SCHEDULE_FROZEN_PRE_SOURCE_OUTCOME",
        "fresh_pool_sha256": POOL_SHA,
        "pilot_split_sha256": SPLIT_SHA,
        "probe_specs_sha256": PROBES_SHA,
        "q02_result_sha256": Q02_SHA,
        "q03_controlled_output_sha256": Q03_SHA,
        "bridge_schema": "c1-controlled-output-mcp-submit-output-v1",
        "allowed_tool": "mcp__c1output__submit_output",
        "action_kind": "bash_action",
        "host_tools_allowed": False,
...
def main() -> None:
    if OUT.exists():
        raise RuntimeError("fresh4 source schedule exists; no overwrite")
    result = prepare()
