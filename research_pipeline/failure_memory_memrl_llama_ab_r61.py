#!/usr/bin/env python3
"""R61: Llama executor replication of the frozen 32-cluster A/B provenance-exposure contrast."""
...
try:
    ...
    from . import failure_memory_memrl_llama_utilization_r60 as r60
except ImportError:
    import failure_memory_memrl_ab_identification_r48 as r48  # type: ignore
    import failure_memory_memrl_llama_utilization_r60 as r60  # type: ignore

PAPER_ID="D2-PAPER-FAILURE-MEMORY-PROVENANCE"
CONTRACT_STATUS="R61_LLAMA_FRESH_AB_REPLICATION_CONTRACT_FROZEN_PRE_PROBE"
AUTH_STATUS="R61_LLAMA_FRESH_AB_CONDITIONAL_AUTHORITY_FROZEN_PRE_PROBE"
...
def preflight(manifest_path:pathlib.Path,r60_auth_path:pathlib.Path,contract_path:pathlib.Path,authority_path:pathlib.Path,
              r54_path:pathlib.Path,selection_path:pathlib.Path,frozen_path:pathlib.Path,source_receipt_path:pathlib.Path,parser_receipt_path:pathlib.Path,util_path:pathlib.Path):
    m,r60auth,contract,auth,q,sel,frozen,src,parser,util=map(load,[manifest_path,r60_auth_path,contract_path,authority_path,r54_path,selection_path,frozen_path,source_receipt_path,parser_receipt_path,util_path])
    if any(x.get("paper_id")!=PAPER_ID for x in [m,r60auth,contract,auth,q,sel,frozen,src,parser,util]): raise RuntimeError("R61-paper-id-drift")
    if contract.get("status")!=CONTRACT_STATUS or auth.get("status")!=AUTH_STATUS: raise RuntimeError("R61-contract-authority-status-drift")
    if not all(valid(x) for x in [contract,auth,util]): raise RuntimeError("R61-receipt-hash-drift")
    # Re-run the Llama-specific R60 preflight so host/runtime/selection/frozen support and parser qualification are revalidated.
    _,_,_,_,_,_,_=r60.preflight(manifest_path,r60_auth_path,r54_path,selection_path,frozen_path,source_receipt_path,parser_receipt_path)
    if util.get("status")!=UTIL_PASS or util.get("pass") is not True or util.get("primary_32_clusters_authorized_next") is not True: raise RuntimeError("R61-utilization-not-pass")
    if int(util.get("primary_confirmatory_outcomes_observed") or 0)!=0: raise RuntimeError("R61-primary-already-opened")
    b=auth.get("bindings") or {}
    checks={"manifest_file_sha256":r48.sha(manifest_path),"contract_file_sha256":r48.sha(contract_path),"r60_authority_file_sha256":r48.sha(r60_auth_path),
    ...
    for k,v in checks.items():
        if b.get(k)!=v: raise RuntimeError(f"R61-binding-drift:{k}")
    if (auth.get("authority") or {}).get("A_B_execution_conditionally_after_R60_PASS") is not True or (auth.get("authority") or {}).get("C_D_execution") is not False: raise RuntimeError("R61-authority-scope-drift")
    e=m["execution_manifest"]; ids=[str(x) for x in e["confirmatory_units"]["representative_ids"]]
    if len(ids)!=32 or ids_hash(ids)!=e["confirmatory_units"]["representative_ids_sha256"] or ids!=[str(x) for x in sel.get("primary_representative_ids") or []]: raise RuntimeError("R61-primary-id-drift")
    records=list(sel.get("primary_records") or []); by={str(x.get("validation_task_id")):x for x in records}
    if set(by)!=set(ids) or len(by)!=32: raise RuntimeError("R61-primary-record-drift")
    make_prompt=r48.prompt_builder(m); surfaces={}; prompt_hashes={}
    for tid in ids:
        selected=list(by[tid].get("selected") or [])
        if not selected or any(x.get("eligible") is not True for x in selected): raise RuntimeError(f"R61-primary-selected-ineligible:{tid}")
        pair=r48.render_pair(selected,tid); surfaces[tid]=pair
        if "source_outcome_success" in pair["A_content_only"]: raise RuntimeError(f"R61-A-provenance-leak:{tid}")
        if pair["audit"].get("actionable_content_identical") is not True or pair["audit"].get("only_executor_visible_difference")!="source_outcome_success": raise RuntimeError(f"R61-adapter-audit-drift:{tid}")
...
def build_plan(m:dict[str,Any],contract:dict[str,Any],surfaces:dict[str,dict[str,Any]],prompt_hashes:dict[str,str])->dict[str,Any]:
    ...
    p={"schema_version":"1.0","paper_id":PAPER_ID,"role":"R61_FRESH_AB_SCHEDULE_FROZEN_PRE_TREATMENT","contract_receipt_sha256":contract["receipt_sha256"],
...
def main()->None:
    ...
    for x in ["manifest","r60-authorization","contract","authority","r54v2-qualification","selection","frozen-retrieval","source-receipt","parser-receipt","utilization-receipt","output-dir"]: p.add_argument("--"+x,type=pathlib.Path,required=True)
    p.add_argument("--resume",action="store_true"); a=p.parse_args(); out=a.output_dir.resolve(); out.mkdir(parents=True,exist_ok=True)
    m,contract,auth,sel,util,surfaces,prompt_hashes=preflight(a.manifest.resolve(),a.r60_authorization.resolve(),a.contract.resolve(),a.authority.resolve(),
        a.r54v2_qualification.resolve(),a.selection.resolve(),a.frozen_retrieval.resolve(),a.source_receipt.resolve(),a.parser_receipt.resolve(),a.utilization_receipt.resolve())
    ...
    if plan.get("plan_sha256")!=r48.digest({k:v for k,v in plan.items() if k!="plan_sha256"}): raise RuntimeError("R61-plan-hash-drift")
    ...
    if sk!=schedule[:len(sk)] or ck!=schedule[:len(ck)]: raise RuntimeError("R61-ledger-not-schedule-prefix")
    if len(started)!=len(complete): raise RuntimeError("R61_EXPOSED_INCOMPLETE_ARM_NO_RETRY")
    if complete and len(complete)<len(schedule) and not a.resume: raise RuntimeError("R61-partial-requires-explicit-resume")
    if len(complete)<len(schedule):
        ...
        for ordinal,tid,arm in schedule[len(complete):]:
            ...
            if hashlib.sha256(ctx.encode()).hexdigest()!=expected["memory_context_sha256"] or hashlib.sha256(prompt.encode()).hexdigest()!=expected["system_prompt_sha256"]: raise RuntimeError("R61-treatment-surface-drift")
    ...
    if len(complete)!=64: raise RuntimeError("R61-A-B-run-incomplete")
    result=r48.analyze(plan,complete,contract); result.update({"role":"R61_LLAMA_FRESH_AB_COMPLETE_ONLY_ANALYSIS","status":"FRESH_AB_IDENTIFICATION_ESTIMATE_COMPLETE",
