#!/usr/bin/env python3
"""Freeze deterministic, branch-blind MSR probe commands for the fresh2 pool.
...
any fresh2 source trajectory is acquired.
"""
...
OUT = ROOT / "paper_drafts/c1-manuscript-strengthening-20260825/c1-pacta-msr-atomgit-qwen38-fresh2-probe-specs-20260903.json"
TOKEN_SALT = "C1-PACTA-MSR-FRESH2-PROBE-TOKEN-v1"
STOP = {
...
def compile_tokens(task: str, task_sha: str) -> list[str]:
    ...
    if len(ranked) < 3:
        raise RuntimeError("STOP_FRESH2_MSR_PROBE_TOKEN_SUPPORT_INSUFFICIENT")
...
def prepare() -> dict[str, Any]:
    if sha256_file(POOL) != POOL_SHA:
        raise RuntimeError("fresh2 pool drift")
    ...
    if len(rows) != 10 or len({row["future_task_id"] for row in rows}) != 10:
        raise RuntimeError("fresh2 probe geometry drift")
    return {
        "schema_version": 1,
        "experiment": "C1-PACTA-MSR-ATOMGIT-QWEN38-FRESH2-PROBE-SPECS-20260903",
        "status": "FRESH2_MSR_10_PROBE_COMMANDS_FROZEN_PRE_SOURCE_OUTCOME",
...
def main() -> None:
    if OUT.exists():
        raise RuntimeError("fresh2 probe specs already exist; no overwrite")
    result = prepare()
