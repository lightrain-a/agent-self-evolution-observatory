(()=>{
  ...
  const e1QuickOverview=(paper,story,detail)=>{
    ...
    return `<section class="cpp-plain cpp-e1-overview panel" id="quick-overview"><div class="cpp-section-kicker">${isZh()?"0 · 先看懂问题":"0 · START HERE"}</div><h2>${isZh()?"30 秒先抓住这篇论文在研究什么":"The paper in 30 seconds"}</h2><p class="cpp-e1-hook">${isZh()?"如果 Agent 会的东西完全没变，仅仅把一个技能拆成四个，它为什么会做出不同决定？":"If an agent's capabilities do not change, why should splitting one skill into four change its decisions?"}</p><div class="cpp-e1-overview-grid"><article><span>${isZh()?"先看一个例子":"ONE EXAMPLE"}</span><p>${E(example)}</p></article><article><span>${isZh()?"本文真正的问题":"THE QUESTION"}</span><p>${E(question)}</p></article><article class="answer"><span>${isZh()?"一句话答案":"THE ANSWER"}</span><p>${E(answer)}</p></article></div><div class="cpp-e1-findings"><b>${isZh()?"如果只想先建立直觉，可以先看这三条":"Three intuition-building takeaways before the full story"}</b><div>${findings.map((x,i)=>`<article><span>0${i+1}</span><p>${E(x)}</p></article>`).join("")}</div></div><div class="cpp-term-strip"><span><b>Skill package</b>${isZh()?"一个被控制器单独看待的技能单元":"a skill unit the controller treats separately"}</span><span><b>Semantic capability</b>${isZh()?"Agent 真正会做什么":"what the agent can actually do"}</span><span><b>Representation invariance</b>${isZh()?"能力不变时，换包装不应改变控制":"repackaging alone should not change control"}</span></div><div class="cpp-e1-status-strip"><span><b>${isZh()?"核心 E1":"Canonical E1"}</b>${isZh()?"3/3 核心窄主张已有证据支持":"3/3 narrow claims supported"}</span><span><b>${isZh()?"外部扩展":"External extension"}</b>${isZh()?"已执行，但当前不纳入论文结论":"executed, but not used in the paper claim"}</span></div></section>`;
  };
  ...
  const e1Work=(paper,story,detail)=>{
    ...
    return `<section class="panel cpp-e1-work" id="mechanism"><div class="cpp-section-kicker">${isZh()?"3 · 我们做了什么":"3 · WHAT WE DID"}</div><h2>${isZh()?"从工程现象到可证伪问题：我们做了哪些工作，为什么都需要？":"From an engineering observation to a falsifiable question: what did we do, and why?"}</h2><div class="cpp-work-grid">${work.map((x,i)=>`<article><span>${String(i+1).padStart(2,"0")}</span><div><b>${E(x[0])}</b><strong>${E(x[1])}</strong><p>${E(x[2])}</p><small>${E(x[3])}</small></div></article>`).join("")}</div><div class="cpp-simple-chain"><b>${isZh()?"先用一条简单链理解整篇论文":"A simple way to read the paper's chain"}</b><div>${chain.map((x,i)=>`${i?'<i>→</i>':''}<span>${E(x)}</span>`).join("")}</div><p>${isZh()?"STRI 先检查“只换表示，控制是否会不合理地改变”，再用 P19 验证这种差异能否继续传到后面的真实行为。":"STRI audits unwanted sensitivity at the representation/control boundary; P19 then checks whether that difference propagates to real behavior."}</p></div>${readerDesignDetails(detail,story)}</section>`;
  };
  const e1Results=(paper,story,detail)=>{
    ...
    return `<section class="panel cpp-e1-results" id="experiment-results"><div class="cpp-section-kicker">${isZh()?"4 · 实验回答了什么":"4 · WHAT THE EXPERIMENTS ANSWER"}</div><h2>${isZh()?"按科学问题读结果：哪些已经回答，哪些边界仍然必须保留？":"Read the evidence by scientific question: what is answered, and what boundaries remain?"}</h2><div class="cpp-e1-rq-grid">${rows.map(x=>`<article><header><span>${E(x[0])}</span><b>${E(x[1])}</b></header><p class="cpp-rq-how"><strong>${isZh()?"怎么测：":"How: "}</strong>${E(x[2])}</p><div class="cpp-rq-answer"><strong>${isZh()?"看到什么":"What we found"}</strong><p>${E(x[3])}</p></div></article>`).join("")}</div><h3 data-toc="false">${isZh()?"目前最关键的证据":"Evidence worth remembering"}</h3><div class="cpp-proof-grid cpp-proof-grid-three">${evidence.map(x=>`<article><strong>${E(x[0])}</strong><p>${E(x[1])}</p></article>`).join("")}</div><div class="cpp-now"><b>${isZh()?"这些结果合起来说明什么":"What the evidence means together"}</b><p>${isZh()?"“技能怎么拆”不是永远无害的实现细节。E1 先把这种表示敏感性单独隔离出来，再给出它什么时候能靠调权重修复、什么时候会受结构限制的精确边界，并在一个冻结 Agent 场景里看到它确实可以传到执行行为。":"Skill packaging is not always a harmless implementation detail. E1 isolates representation sensitivity, identifies when package weights can or cannot repair it, and shows on one frozen agent substrate that it can propagate to executed behavior."}</p></div>${readerEvidenceDetails(detail,story)}</section>`;
  };
  ...
  const e1Evolution=(paper,detail)=>{
    ...
    return `<div class="cpp-evolution-phase"><header><span>${isZh()?"主线时间线":"MAIN TIMELINE"}</span><b>${isZh()?"从工程问题到今天的 STRI，真正发生过哪些转折":"The actual turns from an engineering question to STRI"}</b></header><div class="cpp-evolution cpp-evolution-detailed cpp-e1-evolution">${rows.map(x=>`<article><span>${E(x[0])}</span><div><strong>${E(x[1])}</strong><p>${E(x[2])}</p></div></article>`).join("")}</div></div>${detail?.lineage?.length?`<div class="cpp-evolution-phase decision"><header><span>${isZh()?"关键科研决策":"RESEARCH DECISIONS"}</span><b>${isZh()?"哪些路线被主动放弃，哪些边界必须一直保留":"What was deliberately abandoned, and which boundaries must remain"}</b></header>${lineage(detail)}</div>`:""}<div class="cpp-evolution-current"><b>${isZh()?"为什么今天是这个版本":"Why the paper has this form today"}</b><p>${E(T(paper.thesis))}</p><p>${isZh()?"核心 E1 已经有自己的 exact certificate 与限定行为桥；ReasoningBank 是后来增加的外部有效性扩展，它的 evaluator/provider gate 失败只能让扩展 HOLD，不能倒灌改写 canonical 结论。":"Canonical E1 already has its exact certificate and bounded behavioral bridge. ReasoningBank is a later external-validity extension; its evaluator/provider gate failure can only hold the extension, not rewrite the canonical result."}</p></div>`;
  };
  ...
  const readerGapDetails=(dossier)=>{
    ...
    return `<div class="cpp-reader-context cpp-reader-gap-context"><h3 data-toc="false">${isZh()?"再往深一层：这些缺口为什么真的会影响论文结论？":"One level deeper: why these gaps matter to the scientific claim"}</h3>${gaps.length?`<div class="cpp-reader-gap-grid">${gaps.map(x=>`<article><b>${E(isZh()?(x.title_zh||x.title_en):(x.title_en||x.title_zh))}</b><p>${E(isZh()?(x.text_zh||x.text_en):(x.text_en||x.text_zh))}</p></article>`).join("")}</div>`:""}${approaches.length?`<div class="cpp-reader-approach-list">${approaches.map(a=>`<article><b>${E(a.name||"")}</b><p><strong>${isZh()?"现有方法在做什么：":"What it does: "}</strong>${E(storyText(a,"how"))}</p><p><strong>${isZh()?"为什么还不够：":"Why it is still insufficient: "}</strong>${E(storyText(a,"problem"))}</p></article>`).join("")}</div>`:""}</div>`;
  };
  const readerDesignDetails=(detail,story)=>{
    if(!detail)return "";
    const requirements=(story?.design_requirements||[]);
    return `<div class="cpp-reader-context cpp-reader-design-context"><h3 data-toc="false">${isZh()?"为什么这个实验设计能回答问题，而不是只做一个漂亮对比？":"Why this design identifies the question instead of merely producing a comparison"}</h3>${detail.design?`<p class="cpp-reader-long-lead">${E(T(detail.design))}</p>`:""}${requirements.length?`<div class="cpp-reader-requirements"><b>${isZh()?"设计必须同时满足":"The design must satisfy"}</b>${list(requirements,"good")}</div>`:""}${detail.contract?.length?`<h4>${isZh()?"冻结合同：哪些东西不能跟着 treatment 一起变":"Frozen contract: what cannot change with the treatment"}</h4>${contract(detail)}`:""}${detail.arms?.length?`<h4>${isZh()?"处理组与对照组：每一组到底排除什么解释":"Treatment and controls: what each arm rules out"}</h4>${arms(detail)}`:""}${detail.analysis?.length?`<h4>${isZh()?"分析时真正看什么":"What the analysis actually reads"}</h4>${analysisPlan(detail)}`:""}</div>`;
  };
  const readerEvidenceDetails=(detail,story)=>{
    const fullRqs=storyExperiments(story);
    if(!fullRqs&&!detail?.interpretation)return "";
    return `<div class="cpp-reader-context cpp-reader-evidence-context"><h3 data-toc="false">${isZh()?"完整一点看：哪些问题已经回答，哪些仍然没有答案？":"A fuller reading: what is answered, and what remains unresolved?"}</h3>${fullRqs}${interpretation(detail)}</div>`;
  };
  const evolutionRows=(rows)=>`<div class="cpp-evolution cpp-evolution-detailed">${(rows||[]).map((x,i)=>`<article><span>${String(i+1).padStart(2,"0")}</span><div><strong>${E(T(x.title||x.t))}</strong><p>${E(T(x.body||x.b))}</p></div></article>`).join("")}</div>`;
  const readerEvolution=(paper,detail,spec,customRows=null)=>{
    ...
    return `<section class="panel cpp-beginner-evolution cpp-full-evolution" id="paper-evolution"><div class="cpp-section-kicker">${isZh()?"6 · 完整研究演变与关键决策":"6 · FULL RESEARCH EVOLUTION & DECISIONS"}</div><h2>${E(spec.evolutionTitle)}</h2><p class="cpp-reader-long-lead">${isZh()?"这里不是把论文压成固定的六步模板，而是把真正改变研究方向的节点完整保留下来：早期直觉、反例、失败解释、审稿或 novelty 碰撞、被放弃的路线，以及为什么最后收敛到今天这个版本。":"This is not a fixed six-step template. It preserves the events that actually changed the project: early intuitions, counterexamples, failed explanations, review or novelty collisions, abandoned routes, and the decisions that produced the current formulation."}</p>${chronology.length?`<div class="cpp-evolution-phase"><header><span>${isZh()?"主线时间线":"MAIN TIMELINE"}</span><b>${isZh()?"研究问题和论文故事是怎样一步步改变的":"How the question and paper story changed"}</b></header>${evolutionRows(chronology)}</div>`:""}${decisions.length?`<div class="cpp-evolution-phase decision"><header><span>${isZh()?"关键科研决策":"RESEARCH DECISIONS"}</span><b>${isZh()?"哪些路线被主动放弃，为什么不能再往回走":"What was deliberately abandoned, and why"}</b></header>${lineage(detail)}</div>`:""}<div class="cpp-evolution-current"><b>${isZh()?"为什么今天是这个版本":"Why the paper has this form today"}</b><p>${E(T(paper.thesis))}</p><p><strong>${isZh()?"当前状态：":"Current state: "}</strong>${E(spec.statusHeadline||T(paper.experiment?.headline))}</p><p><strong>${isZh()?"下一步不是随便补实验，而是：":"The next step is not arbitrary extra experimentation: "}</strong>${E(spec.nextPlain||T(paper.next))}</p></div></section>`;
  };
  const beginnerGaps=(paper,spec,dossier)=>`<section class="panel cpp-related-summary cpp-beginner-gaps" id="related-work-comparison"><div class="cpp-section-kicker">${isZh()?"2 · 现有研究缺什么":"2 · WHAT PRIOR WORK STILL MISSES"}</div><h2>${isZh()?"为什么不能直接用现有思路回答？":"Why do existing approaches not answer the exact question?"}</h2><p class="cpp-design-lead">${E(spec.gapLead)}</p><div class="cpp-related-summary-grid">${spec.gaps.map((x,i)=>`<article><span>${String(i+1).padStart(2,"0")}</span><b>${E(x[0])}</b><p><strong>${isZh()?"已经能做：":"What it already does: "}</strong>${E(x[1])}</p><p><strong>${isZh()?"还缺：":"What is still missing: "}</strong>${E(x[2])}</p></article>`).join("")}</div>${summaryLiterature(dossier)}${readerGapDetails(dossier)}<div class="cpp-gap-callout"><b>${isZh()?`${paper.code} 真正切入的位置`:`Where ${paper.code} enters`}</b><p>${E(T(paper.thesis))}</p></div></section>`;
  const beginnerWork=(spec,detail,story)=>`<section class="panel cpp-e1-work cpp-beginner-work" id="mechanism"><div class="cpp-section-kicker">${isZh()?"3 · 我们具体做了什么":"3 · WHAT WE DID"}</div><h2>${isZh()?"我们围绕这个问题做了哪些工作，为什么每一层都需要？":"What did we build around this question, and why is each layer necessary?"}</h2><div class="cpp-work-grid">${spec.work.map((x,i)=>`<article><span>${String(i+1).padStart(2,"0")}</span><div><b>${E(x[0])}</b><strong>${E(x[1])}</strong><p>${E(x[2])}</p></div></article>`).join("")}</div><div class="cpp-simple-chain"><b>${isZh()?"先用一条简单链理解整篇论文":"A simple chain for the whole paper"}</b><div>${spec.chain.map((x,i)=>`${i?'<i>→</i>':''}<span>${E(x)}</span>`).join("")}</div></div>${readerDesignDetails(detail,story)}</section>`;
  const beginnerResults=(spec,detail,story)=>`<section class="panel cpp-e1-results cpp-beginner-results" id="experiment-results"><div class="cpp-section-kicker">${isZh()?"4 · 实验现在回答了什么":"4 · WHAT THE EVIDENCE ANSWERS"}</div><h2>${isZh()?"按科学问题读结果：已经回答什么，还没回答什么？":"Read the evidence by scientific question: what is answered and what is not?"}</h2><div class="cpp-e1-rq-grid">${spec.rqs.map(x=>`<article><header><span>${E(x[0])}</span><b>${E(x[1])}</b></header><p class="cpp-rq-how"><strong>${isZh()?"怎么测：":"How: "}</strong>${E(x[2])}</p><div class="cpp-rq-answer"><strong>${isZh()?"当前答案":"Current answer"}</strong><p>${E(x[3])}</p></div></article>`).join("")}</div><h3 data-toc="false">${isZh()?"目前最关键的证据":"Evidence worth remembering"}</h3><div class="cpp-proof-grid cpp-proof-grid-three">${spec.evidence.map(x=>`<article><strong>${E(x[0])}</strong><p>${E(x[1])}</p></article>`).join("")}</div>${readerEvidenceDetails(detail,story)}</section>`;
  const beginnerContributions=(spec,detail)=>`<section class="panel cpp-e1-contributions cpp-beginner-contributions" id="claim-boundary"><div class="cpp-section-kicker">${isZh()?"5 · 最终贡献与边界":"5 · CONTRIBUTIONS & BOUNDARIES"}</div><h2>${isZh()?"所以这篇论文 / 科学对象真正让我们多知道了什么？":"What do we know because of this paper or scientific object?"}</h2><div class="cpp-contribution-grid">${spec.contributions.map(x=>`<article><span>${E(x[0])}</span><div><b>${E(x[1])}</b><p>${E(x[2])}</p></div></article>`).join("")}</div>${detail?.interpretation?.importance?`<div class="cpp-contribution-why"><b>${isZh()?"为什么读者应该在意这件事":"Why this matters beyond this one experiment"}</b><p>${E(T(detail.interpretation.importance))}</p></div>`:""}<div class="cpp-boundary-box"><b>${isZh()?"同样重要：现在明确不能写成什么":"Equally important: what cannot currently be claimed"}</b>${list(spec.limits.map(x=>({zh:x,en:x})),"boundary")}</div></section>`;
  ...
  const renderBeginnerPage=(pageId,paper,reg,detail,story)=>{
    ...
    return `<main class="cpp-page cpp-e1-page cpp-beginner-page" data-paper-order="${paper.order}"><header class="cpp-hero"><div class="cpp-hero-top"><span class="cpp-index">${String(paper.order).padStart(2,"0")}</span><div class="cpp-badges">${statusBadge(paper,reg)}</div></div><div class="eyebrow">${E(T(paper.area))}</div><h1>${E(T(paper.title))}</h1><p class="cpp-hero-subtitle">${E(spec.hook)}</p><p class="cpp-canonical-title">${E(paper.canonicalTitle)}</p>${heroLinks(paper,reg)}</header>${beginnerQuick(paper,spec)}${beginnerOrigin(spec)}${beginnerGaps(paper,spec,story||detail)}${beginnerWork(spec,detail,story)}${beginnerResults(spec,detail,story)}${beginnerContributions(spec,detail)}${readerEvolution(paper,detail,spec)}${beginnerStatus(paper,reg,spec)}${beginnerDeepDive(paper,reg,detail,story,spec)}<div class="cpp-back-collection"><a href="selected-paper.html">${isZh()?"← 返回当前论文合集":"← Back to current paper collection"}</a></div></main>`;
  };
