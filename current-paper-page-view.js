(()=>{
  ...
  const beginnerGaps=(paper,spec,dossier,detail)=>`<section class="panel cpp-related-summary cpp-beginner-gaps" id="related-work-comparison"><div class="cpp-section-kicker">${isZh()?"2 · 现有研究缺什么":"2 · WHAT PRIOR WORK STILL MISSES"}</div><h3 class="cpp-subsection-title">${isZh()?"为什么不能直接用现有思路回答？":"Why do existing approaches not answer the exact question?"}</h3><p class="cpp-design-lead">${E(spec.gapLead)}</p><div class="cpp-related-summary-grid">${spec.gaps.map((x,i)=>`<article><span>${String(i+1).padStart(2,"0")}</span><b>${E(x[0])}</b><p><strong>${isZh()?"已经能做：":"What it already does: "}</strong>${E(x[1])}</p><p><strong>${isZh()?"还缺：":"What is still missing: "}</strong>${E(x[2])}</p></article>`).join("")}</div>${summaryLiterature(dossier)}${readerGapDetails(dossier)}${readerPaperCase(paper,dossier,detail)}<div class="cpp-gap-callout"><b>${isZh()?`${paper.code} 真正切入的位置`:`Where ${paper.code} enters`}</b><p>${E(T(paper.thesis))}</p></div></section>`;
