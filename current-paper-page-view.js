(()=>{
  ...
  const experimentProvenance=(pageId)=>{
    ...
    if(!e)return "";
    const sources=(e.sources||[]).map((s,i)=>`<article class="cpp-exp-source"><header><span>${String(i+1).padStart(2,"0")}</span><div><b>${E(s.name||"")}</b><small>${E(s.paper||"")}</small></div>${sourceBadge(s)?`<em class="cpp-venue-tag">${E(sourceBadge(s))}</em>`:""}</header>${s.url?`<a class="cpp-source-link" href="${E(s.url)}" target="_blank" rel="noopener">${isZh()?"原始论文 / 来源 ↗":"Source paper ↗"}</a>`:""}<dl><div><dt>${isZh()?"原始数据是什么":"Original source"}</dt><dd>${E(T(s.original))}</dd></div><div><dt>${isZh()?"我们实际用了哪一部分":"Our slice"}</dt><dd>${E(T(s.slice))}</dd></div><div><dt>${isZh()?"我们在此基础上怎么改":"How we transform it"}</dt><dd>${E(T(s.transform))}</dd></div></dl></article>`).join("");
    const models=(e.models||[]).map(m=>`<article><b>${E(m.name||"")}</b><p>${E(T(m.role))}</p><small>${E(T(m.status))}</small></article>`).join("");
    const quantities=(e.quantities||[]).map(q=>`<span><b>${E(q.v||"")}</b><small>${E(T(q.k))}</small></span>`).join("");
    const readouts=(e.readouts||[]).map(x=>`<span>${E(x)}</span>`).join("");
    return `<div class="cpp-experiment-provenance" id="experiment-provenance"><header><div><span>${isZh()?"实验全景":"EXPERIMENT AT A GLANCE"}</span><h4>${isZh()?"数据从哪篇论文来、我们怎么改、测什么模型、到底有多少实验单位":"Source → our slice → treatment → models → exact scale"}</h4></div><p>${E(T(e.intro))}</p></header><section><h4>${isZh()?"A · 数据 / benchmark 来源，以及我们怎么改造成实验":"A · Dataset / benchmark provenance and transformation"}</h4><div class="cpp-exp-source-grid">${sources}</div></section><section><h4>${isZh()?"B · 被测模型 / evaluator / backbone":"B · Tested models / evaluators / backbones"}</h4><div class="cpp-exp-model-grid">${models}</div></section><section><h4>${isZh()?"C · 实验量级：这些数字分别在数什么":"C · Experimental scale and what each count means"}</h4><div class="cpp-exp-quantity-strip">${quantities}</div></section><div class="cpp-exp-identification"><article><b>${isZh()?"真正的统计 / treatment unit":"Statistical / treatment unit"}</b><p>${E(T(e.unit))}</p></article><article><b>${isZh()?"我们唯一改了什么":"What is actually manipulated"}</b><p>${E(T(e.treatment))}</p></article><article class="readouts"><b>${isZh()?"最后读哪些结果":"Readouts"}</b><div>${readouts}</div></article></div></div>`;
  };
