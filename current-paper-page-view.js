(()=>{
  ...
  const readerPaperCase=(paper,dossier,detail)=>{
    ...
    const prior=(dossier?.approaches||[]).map(a=>a.name).filter(Boolean).join(" · ");
    const object=T(dossier?.missing_scientific_object)||T(audit?.defended_residual)||T(audit?.surviving_axis)||T(paper?.thesis);
    const why=T(dossier?.why_better)||T(detail?.interpretation?.importance);
    const stop=T(audit?.stop_rule)||T(dossier?.boundary);
    const rows=[];
    ...
    if(object)rows.push([isZh()?"真正还缺的 scientific object":"The scientific object that remains",object,"object"]);
    if(why)rows.push([isZh()?"为什么这不是一个普通工程对比":"Why this is more than an engineering comparison",why,"why"]);
    if(stop)rows.push([isZh()?"什么结果会让这个故事停下来":"What would stop or falsify this story",stop,"stop"]);
