(()=>{
  ...
  const datasetPrimer=(e)=>{
    ...
    return `<section class="cpp-dataset-primer"><header><span>${isZh()?"先认识数据 / 环境":"MEET THE DATA / ENVIRONMENT"}</span><h4>${isZh()?"别先记 benchmark 名字：先看一条数据到底长什么样":"Before benchmark names, understand what one task actually looks like"}</h4><p>${isZh()?"这里把数据集、环境、评测器和方法来源分开讲。看完后应该能知道模型实际看到什么、要做什么、原 benchmark 怎么判对错，以及为什么我们选它。":"This separates datasets, environments, evaluators, and method lineages so the actual task/input/output/evaluation is clear."}</p></header><div class="cpp-dataset-primer-grid">${rows.map(({s,p},i)=>`<article><header><span>${String(i+1).padStart(2,"0")}</span><div><b>${E(s.name||"")}</b><small>${E(T(p.kind))}</small></div>${sourceBadge(s)?`<em class="cpp-venue-tag">${E(sourceBadge(s))}</em>`:""}</header><p class="what">${E(T(p.what))}</p><dl><div><dt>${isZh()?"一条数据 / 任务长什么样？":"What does one task look like?"}</dt><dd>${E(T(p.sample))}</dd></div><div><dt>${isZh()?"模型看到什么、要做什么？":"What goes in and out?"}</dt><dd>${E(T(p.io))}</dd></div><div><dt>${isZh()?"原来怎么判对错？":"How is it evaluated?"}</dt><dd>${E(T(p.score))}</dd></div><div><dt>${isZh()?"为什么我们要用它？":"Why do we use it?"}</dt><dd>${E(T(p.why))}</dd></div></dl></article>`).join("")}</div></section>`;
  };
  ...
  const experimentProvenance=(pageId)=>{
    ...
    const quantities=(e.quantities||[]).map(q=>`<span><b>${E(q.v||"")}</b><small>${E(T(q.k))}</small></span>`).join("");
    const readouts=readoutRows.map(x=>`<span>${E(x)}</span>`).join("");
  ...
  const beginnerQuick=(paper,spec)=>`<section class="cpp-plain cpp-e1-overview cpp-beginner-overview panel" id="quick-overview"><div class="cpp-section-kicker">${isZh()?"0 · 先看懂问题":"0 · START HERE"}</div><h3 class="cpp-subsection-title">${isZh()?"30 秒先抓住这篇论文在研究什么":"The paper in 30 seconds"}</h3><p class="cpp-e1-hook">${E(spec.hook)}</p><div class="cpp-e1-overview-grid"><article><span>${isZh()?"先看一个例子":"ONE EXAMPLE"}</span><p>${E(spec.example)}</p></article><article><span>${isZh()?"本文真正的问题":"THE QUESTION"}</span><p>${E(T(paper.question))}</p></article><article class="answer"><span>${isZh()?"一句话答案":"THE ANSWER"}</span><p>${E(spec.answer)}</p></article></div><div class="cpp-term-strip">${spec.terms.map(x=>`<span><b>${E(x[0])}</b>${E(x[1])}</span>`).join("")}</div></section>`;
  ...
  const beginnerStatus=(paper,reg,spec)=>`<section class="panel cpp-next cpp-e1-status cpp-beginner-status" id="next-gate"><div class="cpp-section-kicker">${isZh()?"7 · 当前状态与下一步":"7 · CURRENT STATE & NEXT"}</div><h3 class="cpp-subsection-title">${E(spec.statusHeadline)}</h3><div class="cpp-e1-state-grid">${spec.statusCards.map((x,i)=>`<article class="${i?"hold":""}"><span>${E(x[0])}</span><strong>${E(x[1])}</strong><p>${E(x[2])}</p></article>`).join("")}</div><div class="cpp-next-action"><b>${isZh()?"下一步用一句话说":"Next, in plain language"}</b><p>${E(spec.nextPlain)}</p></div>${registryBox(paper,reg)}</section>`;
