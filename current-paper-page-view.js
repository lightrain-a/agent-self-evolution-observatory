(()=>{
  ...
  const kindLabel=(paper)=>{
    ...
    if(paper.kind==="scientific-object") return isZh()?"独立 Scientific Object":"Independent scientific object";
  ...
  const modelData=(detail)=>{
    ...
    return `<div class="cpp-resource-columns"><section><h3>${isZh()?"实验模型 / 系统":"Models / systems"}</h3><div class="cpp-resource-grid">${cards(detail.models,isZh()?"MODEL":"MODEL")}</div></section><section><h3>${isZh()?"数据集 / 实验环境":"Datasets / environments"}</h3><div class="cpp-resource-grid">${cards(detail.datasets,isZh()?"DATA":"DATA")}</div></section></div>`;
  };
  ...
  const collectionCard=(id,paper)=>{
    ...
    return `<a class="cpp-collection-card" href="${E(paper.href)}"><header><span>${E(label)}</span><em>${E(paperState(paper,reg))}</em></header><h3>${E(T(paper.title))}</h3><p>${E(T(paper.question))}</p><dl><div><dt>${isZh()?"模型":"Model"}</dt><dd>${E(models)}</dd></div><div><dt>${isZh()?"数据":"Data"}</dt><dd>${E(datasets)}</dd></div></dl><footer><span>${E(kindLabel(paper))}</span><b>${isZh()?"打开单篇 →":"Open paper →"}</b></footer></a>`;
  };
  ...
  window.renderCurrentPaperPage=(pageId)=>{
    ...
    return `<main class="cpp-page" data-paper-order="${paper.order}">
      ...
      <section class="panel" id="experiment-design"><div class="cpp-section-kicker">${isZh()?"实验思路":"EXPERIMENT DESIGN"}</div><h2>${isZh()?"实验怎么设计，为什么这样能回答问题":"How the experiment identifies the scientific question"}</h2><p class="cpp-design-lead">${E(T(detail.design))}</p>${storyExperiments(story)}</section>
      <section class="panel" id="experiment-results"><div class="cpp-section-kicker">${isZh()?"结果与证据":"RESULTS / EVIDENCE"}</div><h2>${isZh()?"当前实验结果，以及它真正证明了什么":"What the current evidence actually establishes"}</h2><p class="cpp-status-headline">${E(T(paper.experiment?.headline))}</p>${metrics(paper)}${proof(detail)}<div class="cpp-now"><b>${isZh()?"当前现场解释":"Current interpretation"}</b><p>${E(T(paper.experiment?.now))}</p></div></section>
      <section class="panel" id="paper-evolution"><div class="cpp-section-kicker">${isZh()?"论文演变":"PAPER EVOLUTION"}</div><h2>${isZh()?"这篇论文怎么一步步演变到今天":"How the paper evolved into its current form"}</h2>${evolution(paper,detail)}</section>
