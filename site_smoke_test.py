...
def main() -> None:
    ...
    expected_longitudinal_lessons = int((discovery_cycle.get("summary") or {}).get("failure_lessons") or 0)
    expected_result_analysis_lessons = int((((research_system.get("result_analysis") or {}).get("summary") or {}).get("discovery_lessons")) or 0)
    memory_manifest = research_memory.get("source_manifest") or {}
    expected_discovery_lessons = expected_longitudinal_lessons + expected_result_analysis_lessons
    longitudinal_rows = [row for row in discovery_lessons if row.get("source_artifact") == "longitudinal_safety_discovery_cycle"]
    result_analysis_rows = [row for row in discovery_lessons if row.get("source_artifact") == "result_analysis_ledger"]
    if expected_longitudinal_lessons < 19 or int(memory_manifest.get("discovery_failure_lessons") or 0) != expected_longitudinal_lessons or int(memory_manifest.get("result_analysis_lessons") or 0) != expected_result_analysis_lessons or len(longitudinal_rows) != expected_longitudinal_lessons or len(result_analysis_rows) != expected_result_analysis_lessons or int((research_memory.get("summary") or {}).get("discovery_lessons") or 0) != expected_discovery_lessons or len(discovery_lessons) != expected_discovery_lessons:
        fail(f"Research Memory discovery-lesson projection drifted: longitudinal={expected_longitudinal_lessons}/{len(longitudinal_rows)} result-analysis={expected_result_analysis_lessons}/{len(result_analysis_rows)} summary={(research_memory.get('summary') or {}).get('discovery_lessons')} rows={len(discovery_lessons)}")
    ...
    for marker in ("自动执行", "条件自动", "人工控制", "8/8 Pre-Experiment", "10/10 identifiability", "整体流程与权限：从研究问题到实验，再到论文", "确认是否真的存在新的科学问题", "先压缩 Problem–Insight，再设计最小方法和证据链", "找到最便宜、但足以改变结论的实验", "先小规模验证，弄清失败原因，再决定是否扩量", "冻结论文科学证据", "先选出最佳故事线，再按它形成成稿", "先做成稿完整性审计，再模拟真实拒稿风险并定向修稿", "投稿准备、人工授权与真实审稿", "记住为什么继续、停止、被接收或被拒绝", "当前科学结论记录"):
