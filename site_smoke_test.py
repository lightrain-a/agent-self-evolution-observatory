...
def main() -> None:
    ...
    if shadow_portfolio.get("latest_run_id") != "shadow-20260814-r3" or (latest_summary.get("requested_raw_seeds"),latest_summary.get("expansion_successful_shards"),latest_summary.get("expansion_parse_failures"),latest_summary.get("raw_seeds"),latest_summary.get("semantic_unique"),latest_summary.get("evolved_branches"),latest_summary.get("formulated_candidates"),latest_summary.get("machine_reviewable"),latest_summary.get("semantic_clear"),latest_summary.get("terminal_shadow_survivors")) != (120,18,2,88,47,36,15,2,0,0):
        fail(f"latest Search Portfolio r3 funnel is stale: {latest_shadow}")
    latest_policy = latest_shadow.get("policy") or {}; latest_authority = latest_shadow.get("authority") or {}
    if latest_shadow.get("status") != "SHADOW_TERMINAL_COMPLETE" or latest_shadow.get("scientific_authority") is not False or latest_policy.get("current_source_web_receipt_required_after_semantic_clear") is not True or latest_policy.get("missing_or_failed_current_source_reviewer_is_not_pass") is not True or latest_summary.get("current_source_missing") != 0 or latest_summary.get("live_paper_design_eligible") != 0 or any(latest_authority.get(key) is not False for key in ("live_problem_gate","paper_design","method","experiment","p0","gpu")):
    ...
    if shadow_queue.get("latest_run_id") != "shadow-20260814-r3" or latest_queue_summary.get("terminal_shadow_survivors") != 0 or latest_queue_summary.get("live_paper_design_eligible") != 0:
