...
def main() -> None:
    ...
    if shadow_portfolio.get("latest_run_id") != "shadow-20260814-r2" or (latest_summary.get("raw_seeds"),latest_summary.get("semantic_unique"),latest_summary.get("evolved_branches"),latest_summary.get("formulated_candidates"),latest_summary.get("machine_reviewable"),latest_summary.get("semantic_clear"),latest_summary.get("current_source_blocked"),latest_summary.get("terminal_shadow_survivors")) != (101,54,36,15,3,1,1,0):
        fail(f"latest Search Portfolio r2 funnel is stale: {latest_shadow}")
    ...
    if shadow_queue.get("latest_run_id") != "shadow-20260814-r2" or latest_queue_summary.get("terminal_shadow_survivors") != 0 or latest_queue_summary.get("live_paper_design_eligible") != 0:
