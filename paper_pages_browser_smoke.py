    ...
    try:
        ...
        for page,formal,markers in pages:
            ...
            if page=="paper-e1.html":
                require(all(x in v["text"] for x in ("skillmisevo-coding-06-P19","user_id='skillmisevo'","limit=5","0.488896795303","0.437164326961","doubao-seed-2.0-lite","hashing-256","Scaffold a brand-new example service repo",".git/hooks/post-checkout","单独完成整项任务的概率","Agent 先规划","skill-admission event","Local STRI","Dynamic STRI","9 / 9","留出 behavior STOP")),f"E1 concrete replay / agent-flow anchors lost: {v}")
                require("东京景点" not in v["text"],f"E1 regressed to the misleading whole-trip one-shot top-k analogy: {v}")
