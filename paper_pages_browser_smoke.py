...
def main():
    ...
    try:
        ...
        for page,formal,markers in pages:
            ...
            v=execute(sid,"""return {quick:document.querySelector('#quick-overview .cpp-section-kicker')?.textContent.trim()||'',placeholder:document.querySelector('#site-search')?.getAttribute('placeholder')||'',other:document.querySelectorAll('.cpp-collection-card,.cpp-shelf-card,.cpp-pager').length,snapshot:document.querySelectorAll('#paper-snapshot .cpp-snapshot-grid article').length,models:document.querySelectorAll('#models-data .cpp-resource-columns>section:first-child .cpp-resource-card').length,data:document.querySelectorAll('#models-data .cpp-resource-columns>section:last-child .cpp-resource-card').length,contract:document.querySelectorAll('#experiment-contract .cpp-contract-grid article').length,arms:document.querySelectorAll('#experiment-design .cpp-arm-grid article').length,analysis:document.querySelectorAll('#experiment-design .cpp-analysis-grid article').length,design:document.querySelectorAll('#experiment-design .cpp-design-lead').length,proof:document.querySelectorAll('#experiment-results .cpp-proof-grid article').length,interpretation:document.querySelectorAll('#experiment-results .cpp-interpretation>section').length,evolution:document.querySelectorAll('#paper-evolution .cpp-evolution article').length,lineage:document.querySelectorAll('#paper-evolution .cpp-lineage article').length,replay:document.querySelectorAll('#replay-notes').length,origin:document.querySelectorAll('#problem-origin .cpp-origin-grid article').length,related:document.querySelectorAll('#related-work-comparison .cpp-related-approach').length,nearest:document.querySelectorAll('#related-work-comparison .cpp-nearest-table tbody tr').length,storyArchive:document.querySelectorAll('#paper-story-complete').length,novelty:document.querySelectorAll('#legacy-paper-audit .paper-novelty-detail').length,externalReview:document.querySelectorAll('#legacy-paper-audit .paper-external-review-detail').length,objection:document.querySelectorAll('#legacy-paper-audit .reviewer-objection-detail').length,acceptance:document.querySelectorAll('#legacy-paper-audit .paper-acceptance-workflow').length,registry:document.querySelectorAll('#paper-state').length,overflow:document.documentElement.scrollWidth>document.documentElement.clientWidth+2,text:document.body.textContent||''};""")
            ...
            require((v["origin"]>=3)==formal and (v["related"]>=1)==formal and (v["nearest"]>=1)==formal and (v["storyArchive"]==1)==formal and (v["novelty"]==1)==formal and (v["externalReview"]==1)==formal and (v["objection"]==1)==formal and (v["acceptance"]==1)==formal and (v["registry"]==1)==formal,f"formal migration failed {page}: {v}")
        ...
        for mobile_page in ("paper-e1.html","paper-b1.html","paper-e2.html"):
