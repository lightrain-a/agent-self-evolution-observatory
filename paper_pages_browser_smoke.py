    ...
    try:
        ...
        for page,formal,markers in pages:
            ...
            if page=="paper-e1.html":
                require(v["e1WhySplit"]==1 and v["e1SplitReasons"]==4 and v["e1SplitFlight"]==4 and v["e1WorkedExample"]==1 and v["e1Packages"]==4 and v["e1Flights"]==3 and v["e1SystemDataflow"]==1 and v["e1SystemRoles"]==7 and v["e1SystemFlow"]==8 and v["e1WhoSelects"]==3 and v["e1P19Concrete"]==1 and v["e1P19Contrast"]==2 and v["e1P19Controls"]==2 and v["e1MethodGlance"]==1 and v["e1MethodSteps"]==5 and v["e1BeginnerRule"]==1 and v["e1BeginnerTech"]==1 and not v["e1BeginnerTechOpen"] and v["e1DeepTech"]==1 and not v["e1DeepTechOpen"] and v["e1EvidenceGlance"]==3 and v["e1EvidenceLadder"]==1 and v["e1ExperimentArc"]==6 and v["e1ClaimRows"]==3 and v["e1Review"]==1 and "6.1" in v["e1ReviewScore"] and "Accept" in v["e1ReviewScore"] and v["e1ReviewHistory"]==1 and not v["e1ReviewHistoryOpen"] and v["e1Budget"]==1,f"E1 golden-template contract failed: {v}")
            else:
            ...
            require(v["tocMain"]==["先理解这篇论文","为什么现有研究还不够","我们怎么验证这个问题" if page!="paper-e1.html" else "我们怎么把问题变成可验证实验","结论、边界与完整研究演变" if page=="paper-e1.html" else "结论、评审、成本与研究演变"],f"main paper TOC hierarchy drifted {page}: {v['tocMain']}")
