(() => {
  ...
  function renderCurrentIdeas() {
    ...
    const advisorPriority = window.ADVISOR_PRIORITY_IDEAS || {primary_shortlist:[],meta_review_status:{}};
    ...
    <section class="system-advisor-shortlist system-section"><div class="system-decision-head"><div><h3>${pick("22 个严格 PASS → 8 个首轮讨论方向","22 strict PASS → 8 first discussion directions")}</h3><p>${pick("这一步做相对比较而不是再次判断 novelty：同族方向只保留最强代表，并综合问题重要性、机制独立性、证伪性、P0 成本与 ICLR 叙事。","This is a relative portfolio comparison rather than another novelty screen: overlapping theses are represented once and ranked by problem importance, mechanism independence, falsifiability, P0 cost, and ICLR story.")}</p></div><a class="link-btn" href="paper-ideas.html#advisor-priority-shortlist">${pick("查看 shortlist 与 22 项相对排名 →","View shortlist and all 22 relative ranks →")}</a></div><div class="system-advisor-grid">${(advisorPriority.primary_shortlist||[]).map((idea,index)=>`<article><span>#${index+1}</span><b>${esc(titleOf(idea))}</b><small>${pick(idea.relative_tier==="fast-pilot"?"快速 P0":idea.relative_tier==="high-upside"?"高上限":"主线",idea.relative_tier||"")} · ${pick(idea.first_pilot_priority==="high"?"P0 优先":"P0 中等",idea.first_pilot_priority||"")}</small></article>`).join("")}</div></section>
