(() => {
  window.renderSystemOverview = function renderSystemOverview(config) {
    ...
    const bodies = [
      `${sections.renderPurpose ? sections.renderPurpose(state,s2) : ""}${sections.renderReaderRoadmap ? sections.renderReaderRoadmap(state) : ""}${sections.renderAuthorityModel ? sections.renderAuthorityModel(state) : ""}${deep("机器架构：11-stage、6 层职责与横向控制","Machine architecture: 11 stages, six layers, and cross-cutting controls",architectureDetail)}`,
      `${sections.renderProblemDiscoveryPhase ? sections.renderProblemDiscoveryPhase(state) : ""}${deep("Evidence intake 与 AI 会诊节点","Evidence intake and AI consultation checkpoints",sections.renderResearchIntake ? sections.renderResearchIntake(state) : "")}`,
      `${sections.renderPaperDesignPhase ? sections.renderPaperDesignPhase(state) : ""}`,
      `${sections.renderExperimentCompilePhase ? sections.renderExperimentCompilePhase(state) : ""}${deep("完整机器 Gate / Problem Discovery / Paper Design 编译细节","Full machine gates, discovery, and paper-design compiler detail",machineCompileDetail)}`,
      `${sections.renderValidationScalePhase ? sections.renderValidationScalePhase(state) : ""}${sections.renderGovernanceV2 ? sections.renderGovernanceV2(state) : ""}${sections.renderFailureSemantics ? sections.renderFailureSemantics(state) : ""}${deep("运行时、GPU、Artifact、Automation 与组件清单","Runtime, GPU, artifacts, automation, and component inventory",runtimeReference)}`,
