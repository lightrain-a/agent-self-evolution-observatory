(() => {
  ...
  const ideaHref = (idea) => idea?.final_status ? "paper-ideas.html#new-ideas" : "paper-ideas.html#discussed-ideas";
  ...
  function renderLiveArchitecture(state, s2) {
    ...
    const discussionPortfolio = window.DISCUSSION_READY_IDEAS || {};
    const finalSummary = discussionPortfolio.final_summary || {};
    const finalPass = Number(finalSummary.pass ?? discussionPortfolio.count ?? 0);
    const finalTarget = Number(discussionPortfolio.target || 20);
    ...
    return `<section class="system-live-summary system-section"><h3>${pick("当前运行快照","Current runtime snapshot")}</h3><p class="section-intro">${pick("统计值直接读取后端发布的研究状态与文献快照，不在页面中手工维护。","The values below are read from backend-published research and literature snapshots rather than maintained manually in the page.")}</p><div class="system-stat-grid">
      ...
      ${stat(`${finalPass}/${finalTarget}`,"个 FINAL PASS 师兄讨论项","FINAL-PASS advisor items",finalPass === finalTarget ? "good" : "warn")}
    ...
    <section class="system-architecture system-section"><h3>${pick("后台分层架构","Backend layered architecture")}</h3><p class="section-intro">${pick("各层通过明确的静态工件连接。上一层失败时不会覆盖上一份有效快照，前端始终只读取最后一次通过校验的结果。","Layers communicate through explicit artifacts. A failed layer never overwrites the previous valid snapshot, and the frontend reads only the latest validated result.")}</p><div class="system-layer-grid">
      ...
      ${layerCard(5,"REVIEW","审查编排层","Review orchestration","执行 R1/R2、原始 R3、定向 R3.1/R3.2 修复复审和最终 primary-source 碰撞门槛；历史结论与当前 FINAL gate 分开保存。","Runs R1/R2, historical R3, targeted R3.1/R3.2 repair review, and the final primary-source collision gate while keeping historical diagnostics separate from the current FINAL gate.",["R1/R2 provenance","R3.1/R3.2","2-reviewer gate","fresh collision gate"])}
  ...
  function renderPipelineStages() {
    return `<section class="system-pipeline-panel system-section"><h3>${pick("详细后台数据流","Detailed backend data flow")}</h3><div class="system-pipeline">
      ...
      ${pipelineStage(10,"FINAL","最终师兄讨论门槛","Run final pre-advisor gate","R2 入池项、原始 R3 诊断、R3.1/R3.2 当前页面版本、两位独立 reviewer 和最新 primary-source 文献","R2 provenance, historical R3 diagnostics, current R3.1/R3.2 page versions, two independent reviewers, and fresh primary sources","先按绝对门槛做 R3 诊断并定向修复；当前版本必须同时获得 GLM-5.2 与 DeepSeek V4 Pro PASS，再对全部当前候选做 2026-08-08 最新机制级碰撞复核","Use R3 as an absolute diagnostic, repair only material boundaries, require unanimous GLM-5.2 and DeepSeek V4 Pro PASS on the current version, then rerun a fresh mechanism-level primary-source collision check for every current candidate","20 个 FINAL PASS 当前版本；REVISE/BLOCK 不进入师兄讨论池，旧版本仅保留历史追溯","20 current FINAL-PASS versions; REVISE/BLOCK stay out of the advisor pool while historical versions remain traceable")}
      ${pipelineStage(11,"PRE-P0","实验可识别性前置审计","Audit Pre-P0 identifiability","冻结论文主张、训练目标、最强简化、候选更新/任务池和执行预算","Frozen paper claim, learning objective, strongest simplification, candidate update/task pool, and execution budget","在 GPU 前检查 claim alignment、target/effect variation、baseline disagreement、representability、tiny overfit、competence window、cost/provenance 和 outcome interpretation matrix","Before GPU execution, check claim alignment, target/effect variation, baseline disagreement, representability, tiny-set overfit, competence window, cost/provenance, and the outcome-interpretation matrix","Pre-P0 PASS 或明确 blocker；任一硬门失败时 execution_authorized=false","Pre-P0 PASS or explicit blockers; any failed hard gate forces execution_authorized=false")}
      ${pipelineStage(12,"PILOT","实验注册与结果回流","Register experiments and ingest results","Pre-P0 PASS、冻结 P0/P1/P2 协议和结果 schema","Pre-P0 PASS plus frozen P0/P1/P2 protocol and result schema","验证结果完整性、预算、seed、对照和 Stop 规则","Validate completeness, budget, seeds, controls, and Stop rule","结构化 Pilot 工件、学习曲线、简化对照和结果证据","Structured pilot artifacts, learning curves, simplification controls, and result evidence")}
      ${pipelineStage(13,"DIAGNOSE","实验诊断与原子修复","Diagnose experiments and create atomic repairs","Pilot 工件、qualification、fit 曲线、hidden 使用状态和 matched baseline","Pilot artifacts, qualification, fit curves, hidden-use state, and matched baselines","先判断实验是否有资格评价方法，再区分 execution／substrate／optimization／mechanism／scientific-boundary；每个 child 只改一个变量","First decide whether the experiment can evaluate the mechanism, then separate execution, substrate, optimization, mechanism, and scientific-boundary failures; each child changes one variable only","原子修复子节点、merge/stop 决策或等待人工批准的扩大建议","Atomic repair children, merge/stop decisions, or a scale-up proposal awaiting human approval")}
    </div></section>`;
  }
  ...
  function renderDataContracts() {
    const rows = [
      ...
      ["S9",pick("FINAL 师兄门槛","FINAL pre-advisor gate"),pick("R2 provenance、R3 诊断、R3.1/R3.2 当前版本、双 reviewer 与最新 primary-source 碰撞复核","R2 provenance, R3 diagnostics, current R3.1/R3.2 versions, two reviewers, and fresh primary-source collision recheck"),pick("当前 FINAL PASS／REVISE／BLOCK、存活边界和 retired 历史版本","Current FINAL PASS/REVISE/BLOCK, surviving boundary, and retired historical versions"),pick("只有双 reviewer PASS 且 fresh collision PASS 才计入 20 个正式讨论项","Count an idea toward the formal 20 only after unanimous reviewer PASS plus fresh collision PASS")],
      ["S10",pick("Pre-P0 可识别性","Pre-P0 identifiability"),pick("冻结 claim/objective、任务与更新池、简化 baseline、成本和追溯合同","Frozen claim/objective, task/update pool, simplification baseline, cost and provenance contracts"),pick("十项硬门结果、blockers、estimated VOI 与 execution_ready","Ten hard-gate results, blockers, estimated VOI, and execution_ready"),pick("任一硬门失败都禁止 P0；不得人工绕过未记录的 blocker","Any failed hard gate blocks P0; undocumented manual override is forbidden")],
      ["S11",pick("实验回流","Experiment feedback"),pick("Pre-P0 PASS、冻结协议与结构化结果文件","Pre-P0 PASS, frozen protocol, and structured result files"),pick("qualification、fit、matched baseline 与结果证据","Qualification, fit, matched-baseline, and result evidence"),pick("结果不完整或预算不匹配时拒绝进入科学诊断","Reject scientific diagnosis if results or budgets are invalid")],
      ["S12",pick("实验诊断与修复","Experiment diagnosis and repair"),pick("Pilot 工件、学习曲线、简化对照和 hidden 使用状态","Pilot artifacts, learning curves, simplification controls, and hidden-use state"),pick("诊断层级、experiment_identifiable、原子 repair children、merge/stop／scale-up proposal","Diagnosis layer, experiment_identifiable, atomic repair children, merge/stop or scale-up proposal"),pick("不可识别实验不得改变科学判断；正向 P0 仍需人工批准才可扩大","Non-identifiable pilots cannot update scientific belief; positive P0 still needs human approval before scale-up")],
    ];
  ...
  function renderArtifacts() {
    return `<section class="system-artifact-panel system-section"><h3>${pick("持久化工件与前后端边界","Persistent artifacts and frontend/backend boundary")}</h3><p class="section-intro">${pick("后台生成完整研究状态，再只把允许公开的静态快照编译到 Pages。原始响应、缓存、实验数据、密钥和执行代码不进入前端。","The backend builds the full research state, then compiles only approved static snapshots into Pages. Raw responses, caches, experiments, secrets, and execution code stay off the frontend.")}</p><div class="history-table-scroll"><table class="matrix system-artifact-table"><thead><tr><th>${pick("工件","Artifact")}</th><th>${pick("可见性","Visibility")}</th><th>${pick("用途","Purpose")}</th><th>${pick("生成者","Producer")}</th></tr></thead><tbody>
      ...
      ${artifactRow("generated/research-system-state.json/js","public","系统健康、证据规模、碰撞、谱系、Pilot 与实验诊断／原子修复树摘要","System health, evidence scale, collisions, lineage, pilot state, and experiment diagnosis/atomic-repair tree","每日确定性重建","Daily deterministic rebuild")}
      ...
      ${artifactRow("generated/r3-final-audit.json/js","public","22 个历史 R2-PASS 版本的严格 R3 诊断；仅用于追溯，不代表当前状态","Strict historical R3 diagnostic for the 22 R2-PASS versions; provenance only, not current readiness","R3 final audit compiler","R3 final audit compiler")}
      ${artifactRow("generated/current-final-ideas.json/js","public","20 个经 R3.1/R3.2 修复后的纯 page-facing 当前版本；原始 reviewer/model 元数据仅留后台","20 sanitized current page-facing versions after R3.1/R3.2 repair; raw reviewer/model metadata stays backend-only","Current-final compiler","Current-final compiler")}
      ${artifactRow("generated/final-internal-review-gate.json","public","20 个当前版本的精简双 reviewer verdict；原始 reviewer response 仅留后台","Sanitized two-reviewer verdicts for the 20 current versions; raw reviewer responses stay backend-only","Internal review gate compiler","Internal review gate compiler")}
      ${artifactRow("generated/final-collision-recheck.json/js","public","20 个当前版本在 2026-08-08 的最新 primary-source 机制级碰撞复核","Fresh 2026-08-08 primary-source mechanism collision recheck for all 20 current versions","Final collision review","Final collision review")}
      ${artifactRow("generated/final-advisor-audit.json/js","public","双 reviewer + 最新碰撞门槛汇总；只有 FINAL PASS 才进入师兄讨论池","Aggregated two-reviewer + fresh-collision gate; only FINAL PASS enters the advisor pool","Final advisor audit compiler","Final advisor audit compiler")}

      ${artifactRow("generated/discussion-ready-ideas.json/js","public","正式师兄讨论池：20 个当前 FINAL PASS 版本，不含 shortlist","Formal advisor discussion pool: 20 current FINAL-PASS versions, with no shortlist","Discussion portfolio compiler","Discussion portfolio compiler")}
  ...
  function renderCurrentIdeas() {
    ...
    const discoveryV5 = window.IDEA_DISCOVERY_V5 || {summary:{},finalists:[],all_candidates:[],pareto_front_ids:[]};
    ...
    const v5Summary = discoveryV5.summary || {};
    ...
    const coreDiscussionReady = Number(finalSummary.pass ?? discussionPortfolio.count ?? 0);
    const discussionTarget = Number(discussionPortfolio.target || 20);
    const finalRevise = Number(finalSummary.revise || 0);
    const finalBlock = Number(finalSummary.block || 0);
    const r31FinalPass = (discussionPortfolio.ideas || []).filter((idea) => idea.revision === "R3.1").length;
    const r32FinalPass = (discussionPortfolio.ideas || []).filter((idea) => idea.revision === "R3.2").length;
    const freshCollisionPass = Number(finalSummary.fresh_collision_rechecks || 0);

    return `<section class="system-decision-summary system-section"><div class="system-decision-head"><div><h3>${pick("主 ICLR Idea Bank","Main ICLR idea bank")}</h3><p>${pick("R1 是结构与可行性筛查，R2 是正式来源约束下的独立机制审查。PASS 表示可以进入实验，不表示论文结论已经成立。","R1 screens structure and feasibility; R2 is an independent mechanism audit under official-source constraints. PASS means ready for experiments, not that the paper claim is established.")}</p></div><a class="link-btn system-primary-link" href="paper-ideas.html#discussed-ideas">${pick("打开完整 Idea 页面 →","Open the complete idea page →")}</a></div><div class="system-funnel"><div><b>${get(summary.raw_candidates,41)}</b><span>${pick("原始候选","raw")}</span></div><i>→</i><div><b>${get(summary.structured_candidates,29)}</b><span>${pick("结构化候选","structured")}</span></div><i>→</i><div><b>${get(summary.passed,26)}</b><span>${pick("R1 通过","R1 pass")}</span></div><i>→</i><div class="pass"><b>${pass.length}</b><span>PASS</span></div><div class="revise"><b>${revise.length}</b><span>REVISE</span></div><div class="block"><b>${block.length}</b><span>BLOCK</span></div></div>
