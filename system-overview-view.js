(() => {
  ...
  function renderPipelineStages() {
    return `<section class="system-pipeline-panel system-section"><h3>${pick("详细后台数据流","Detailed backend data flow")}</h3><div class="system-pipeline">
      ...
      ${pipelineStage(11,"PILOT","实验注册与结果回流","Register experiments and ingest results","冻结 FINAL PASS Idea、P0/P1/P2 协议和结果 schema","Frozen FINAL-PASS idea, P0/P1/P2 protocol, result schema","验证结果完整性、预算、seed、对照和 Stop 规则","Validate completeness, budget, seeds, controls, and Stop rule","Idea 状态变化、失败记录和下一轮修订输入","Idea state transition, failure record, next repair input")}
    </div></section>`;
  }
  ...
  function renderDataContracts() {
    const rows = [
      ...
      ["S10",pick("实验回流","Experiment feedback"),pick("冻结协议与结构化结果文件","Frozen protocol and structured result files"),pick("planned／pilot-ready／selected-ready／stop","State transition"),pick("结果不完整或预算不匹配时拒绝晋级","Reject promotion if results or budgets are invalid")],
    ];
