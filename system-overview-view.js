(() => {
  ...
  function renderCurrentIdeas() {
    ...
    const advisorPriority = window.ADVISOR_PRIORITY_IDEAS || {discussion_pool:[],priority_first_read:[],meta_review_status:{}};
    ...
    const coreDiscussionReady = Number(discussionPortfolio.count ?? (pass.length + Number(v4Summary.external_pass || 0) + Number(v5Summary.external_pass || 0)));
    const discussionTarget = Number(discussionPortfolio.target || 20);
    const v5RepairPass = (discussionPortfolio.ideas || []).filter((idea) => ["v51-r2","v52-r2","v53-r2"].includes(idea.source)).length;
    ...
    <section class="system-advisor-shortlist system-section"><div class="system-decision-head"><div><h3>${pick("22 个严格 PASS：全部进入师兄讨论池","22 strict PASS: all enter senior discussion")}</h3><p>${pick("22 个方向都会交给师兄讨论；相对元审查只提供 1–22 排名、同族关系和 8 个优先阅读建议，不再替师兄删减候选。","All 22 directions are presented for senior discussion. The comparative meta-review supplies a 1–22 ranking, overlap relationships, and eight suggested first reads without removing candidates on the senior's behalf.")}</p></div><a class="link-btn" href="paper-ideas.html#advisor-priority-shortlist">${pick("查看 22 个讨论 Idea 与优先阅读 →","View all 22 ideas and first-read priorities →")}</a></div><div class="system-advisor-pool-count"><b>${(advisorPriority.discussion_pool||[]).length || coreDiscussionReady}</b><span>${pick("正式讨论 Idea","discussion-ready ideas")}</span><small>${pick("最低门槛 20；当前全部保留","minimum 20; all retained")}</small></div><h4 data-toc="false">${pick("建议先读的 8 个","Eight suggested first reads")}</h4><div class="system-advisor-grid">${(advisorPriority.priority_first_read||[]).map((idea,index)=>`<article><span>#${index+1}</span><b>${esc(titleOf(idea))}</b><small>${pick(idea.relative_tier==="fast-pilot"?"快速 P0":idea.relative_tier==="high-upside"?"高上限":"主线",idea.relative_tier||"")} · ${pick(idea.first_pilot_priority==="high"?"P0 优先":"P0 中等",idea.first_pilot_priority||"")}</small></article>`).join("")}</div></section>
    <section class="system-v5-summary system-section"><div class="system-decision-head"><div><h3>${pick("师兄讨论候选进度：严格二审 PASS","Discussion-ready progress: strict R2 PASS")}</h3><p>${pick(`只统计主 Bank、v4、v5 和 v5.x 定向修订轮的正式 R2 PASS，不把 REVISE、内部短名单或网络灵感补充批次计入。当前 ${coreDiscussionReady}/${discussionTarget}，已达到讨论门槛，系统停止继续扩张。`,`Counts only formal R2 PASS from the main bank, v4, v5, and targeted v5.x repair rounds; REVISE, internal shortlists, and the supplementary inspired batch are excluded. Current progress is ${coreDiscussionReady}/${discussionTarget}; the discussion threshold is reached and further expansion stops.`)}</p></div><a class="link-btn" href="paper-ideas.html#idea-discovery-v5">${pick("查看 v5 扩展池 →","Open v5 expansion →")}</a></div><div class="system-v5-progress"><span><b>${pass.length}</b>${pick("主 Bank PASS","main-bank PASS")}</span><span><b>${v4Summary.external_pass || 0}</b>v4 PASS</span><span><b>${v5Summary.external_pass || 0}</b>v5 PASS</span><span><b>${v5RepairPass}</b>v5.x PASS</span><span class="target"><b>${coreDiscussionReady}</b>/ ${discussionTarget}</span></div><div class="system-v5-bar"><i style="width:${Math.min(100,coreDiscussionReady/discussionTarget*100)}%"></i></div><p>${pick(`v5：${v5Summary.raw_candidates || 0} 个候选、${(discoveryV5.finalists || []).length} 个 finalist/revival、已二审 ${v5Summary.external_reviewed || 0}，其中 ${v5Summary.external_pass || 0} PASS / ${v5Summary.external_revise || 0} REVISE / ${v5Summary.external_block || 0} BLOCK。REVISE 会继续生成定向 child，BLOCK 仍保留为组件或复活源。`,`v5: ${v5Summary.raw_candidates || 0} candidates, ${(discoveryV5.finalists || []).length} finalists/revivals, ${v5Summary.external_reviewed || 0} reviewed: ${v5Summary.external_pass || 0} PASS / ${v5Summary.external_revise || 0} REVISE / ${v5Summary.external_block || 0} BLOCK. REVISE generates targeted children; BLOCK remains a component or revival source.`)}</p></section>
