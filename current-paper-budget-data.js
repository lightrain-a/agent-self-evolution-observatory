window.CURRENT_PAPER_BUDGET={
  ...
  rows:[
    ...
    },
    {
      ...
      api:{zh:"旧 4+8 / 144-episode 包络已不再适用。MiMo capability PASS，但旧 F0 source 8/8 success 后 mandatory stop。当前只冻结 12-case Direct-SFQ-A0。2026-09-05 readiness R1 因 approved AA_API_KEY 未注入而在 dispatch 前停止，provider request=0，所以当前 credit 状态未知；必须先恢复受控 secret injection，再新授权 readiness，之后才可能进入 Flash V4-R2。",en:"The old 4+8 / 144-episode envelope no longer applies. MiMo passed capability, but the old F0 source stopped after 8/8 successes. Only the 12-case Direct-SFQ-A0 is currently frozen. Readiness R1 on 2026-09-05 stopped before dispatch because the approved AA_API_KEY was not injected, with zero provider requests, so current credit state is unknown; controlled secret injection must be restored and readiness freshly authorized before Flash V4-R2 can open."},
      envelope:{zh:"恢复 approved secret injection → 新 provider-readiness authority → 单次 zero-tool/zero-retry readiness。只有 PASS 后再单独授权 R2 capability → Direct-SFQ-A0。source gate 通过后：24-family reserve → TARGET_ONLY_VERIFICATION 冻结 eligibility → outcome 前冻结 N*/R*。进入 I/L/H 后 target outcome 只报告、不筛 family；禁止 effect-driven 扩样。",en:"Restore approved secret injection → fresh provider-readiness authority → one zero-tool/zero-retry readiness request. Only after PASS may separate R2 capability authority open → Direct-SFQ-A0. After the source gate: 24-family reserve → TARGET_ONLY_VERIFICATION freezes eligibility → freeze N*/R* before outcomes. After entry into I/L/H, target outcomes are reported rather than filtered; no effect-driven expansion."},
