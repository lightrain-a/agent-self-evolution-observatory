window.CURRENT_PAPER_BUDGET={
  ...
  rows:[
    ...
    },
    {
      ...
      envelope:{zh:"credit 恢复后按 readiness → R2 capability → Direct-SFQ-A0。source gate 通过后：24-family reserve → TARGET_ONLY_VERIFICATION 冻结 eligibility → outcome 前冻结 N*/R*。进入 I/L/H 后 target outcome 只报告、不筛 family；禁止 effect-driven 扩样。",en:"After credit recovery: readiness → R2 capability → Direct-SFQ-A0. After the source gate: 24-family reserve → TARGET_ONLY_VERIFICATION freezes eligibility → freeze N*/R* before outcomes. After entry into I/L/H, target outcomes are reported rather than filtered; no effect-driven expansion."},
