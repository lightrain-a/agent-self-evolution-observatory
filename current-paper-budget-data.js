window.CURRENT_PAPER_BUDGET={
  ...
  rows:[
    ...
    },
    {
      ...
      api:{zh:"旧 4+8 / 144-episode 包络已不再适用。MiMo capability PASS，但旧 F0 source 8/8 success 后 mandatory stop。当前只冻结 12-case Direct-SFQ-A0；需先恢复 clean direct provider 并完成 Flash V4-R2。RQ1–RQ4 的预算等 source gate 通过后再冻结。",en:"The old 4+8 / 144-episode envelope no longer applies. MiMo passed capability, but the old F0 source stopped after 8/8 successes. Only the 12-case Direct-SFQ-A0 is currently frozen; clean direct provider access and Flash V4-R2 must pass first. RQ1–RQ4 budgets are frozen only after the source gate passes."},
      envelope:{zh:"provider credit 恢复后按 readiness → 新 R2 capability → Direct-SFQ-A0 推进。后续先生成候选 family，再在 collateral outcome 前冻结 N* 与 repeat 数；禁止看 effect 后补样本，也不重跑旧 R1 / 回退 contaminated CodingPlan actor。",en:"After credit recovery: readiness → fresh R2 capability → Direct-SFQ-A0. Later work generates candidate families first and freezes N* plus repeat count before collateral outcomes; no effect-driven expansion, old-R1 replay, or contaminated CodingPlan fallback."},
      cash:{zh:"中 · 近期只需 capability recovery + 12-case SFQ。若 source gate 通过，默认高效主机制预算约为 N*=16、R*=2（192 probes）+ 8-family sham control（32）；RQ3/RQ4/跨模型均按前一层 PASS 条件打开，而不是预先跑满最大 envelope。",en:"Medium · near-term cost is limited to capability recovery plus the 12-case SFQ. If the source gate passes, the efficient default mechanism budget is roughly N*=16, R*=2 (192 probes) plus an 8-family sham control (32); RQ3, RQ4, and cross-model checks open only after upstream PASS rather than exhausting a maximum envelope up front."},
