(() => {
  ...
  pages["selected-paper"] = {
    ...
    callout:{en:"Current canonical PaperRegistry now projects all five append-only paper ledgers. Four papers are at SUBMISSION_READY; Failure-Memory Provenance remains in TARGETED_REPAIR. STRI and Agent Safety retain their ResearchItem parents, while the three D2 papers retain paper-first discovery-candidate provenance rather than being forced into an A–G parent. Submission readiness is a manuscript-workflow state, not evidence completion: external support debt remains explicit where registered. Automatic scientific, experiment, GPU, and submission authority remain zero.",zh:"当前 canonical PaperRegistry 已直接投影全部 5 个 append-only paper ledger：4 篇达到 SUBMISSION_READY，Failure-Memory Provenance 仍处于 TARGETED_REPAIR。STRI 与 Agent Safety 保留各自 ResearchItem 父项；3 篇 D2 论文保留 paper-first discovery candidate 来源，不强行挂到某个 A–G ResearchItem。Submission Ready 是论文工作流状态，不等于证据债自动消失；已登记的外部 support / experiment debt 继续显式保留。科学、实验、GPU 与自动投稿权限仍全部为 0。"},
