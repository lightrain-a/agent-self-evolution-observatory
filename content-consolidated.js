(() => {
  ...
  pages["selected-paper"] = {
    ...
    callout:{en:"Current canonical state: STRI is READY scientifically and has reached SUBMISSION_READY after Story Search, both Mock PC modes, Targeted Repair, Claim Audit, 9/9 Manuscript CI, PDF QA, and a 10-objection Prebuttal; Submission Ready=YES. Agent Safety remains PAPER_EVIDENCE / CAUSAL_HOLD. Automatic scientific, experiment, GPU, and submission authority remain zero, so SUBMITTED still requires external human action.",zh:"当前 canonical 状态：STRI 的科学状态为 READY，并已在完成 Story Search、双模式 Mock PC、Targeted Repair、Claim Audit、9/9 Manuscript CI、PDF QA 与覆盖 10 个 decision-critical objection 的 Prebuttal 后达到 SUBMISSION_READY，Submission Ready=YES；Agent Safety 仍保持 PAPER_EVIDENCE / CAUSAL_HOLD。论文工作流的科学、实验、GPU 与自动投稿权限全部为 0，因此 SUBMITTED 仍必须由外部人工真实提交触发。"},
