(() => {
  ...
  pages["selected-paper"] = {
    ...
    title:{en:"PaperRegistry: five complete paper arguments from scenario to evidence",zh:"PaperRegistry：把 5 篇论文从场景、方法到实验结论完整讲明白"},
    lead:{en:"Read every paper through the same argument chain: scenario and practical value → concrete failure → existing approaches and their limits → three gaps → design motivation → component-to-gap mapping → experimental questions and strongest baselines → mechanism/ablation evidence → claim boundary and paper outline. PaperState, receipts, hashes, external reviews, and the 12-stage workflow remain collapsed audit material.",zh:"五篇论文统一按一条真正的论文论证链阅读：场景与实际价值 → 具体失败 → 现有方法及各自问题 → 3 个核心 gap → 设计动机 → 方法组件如何逐一补 gap → 实验 RQ 与 strongest baseline → 哪些组件/对照真正生效 → 最终 claim boundary 与推荐论文大纲。PaperState、receipt、hash、External Review 和 12-stage workflow 继续保留在默认折叠的审计层。"},
