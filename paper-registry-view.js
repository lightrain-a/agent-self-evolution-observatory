(() => {
  ...
  const paperCard = (paper, index) => {
    const prep=paper.paper_preparation||{}, freeze=paper.submission_freeze||{}, handoff=paper.submission_handoff||{}, signoff=paper.human_signoff||{}, actual=paper.actual_submission||{}, gates=paper.gates||{}, layers=paper.layers||{};
    ...
    let nextStep;
    if(submitted) nextStep=actual.status==="VENUE_SUBMISSION_CONFIRMED"?pickR("真实 venue submission receipt 已与当前 freeze/handoff/signoff 绑定。后续只在 rebuttal、camera-ready 或 venue 明确要求时生成新的受控版本。","The real venue submission receipt is bound to the current freeze/handoff/signoff lineage. Create a new controlled version only for rebuttal, camera-ready, or explicit venue requests."):pickR("Paper Acceptance 显示 SUBMITTED，但实际 submission receipt 无效；必须视为 ledger 异常，不能继续后续流程。","Paper Acceptance says SUBMITTED but the actual submission receipt is invalid; treat this as a ledger error and stop downstream workflow.");
