window.ADVISOR_MEETING_DATA = {
  ...
  "papers": [
    ...
    },
    {
      ...
      "evidence_state": "R76：历史 Llama 4 个 terminal flips 在 temperature=0 exact rerun 下 4/4 复现；R77：其中 2 个表现为 same-state decision-boundary shift，另 2 个更依赖 transcript/self-conditioning；相同 4 个 task 在 Qwen 上均 concordant。R80 已冻结 321-run 并只授予 Qwen/Llama execution authority；analysis/claim change/30B execution 均关闭。",
      "next_closure": "完成 sealed Qwen 189 → Llama 132，共 321 trajectories；只在 stage seals 完成后另开 analysis authority。Qwen3.5-27B 只按未来 discordant 数 D 触发 4D targeted check，目前不授权。",
      "cost_class": "MEDIUM · ALREADY_RUNNING",
      "cost_to_next_decision": "本地 A100/open-weight 推理，直接 API cash≈0；主成本是 A100 占用与 321-run wall-time。当前 Qwen execution 已启动，不能因中间结果改 schedule。",
      "dependencies": [
        "clean MemRL checkout pinned to c1b322ca…",
        "local OpenAI-compatible Qwen/Llama runtime",
        "sealed Qwen then Llama stage order",
        "separate post-seal analysis authorization"
      ],
      "default_action": "COMPLETE_FROZEN_321_NO_SCOPE_CHANGE",
    ...
    },
    {
      ...
      "evidence_state": "旧 F0 因 8/8 target success 按协议 STOP。新的 Direct-SFQ-A0 12-case source qualification 已完成 public-oracle reachability/freshness 12/12，但 direct provider scientific execution 仍被 credit/provider readiness 阻塞；无 confirmatory collateral outcome。",
      "next_closure": "恢复 direct provider 后只执行 12-case Direct-SFQ-A0，先确认真实 failure/repair availability。只有 source gate PASS 才冻结 N*；高效默认 N*=16、R*=2，之后主 mechanism≈192 probes + 8-family sham≈32。",
      "cost_class": "LOW_NOW / MEDIUM_IF_QUALIFIED",
      "cost_to_next_decision": "眼前只有 12 个 source-qualification provider cases。192+32 的主实验预算在 source gate 前不应视为已承诺成本。",
      "dependencies": [
        "direct provider credit/readiness",
        "strict target-local repair writer",
        "outcome-blind topology graph",
        "TARGET_ONLY verification before topology analysis"
      ],
      "default_action": "QUALIFY_SOURCE_FAILURE_ONLY",
      "override_trigger": "12-case SFQ 无足够 repairable failures → HOLD/STOP current substrate；不得通过换 topology 或扩大 workload 制造现象。",
