    ...
    try:
        ...
        for page, expected in EXPECTATIONS.items():
            ...
            if page == "experiment-costs":
                costs = execute(session_id, """const paperCells=[...document.querySelectorAll('.ec-portfolio .ec-paper-cell')];const heads=[...document.querySelectorAll('.ec-portfolio thead th')];const rows=[...document.querySelectorAll('.ec-portfolio tbody tr')];const vendorHeaders=[...document.querySelectorAll('.ec-vendor-matrix thead th')].map(x=>x.textContent.trim());const junliaiRows=document.querySelectorAll('.ec-vendor-matrix td.ec-junliai').length;const vendorRows=[...document.querySelectorAll('.ec-vendor-matrix tbody tr')];const expectedFamilyOrder=['qwen','deepseek','kimi','glm','minimax','doubao','hunyuan','ernie','openai / gpt','claude','gemini','grok'];const familySequence=[...new Set(vendorRows.map(r=>(r.dataset.family||'').toLowerCase()))];const sortedWithinFamilies=expectedFamilyOrder.every(f=>{const p=vendorRows.filter(r=>(r.dataset.family||'').toLowerCase()===f).map(r=>r.dataset.sortPrice===''?Infinity:Number(r.dataset.sortPrice));return p.every((v,i)=>i===0||p[i-1]<=v);});const modelFilter=document.querySelector('#ec-model-filter-input');const q8=rows.find(r=>r.querySelector('.ec-model b')?.textContent.trim()==='Qwen3-8B');const arkE1=rows.find(r=>r.querySelector('.ec-model b')?.textContent.trim()==='DeepSeek V4 Pro · Ark Plan');const e1=paperCells.find(x=>x.textContent.includes('E1 · STRI'));const text=document.querySelector('#dynamic-page')?.textContent||'';return {sections:document.querySelectorAll('.topic-section').length,tables:document.querySelectorAll('.advisor-table-scroll>.matrix').length,rows:document.querySelectorAll('.ec-master tbody tr').length,portfolioRows:rows.length,paperCells:paperCells.length,paperRowspanTotal:paperCells.reduce((s,x)=>s+(Number(x.getAttribute('rowspan'))||1),0),e1Rowspan:Number(e1?.getAttribute('rowspan')||0),visibleHeaders:heads.filter(x=>getComputedStyle(x).display!=='none').map(x=>x.textContent.trim()),priceBefore:document.querySelectorAll('.ec-portfolio .ec-price-used').length,priceAfter:document.querySelectorAll('.ec-portfolio .ec-price-plan').length,tokenEstimates:document.querySelectorAll('.ec-portfolio .ec-token-estimate').length,gpuPriceCells:document.querySelectorAll('.ec-portfolio .ec-price-gpu').length,tokenPriceCells:document.querySelectorAll('.ec-portfolio .ec-price-token').length,qwen8bGpu:q8?.classList.contains('ec-resource-gpu')===true,qwen8bTokenEstimate:q8?.querySelectorAll('.ec-token-estimate').length||0,arkE1Token:arkE1?.classList.contains('ec-resource-token')===true,arkE1Usage:(arkE1?.textContent||'').includes('1,265 calls')&&(arkE1?.textContent||'').includes('1943.2 万 Token'),e1StagedBudget:text.includes('不是“整个 V4 只要 ¥20”'),facts:document.querySelectorAll('.ec-facts article').length,toc:[...document.querySelectorAll('#page-toc a')].map(a=>a.textContent.trim()),vendorHeaders,junliaiRows,aihubmixRows:document.querySelectorAll('.ec-vendor-matrix td.ec-provider-aihubmix').length,siliconflowRows:document.querySelectorAll('.ec-vendor-matrix td.ec-provider-siliconflow').length,ppioRows:document.querySelectorAll('.ec-vendor-matrix td.ec-provider-ppio').length,ai302Rows:document.querySelectorAll('.ec-vendor-matrix td.ec-provider-302').length,modelFilter:!!modelFilter,modelFilterCount:document.querySelector('#ec-model-filter-count')?.textContent||'',gptRows:vendorRows.filter(r=>(r.dataset.family||'').includes('openai')).length,allModelMarkers:['gpt-5.6-sol','gpt-4.1-mini-2025-04-14','claude-opus-5','gemini-3.8-flash','qwen3.8-max'].every(m=>vendorRows.some(r=>r.dataset.model===m)),familySequence,sortedWithinFamilies,arkSortPrice:vendorRows.find(r=>r.dataset.model==='glm-5.3-flash')?.dataset.sortPrice||'',tieredDianming:document.querySelectorAll('.ec-vendor-matrix td.ec-tiered-price').length,qwenCoderTierCount:vendorRows.find(r=>r.dataset.model==='qwen3-coder-next')?.querySelectorAll('.ec-price-tier').length||0,contextTierRule:text.includes('128K')&&text.includes('0/1,265')&&text.includes('per-call'),status:document.querySelectorAll('.project-status-strip').length,generic3k:text.includes('¥3k')||text.includes('约 ¥3k'),overflow:document.documentElement.scrollWidth>document.documentElement.clientWidth+2};""")
                if costs.get("sections") != 4 or costs.get("tables") != 2 or costs.get("rows") != 160 or costs.get("portfolioRows") != 25 or costs.get("paperCells") != 9 or costs.get("paperRowspanTotal") != 25 or costs.get("e1Rowspan") != 4 or len(costs.get("visibleHeaders",[])) != 7 or not any("之前已经用了" in x for x in costs.get("visibleHeaders",[])) or not any("之前价格" in x for x in costs.get("visibleHeaders",[])) or not any("之后还要用" in x for x in costs.get("visibleHeaders",[])) or not any("之后价格" in x for x in costs.get("visibleHeaders",[])) or costs.get("priceBefore") != 25 or costs.get("priceAfter") != 25 or costs.get("tokenEstimates",0) < 7 or costs.get("gpuPriceCells",0) < 14 or costs.get("tokenPriceCells",0) < 20 or not costs.get("qwen8bGpu") or costs.get("qwen8bTokenEstimate") != 0 or not costs.get("arkE1Token") or not costs.get("arkE1Usage") or not costs.get("e1StagedBudget") or costs.get("vendorHeaders") != ["Exact model","Ark Agent Plan","典名词元","ChatAnywhere","君黎 API*","AIHubMix","SiliconFlow","PPIO","302.AI"] or costs.get("junliaiRows",0) < 15 or costs.get("aihubmixRows",0) < 30 or costs.get("siliconflowRows",0) < 10 or costs.get("ppioRows",0) < 15 or costs.get("ai302Rows",0) < 12 or not costs.get("modelFilter") or costs.get("modelFilterCount") != "160" or costs.get("gptRows",0) < 60 or not costs.get("allModelMarkers") or costs.get("familySequence") != ["qwen","deepseek","kimi","glm","minimax","doubao","hunyuan","ernie","openai / gpt","claude","gemini","grok"] or not costs.get("sortedWithinFamilies") or costs.get("arkSortPrice") != "0.05" or costs.get("tieredDianming",0) < 15 or costs.get("qwenCoderTierCount") != 3 or not costs.get("contextTierRule") or costs.get("facts") != 8 or len(costs.get("toc", [])) != 4 or costs.get("status") != 0 or costs.get("generic3k") or costs.get("overflow"):
