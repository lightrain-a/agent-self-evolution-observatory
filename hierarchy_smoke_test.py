    ...
    try:
        ...
        for page, expected in EXPECTATIONS.items():
            ...
            if page == "experiment-costs":
                costs = execute(session_id, """const paperCells=[...document.querySelectorAll('.ec-portfolio .ec-paper-cell')];return {sections:document.querySelectorAll('.topic-section').length,tables:document.querySelectorAll('.advisor-table-scroll>.matrix').length,rows:document.querySelectorAll('.ec-master tbody tr').length,portfolioRows:document.querySelectorAll('.ec-portfolio tbody tr').length,paperCells:paperCells.length,paperRowspanTotal:paperCells.reduce((s,x)=>s+(Number(x.getAttribute('rowspan'))||1),0),facts:document.querySelectorAll('.ec-facts article').length,toc:[...document.querySelectorAll('#page-toc a')].map(a=>a.textContent.trim()),status:document.querySelectorAll('.project-status-strip').length,generic3k:(document.querySelector('#dynamic-page')?.textContent||'').includes('¥3k')||(document.querySelector('#dynamic-page')?.textContent||'').includes('约 ¥3k'),overflow:document.documentElement.scrollWidth>document.documentElement.clientWidth+2};""")
                if costs.get("sections") != 4 or costs.get("tables") != 2 or costs.get("rows") != 11 or costs.get("portfolioRows") != 24 or costs.get("paperCells") != 9 or costs.get("paperRowspanTotal") != 24 or costs.get("facts") != 7 or len(costs.get("toc", [])) != 4 or costs.get("status") != 0 or costs.get("generic3k") or costs.get("overflow"):
