    ...
    try:
        ...
        selected = execute(session_id, """
          ...
          return {
            ...
            registryDownloadSupplements: papers.map(x => x.downloads?.supplement_zip || '').filter(Boolean),
        ...
        require(summary.get("discovery_aliases") == ["DI2-02","DI2-05","DI2-06","DI2-01","DI2-04"], f"Reader discovery aliases drifted: {summary}")
        require((discovery_provenance.get("D2-PAPER-PROXY-REWARD-MEMORY-VARIANCE") or {}).get("candidate_aliases") == ["DI2-02","DI2-05"], f"Proxy Reward discovery aliases drifted: {discovery_provenance}")
        require((discovery_provenance.get("D2-PAPER-TEMPORAL-SKILL-CAUSAL-BOTTLENECK") or {}).get("candidate_aliases") == ["DI2-06"], f"Temporal Skill discovery alias drifted: {discovery_provenance}")
        require((discovery_provenance.get("D2-PAPER-FAILURE-MEMORY-PROVENANCE") or {}).get("candidate_aliases") == ["DI2-01","DI2-04"], f"Failure Memory discovery aliases drifted: {discovery_provenance}")
        ...
        require(all(alias in " | ".join(selected["registryProvenanceLines"] + selected["discoveryDetailHeaders"]) for alias in ("DI2-01","DI2-02","DI2-04","DI2-05","DI2-06")), f"Reader-facing DI aliases are incomplete: {selected['registryProvenanceLines']} / {selected['discoveryDetailHeaders']}")
