...
def main() -> None:
    ...
    try:
        ...
        require(selected["chapters"] == 5 and selected["currentSTRI"] == 1 and selected["archive"] == 1 and selected["registryCards"] == 5, f"selected-paper must render five canonical PaperRegistry cards plus the STRI detail and four historical archive chapters: {selected}")
        registry_summary = selected["registry"].get("summary") or {}
        require((registry_summary.get("papers"),registry_summary.get("submission_ready"),registry_summary.get("targeted_repair"),registry_summary.get("preparation_pass"),registry_summary.get("legacy_ready_needs_preparation_migration")) == (5,4,1,1,3), f"PaperRegistry canonical projection is stale: {registry_summary}")
