    ...
    try:
        ...
        for page, expected in expected_hubs.items():
            ...
            if page == "/selected-paper.html":
                require("PaperRegistry" in result["text"] and "SUBMISSION_READY" in result["text"] and "AGENT-SAFETY-R9" in result["text"] and "STRI" in result["text"] and "When Reusable Temporal Skills Become Causal Bottlenecks" in result["text"] and "Failure Memories Are Not Neutral" in result["text"] and all(stage in result["text"] for stage in set(expected_registry_stages.values())), "canonical five-paper PaperRegistry is missing or incomplete")
        ...
        require("当前控制面 · 跟踪不等于待办" in research_map["text"] and "控制面跟踪 6 个 ResearchItem" in research_map["text"] and "machine-actionable=0" in research_map["text"] and "领域全景" in research_map["text"] and "研究组合：完整证据" in research_map["text"] and "A–G 快速总览" in research_map["text"] and "把“我们做得多不多”和“外部已经拥不拥挤”分开看" in research_map["text"] and "外部文献更密集" in research_map["text"] and "完整知识图谱技术结构" in research_map["text"], "current research map reading chain/control-board/coverage-density summary is incomplete")
        ...
        require(registry_summary.get("submission_ready") == 5 and registry_summary.get("gate_clean_submission_ready") == 4 and registry_summary.get("paper_preparation_failed") == 1 and registry_summary.get("immediate_submission_holds") == 1 and registry_summary.get("internal_action_required") == 1 and registry_summary.get("no_internal_action") == 4 and temporal_registry.get("gate_clean_submission_ready") is False and temporal_registry.get("immediate_submission_hold") is True and (temporal_registry.get("primary_next_action") or {}).get("action_class") == "PAPER_REPAIR_REQUIRED" and (temporal_registry.get("latest_paper_preparation") or {}).get("pass") is False and ((temporal_registry.get("latest_paper_preparation") or {}).get("passed_gates"),(temporal_registry.get("latest_paper_preparation") or {}).get("required_gates")) == (5,8) and (temporal_registry.get("submission_readiness_context") or {}).get("recommended_immediate_submission") == "READY_FOR_HUMAN_SUBMISSION" and (temporal_evidence.get("runtime_valid_rows"),temporal_evidence.get("distinct_endpoints"),temporal_evidence.get("institutional_systems")) == (1326,35,3) and temporal_review.get("scores") == [8,8,7] and failure_registry.get("paper_stage") == "SUBMISSION_READY" and failure_registry.get("gate_clean_submission_ready") is True and failure_registry.get("active_unrefuted_claims") == 2, f"Research Portfolio must preserve five historical ledger-ready papers while exposing the latest Temporal Skill repair hold: {idea_portfolio['paperRegistry']}")
        ...
        require(selected["externalReviewScores"] == {"STRI":"5.8","AGENT-SAFETY-R9":"6.5","D2-PAPER-PROXY-REWARD-MEMORY-VARIANCE":"6.7","D2-PAPER-TEMPORAL-SKILL-CAUSAL-BOTTLENECK":"5.8","D2-PAPER-FAILURE-MEMORY-PROVENANCE":"5.6"}, f"Stanford external review scores drifted: {selected['externalReviewScores']}")
        require(all(marker in selected["externalReviewText"] for marker in ("Weak Accept","Weak Reject","Lean Accept","Borderline Reject","非官方 ICLR 分数","文稿与定位","现有数据可补分析","需要新增证据 / 实验","Proxy Reward → STRI → Temporal Skill → Agent Safety → Failure Provenance")), f"external review/repair layer is missing reviewer recommendations or repair-plan markers: {selected['externalReviewText'][:2200]}")
        ...
        require(selected["paperRegistrySummary"].get("submission_ready") == 5 and selected["paperRegistrySummary"].get("gate_clean_submission_ready") == 4 and selected["paperRegistrySummary"].get("paper_preparation_failed") == 1 and selected["paperRegistrySummary"].get("immediate_submission_holds") == 1 and selected["paperRegistrySummary"].get("internal_action_required") == 1 and selected["paperRegistrySummary"].get("no_internal_action") == 4 and selected["paperRegistryGateClean"].get("D2-PAPER-TEMPORAL-SKILL-CAUSAL-BOTTLENECK") == "false" and selected["paperRegistryGateClean"].get("D2-PAPER-FAILURE-MEMORY-PROVENANCE") == "true" and selected["paperRegistryNextActions"].get("D2-PAPER-TEMPORAL-SKILL-CAUSAL-BOTTLENECK") == "PAPER_REPAIR_REQUIRED" and selected["paperRegistryNextActions"].get("D2-PAPER-FAILURE-MEMORY-PROVENANCE") == "NO_INTERNAL_ACTION" and (selected["temporalPaper"].get("latest_paper_preparation") or {}).get("pass") is False and ((selected["temporalPaper"].get("latest_paper_preparation") or {}).get("passed_gates"),(selected["temporalPaper"].get("latest_paper_preparation") or {}).get("required_gates")) == (5,8) and (selected["temporalPaper"].get("submission_readiness_context") or {}).get("support_blocker") == "" and (selected["temporalPaper"].get("submission_readiness_context") or {}).get("recommended_immediate_submission") == "READY_FOR_HUMAN_SUBMISSION" and ((selected["temporalPaper"].get("latest_mock_review") or {}).get("summary") or {}).get("scores") == [8,8,7] and ((selected["temporalPaper"].get("source_native_evidence") or {}).get("runtime_valid_rows"),(selected["temporalPaper"].get("source_native_evidence") or {}).get("distinct_endpoints"),(selected["temporalPaper"].get("source_native_evidence") or {}).get("institutional_systems")) == (1326,35,3) and selected["failureMemoryPaper"].get("active_unrefuted_claims") == 2, f"PaperRegistry must show the current four gate-clean / one Temporal repair-hold boundary: {selected}")
