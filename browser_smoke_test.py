    ...
    try:
        ...
        for page, expected in expected_hubs.items():
            ...
            if page == "/selected-paper.html":
                require("PAPER REGISTRY" in result["text"] and "CURRENT SELECTED PAPER" in result["text"] and "Former Regression-Gated Self-Evolution workspace" in result["text"] and "No experiment from this old project is currently allowed to launch" in result["text"], "PaperRegistry, STRI detail, or plain-language historical STOP archive is missing")
        ...
        require(all(marker in shell_zh["status"] for marker in ("投稿就绪论文","还缺的论文证据","通过正式问题检查的新研究问题","因缺证据暂缓的新现象","已关闭的精确候选表述","真正关闭到核心原理层","整个基准或现象也被判定不成立")), f"shared current-status strip is not plain-language Chinese: {shell_zh['status']}")
        ...
        require(idea_portfolio["currentLedger"] == 1, f"Paper Ideas must render the unified canonical current ledger, got {idea_portfolio}")
        ...
        selected = execute(session_id, """return {
          ...
          registry: window.PAPER_REGISTRY_STATE || {},
          registryCards: document.querySelectorAll('.paper-registry-paper').length,
          title: document.title,
          text: document.body.textContent || ''
        };""")
        registry_papers = selected["registry"].get("papers") or []
        require(selected["chapters"] == 5 and selected["currentSTRI"] == 1 and selected["archive"] == 1 and selected["registryCards"] == len(registry_papers) and len(registry_papers) >= 5, f"selected-paper must render every canonical PaperRegistry card plus the STRI detail and four historical archive chapters: {selected}")
        registry_summary = selected["registry"].get("summary") or {}
        expected_ready = sum(p.get("current_state") == "SUBMISSION_READY" for p in registry_papers)
        expected_repair = sum(p.get("current_state") == "TARGETED_REPAIR" for p in registry_papers)
        expected_prep_pass = sum((p.get("paper_preparation") or {}).get("status") == "PASS" for p in registry_papers)
        expected_prep_blocked = sum((p.get("paper_preparation") or {}).get("status") == "BLOCKED" for p in registry_papers)
        expected_legacy = sum((p.get("paper_preparation") or {}).get("status") == "LEGACY_READY_NEEDS_PREPARATION_MIGRATION" for p in registry_papers)
        expected_frozen = sum((p.get("submission_freeze") or {}).get("status") == "MACHINE_FROZEN_HUMAN_SIGNOFF_PENDING" for p in registry_papers)
        expected_stale = sum((p.get("submission_freeze") or {}).get("status") == "MACHINE_FREEZE_STALE" for p in registry_papers)
        expected_machine_handoff = sum((p.get("submission_handoff") or {}).get("status") == "MACHINE_HANDOFF_READY_HUMAN_CONFIRMATION_REQUIRED" for p in registry_papers)
        expected_handoff_stale = sum((p.get("submission_handoff") or {}).get("status") == "MACHINE_HANDOFF_STALE" for p in registry_papers)
        expected_venue_form_pending = sum((p.get("venue_form_consistency") or {}).get("status") == "PENDING_FORM_SNAPSHOT_AND_AUDIT" for p in registry_papers)
        expected_venue_form_pass = sum((p.get("venue_form_consistency") or {}).get("status") == "PASS_VENUE_FORM_CONSISTENCY_AUDIT" for p in registry_papers)
        expected_venue_form_blocked_or_stale = sum((p.get("venue_form_consistency") or {}).get("status") in {"VENUE_FORM_AUDIT_BLOCKED", "VENUE_FORM_AUDIT_STALE"} for p in registry_papers)
        expected_signoff_locked = sum((p.get("submission_handoff") or {}).get("status") == "MACHINE_HANDOFF_READY_HUMAN_CONFIRMATION_REQUIRED" and (p.get("venue_form_consistency") or {}).get("status") != "PASS_VENUE_FORM_CONSISTENCY_AUDIT" for p in registry_papers)
        expected_signoff_pending = sum((p.get("venue_form_consistency") or {}).get("status") == "PASS_VENUE_FORM_CONSISTENCY_AUDIT" and (p.get("human_signoff") or {}).get("status") == "PENDING_HUMAN_CONFIRMATION" for p in registry_papers)
        expected_signoff_complete = sum((p.get("human_signoff") or {}).get("status") == "HUMAN_SIGNOFF_COMPLETE_ACTUAL_SUBMISSION_PENDING" for p in registry_papers)
        expected_signoff_stale = sum((p.get("human_signoff") or {}).get("status") == "HUMAN_SIGNOFF_STALE" for p in registry_papers)
        expected_submitted = sum(p.get("current_state") == "SUBMITTED" for p in registry_papers)
        expected_submitted_bound = sum((p.get("actual_submission") or {}).get("status") == "VENUE_SUBMISSION_CONFIRMED" for p in registry_papers)
        require((registry_summary.get("papers"),registry_summary.get("submission_ready"),registry_summary.get("targeted_repair"),registry_summary.get("preparation_pass"),registry_summary.get("preparation_blocked"),registry_summary.get("legacy_ready_needs_preparation_migration"),registry_summary.get("machine_frozen_candidates"),registry_summary.get("machine_freeze_stale"),registry_summary.get("machine_handoff_ready"),registry_summary.get("machine_handoff_stale"),registry_summary.get("venue_form_pending"),registry_summary.get("venue_form_pass"),registry_summary.get("venue_form_blocked_or_stale"),registry_summary.get("human_signoff_locked_by_venue_form"),registry_summary.get("human_submission_signoff_pending"),registry_summary.get("human_signoff_complete"),registry_summary.get("human_signoff_stale"),registry_summary.get("submitted"),registry_summary.get("submitted_receipt_bound")) == (len(registry_papers),expected_ready,expected_repair,expected_prep_pass,expected_prep_blocked,expected_legacy,expected_frozen,expected_stale,expected_machine_handoff,expected_handoff_stale,expected_venue_form_pending,expected_venue_form_pass,expected_venue_form_blocked_or_stale,expected_signoff_locked,expected_signoff_pending,expected_signoff_complete,expected_signoff_stale,expected_submitted,expected_submitted_bound), f"PaperRegistry summary is not a projection of its paper rows: {registry_summary}")
        temporal = next((p for p in registry_papers if p.get("paper_id") == "D2-PAPER-TEMPORAL-SKILL-CAUSAL-BOTTLENECK"), {})
        require((temporal.get("paper_preparation") or {}).get("status") == "BLOCKED" and (temporal.get("submission_freeze") or {}).get("status") == "PREPARATION_BLOCKED" and (temporal.get("submission_handoff") or {}).get("status") == "PREPARATION_BLOCKED" and (temporal.get("venue_form_consistency") or {}).get("status") == "NOT_ELIGIBLE" and (temporal.get("paper_preparation") or {}).get("human_submission_signoff_pending") is False, f"Temporal Skill preparation blocker must remain distinct from legacy migration, freeze, machine handoff, and venue-form audit: {temporal}")
        ...
        require("PaperRegistry" in selected["title"] and "Noisy Rewards Write Noisy Memories" in selected["text"] and "Temporal Skills Are a Causal Bottleneck" in selected["text"] and "A Static Safety Pass Is Not a Temporal Certificate" in selected["text"] and "Venue Form Consistency" in selected["text"] and "等待最终表单快照" in selected["text"] and ("当前选中论文" in selected["text"] or "CURRENT SELECTED PAPER" in selected["text"]) and "2026-09-18" in selected["text"] and "2026-09-25" in selected["text"] and ("Official author-facing ICLR pages are aligned" in selected["text"] or "面向作者的 ICLR 官方页面已一致") and ("Former Regression-Gated Self-Evolution workspace" in selected["text"] or "旧 Regression-Gated Self-Evolution 工作区" in selected["text"]), f"selected-paper registry/current/historical hierarchy or deadline handoff is wrong: {selected}")
        ...
        selected_markers = ("科学主张","N1 · AutoSkill P19 动态机制证据","6/6","0/6","post-checkout 加回=3/3","匹配清理对照=0/3","精确 p=1/20","Stage-3 复放=18/18","独立审稿后决策","4/10","6/10","哪些动态实验进入了主张，哪些没有","论文证据是否齐全且能追到固定文件","ICLR 投稿包","可视化证据","人工提交交接","已发布控制平面上的表示敏感性","是否声称求解算法本身是新贡献","等待人工签字")
        ...
        require("作者需审阅最终稿与 AI 使用声明并承担责任" in selected["text"] and "Human authors review and accept responsibility" not in selected["text"], "Selected Paper Chinese deadline handoff leaked the raw English next_action")
