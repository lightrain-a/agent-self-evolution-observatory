...
def main() -> None:
    ...
    try:
        ...
        require((idea_portfolio["currentLedger"],idea_portfolio["currentRows"],idea_portfolio["leadingPaperTracks"]) == (1,7,1), f"unified current idea ledger is incomplete: {idea_portfolio}")
        ...
        require((cs.get("paper_ready"),cs.get("paper_quality_hold"),cs.get("paper_quality_evidence_debt"),cs.get("canonical_live_ideas"),cs.get("launchable_formal_experiments"),cs.get("shadow_dead_ends"),cs.get("shadow_holds"),cs.get("shadow_qualification_ready"),cs.get("legacy_p0_lifecycle")) == (1,0,0,0,0,12,3,0,27), f"current status snapshot is wrong: {cs}")
