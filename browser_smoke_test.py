...
def main() -> None:
    ...
    try:
        ...
        require(autoskill.get("status") == "GO_STAGE3_DYNAMIC_BEHAVIORAL_PROPAGATION" and groups.get("A_original",{}).get("destructive_signature_positive") == 6 and groups.get("B_split4",{}).get("destructive_signature_positive") == 0 and groups.get("C_id_placebo",{}).get("destructive_signature_positive") == 3 and groups.get("D_quotient_control",{}).get("destructive_signature_positive") == 3 and abs(float(autoskill.get("fisher_exact_p") or 1.0) - 0.0010822510822510823) < 1e-12 and autoskill.get("judge_calls") == 0 and autoskill.get("fresh_container_per_run") is True, f"selected-paper AutoSkill P19 dynamic evidence is stale: {selected}")
        ...
        require(all(marker in selected["text"] for marker in ("科学主张","N1 · AutoSkill P19 动态行为证据","6/6","0/6","Fisher p=0.00108","哪些动态实验进入了主张，哪些没有","论文证据是否齐全且能追到固定文件","ICLR 投稿包","可视化证据","人工提交交接","已发布控制平面上的表示敏感性","是否声称求解算法本身是新贡献","等待人工签字")), "selected-paper Chinese-first current-paper UI is incomplete")
        ...
        require(all(marker in experiments_zh for marker in ("先看当前实验结论","N1 新增动态证据：仅改变 package 表示会不会改变真实执行行为","6/6","0/6","Fisher p=0.00108","额外检查 A：提案器是否具备继续实验的基本能力","额外检查 E：更完整策略是否真的会改变最终结果","以前的记忆效应 · 为什么不继续","0 个正式实验可启动")), "Experiments Chinese-first current-evidence UI is incomplete")
