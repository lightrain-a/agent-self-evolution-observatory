...
def main() -> None:
    ...
    try:
        ...
        require(selected["objectionMatrixPortfolio"] == 1 and selected["objectionDetails"] == 5 and selected["objectionRows"] == 33 and selected["objectionDispositionCounts"] == {"RESOLVED":10,"EXISTING_EVIDENCE_ACTIONABLE":6,"REQUIRES_SCIENTIFIC_REOPEN":9,"PERMANENT_CLAIM_BOUNDARY":8}, f"Stanford Round-2 objection matrix must render 33 objections across the four legal dispositions: {selected}")
        require(all(marker in selected["objectionText"] for marker in ("33 条 objection","10 已解决","6 现有证据可补","9 需 scientific reopen","8 永久边界","0 AUTO ACTION","SAFETY-O3","SAFETY-O4","SAFETY-O5","TEMP-O3","88/88","TEMP-O5","PROXY-O5","FAILURE-O5")), f"Reviewer Objection Matrix is missing decision-critical disposition markers: {selected['objectionText'][:3200]}")
