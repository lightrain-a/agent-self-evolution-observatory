...
def main() -> None:
    ...
    try:
        ...
        require(selected["objectionMatrixPortfolio"] == 1 and selected["objectionDetails"] == 5 and selected["objectionRows"] == 33 and selected["objectionDispositionCounts"] == {"RESOLVED":12,"EXISTING_EVIDENCE_ACTIONABLE":4,"REQUIRES_SCIENTIFIC_REOPEN":9,"PERMANENT_CLAIM_BOUNDARY":8}, f"Stanford Round-2 objection matrix must render 33 objections across the four legal dispositions: {selected}")
        require(all(marker in selected["objectionText"] for marker in ("33 条 objection","12 已解决","4 现有证据可补","9 需 scientific reopen","8 永久边界","0 AUTO ACTION","SAFETY-O3","SAFETY-O4","SAFETY-O5","TEMP-O3","88/88","TEMP-O5","PROXY-O5","FAILURE-O3","FAILURE-O4","FAILURE-O5","FAILURE-O6")), f"Reviewer Objection Matrix is missing decision-critical disposition markers: {selected['objectionText'][:3200]}")
        ...
        temporal_selected_evidence=temporal_selected.get("source_native_evidence") or {}
        require(selected["paperRegistrySummary"] == expected_registry_summary and selected["paperRegistryGateClean"].get("D2-PAPER-TEMPORAL-SKILL-CAUSAL-BOTTLENECK") == ("true" if temporal_selected_clean else "false") and selected["paperRegistryGateClean"].get("D2-PAPER-FAILURE-MEMORY-PROVENANCE") == "true" and selected["paperRegistryNextActions"].get("D2-PAPER-TEMPORAL-SKILL-CAUSAL-BOTTLENECK") == temporal_selected_action and selected["paperRegistryNextActions"].get("D2-PAPER-FAILURE-MEMORY-PROVENANCE") == "NO_INTERNAL_ACTION" and int(temporal_selected_prep.get("required_gates") or 0) == 8 and (temporal_selected_clean or bool(temporal_selected_prep.get("blockers"))) and int(temporal_selected_evidence.get("runtime_valid_rows") or 0) > 0 and int(temporal_selected_evidence.get("distinct_endpoints") or 0) > 0 and int(temporal_selected_evidence.get("institutional_systems") or 0) > 0 and bool(temporal_selected.get("latest_mock_review")) and selected["failureMemoryPaper"].get("active_unrefuted_claims") == 2, f"PaperRegistry effective readiness must track latest receipts without pinning a historical Temporal revision: {selected}")
        ...
        require(safety_paper.get("source_research_item") == "G-1" and safety_paper.get("paper_stage") == "PREBUTTAL" and safety_paper.get("scientific_status") == "READY" and safety_paper.get("submission_ready") is False and (safety_paper.get("latest_story_search") or {}).get("selected_story_id") == "erta-evaluator-relative-temporal-audit-r8" and all((safety_paper.get("mock_pc_modes") or {}).get(mode) for mode in ("BLIND_MANUSCRIPT","ARTIFACT_AWARE")) and (safety_paper.get("latest_claim_audit") or {}).get("pass") is True and ((safety_paper.get("latest_manuscript_ci") or {}).get("passed"),(safety_paper.get("latest_manuscript_ci") or {}).get("required")) == (9,9) and (safety_paper.get("latest_prebuttal") or {}).get("decision_critical") == 6 and int((safety_paper.get("latest_prebuttal") or {}).get("unresolved_decision_critical") or 0) == 4 and (safety_paper.get("latest_submission_readiness") or {}).get("submission_ready") is False and ((safety_paper.get("latest_mock_review") or {}).get("summary") or {}).get("scores") == [5] and ((safety_paper.get("latest_mock_review") or {}).get("summary") or {}).get("recommendations") == ["borderline"] and (safety_paper.get("latest_paper_preparation") or {}).get("passed_gates") == 7 and (safety_paper.get("latest_paper_preparation") or {}).get("required_gates") == 8 and (safety_paper.get("primary_next_action") or {}).get("action_class") == "EXTERNAL_EVIDENCE_REQUIRED" and (safety_paper.get("primary_next_action") or {}).get("blocking_on") == "HUMAN_SEMANTIC_LABEL_EVIDENCE_REQUIRED" and (safety_paper.get("authority") or {}).get("submission") is False, f"Agent Safety bounded R9 PaperState projection is stale: {safety_paper}")
        safety_acceptance=selected["agentSafetyAcceptance"]
        require(safety_acceptance.get("current_state") == "PREBUTTAL" and safety_acceptance.get("scientific_status") == "READY" and (safety_acceptance.get("latest_story_search") or {}).get("selected_story_id") == "erta-evaluator-relative-temporal-audit-r8" and (safety_acceptance.get("latest_claim_audit") or {}).get("pass") is True and ((safety_acceptance.get("latest_manuscript_ci") or {}).get("passed"),(safety_acceptance.get("latest_manuscript_ci") or {}).get("required")) == (9,9) and (safety_acceptance.get("latest_prebuttal") or {}).get("decision_critical") == 6 and int((safety_acceptance.get("latest_prebuttal") or {}).get("unresolved_decision_critical") or 0) == 4 and (safety_acceptance.get("latest_submission_readiness") or {}).get("submission_ready") is False and (safety_acceptance.get("primary_next_action") or {}).get("action_class") == "EXTERNAL_EVIDENCE_REQUIRED" and safety_acceptance.get("authority") == {"scientific":False,"experiment":False,"gpu":False,"submission":False}, f"canonical Agent Safety Paper Acceptance projection is stale or unsafe: {selected}")
        ...
        require("PaperRegistry" in selected["title"] and "PaperRegistry" in selected["text"] and "SUBMISSION_READY" in selected["text"] and "AGENT-SAFETY-R9" in selected["text"] and "ERTA 双评测器审计" in selected["text"] and "measurement-inconclusive" in selected["text"] and "2026-09-18" in selected["text"] and "2026-09-25" in selected["text"] and ("Former Regression-Gated Self-Evolution workspace" in selected["text"] or "旧 Regression-Gated Self-Evolution 工作区" in selected["text"]), f"selected-paper PaperRegistry/current/historical hierarchy is wrong: {selected}")
