...
def main() -> None:
    ...
    try:
        ...
        require(system_overview["memorySummary"].get("entries") >= 50 and int(system_overview["memorySummary"].get("scientific_closures") or 0) == int(system_overview["searchClosureSummary"].get("core_principle_dead_ends") or 0) and int(system_overview["memorySummary"].get("search_closures") or 0) + int(system_overview["memorySummary"].get("scientific_closures") or 0) == int(system_overview["searchClosureSummary"].get("shadow_closed_basins") or 0) and system_overview["memorySummary"].get("failure_assets") >= 3 and system_overview["memorySummary"].get("success_assets") >= 3 and int(system_overview["memorySummary"].get("discovery_lessons") or 0) == 19 and system_overview["memorySummary"].get("paper_development_guidance") == 1 and system_overview["memoryLint"].get("errors") == 0, f"Research Memory Wiki state is missing or inconsistent with canonical typed closures/discovery lessons/development guidance: summary={system_overview['memorySummary']} canonical={system_overview['searchClosureSummary']} lint={system_overview['memoryLint']}")
        require(system_overview["discoveryLessonSection"] == 1 and system_overview["discoveryLessonCards"] == 19 and "DISCOVERY_LESSON" in system_overview["text"], f"all 19 Discovery Lessons must be visibly rendered from Research Memory: section={system_overview['discoveryLessonSection']} cards={system_overview['discoveryLessonCards']}")
