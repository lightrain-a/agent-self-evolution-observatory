...
def main() -> None:
    ...
    try:
        ...
        require(selected["objectionMatrixPortfolio"] == 1 and selected["objectionDetails"] == 5 and selected["objectionRows"] == 33 and selected["objectionDispositionCounts"] == {"RESOLVED":7,"EXISTING_EVIDENCE_ACTIONABLE":9,"REQUIRES_SCIENTIFIC_REOPEN":9,"PERMANENT_CLAIM_BOUNDARY":8}, f"Stanford Round-2 objection matrix must render 33 objections across the four legal dispositions: {selected}")
        require(all(marker in selected["objectionText"] for marker in ("33 条 objection","7 已解决","9 现有证据可补","9 需 scientific reopen","8 永久边界","0 AUTO ACTION","TEMP-O2","TEMP-O5","PROXY-O5","FAILURE-O5")), f"Reviewer Objection Matrix is missing decision-critical disposition markers: {selected['objectionText'][:3200]}")
        ...
        require(safety_paper.get("source_research_item") == "G-1" and safety_paper.get("paper_stage") == "SUBMISSION_READY" and safety_paper.get("scientific_status") == "READY" and safety_paper.get("submission_ready") is True and (safety_paper.get("latest_story_search") or {}).get("selected_story_id") == "S1-TEMPORAL-CERTIFICATE-CONTROL" and all((safety_paper.get("mock_pc_modes") or {}).get(mode) for mode in ("BLIND_MANUSCRIPT","ARTIFACT_AWARE")) and (safety_paper.get("latest_claim_audit") or {}).get("pass") is True and ((safety_paper.get("latest_manuscript_ci") or {}).get("passed"),(safety_paper.get("latest_manuscript_ci") or {}).get("required")) == (9,9) and (safety_paper.get("latest_prebuttal") or {}).get("decision_critical") == 12 and (safety_paper.get("latest_submission_readiness") or {}).get("submission_ready") is True and (safety_paper.get("authority") or {}).get("submission") is False, f"Agent Safety bounded R9 PaperState projection is stale: {safety_paper}")
        safety_acceptance=selected["agentSafetyAcceptance"]
        require(safety_acceptance.get("current_state") == "SUBMISSION_READY" and safety_acceptance.get("scientific_status") == "READY" and (safety_acceptance.get("latest_story_search") or {}).get("selected_story_id") == "S1-TEMPORAL-CERTIFICATE-CONTROL" and (safety_acceptance.get("latest_claim_audit") or {}).get("pass") is True and ((safety_acceptance.get("latest_manuscript_ci") or {}).get("passed"),(safety_acceptance.get("latest_manuscript_ci") or {}).get("required")) == (9,9) and (safety_acceptance.get("latest_prebuttal") or {}).get("decision_critical") == 12 and (safety_acceptance.get("latest_submission_readiness") or {}).get("submission_ready") is True and safety_acceptance.get("authority") == {"scientific":False,"experiment":False,"gpu":False,"submission":False}, f"canonical Agent Safety Paper Acceptance projection is stale or unsafe: {selected}")
