...
def main() -> None:
    ...
    try:
        ...
        require(all(marker in shell_zh["status"] for marker in ("投稿就绪论文","还缺的论文证据","通过正式问题检查的新研究问题","因缺证据暂缓的新现象","已关闭的精确候选表述","core_principle 层 scoped STOP","更宽 benchmark/现象整体被证伪")), f"shared current-status strip is not plain-language Chinese: {shell_zh['status']}")
