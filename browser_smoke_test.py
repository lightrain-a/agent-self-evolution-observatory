...
def main() -> None:
    ...
    try:
        ...
        require(registry_summary.get("submission_ready") == 5 and registry_summary.get("gate_clean_submission_ready") == 4 and registry_summary.get("paper_preparation_failed") == 1 and registry_summary.get("immediate_submission_holds") == 1 and temporal_registry.get("gate_clean_submission_ready") is False and temporal_registry.get("immediate_submission_hold") is True and (temporal_registry.get("latest_paper_preparation") or {}).get("pass") is False and (temporal_registry.get("submission_readiness_context") or {}).get("recommended_immediate_submission") == "HOLD_FOR_EVIDENCE" and failure_registry.get("paper_stage") == "SUBMISSION_READY" and failure_registry.get("gate_clean_submission_ready") is True and failure_registry.get("active_unrefuted_claims") == 2, f"Research Portfolio must preserve the five ledger-ready / four gate-clean / one readiness-hold boundary: {idea_portfolio['paperRegistry']}")
