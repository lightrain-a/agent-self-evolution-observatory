...
def main() -> None:
    ...
    try:
        ...
        require(registry_summary.get("submission_ready") == 5 and registry_summary.get("gate_clean_submission_ready") == 5 and registry_summary.get("paper_preparation_failed") == 0 and registry_summary.get("immediate_submission_holds") == 0 and registry_summary.get("internal_action_required") == 0 and registry_summary.get("no_internal_action") == 5 and temporal_registry.get("gate_clean_submission_ready") is True and temporal_registry.get("immediate_submission_hold") is False and (temporal_registry.get("primary_next_action") or {}).get("action_class") == "NO_INTERNAL_ACTION" and (temporal_registry.get("latest_paper_preparation") or {}).get("pass") is True and (temporal_registry.get("submission_readiness_context") or {}).get("recommended_immediate_submission") == "READY_FOR_HUMAN_SUBMISSION" and (temporal_evidence.get("runtime_valid_rows"),temporal_evidence.get("distinct_endpoints"),temporal_evidence.get("institutional_systems")) == (1326,35,3) and temporal_review.get("scores") == [8,8,7] and failure_registry.get("paper_stage") == "SUBMISSION_READY" and failure_registry.get("gate_clean_submission_ready") is True and failure_registry.get("active_unrefuted_claims") == 2, f"Research Portfolio must preserve the five ledger-ready / five gate-clean / zero-readiness-hold / zero-internal-action boundary: {idea_portfolio['paperRegistry']}")
