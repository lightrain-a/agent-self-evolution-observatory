(() => {
  ...
  function renderCostDependencies(){
    ...
    const rows=orderedPapers().map(p=>{const r=resource(p),d=dims(p),op=r.operational_snapshot||{};return `<tr><td><b>${paperSeq(p)} ${esc(shortPaperName(p))}</b><br><span class="advisor-route ${routeClass(p.route)}">${esc(routeLabel(p.route))}</span>${op.observed_at?`<small>Live · ${esc(op.observed_at)}</small>`:''}</td><td><p>${esc(d.api_cash||'UNKNOWN')}</p></td><td><p>${esc(d.local_gpu_occupancy||'UNKNOWN')}</p></td><td><p>${esc(d.human_time||'UNKNOWN')}</p></td><td><p>${esc(d.provider_credential_dependency||'UNKNOWN')}</p></td><td><p>${esc(d.calendar_latency||'UNKNOWN')}</p></td><td><p><b>NEXT</b> ${esc(r.next_if_pass||'')}</p><p><b>CONDITIONAL</b> ${esc(r.conditional_envelope||'')}</p></td></tr>`}).join('');
    return `<section class="advisor-section" id="cost-dependencies"><header><div><h2>${zh('资源、成本与并行调度','Resource, cost, and parallel scheduling')}</h2><p>${zh('每篇统一按 API cash → local GPU occupancy → human time → provider/credential dependency → calendar latency 五个维度展示。Human 是会后执行工时，不是 14:00–17:00 的会议分钟；只有明确 authority 下的资源才算当前承诺，NEXT / CONDITIONAL 都不提前占预算。','Every paper uses the same five resource dimensions: API cash → local GPU occupancy → human time → provider/credential dependency → calendar latency. Human is post-meeting execution effort, not meeting minutes; only resources under explicit current authority are commitments, and NEXT / CONDITIONAL are not pre-booked.')}</p></div></header><div class="advisor-scheduling-grid">${lanes}</div><div class="advisor-table-wrap"><table class="matrix advisor-cost-table"><thead><tr><th>Paper / Route</th><th>API cash</th><th>Local GPU</th><th>Human</th><th>Provider / Credential</th><th>Calendar</th><th>Next / Conditional</th></tr></thead><tbody>${rows}</tbody></table></div></section>`;
  }
