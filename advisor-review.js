(() => {
  ...
  function decisionControl(p){
    ...
    return `<section class="advisor-live-decision" data-decision-paper="${esc(p.paper_id)}"><div class="advisor-live-decision-head"><div><b>${zh('笔记','NOTES')}</b></div><small>${zh('这里只记录师兄是否建议改变当前路线；不会自动改变科学结论或实验权限','Records the advisor override only; never changes scientific authority automatically')}</small></div><div class="advisor-live-decision-actions">${options.map(([key,label])=>`<button type="button" data-decision-choice="${key}" class="${current.status===key?'selected':''}">${esc(label)}</button>`).join('')}</div><input type="text" data-decision-note value="${esc(current.note||'')}" placeholder="${zh('一句话备注：为什么改 / 谁 follow-up / 何时回看','一句话备注: why / owner / follow-up trigger')}"></section>`;
  }
