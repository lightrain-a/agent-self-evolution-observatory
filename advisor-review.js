(() => {
  ...
  function renderToc(){
    ...
    toc.innerHTML=`<div class="toc-title">${zh('会议导航 · 14:00–17:00','Meeting path · 14:00–17:00')}</div><div class="toc-links"><a href="#meeting-path">${zh('3 小时单一路线','3-hour single route')}</a><a href="#portfolio">${zh('九篇总览','Portfolio')}</a>${paperLinks}<a href="#shared-risks">${zh('跨论文 Exceptions / Shared Risks','Cross-paper exceptions / shared risks')}</a><a href="#cost-dependencies">${zh('资源与 Authority','Resources & authority')}</a><a href="#decision-ledger">${zh('锁 Decision Ledger','Lock decision ledger')}</a><a href="#readback">${zh('最终 Read-back','Final read-back')}</a><a href="#spinoffs">${zh('会后分叉候选','Post-meeting spinoffs')}</a></div>`;
  }
  function renderHero(){
