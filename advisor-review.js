(() => {
  ...
  function renderToc(){
    ...
    toc.innerHTML=`<div class="toc-title">${zh('会议导航','Meeting path')}</div><div class="toc-links"><a href="#portfolio">${zh('九篇总览','Portfolio')}</a><a href="#shared-risks">${zh('共同风险','Shared risks')}</a><a href="#cost-dependencies">${zh('成本与依赖','Cost & dependencies')}</a><a href="#paper-review">Decision Cards</a><a href="#schedule">${zh('3 小时议程','Schedule')}</a></div>`;
  }
  function renderHero(){
  ...
  function renderCostDependencies(){
    const rows=DATA.papers.map(p=>`<tr><td><b>${esc(p.paper_id)}</b><br><span class="advisor-route ${routeClass(p.route)}">${esc(routeLabel(p.route))}</span></td><td><strong>${esc(p.cost_class)}</strong><p>${esc(p.cost_to_next_decision)}</p></td><td>${esc(depText(p))}</td><td>${esc(p.default_action)}</td><td>${esc(p.override_trigger)}</td></tr>`).join('');
    return `<section class="advisor-section" id="cost-dependencies"><header><div><h2>${zh('Cost / Dependency / Override Matrix','Cost / dependency / override matrix')}</h2><p>${zh('这里只统计到下一道会改变 scientific route 的 closure。后续 conditional workload 不提前记成承诺成本。','Only the next decision-changing closure is costed here. Conditional downstream workload is not pre-committed.')}</p></div></header><div class="advisor-table-wrap"><table class="matrix advisor-cost-table"><thead><tr><th>Paper / Route</th><th>${zh('Cost to next decision','Cost to next decision')}</th><th>${zh('关键依赖','Dependencies')}</th><th>${zh('默认动作','Default action')}</th><th>${zh('Override trigger','Override trigger')}</th></tr></thead><tbody>${rows}</tbody></table></div></section>`;
  }
