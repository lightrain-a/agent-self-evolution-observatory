(() => {
  ...
  function renderToc(){
    ...
    toc.innerHTML=`<div class="toc-title">${zh('会议导航','Meeting path')}</div><div class="toc-links"><a href="#portfolio">${zh('九篇总览','Portfolio')}</a><a href="#shared-risks">${zh('共同风险 / Ownership','Shared risks / ownership')}</a><a href="#meeting-outputs">${zh('17:00 输出','17:00 outputs')}</a><a href="#cost-dependencies">${zh('成本与依赖','Cost & dependencies')}</a><a href="#paper-review">Decision Cards</a><a href="#schedule">${zh('3 小时议程','Schedule')}</a></div>`;
  }
  function renderHero(){
