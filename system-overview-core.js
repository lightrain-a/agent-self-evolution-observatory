(() => {
  ...
  api.renderPurpose = function renderPurpose(state, s2) {
    ...
    return `<section class="system-hero system-section">
      <div class="system-hero-copy"><span class="system-kicker">${pick("科研到论文的统一流程","RESEARCH-TO-PAPER WORKFLOW")}</span><h3>${pick("整套系统依次回答三个问题：这个问题值得继续研究吗？最小实验能不能判清？证据最终够不够形成论文？","The system answers three questions in order: is this problem worth pursuing, can the smallest experiment decide it, and is the final evidence sufficient for a paper?")}</h3><p>${pick("实际顺序是：先从论文和已有实验找到一个具体问题，确认已有工作或简单解释还不能覆盖；再写清科学贡献、方法和最强基线；然后只跑最便宜的决定性实验；证据稳定后才扩量并进入论文流程。AI 评审、GPU 调度、失败诊断和历史记忆都只是这条主流程的辅助工具。","The actual order is: find a concrete problem from papers or prior experiments; verify that prior work or a simpler explanation does not already cover it; write down the scientific contribution, method, and strongest baseline; run only the cheapest decisive test; and scale into the paper workflow only after the evidence is stable. AI review, GPU scheduling, failure diagnosis, and research memory support this flow.")}</p></div>
