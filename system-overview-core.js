(() => {
  ...
  api.renderPurpose = function renderPurpose(state, s2) {
    ...
    return `<section class="system-hero system-section">
      <div class="system-hero-copy"><span class="system-kicker">${pick("PAPER-FIRST 科研操作系统","PAPER-FIRST RESEARCH OS")}</span><h3>${pick("后端只保留一个科研主逻辑：先设计一篇具有明确 Novelty 的论文，再设计方法和实验，局部验证后冻结，最后收集全量证据。","The backend now has one research logic: design a paper-worthy contribution first, then the method and experiments, validate locally, freeze, and only then collect full-scale evidence.")}</h3><p>${pick("研究想法（Idea）生成、AI 会诊、P0、实验调度、失败诊断和系统记忆不再各自形成平行流程，而是共同服务于同一条 Paper-first 生命周期。后端架构清单（Architecture Manifest）把每个组件归到唯一主责层，前端直接读取这份结构。","Idea generation, AI consultation, P0, experiment scheduling, failure diagnosis, and scientific memory are no longer separate workflows; each serves the same paper-first lifecycle. A backend Architecture Manifest assigns every component to one primary responsibility layer and the frontend reads that structure directly.")}</p></div>
    ...
    <section class="system-principles system-section"><h3>${pick("系统最终保留的六条科研硬约束","Six final research invariants")}</h3><p class="section-intro">${pick("这些约束比某个具体 Idea、模型或实验批次更高层；任何新模块都必须服从它们。","These constraints sit above any individual idea, model, or experiment batch; every new module must obey them.")}</p><div class="system-principle-grid">
      ${principle(1,"证据先于主张","Evidence before claims","文献、最近邻工作、Baseline、局部实验（Pilot）与失败结论都必须能回到真实来源；未知项必须明确保持未知。","Literature, closest work, baselines, pilots, and failure claims remain traceable to real sources; unknown stays unknown.")}
      ${principle(2,"Novelty 先于实现","Novelty before implementation","先回答论文为什么值得发表、与最近邻工作（closest work）的不可约差异是什么；说不清核心贡献就不进入实现。","First establish why the paper is publishable and what is irreducibly different from the closest work; no clear contribution means no implementation.")}
      ${principle(3,"原理、方法和实验蓝图先于局部实验","Principle, method, and blueprint before pilots","先冻结机制、方法组件、最强简化对照和“论文主张→实验”关系；局部实验只能验证预注册风险，不能边跑边发明核心方法。","Freeze mechanism, method components, strongest simplification, and Claim→Experiment mapping first; local pilots test preregistered risks and never invent the core method on the fly.")}
      ${principle(4,"先证明值得且可辨识，再消耗 GPU","Economy and identifiability before GPU","先用最强简化对照、实验底座库存、因果单元、信息价值（VOI）、协议有效性与 8 道实验门，判断最便宜的决定性实验是否有资格启动。","Use strongest simplifications, substrate inventory, causal units, VOI, protocol checks, and eight gates to qualify the cheapest decisive test before GPU spend.")}
      ...
      ${principle(6,"方法冻结后再扩量；运行可恢复，最终主张有人类门","Freeze before scale; recoverable runs and human claim authority","核心方法若因局部结果改变，就回到 Novelty / 方法 / 实验蓝图；全量实验只执行冻结方案，预算扩张、原理解释和最终论文主张保留人工科学权限。","If local evidence changes the core method, return to Novelty/Method/Blueprint. Full experiments execute only frozen designs, while budget escalation, principle interpretation, and final paper claims remain under human scientific authority.")}
    </div></section>`;
  };
})();
