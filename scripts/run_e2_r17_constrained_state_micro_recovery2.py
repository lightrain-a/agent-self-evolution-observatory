...
def main()->int:
    ...
    try:
        ...
        for arm,task in allowed:
            ...
            lp,ledger=ledgers[arm]; before=ledger.snapshot().total_claimed; out=eroot/'evaluation_summary.json'; skill_path=Path(states[arm]['skill_path']); skill_dir=str(skill_path.parent if arm=='g0_base' else (ROOT/skill_path).resolve().parent)
            cmd=[str(actor_py),str(ROOT/'scripts/run_e2_r17_actor_pool_constrained_state_micro.py'),'--env-file',c['env_file'],'--suite-root',c['suite']['root'],'--mindmemos-root',c['mindmemos']['root'],'--run-root',str(eroot),'--identity',str(identity),'--authorization',str(args.authorization.resolve()),'--skill-source',skill_dir,'--mode','e1','--model',c['actor']['requested_model'],'--task-id',task,'--k','1','--prefix-ks','1','--max-turns',str(c['actor']['max_turns']),'--max-output-tokens',str(c['actor']['max_output_tokens']),'--concurrency','1','--provider-budget-ledger',str(lp),'--provider-total-call-limit','123','--provider-per-unit-call-limit','11','--output',str(out)]
            if arm!='g0_base': cmd += ['--updater-receipt',str(receipts[arm])]
