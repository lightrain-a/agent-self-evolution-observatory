...
STATUS="AUTHORIZED_E2_R17_SINGLE_CASE_FIRST_FAIL_STABILITY_ANALYSIS"; AUDIT="PASS_FIRST_FAIL_STABILITY_FULL_INTEGRITY_READY_FOR_ANALYSIS"
...
def main()->int:
    ap=argparse.ArgumentParser(); ap.add_argument('--contract',type=Path,required=True); ap.add_argument('--execution-authorization',type=Path,required=True); ap.add_argument('--completion-audit',type=Path,required=True); ap.add_argument('--run-summary',type=Path,required=True); ap.add_argument('--analyzer',type=Path,required=True); ap.add_argument('--analysis-output',type=Path,required=True); ap.add_argument('--output',type=Path,required=True); a=ap.parse_args(); req(not a.output.exists() and not a.analysis_output.exists(),'stability analysis auth/output exists')
