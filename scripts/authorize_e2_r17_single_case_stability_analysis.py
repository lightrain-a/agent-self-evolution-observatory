...
def main()->int:
    ...
    req(not a.output.exists() and not a.analysis_output.exists(),"stability analysis authority/output already exists")
    for p in (a.contract,a.execution_authorization,a.completion_audit,a.run_summary,a.analyzer): req(p.is_file(),f"missing bound artifact {p}")
    c=load(a.contract); ea=load(a.execution_authorization); au=load(a.completion_audit); s=load(a.run_summary); csha=sha(a.contract); easha=sha(a.execution_authorization); ssha=sha(a.run_summary)
    ...
    req(au.get("status")==AUDIT_STATUS and au.get("contract_sha256")==csha and au.get("execution_authorization_sha256")==easha and au.get("run_summary_sha256")==ssha,"stability audit drift")
    req(au.get("scientific_scores_read") is False and au.get("partial_effect_read") is False and au.get("analyzer_run") is False,"stability audit crossed outcome boundary")
    req(s.get("status")=="COMPLETED_PENDING_SEPARATE_STABILITY_ANALYSIS","stability run incomplete")
    payload={"schema_version":"1.0","artifact_type":"e2-r17-single-case-first-fail-stability-analysis-authorization","created_at_utc":datetime.now(timezone.utc).isoformat(timespec="seconds"),"status":AUTH_STATUS,"contract_path":str(a.contract),"contract_sha256":csha,"execution_authorization_path":str(a.execution_authorization),"execution_authorization_sha256":easha,"completion_audit_path":str(a.completion_audit),"completion_audit_sha256":sha(a.completion_audit),"run_summary_path":str(a.run_summary),"run_summary_sha256":ssha,"analyzer_path":str(a.analyzer),"analyzer_sha256":sha(a.analyzer),"analysis_output_path":str(a.analysis_output),"single_use":True,"authority":{"analyzer":True,"read_complete_stability_effect_once":True,"scientific_experiment":False,"provider_io":False,"updater":False,"heldout_evaluation":False,"second_backbone":False,"public_benchmark":False,"e3_confirmation":False,"s2_execution":False,"paper_promotion":False,"submission":False}}
