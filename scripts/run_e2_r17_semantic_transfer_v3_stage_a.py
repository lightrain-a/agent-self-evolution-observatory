...
EXPECTED_AUTH_STATUS = "AUTHORIZED_SEMANTIC_TRANSFER_V3_STAGE_A"
...
def validate_contract_auth(contract_path: Path, auth_path: Path) -> tuple[dict[str, Any], dict[str, Any], str, str]:
    ...
    require(contract.get("status") == EXPECTED_CONTRACT_STATUS, "Stage-A V3 contract status invalid")
    require(auth.get("status") == EXPECTED_AUTH_STATUS, "Stage-A V3 authorization status invalid")
    require(auth.get("contract_sha256") == contract_sha, "authorization does not bind exact Stage-A V3 contract")
...
def verify_authorization_scope(contract: dict[str, Any], auth: dict[str, Any], all_tasks: list[str], heldout: list[str]) -> None:
    ...
    require(scope.get("exact_prefix_ks") == [1, 2, 4, 8], "authorization prefix K drift")
    require(int(scope.get("exact_concurrency") or 0) == int(contract["actor"]["concurrency"]), "authorization concurrency drift")
    require(scope.get("required_run_root") == contract["run_root"], "authorization run-root drift")
    require(scope.get("runner_lease_required") is True, "authorization must require first-run runner lease")
...
def main() -> int:
    ...
    require(args.env_file.resolve() == bound_env, "Stage-A V3 env-file path drift")
    ...
    require(list(streams) == stream_ids, "Stage-A V3 stream ordering drift")
    all_tasks = [task for stream_id in stream_ids for task in streams[stream_id]]
    require(len(all_tasks) == 160 and len(set(all_tasks)) == 160, "Stage-A V3 task set drift")
    heldout = [str(x) for x in split["e1_common_heldout_probe"]]
    require(len(heldout) == 20 and len(set(heldout)) == 20 and set(all_tasks).isdisjoint(heldout), "Stage-A heldout separation drift")
    ...
    require(not run_root.exists(), "Stage-A V3 run root already exists; resume requires separate adjudication")
    require(not summary_path.exists(), "Stage-A V3 terminal summary already exists")

    global_lease_path = Path(contract["global_lease_path"])
    lease_payload = {
        "schema_version": "1.0",
        "artifact_type": "e2-r17-semantic-transfer-v3-stage-a-global-lineage-lease",
    ...
    try:
        ...
        require(not manifest_path.exists(), "first-run Stage-A V3 unexpectedly has a completed-stream manifest")
        ...
        for stream_id in stream_ids:
            ...
            if result.returncode != 0:
                ...
                raise RuntimeError(f"Stage-A V3 stream failed: {stream_id}")
        ...
        # Only after all 20 streams have completed do we open/validate stream summaries.
        require(len(completed_rows) == 20, "Stage-A V3 did not complete all streams")
        total_rollouts = 0
        ...
        require(total_rollouts == 1280, f"Stage-A V3 rollout count drift: {total_rollouts}")
        ...
        final = {
            "schema_version": "1.0",
            "artifact_type": "e2-r17-semantic-transfer-v3-stage-a-pool-freeze-summary",
            "created_at_utc": datetime.now(timezone.utc).isoformat(timespec="seconds"),
            "status": "COMPLETED_ALL_160_POOLS_PENDING_EQUAL_DOSE_ADJUDICATION",
            "contract_sha256": contract_sha,
            "authorization_sha256": auth_sha,
        ...
        terminal_lease = {
            **lease_payload,
            "status": "COMPLETED_STAGE_A_V3_PENDING_EQUAL_DOSE_ADJUDICATION",
