...
SRC=ROOT/'paper_drafts/e2-temporal-skill-r14-20260824'; DL=ROOT/'downloads'; GEN=ROOT/'generated'
PDF=SRC/'main.pdf'; SOURCE=DL/'E2-Temporal-Skill-r14-20260824-source.zip'; SUPP=DL/'E2-Temporal-Skill-r14-20260824-supplement.zip'; OUTPDF=DL/'E2-Temporal-Skill-r14-20260824.pdf'
NAMED=DL/'D2-PAPER-TEMPORAL-SKILL-CAUSAL-BOTTLENECK-r14-20260824'
...
def main():
 if not PDF.exists():raise SystemExit('missing final R14 PDF')
 # source archive
 ...
 # supplement: inherit complete R13 supplement, append R14 audit/review records
 old=DL/'E2-Temporal-Skill-r13-20260824-supplement.zip';entries={}
 with zipfile.ZipFile(old) as z:
  for n in z.namelist(): entries[n]=z.read(n)
 additions=[
  GEN/'temporal-skill-r14-identification-audit-20260824.json',GEN/'temporal-skill-r14-contrast-decomposition-20260824.csv',GEN/'temporal-skill-r14-review-closure-20260824.json',GEN/'temporal-skill-r14-claim-audit-20260824.json',
  ROOT/'scripts/audit_temporal_skill_r14_identification.py',ROOT/'scripts/run_temporal_skill_r14_internal_review.py']
 for d in [GEN/'temporal-skill-r14-internal-review-v1-20260824',GEN/'temporal-skill-r14-internal-review-v2-20260824',GEN/'temporal-skill-r14-internal-review-v3-20260824']:
  additions.extend(sorted(d.glob('*.json')))
 for p in additions:
  if not p.exists():raise RuntimeError(f'missing supplement addition {p}')
  if p.is_relative_to(GEN): arc='supplement/r14/'+p.relative_to(GEN).as_posix()
  else: arc='supplement/r14/code/'+p.name
 ...
 with tempfile.TemporaryDirectory(prefix='e2r14-') as td:
 ...
 with tempfile.TemporaryDirectory(prefix='e2r14-raster-') as rd:
 ...
 verification={'schema_version':'1.0','receipt_type':'temporal-r14-package-verification','paper_id':'D2-PAPER-TEMPORAL-SKILL-CAUSAL-BOTTLENECK','revision':'r14','checks':{'pages':pages,'conclusion_page':conc,'references_start_page':refs,'source_zip_independent_rebuild':True,'source_zip_rebuild_text_identical':text_equal,'compile_warning_count':len(warnings),'nonembedded_font_rows':len(nonembedded),'raster_pages_checked':len(pngs),'raster_border_clipping_flags':len(clipped),'supplement_zip_test':bad is None,'supplement_entries':len(names),'supplement_raw_json_receipts':raw,'supplement_results_csv_files':csv,'supplement_checkpoint_files':checkpoints,'manuscript_internal_term_hits':internal_terms,'pdf_private_path_hits':private_hits},'pass':bool(pages==14 and conc==9 and refs==10 and text_equal and not warnings and not nonembedded and len(pngs)==14 and not clipped and bad is None and raw>=700 and csv>=5 and checkpoints>=5 and not internal_terms and not private_hits),'scientific_authority':False,'submission_authority':False}
 vp=GEN/'temporal-skill-r14-verification-20260824.json';vp.write_text(json.dumps(verification,ensure_ascii=False,indent=2)+'\n')
 artifacts={'pdf':OUTPDF,'source_zip':SOURCE,'supplement_zip':SUPP,'identification_audit':GEN/'temporal-skill-r14-identification-audit-20260824.json','review_closure':GEN/'temporal-skill-r14-review-closure-20260824.json','claim_audit':GEN/'temporal-skill-r14-claim-audit-20260824.json','verification':vp}
 manifest={'schema_version':'1.0','paper_id':'D2-PAPER-TEMPORAL-SKILL-CAUSAL-BOTTLENECK','revision':'r14','title':'Do Temporal Skills Really Repair Agents? An Intervention Audit of Repair and Attribution','artifacts':{k:{'path':str(p.relative_to(ROOT)),'sha256':sha(p)} for k,p in artifacts.items()},'new_scientific_execution':False,'new_experiment_after_r14_review_stop':False,'stable_aliases_modified':False,'verification_pass':verification['pass'],'scientific_authority':False,'submission_authority':False}
 mp=GEN/'temporal-skill-r14-package-manifest-20260824.json';mp.write_text(json.dumps(manifest,ensure_ascii=False,indent=2)+'\n')
 ...
 if not verification['pass']:raise SystemExit('R14 verification failed')
