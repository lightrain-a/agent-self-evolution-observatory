...
ARMS = ("win_c", "mrw")
UPDATE_ORDER_SALT = "E2-R17-E1B-MRW-UPDATE-ORDER-v1"
EVAL_ORDER_SALT = "E2-R17-E1B-MRW-EVAL-PAIR-ORDER-v1"
...
def validate_contract_auth(contract_path: Path, auth_path: Path) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    require(contract.get("status") == "FROZEN_E1_B_MRW_CONTEMPORANEOUS_FULL", "MRW causal contract not frozen")
    require(auth.get("status") == "AUTHORIZED_E1", "MRW causal authorization invalid")
    ...
    require(authority.get("e1_b") is True and authority.get("e1_b_mrw") is True, "MRW authority bit absent")
    require(authority.get("mrw_causal_comparison") is True, "MRW causal comparison authority absent")
    ...
    require(gate_payload.get("status") == "PASS_NEGATIVE_CONTROL_EQUIVALENCE_READY_FOR_MRW_CONTRACT", "MRW remains unauthorized because negative-control equivalence did not PASS")
...
async def ensure_update(*, contract: dict[str, Any], contract_sha: str, auth_sha: str, stream_id: str, arm: str, pools: list[Any], evidence_units_for_arm: list[Any], projection: ProjectionName, initial_skill: str, initial_sha: str, mind_head: str, requested: str, resolved: str, settings: ArkSettings, state_root: Path, ledger: ProviderBudgetLedger) -> dict[str, Any]:
    ...
    stream = project_stream(stream_id=stream_id, initial_skill_sha256=initial_sha, pools=pools, projection=projection)
    ...
    require(result.provider_calls == 10 and len(receipts) == 10, "each MRW-tranche update must use exact nominal 10 calls")
    require(not any(r.get("parse_error") for r in receipts), "MRW-tranche update parse error")
...
async def main_async(args: argparse.Namespace) -> dict[str, Any]:
    ...
    try:
        ...
        for stream_id in contract["streams"]:
            ...
            evidence_payload={"stream_id":stream_id,"win_c_evidence_bundle_sha256":win_bundle_sha,"mrw_evidence_bundle_sha256":mrw_bundle_sha,"receipts":evidence_receipts,"mrw_provider_execution_authorized":True,"primary_control":"fresh_contemporaneous_win_c"}
            ...
            for arm in ordered_arms(stream_id,UPDATE_ORDER_SALT):
                ...
                updates[arm]=await ensure_update(contract=contract,contract_sha=contract_sha,auth_sha=auth_sha,stream_id=stream_id,arm=arm,pools=pools,evidence_units_for_arm=units_for_arm,projection=projection_for_arm,initial_skill=initial_skill,initial_sha=initial_sha,mind_head=mind_head,requested=requested,resolved=resolved,settings=settings,state_root=state_root,ledger=ledger)
            ...
            stream_summary={"schema_version":"1.0","artifact_type":"e2-r17-e1-b-mrw-contemporaneous-stream","created_at_utc":datetime.now(timezone.utc).isoformat(timespec="seconds"),"status":"COMPLETED","stream_id":stream_id,"pool_ids":[p.pool_id for p in pools],"evidence_windows_sha256":sha_file(evidence_path),"update_order":ordered_arms(stream_id,UPDATE_ORDER_SALT),"heldout_task_ids":contract["heldout"]["task_ids"],"states":states,"mrw_executed":True,"primary_control":"win_c","paper_promotion_authority":False}
        ...
        require(set(completed_streams)==set(contract["streams"]), "MRW contemporaneous tranche did not complete all streams")
        final={"schema_version":"1.0","artifact_type":"e2-r17-e1-b-mrw-contemporaneous-full-summary","created_at_utc":datetime.now(timezone.utc).isoformat(timespec="seconds"),"status":"COMPLETED_PENDING_SEPARATE_MRW_ADJUDICATION","contract_sha256":contract_sha,"authorization_sha256":auth_sha,"streams":len(contract["streams"]),"arms":list(ARMS),"learned_states":len(contract["streams"])*2,"heldout_tasks_per_state":len(contract["heldout"]["task_ids"]),"heldout_rollout_units":len(contract["streams"])*2*len(contract["heldout"]["task_ids"]),"mrw_executed":True,"primary_control":"win_c","mrw_inference_performed":False,"paper_promotion_authority":False,"completed_stream_manifest":str(stream_manifest),"completed_stream_manifest_sha256":sha_file(stream_manifest)}
        atomic_json(run_root/"summary/e1_b_mrw_contemporaneous_full_summary.json",final); success=True; return final
    finally:
        os.close(lock_fd)
...
def main() -> int:
    ...
    payload=asyncio.run(main_async(args)); print(json.dumps(payload,ensure_ascii=False,indent=2,sort_keys=True)); return 0 if payload["status"]=="COMPLETED_PENDING_SEPARATE_MRW_ADJUDICATION" else 2
