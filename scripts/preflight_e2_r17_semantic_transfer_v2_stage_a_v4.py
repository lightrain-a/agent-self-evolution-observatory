...
CONTRACT_STATUS = "FROZEN_SEMANTIC_TRANSFER_V2_STAGE_A_V4"
...
def main() -> int:
    ...
    req(not args.output.exists(), "V4 preflight output already exists")
    contract = load(args.contract)
    contract_sha = sha(args.contract)
    req(contract.get("status") == CONTRACT_STATUS, "V4 contract status invalid")
    req(contract.get("authority", {}).get("stage_a_provider_execution") is False, "draft V4 contract cannot self-authorize")
    ...
    req(len(all_tasks) == 144 and len(set(all_tasks)) == 144, "V4 update task shape invalid")
    req(len(heldout) == 18 and len(set(heldout)) == 18 and set(all_tasks).isdisjoint(heldout), "V4 heldout separation invalid")
    ...
    req(not run_root.exists(), "V4 run root already exists")
    req(not lease_path.exists(), "V4 global lease already exists")
    ...
    # Import the actual V4 control-plane modules under the frozen runtime/Python
    ...
    for path in (runner_path, adjudicator_path, authorizer_path):
        ...
        req(check.returncode == 0, f"V4 bound code compile failed: {path.name}")

    payload = {
        "schema_version": "1.0",
        "artifact_type": "e2-r17-semantic-transfer-v2-stage-a-v4-zero-provider-preflight",
        "created_at_utc": datetime.now(timezone.utc).isoformat(timespec="seconds"),
        "status": "PASS_ZERO_PROVIDER_SEMANTIC_TRANSFER_V2_STAGE_A_V4_PREFLIGHT",
        ...
        "next_gate": "LOCKED_DUAL_PREEXECUTION_REVIEW_AND_FRESH_IDENTITY_BEFORE_SINGLE_USE_STAGE_A_V4_AUTHORIZATION"
    }
