...
EXPECTED_PREFLIGHT_STATUS = "PASS_ZERO_PROVIDER_SEMANTIC_TRANSFER_V2_STAGE_A_V5_PREFLIGHT"
...
def main() -> int:
    ...
    req(not args.output.exists(), "Stage-A V5 authorization already exists")
    ...
    req(contract.get("status") == EXPECTED_CONTRACT_STATUS, "Stage-A V5 contract not frozen")
    req(preflight.get("status") == EXPECTED_PREFLIGHT_STATUS, "Stage-A V5 preflight not passing")
    ...
    req(identity_created > contract_created, "fresh identity must be qualified after Stage-A V5 contract freeze")
    ...
    req(not lease_path.exists(), "global Stage-A V5 lineage lease already exists")
    req(not Path(contract["run_root"]).exists(), "Stage-A V5 run root already exists")
    ...
    payload = {
        "schema_version": "1.0",
        "artifact_type": "e2-r17-semantic-transfer-v2-stage-a-v5-authorization",
        "created_at_utc": datetime.now(timezone.utc).isoformat(timespec="seconds"),
        "status": "AUTHORIZED_SEMANTIC_TRANSFER_V2_STAGE_A_V5",
