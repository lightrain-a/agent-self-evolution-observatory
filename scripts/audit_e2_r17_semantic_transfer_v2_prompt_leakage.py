...
def main() -> int:
    ...
    env = SpreadsheetBenchEnv(args.suite_root, Path("/tmp/e2-r17-semantic-transfer-v2-prompt-audit"))
    ...
    req(len(rows) == 216, f"unexpected task count: {len(rows)}")
    ...
    payload = {
        "schema_version": "1.0",
        "artifact_type": "e2-r17-semantic-transfer-v2-model-visible-prompt-leakage-audit",
        "created_at_utc": datetime.now(timezone.utc).isoformat(timespec="seconds"),
        "status": "PASS_SEMANTIC_TRANSFER_V2_MODEL_VISIBLE_PROMPT_LEAKAGE_AUDIT",
