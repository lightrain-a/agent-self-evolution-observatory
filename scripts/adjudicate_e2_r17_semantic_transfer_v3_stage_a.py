...
def choose_four(stream_id: str, mixed_task_ids: list[str]) -> list[str]:
    ...
    return sorted(
        ...
        key=lambda task_id: hashlib.sha256(
            f"semantic-transfer-mrw4-v3|{stream_id}|{task_id}".encode("utf-8")
        ).hexdigest(),
    )[:4]


def select_failed_witness(trajectories: list[dict[str, Any]], acting_winner_index: int) -> dict[str, Any]:
    failed_nonwinners = [
        row for row in trajectories
        if float(row["score"]) == 0.0 and int(row["rollout_index"]) != int(acting_winner_index)
    ]
    require(failed_nonwinners, "mixed pool lacks failed nonwinner")
    witness = min(failed_nonwinners, key=lambda row: int(row["rollout_index"]))
    return {
        "rollout_index": int(witness["rollout_index"]),
        "trajectory_path": str(witness["trajectory_path"]),
        "trajectory_sha256": str(witness["trajectory_sha256"]),
        "score": float(witness["score"]),
        "selector": "lowest original rollout index among verifier-failure nonwinner trajectories",
    }


def choose_ten_streams(stream_ids: list[str], scores: dict[str, int], *, descending: bool, salt: str) -> list[str]:
    require(len(stream_ids) == 20 and set(stream_ids) == set(scores), "reduction-router stream universe drift")
    def key(stream_id: str) -> tuple[int, str]:
        ...
        return primary, tie
    return sorted(stream_ids, key=key)[:10]
...
def main() -> int:
    ...
    require(contract.get("status") == "FROZEN_SEMANTIC_TRANSFER_V3_STAGE_A", "Stage-A V3 contract status invalid")
    require(auth.get("status") == "AUTHORIZED_SEMANTIC_TRANSFER_V3_STAGE_A", "Stage-A V3 authorization status invalid")
    require(auth.get("contract_sha256") == contract_sha, "authorization contract binding drift")
    require(summary.get("status") == "COMPLETED_ALL_160_POOLS_PENDING_EQUAL_DOSE_ADJUDICATION", "Stage-A pool freeze incomplete")
    ...
    require(int(summary.get("streams") or 0) == 20, "Stage-A stream cardinality drift")
    require(int(summary.get("tasks") or 0) == 160, "Stage-A task cardinality drift")
    require(int(summary.get("actor_rollouts") or 0) == 1280, "Stage-A rollout cardinality drift")
    ...
    require(len(all_tasks) == 160 and len(set(all_tasks)) == 160, "Stage-A task set drift")
    forbidden_heldout = set(map(str, split["e1_common_heldout_probe"]))
    require(len(forbidden_heldout) == 20 and forbidden_heldout.isdisjoint(all_tasks), "heldout/update overlap")
    ...
    failed_witness_by_task: dict[str, dict[str, Any]] = {}
    ...
    for stream_id, task_ids in streams.items():
        ...
        for task_id in task_ids:
            ...
            for row in trajectories:
                ...
                scores.append(score)
            require(seen_indices == set(range(8)), f"rollout indices must be exactly 0..7: {task_id}")
            winner = min(trajectories, key=lambda row: (-float(row["score"]), int(row["rollout_index"])))
            winner_index = int(winner["rollout_index"])
            require(int(pool.get("acting_winner_index")) == winner_index, f"acting winner selector drift: {task_id}")
            is_mixed = min(scores) < 1.0 and max(scores) >= 1.0
            if is_mixed:
                mixed_tasks.append(task_id)
                failed_witness_by_task[task_id] = select_failed_witness(trajectories, winner_index)
                ...
                treated_rows.append(
                    {
                        ...
                        "failed_witness": failed_witness_by_task[task_id],
                        "selection_key_sha256": hashlib.sha256(
                            f"semantic-transfer-mrw4-v3|{stream_id}|{task_id}".encode("utf-8")
                        ).hexdigest(),
                    }
                )
        require(len(treated_rows) == 80, "equal-dose treated-pool total must be exactly 80")
    else:
    ...
    if support_pass:
        ...
        difficulty_mrw = choose_ten_streams(
            stream_ids,
            success_rollouts_by_stream,
            descending=False,
            salt="semantic-transfer-difficulty-v3",
        )
        mixedness_mrw = choose_ten_streams(
            stream_ids,
            mixed_by_stream,
            descending=True,
            salt="semantic-transfer-mixedness-v3",
        )
        require(len(difficulty_mrw) == len(set(difficulty_mrw)) == 10, "difficulty router cardinality drift")
        require(len(mixedness_mrw) == len(set(mixedness_mrw)) == 10, "mixedness router cardinality drift")
        reduction_routers = {
            "difficulty_only": {
                "score": "Stage-A successful rollouts / 64; lower success means harder",
                "ordering": "ascending success count; SHA256(semantic-transfer-difficulty-v3|stream_id) tie-break",
            ...
            "mixedness_only": {
                ...
                "ordering": "descending mixed-pool count; SHA256(semantic-transfer-mixedness-v3|stream_id) tie-break",
    ...
    status = (
        "PASS_SEMANTIC_TRANSFER_V3_EQUAL_DOSE_SUPPORT_READY_FOR_STAGE_B_DESIGN"
        if support_pass
        else "HOLD_SEMANTIC_TRANSFER_V3_INSUFFICIENT_EQUAL_DOSE_SUPPORT"
    )
    payload = {
        "schema_version": "1.0",
        "artifact_type": "e2-r17-semantic-transfer-v3-stage-a-equal-dose-adjudication",
        ...
        "equal_dose_treatment_manifest": {
            ...
            "failed_witness_selector": "lowest original rollout index among verifier-failure nonwinner trajectories",
            "failed_witness_frozen_for_every_treated_pool": support_pass and len(treated_rows) == 80,
            "rows": treated_rows,
            "scientific_inclusion": support_pass,
        },
        "stage_a_reduction_routers": reduction_routers,
        "interpretation": (
            "This adjudication reads only complete Stage-A search-pool success/failure support after all 160 pools are sealed. "
            "It does not read or infer any WIN-C/MRW4 learned-skill effect. Passing support freezes an equal-dose 80-pool treatment manifest and two same-information reduction routers before any Stage-B outcome for a separately contracted Stage B."
        ),
        "authority": {
