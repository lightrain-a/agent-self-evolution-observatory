...
def choose_four(stream_id: str, mixed_task_ids: list[str]) -> list[str]:
    ...
    return sorted(
        ...
        key=lambda task_id: hashlib.sha256(
            f"semantic-transfer-mrw4-v1|{stream_id}|{task_id}".encode("utf-8")
        ).hexdigest(),
    )[:4]
...
def main() -> int:
    ...
    require(contract.get("status") == "FROZEN_SEMANTIC_TRANSFER_V1_STAGE_A_R2", "Stage-A R2 contract status invalid")
    require(auth.get("status") == "AUTHORIZED_SEMANTIC_TRANSFER_V1_STAGE_A_R2", "Stage-A authorization status invalid")
    require(auth.get("contract_sha256") == contract_sha, "authorization contract binding drift")
    require(summary.get("status") == "COMPLETED_ALL_96_POOLS_PENDING_EQUAL_DOSE_ADJUDICATION", "Stage-A pool freeze incomplete")
    ...
    require(int(summary.get("streams") or 0) == 12, "Stage-A stream cardinality drift")
    require(int(summary.get("tasks") or 0) == 96, "Stage-A task cardinality drift")
    require(int(summary.get("actor_rollouts") or 0) == 768, "Stage-A rollout cardinality drift")
    ...
    require(len(all_tasks) == 96 and len(set(all_tasks)) == 96, "Stage-A task set drift")
    ...
    if support_pass:
        ...
        require(len(treated_rows) == 48, "equal-dose treated-pool total must be exactly 48")
    else:
    ...
    payload = {
        "schema_version": "1.0",
        "artifact_type": "e2-r17-semantic-transfer-v1-stage-a-equal-dose-adjudication",
        ...
        "equal_dose_treatment_manifest": {
            "selection_rule": "lowest SHA256(semantic-transfer-mrw4-v1|stream_id|task_id) among mixed pools",
        ...
        "interpretation": (
            "This adjudication reads only complete Stage-A search-pool success/failure support after all 96 pools are sealed. "
            "It does not read or infer any WIN-C/MRW4 learned-skill effect. Passing support only freezes an equal-dose 48-pool treatment manifest for a separately contracted Stage B."
        ),
        "authority": {
