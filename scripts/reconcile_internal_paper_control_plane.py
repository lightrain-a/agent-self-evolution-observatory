...
def main() -> None:
    ...
    ledger_index = build_paper_ledger_index(data_root)
    ledger_summary = ledger_index.get("summary") or {}
    if int(ledger_summary.get("invalid_ledgers") or 0) != 0:
        raise RuntimeError("refusing to reconcile an invalid paper ledger index")
    ...
    search_summary = search_design.get("summary") or {}
    fresh_summary = fresh_phenomenon.get("summary") or {}
    top_summary.update({
        ...
        "paper_first_search_portfolio_design_reviewed": int(search_summary.get("reviewed") or 0),
        "paper_first_search_portfolio_design_advance_method": int(search_summary.get("advance_to_method_design") or 0),
        "paper_first_search_portfolio_design_revise": int(search_summary.get("revise_paper_problem") or 0),
        "paper_first_search_portfolio_design_stop": int(search_summary.get("stop_standalone") or 0),
        "paper_first_fresh_phenomenon_status": str(fresh_phenomenon.get("status") or "NO_ACTIVE_F0"),
        "paper_first_fresh_phenomenon_candidates": int(fresh_summary.get("candidates") or 0),
        "paper_first_fresh_phenomenon_active_f0": int(fresh_summary.get("active_f0") or 0),
        "paper_first_fresh_phenomenon_design_ready_f0": int(fresh_summary.get("design_ready_f0") or 0),
        "paper_first_fresh_phenomenon_support_holds": int(fresh_summary.get("hold_support") or 0),
        "paper_first_fresh_phenomenon_execution_holds": int(fresh_summary.get("hold_execution") or 0),
        "paper_first_fresh_phenomenon_ready_problem_review": int(fresh_summary.get("ready_for_problem_review") or 0),
        "paper_first_fresh_phenomenon_archived": int(fresh_summary.get("archived") or 0),
    })
