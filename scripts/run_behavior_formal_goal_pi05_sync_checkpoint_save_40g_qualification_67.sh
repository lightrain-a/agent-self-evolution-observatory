...
AUTH="$ROOT/generated/behavior-formal-goal-coupling-shared26-pi05-sync-checkpoint-save-40g-qualification-authority-20260905.json"
ADJ="$ROOT/generated/behavior-formal-goal-coupling-shared26-pi05-async-checkpoint-resource-adjudication-20260905.json"
OUT=/data/wyt/formal-goal-checkpoint-save-qualification-67-20260905/sync-save-40g
RECEIPT="$ROOT/generated/behavior-formal-goal-coupling-shared26-pi05-sync-checkpoint-save-40g-qualification-result-67-20260905.json"
LOG=/data/wyt/formal-goal-sync-checkpoint-save-40g-qualification-67-20260905.log
exec >>"$LOG" 2>&1
[[ -f "$AUTH" && -f "$ADJ" && -f "$BASE_RECEIPT" ]] || { echo "sync checkpoint authority/input missing"; exit 3; }
[[ ! -e "$OUT" && ! -e "$RECEIPT" ]] || { echo "Refusing sync checkpoint qualification replay: output/receipt already exists"; exit 2; }
exec /usr/bin/taskset -c 0-63 /usr/bin/env \
  ...
  "$PY" "$ROOT/research_pipeline/behavior_formal_goal_coupling_shared26_pi05_sync_checkpoint_save_qualification.py" \
    ...
    --output-root "$OUT" --receipt "$RECEIPT" --expected-memory-max-gib 40
