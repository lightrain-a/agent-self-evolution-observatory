...
def validate_contract_auth(contract_path: Path, auth_path: Path) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    require(contract.get("status") == "FROZEN_E2_R17_DEEPSEEK_V2_REPAIR2_CONTINUATION_V3", "DeepSeek V2 Repair2 V3 contract not frozen")
    require(auth.get("status") == "AUTHORIZED_E2_R17_DEEPSEEK_V2_REPAIR2_V3", "Repair2 V3 authorization invalid")
    ...
    require(authority.get("repair2_continuation_v3") is True, "Repair2 V3 continuation authority absent")
    ...
    require(scope.get("continuation_version") == "repair2_v3", "V3 continuation version drift")
...
async def main_async(args: argparse.Namespace) -> dict[str, Any]:
    ...
    inherited_rows = validate_v3_compatibility_manifest(
    ...
    require(len(inherited_rows) == 15, "V3 must freeze exactly 15 completed prefix pairs")
    ...
    inherited_sources = {"repair1_inherited", "repair2_m1_recovered"}
    require({row["unit_id"] for row in inherited_rows} == {unit_id for unit_id,row in valid_units.items() if row.get("source") in inherited_sources}, "runtime inherited set differs from frozen V3 compatibility manifest")
    validate_valid_rows_v3(list(valid_units.values()), streams=contract["streams"], quarantine=quarantine, require_complete=False)
    ...
    try:
        ...
        for stream_id in contract["streams"]:
            ...
            for replicate in REPLICATES:
                ...
                manifest_row={"unit_id":unit_id,"stream_id":stream_id,"replicate":replicate,"summary_path":str(rep_summary_path),"summary_sha256":sha_file(rep_summary_path),"source":"repair2_v3_fresh","completed_at_utc":datetime.now(timezone.utc).isoformat(timespec="seconds")}
                valid_row={"unit_id":unit_id,"stream_id":stream_id,"replicate_id":replicate,"source":"repair2_v3_fresh","pair_summary_path":str(rep_summary_path),"pair_summary_sha256":sha_file(rep_summary_path),"arms":{}}
                for arm in ARMS:
                    state_root=rep_root/arm
                    update=updates[arm]
        ...
        require(set(completed_units)==expected, "DeepSeek V2 Repair2 V3 did not complete all 48 paired replicate units")
        validate_valid_rows_v3(list(valid_units.values()), streams=contract["streams"], quarantine=quarantine, require_complete=True)
