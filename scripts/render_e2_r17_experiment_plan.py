...
def render(plan: dict[str, Any]) -> str:
    model_rows = plan["model_plan"]["core_candidates_before_pilot"]
    budget = plan["budget"]
    stages = plan["stages"]
    lines = [
        "# E2-R17 Experiment Plan V1",
        "",
        f"Date: {plan['created_at_utc']}",
        f"Status: **{plan['status']}**",
        "",
        "## 1. Scientific question and evidence chain",
        "",
        plan["scientific_question"],
        "",
        "The paper separates two projections over the same generated search object:",
        "",
        f"- **Acting projection:** {plan['story']['acting_projection']}",
        f"- **Learning projection:** {plan['story']['learning_projection']}",
        "",
        f"Central claim: {plan['story']['central_claim']}",
        "",
        f"Minimal intervention: **{plan['story']['minimal_method']}**",
        "",
        ...
        "## 2. Current E0 status",
        "",
        f"The frozen E0 summary is `{plan['current_evidence']['e0_summary_sha256']}`. Success@1 is 11/12 and Success@4/8 is 12/12. One rescue task in one family makes censoring nonzero, and the exact observed-pool identity holds, but the frozen threshold is at least six rescue tasks across at least three families.",
        "",
        f"Decision: **{plan['current_evidence']['decision']}**. E1 remains unauthorized.",
        "",
        "Two independently generated analyses are retained rather than overwritten:",
        "",
        f"- `{plan['current_evidence']['canonical_parallel_analysis']}`",
        f"- `{plan['current_evidence']['independent_analysis']}`",
        "",
    ...
    for row in model_rows:
        lines.append(f"| {row['role']} | {row['model']} | {row['status']} |")
    pilot = plan["model_plan"]["model_pilot"]
    checks = pilot["promotion_checks"]
    lines += [
        "",
        f"Each candidate receives K=1 on {pilot['development_tasks']} fixed development tasks and K=4 on four predeclared search-smoke tasks. Promotion requires 100% task loading and identity stability, >=95% tool parsing and verifier completion, >=90% artifact writing, <=5% technical failures, a verified missing-unit-only resume, and a K=1 result that is neither all-zero nor all-one. No R17 effect size is consulted.",
        "",
        f"The V1 core updater is `{plan['model_plan']['core_updater_v1']}`. This is a proposal for review, not a frozen full-contract choice.",
        "",
        ...
        "",
        f"Run only the {stages['S0_E0_full_support']['extension_tasks']} missing predeclared tasks; never rerun the completed 12. All tasks use K=8 once and derive K=1/2/4/8 from nested prefixes. GO requires >=6 rescue tasks across >=3 families with all integrity checks passing. Insufficient support stops this substrate before any updater intervention.",
        "",
    ...
    e1 = budget["E1_estimate"]
    e2 = budget["E2_verified_core_primary_partition_formula"]["example_M4_C4"]
    lines += [
        "",
        "## 8. Calls, tokens, and compute budget",
        "",
        f"Empirical E0 rate: {budget['empirical_rate_source']}, or {budget['empirical_actor_rate']['provider_calls_per_rollout']:.3f} provider calls and {budget['empirical_actor_rate']['tokens_per_rollout']:.0f} tokens per actor rollout.",
        "",
        "| Stage | Actor rollouts | Provider/update calls | Actor tokens | Compute placement |",
        "|---|---:|---:|---:|---|",
        f"| S0 missing 42 tasks | {budget['S0_extension_estimate']['actor_rollouts']} | ~{budget['S0_extension_estimate']['provider_calls']} actor calls | ~{budget['S0_extension_estimate']['actor_tokens']} | Ark + CPU workbook execution on 69 |",
        f"| E1 pools + frozen evaluation | {e1['actor_rollouts_total']} | ~{e1['actor_provider_calls_estimate']} actor + {e1['updater_calls']} updater calls | ~{e1['actor_tokens_estimate']} actor tokens | Ark + CPU workbook execution on 69 |",
        f"| Verified core, 4 models x 4 methods, primary partition | {e2['pool_rollouts'] + e2['validation_and_test_rollouts']} | {e2['updater_calls']} updater calls plus actor-call expansion measured in Pilot | fixed after per-model Pilot | API models on 69; Qwen 4B on 60/52; Qwen 35B preferably 232 |",
        f"| SkillEvolBench example, 4 models x 4 methods, K=8 | {budget['E2_skill_evolbench_formula']['example_M4_C4_K8']['acquisition_rollouts'] + budget['E2_skill_evolbench_formula']['example_M4_C4_K8']['evaluation_rollouts']} | native updater budget frozen after adapter Pilot | fixed after Pilot | official harness-qualified placement |",
        "",
...
def main() -> int:
    ...
    atomic_write(args.output, render(load(args.input)))
    return 0
