...
def build() -> Path:
    ...
    paper_signoff_root = os.environ.get("PAPER_HUMAN_SIGNOFF_ROOT", "").strip()
    ...
    research_p0_auth_root = os.environ.get("RESEARCH_P0_AUTH_ROOT", "").strip()
    research_p0_plan_root = os.environ.get("RESEARCH_P0_PLAN_ROOT", "").strip()
    research_p0_pre_experiment_root = os.environ.get("RESEARCH_P0_PRE_EXPERIMENT_ROOT", "").strip()
    research_p0_lease_request_root = os.environ.get("RESEARCH_P0_LEASE_REQUEST_ROOT", "").strip()
    ...
    if paper_ledger_root:
        ...
        signoff_path = Path(paper_signoff_root).expanduser().resolve() if paper_signoff_root else audit_root / "paper-human-signoffs"
        if signoff_path.is_dir():
            command.extend(["--signoff-root", str(signoff_path)])
        attempt_path = Path(paper_attempt_root).expanduser().resolve() if paper_attempt_root else audit_root / "paper-submission-attempts"
        if attempt_path.is_dir():
            command.extend(["--attempt-root", str(attempt_path)])
        attempt_workflow_path = Path(paper_attempt_workflow_root).expanduser().resolve() if paper_attempt_workflow_root else audit_root / "paper-submission-attempt-workflows"
        if attempt_workflow_path.is_dir():
            command.extend(["--attempt-workflow-root", str(attempt_workflow_path)])
        ...
        run_start_path = Path(research_run_start_root).expanduser().resolve() if research_run_start_root else audit_root / "scientific-contract-run-starts"
        if run_start_path.is_dir():
        ...
        p0_auth_path = Path(research_p0_auth_root).expanduser().resolve() if research_p0_auth_root else audit_root / "scientific-contract-p0-authority"
        if p0_auth_path.is_dir():
            command.extend(["--p0-auth-root", str(p0_auth_path)])
        p0_plan_path = Path(research_p0_plan_root).expanduser().resolve() if research_p0_plan_root else audit_root / "scientific-contract-p0-plans"
        if p0_plan_path.is_dir():
            command.extend(["--p0-plan-root", str(p0_plan_path)])
        p0_pre_experiment_path = Path(research_p0_pre_experiment_root).expanduser().resolve() if research_p0_pre_experiment_root else audit_root / "scientific-contract-p0-pre-experiment"
        if p0_pre_experiment_path.is_dir():
            command.extend(["--p0-pre-experiment-root", str(p0_pre_experiment_path)])
        p0_lease_request_path = Path(research_p0_lease_request_root).expanduser().resolve() if research_p0_lease_request_root else audit_root / "scientific-contract-p0-lease-requests"
        if p0_lease_request_path.is_dir():
            command.extend(["--p0-lease-request-root", str(p0_lease_request_path)])
        ...
        scientific_problem_gate_path = Path(research_scientific_problem_gate_root).expanduser().resolve() if research_scientific_problem_gate_root else audit_root / "scientific-contract-problem-gates"
        if scientific_problem_gate_path.is_dir():
            command.extend(["--scientific-problem-gate-root", str(scientific_problem_gate_path)])
