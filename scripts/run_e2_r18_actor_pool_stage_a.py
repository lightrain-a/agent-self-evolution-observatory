...
def validate_authority(
    *,
    ...
    if status not in {"AUTHORIZED_E2_R18_DIAGNOSTIC_VALUE_STAGE_A_PREFLIGHT", "AUTHORIZED_E2_R18_DIAGNOSTIC_VALUE_STAGE_A"}:
        raise RuntimeError("authorization artifact does not authorize R18 Stage A path")
    authority = payload.get("authority") or {}
    if status.endswith("_PREFLIGHT"):
        if authority.get("r18_stage_a_preflight") is not True or authority.get("provider_io") is not False or authority.get("scientific_experiment") is not False:
            raise RuntimeError("R18 Stage A preflight authority malformed")
    else:
        if authority.get("r18_stage_a_pool_support") is not True or authority.get("provider_io") is not True or authority.get("scientific_experiment") is not True:
            raise RuntimeError("R18 Stage A execution authority malformed")
    for forbidden in ("updater", "heldout_evaluation", "analyzer", "paper_promotion", "second_backbone", "public_benchmark"):
        if authority.get(forbidden) is not False:
            raise RuntimeError(f"R18 Stage A authorization overbroad: {forbidden}")
...
async def main_async(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if authorization_payload is not None:
        auth_status = str(authorization_payload.get("status") or "")
        if args.preflight_only and auth_status != "AUTHORIZED_E2_R18_DIAGNOSTIC_VALUE_STAGE_A_PREFLIGHT":
            raise RuntimeError("R18 actor preflight requires the dedicated preflight authorization")
        if not args.preflight_only and auth_status != "AUTHORIZED_E2_R18_DIAGNOSTIC_VALUE_STAGE_A":
            raise RuntimeError("R18 scientific provider execution requires the dedicated Stage A execution authorization")
    contract_sha = (
    ...
    if args.preflight_only:
        return {
            "schema_version": "1.0",
            "artifact_type": "e2-r18-stage-a-actor-actual-path-preflight",
            "created_at_utc": datetime.now(timezone.utc).isoformat(timespec="seconds"),
            "status": "PASS_BEFORE_PROVIDER_IO",
            "mode": args.mode,
            "task_ids": task_ids,
            "k": args.k,
            ...
            "contract_sha256": contract_sha,
            "authorization_sha256": authorization_sha,
            "provider_claims": provider_budget_ledger.snapshot().total_claimed if provider_budget_ledger is not None else 0,
            "provider_calls": 0,
            "stopped_before_provider_io": True,
        }
    ...
    return {
        "schema_version": "1.0",
        "artifact_type": "e2-r18-stage-a-actor-pool-run-summary",
        "created_at_utc": datetime.now(timezone.utc).isoformat(timespec="seconds"),
        "status": "COMPLETED",
        "mode": args.mode,
