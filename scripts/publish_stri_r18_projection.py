...
PKG = Path("/data/wyt/agent-self-evolution-observatory/submission-packages/stri-e1-r18-second-substrate-audit-20260824")
PDF = PKG / "E1-STRI-R18-second-substrate-audit.pdf"
SOURCE = PKG / "E1-STRI-R18-second-substrate-audit-source.zip"
SUPPLEMENT = PKG / "E1-STRI-R18-second-substrate-audit-supplement.zip"
MANIFEST = PKG / "R18-MANIFEST.json"

PDF_SHA = "a841d3a51f85630b43e824b7eac48980ede4becfd9606d449ac1ab8d5c225e98"
SOURCE_SHA = "0c14d03615446918865777c25bf3bb1901d9cec2dc38c65cd5cd8ad00daa137a"
SUPPLEMENT_SHA = "3dc94258ccf333f277c3779fd39267a112eb160461e295f6db5cda97de056753"
SUPPLEMENT_MANIFEST_SHA = "9e5431baf09a22e44b6e4c22a864322462cb8c8cc16feada02c907bc59021105"
PACKAGE_METADATA_SHA = "67b0f88554021470994ebc4b45c4454fbcee0ca6acdda896c901f27b7c544868"
MANIFEST_FILE_SHA = "9bcaf956966a2719e37ca9cc2ac2e3ab753b97fc13f15b5efad06b19104c673a"
MANIFEST_CANONICAL_SHA = "8a086f72857fc09ab198a899eda9771745f5ea419e1ad4e0938748dab8888c0c"
CANONICAL_BASE = "67228aef9ea47a388c5a529c0a46cad5eb45e15a"
INTEGRATION_SOURCE_HEAD = "3d73b9fc612dd39b80559024ad4b881f4bc351b9"
...
PROJECTION_RECEIPT = GEN / "asset-first-stri-r18-canonical-projection-20260824.json"
...
def verify_package() -> dict:
    ...
    for path, digest in expected.items():
        if not path.is_file() or sha(path) != digest:
            raise RuntimeError(f"R18 artifact mismatch: {path}")
    manifest = load(MANIFEST)
    if manifest.get("candidate") != "R18-second-substrate-audit" or manifest.get("revision") != "R18" or manifest.get("manifest_sha256") != MANIFEST_CANONICAL_SHA:
        raise RuntimeError("R18 manifest identity/digest mismatch")
    if manifest.get("canonical_base") != CANONICAL_BASE or manifest.get("integration_source_head") != INTEGRATION_SOURCE_HEAD:
        raise RuntimeError("R18 manifest base/head mismatch")
    if ((manifest.get("artifacts") or {}).get("supplement_zip") or {}).get("unit_tests") != "31/31 PASS":
        raise RuntimeError("R18 supplement unit-test contract missing")
    qa = manifest.get("qa") or {}
    if (int(qa.get("paper_quality_referenced_files") or 0), int(qa.get("paper_quality_registered_digests") or 0), int(qa.get("citation_count") or 0)) != (64, 67, 21):
        raise RuntimeError("R18 paper-quality/citation contract missing")
    audit = manifest.get("second_substrate_audit") or {}
    agent = audit.get("agentskillos") or {}
    if not (
        int(audit.get("candidates_screened") or 0) == 5
        and int(audit.get("new_exact_support_substrates", -1)) == 0
        and int(audit.get("new_external_analogues") or 0) == 1
        and audit.get("exact_support_search_disposition") == "NO_SECOND_EXACT_SUPPORT_SUBSTRATE_QUALIFIED"
        and audit.get("exact_support_fail_closed") is True
        and (int(agent.get("tasks") or 0), int(agent.get("categories") or 0), int(agent.get("unique_oracle_skills") or 0), int(agent.get("multi_skill_tasks") or 0)) == (30, 5, 19, 20)
        and abs(float(agent.get("full_uniform_exposure_ratio") or -99.0) - 4.0) < 1e-12
        and abs(float(agent.get("full_oracle_set_R_star_analogue") or -99.0) - 2.5) < 1e-12
        and agent.get("exact_support_certificate_authorized") is False
    ):
        raise RuntimeError("R18 second-substrate audit contract missing")
    return manifest
...
def compile_and_refresh_qa() -> dict:
    ...
    with tempfile.TemporaryDirectory(prefix="stri-r18-source-binding-") as tmp:
        ...
        for source in (MAIN := PAPER / "stri-20260816-iclr2027-main.tex", BODY := PAPER / "stri-20260816-narrow-body.tex", TABLES := PAPER / "stri-20260816-tables.tex", BIB := PAPER / "stri-20260816-references.bib"):
            ...
            if not packaged.is_file() or packaged.read_bytes() != source.read_bytes():
                raise RuntimeError(f"R18 source binding mismatch: {source.name}")
    qa = current_qa()
...
def update_stri_states() -> None:
    ...
    final["canonical_reconciliation"] = {
        ...
        "paper_quality_registered_digests": 67,
    ...
    supplement["isolated_verification"] = {
        ...
        "figure_regeneration": "R18_BOUNDARY_FIGURE_REGENERATED_AND_SOURCE_BOUND",
        "paper_quality_v2_reproduction": "PASS_MANUSCRIPT_EVIDENCE",
    }
    ...
    mv.update({
        ...
        "r18_canonical_reconciliation": True,
        "claim_expansion_for_r18": False,
        "new_model_calls_for_r18": 0,
        "new_gpu_runs_for_r18": 0,
    })
...
def append_r18_ledger(root: Path) -> dict:
    ...
    existing = [e for e in before.get("events") or [] if isinstance(e, dict) and e.get("event_type") == "source-native-r18-finalization"]
    contract = contract_from_ledger(before)
    if existing:
        after = load_paper_ledger(root, PID)
        errors = validate_paper_ledger(after)
        if errors:
            raise RuntimeError("existing R18 ledger is invalid: " + "; ".join(errors))
        return after
    ...
    def append(kind: str, payload: dict) -> None:
        ...
        _append(root, contract, "stri-r18-canonical-publisher", event)

    append("paper-preparation-r18", {
        "receipt_type": "paper-preparation",
        "protocol_version": "1.0+r18-second-substrate-audit",
        "pass": True,
        "required_gates": 8,
        "passed_gates": 8,
        "gate_pass": gates,
        ...
        "citation_count": 21,
        "paper_quality": "PASS_MANUSCRIPT_EVIDENCE",
        "paper_quality_referenced_files": 64,
        "paper_quality_registered_digests": 67,
        "supplement_unit_tests": "31/31 PASS",
        "new_experiment_unit_tests": "22/22 PASS",
        "new_external_reviewer_run": False,
    })
    append("submission-readiness-r18", {
    ...
    append("submission-readiness-context-r18", {
    ...
    append("source-native-r18-finalization", {
    ...
    errors = validate_paper_ledger(after)
    if errors:
        raise RuntimeError("R18 ledger invalid: " + "; ".join(errors))
    return after
...
def selective_projection(root: Path) -> dict:
    ...
    if pub.get("gate_clean_submission_ready") is not True or pub.get("immediate_submission_hold") is True:
        raise RuntimeError("live STRI R18 readiness is not gate-clean")
    prep = pub.get("latest_paper_preparation") or {}
    if prep.get("protocol_version") != "1.0+r18-second-substrate-audit":
        raise RuntimeError("live STRI paper-preparation is not R18")
    ...
    if errors:
        raise RuntimeError("R18 STRI public status invalid: " + "; ".join(errors))
    if status.get("status") != "READY_NARROW_ICLR":
        raise RuntimeError(f"R18 public status is not ready: {status.get('gates')}")
...
def main() -> None:
    ...
    receipt = {
        ...
        "revision": "R18",
        "status": "R18_CANONICAL_PROJECTION_PUBLISHED",
        "canonical_base": CANONICAL_BASE,
        "integration_source_head": INTEGRATION_SOURCE_HEAD,
        "release_manifest": str(MANIFEST),
        "release_manifest_sha256": MANIFEST_FILE_SHA,
