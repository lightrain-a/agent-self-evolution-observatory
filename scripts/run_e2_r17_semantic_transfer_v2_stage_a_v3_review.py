...
CONTRACT = ROOT / "generated/e2-r17-semantic-transfer-v2-stage-a-v3-contract-20260903.json"
DEFAULT_OUT_ROOT = ROOT / "generated/e2-r17-semantic-transfer-v2-stage-a-v3-review-20260903"
...
DOSSIER = (
    ...
    ("stage_a_v3_contract", CONTRACT),
    ("stage_a_v3_preflight", ROOT / "generated/e2-r17-semantic-transfer-v2-stage-a-v3-preflight-20260903.json"),
    ...
    ("stage_a_v3_runner", ROOT / "scripts/run_e2_r17_semantic_transfer_v2_stage_a_v3.py"),
    ("equal_dose_adjudicator_v2", ROOT / "scripts/adjudicate_e2_r17_semantic_transfer_v2_stage_a.py"),
    ("authorization_minter_v3", ROOT / "scripts/authorize_e2_r17_semantic_transfer_v2_stage_a_v3.py"),
    ("preflight_code_v3", ROOT / "scripts/preflight_e2_r17_semantic_transfer_v2_stage_a_v3.py"),
...
def prompt_for(model: str, bound: str, contract_sha: str) -> str:
    ...
    return f"""You are an independent adversarial pre-execution reviewer for E2-R17 Selective-MRW Semantic-Transfer V2. You are a reviewer only. No V2 Stage-A search-pool outcomes and no Stage-B learning outcomes are supplied. You may recommend at most a separately SHA-bound Stage-A V3 pool-acquisition authorization. You may NOT authorize Stage B, heldout evaluation, analyzer, second backbone, public benchmark, or paper claims.

Reviewer endpoint: {model}
Exact Stage-A V3 contract SHA-256: {contract_sha}
...
14. AUTHORITY: PASS may recommend only a separate single-use Stage-A V3 authorization. Stage B and paper_claim_authority must remain false.
...
def identity_map(deepseek_path: Path, kimi_path: Path, contract_created_at: str) -> dict[str, str]:
    ...
    if datetime.fromisoformat(str(deepseek["created_at_utc"])) <= contract_time or datetime.fromisoformat(str(kimi["created_at_utc"])) <= contract_time:
        raise RuntimeError("reviewer identity qualification must occur after Stage-A V3 contract freeze")
