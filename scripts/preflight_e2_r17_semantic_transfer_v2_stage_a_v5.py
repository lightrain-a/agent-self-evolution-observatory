...
CONTRACT_STATUS = "FROZEN_SEMANTIC_TRANSFER_V2_STAGE_A_V5"
...
def main() -> int:
    ...
    req(not args.output.exists(), "V5 preflight output already exists")
    contract = load(args.contract)
    contract_sha = sha(args.contract)
    req(contract.get("status") == CONTRACT_STATUS, "V5 contract status invalid")
    req(contract.get("authority", {}).get("stage_a_provider_execution") is False, "draft V5 contract cannot self-authorize")
    ...
    req(len(all_tasks) == 144 and len(set(all_tasks)) == 144, "V5 update task shape invalid")
    req(len(heldout) == 18 and len(set(heldout)) == 18 and set(all_tasks).isdisjoint(heldout), "V5 heldout separation invalid")
    ...
    req(not run_root.exists(), "V5 run root already exists")
    req(not lease_path.exists(), "V5 global lease already exists")
    ...
    synthetic = {
        ...
        "execution_scope": {
            ...
            "provider_budget": {"required": True, "total_limit": 11520, "per_unit_limit": 10},
        },
    }
    ...
    # Import the actual V5 control-plane modules under the frozen runtime/Python
    ...
    for path in (runner_path, adjudicator_path, authorizer_path):
        ...
        req(check.returncode == 0, f"V5 bound code compile failed: {path.name}")

    payload = {
        "schema_version": "1.0",
        "artifact_type": "e2-r17-semantic-transfer-v2-stage-a-v5-zero-provider-preflight",
        "created_at_utc": datetime.now(timezone.utc).isoformat(timespec="seconds"),
        "status": "PASS_ZERO_PROVIDER_SEMANTIC_TRANSFER_V2_STAGE_A_V5_PREFLIGHT",
        ...
        "next_gate": "LOCKED_DUAL_PREEXECUTION_REVIEW_AND_FRESH_IDENTITY_BEFORE_SINGLE_USE_STAGE_A_V5_AUTHORIZATION"
    }
