...
def validate_authority(
    *,
    ...
    if payload.get("status") != "AUTHORIZED_E2_R17_DEEPSEEK_V2_REPAIR2_V3":
        raise RuntimeError("authorization artifact is not a Repair2 Continuation V3 authorization")
    authority = payload.get("authority") or {}
    if authority.get("scientific_experiment") is not True or authority.get("repair2_continuation_v3") is not True:
        raise RuntimeError("authorization has no Repair2 Continuation V3 scientific authority")
    if authority.get("analyzer") is not False or authority.get("paper_promotion") is not False:
        raise RuntimeError("V3 execution authorization must forbid analyzer and paper authority")
    ...
    if scope.get("continuation_version") != "repair2_v3":
        raise RuntimeError("V3 authorization continuation version drift")
...
async def main_async(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if args.stop_before_provider_io:
        ...
        return {
            "schema_version": "1.0",
            "artifact_type": "e2-r17-repair2-v3-actual-actor-authorization-path-preflight-unit",
