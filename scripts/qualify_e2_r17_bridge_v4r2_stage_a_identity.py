...
PROMPT = (
    "This is a non-scientific E2-R17 Bridge V4-R2 Stage-A Ark Plan model-identity qualification. "
    "Return exactly BRIDGE_V4R2_STAGE_A_IDENTITY_OK and nothing else."
)
...
def qualify_once(client: ClientLike) -> dict[str, Any]:
    ...
    try:
        ...
        checks = {
            "text_exact": text.strip() == "BRIDGE_V4R2_STAGE_A_IDENTITY_OK",
...
def build_payload(*, row: dict[str, Any], route: str, source_default_model: str | None) -> dict[str, Any]:
    ...
    status = IDENTITY_STATUS if pass_row else str(row.get("status") or "HOLD")
    return {
        "schema_version": "1.0",
...
def main() -> int:
    ...
    if args.output.exists():
        raise RuntimeError("Bridge V4-R2 Stage-A identity output already exists")

    load_env_file(args.env_file)
    source = ArkSettings.from_env(required=True)
    if source.base_url.rstrip("/") != PLAN_ROUTE:
        raise RuntimeError("Bridge V4-R2 Stage-A identity qualification refuses any non-Ark-Plan route")
    ...
    return 0 if payload["status"] == IDENTITY_STATUS else 2
