...
EXPECTED_AUTH_STATUS = "AUTHORIZED_SEMANTIC_TRANSFER_V2_STAGE_A_V4"
...
def validate_contract_auth(contract_path: Path, auth_path: Path) -> tuple[dict[str, Any], dict[str, Any], str, str]:
    ...
    require(contract.get("status") == EXPECTED_CONTRACT_STATUS, "Stage-A V4 contract status invalid")
    require(auth.get("status") == EXPECTED_AUTH_STATUS, "Stage-A V4 authorization status invalid")
    require(auth.get("contract_sha256") == contract_sha, "authorization does not bind exact Stage-A V4 contract")
...
def main() -> int:
    ...
    require(args.env_file.resolve() == bound_env, "Stage-A V4 env-file path drift")
    ...
    require(list(streams) == stream_ids, "Stage-A V4 stream ordering drift")
    all_tasks = [task for stream_id in stream_ids for task in streams[stream_id]]
    require(len(all_tasks) == 144 and len(set(all_tasks)) == 144, "Stage-A V4 task set drift")
    ...
    require(model_row.get("thinking") == "disabled" and int(model_row.get("provider_retry_limit") or -1) == 0, "fresh identity runtime flags drift")
    ...
    require(not run_root.exists(), "Stage-A V4 run root already exists; resume requires separate adjudication")
    require(not summary_path.exists(), "Stage-A V4 terminal summary already exists")

    global_lease_path = Path(contract["global_lease_path"])
    lease_payload = {
        "schema_version": "1.0",
        "artifact_type": "e2-r17-semantic-transfer-v2-stage-a-global-lineage-lease",
        "status": "RUNNING_STAGE_A_V4",
    ...
    try:
        ...
        require(not manifest_path.exists(), "first-run Stage-A V4 unexpectedly has a completed-stream manifest")
        ...
        for stream_id in stream_ids:
            ...
            if result.returncode != 0:
                ...
                raise RuntimeError(f"Stage-A V4 stream failed: {stream_id}")
        ...
        require(len(completed_rows) == 18, "Stage-A V4 did not complete all streams")
        total_rollouts = 0
        ...
        require(total_rollouts == 1152, f"Stage-A V4 rollout count drift: {total_rollouts}")
        ...
        require(not touched_heldout, f"Stage-A V4 touched forbidden heldout tasks: {touched_heldout}")
        ...
        terminal_lease = {
            **lease_payload,
            "status": "COMPLETED_STAGE_A_V4_PENDING_EQUAL_DOSE_ADJUDICATION",
