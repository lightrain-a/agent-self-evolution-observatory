...
PID="D2-PAPER-TEMPORAL-SKILL-CAUSAL-BOTTLENECK"; TITLE="When Do Reusable Temporal Procedures Actually Repair Agents?"
PDF="80580931de3e85b5fc68756772a8b5186cbf50c1d29d77dd6935e8fa3b9290de"; SRC="1b6a85d3942af1899b9f52df368c00ac981861c048b3fcd8760bc8d4d8da5a8c"; SUP="710ebd1b56fdec3c984572c68b19d0cf86c368cd55eb15df69beaf45cde705d9"
...
def main():
 ...
 srcs=[(DL/'E2-Temporal-Skill-r13-20260824.pdf',DL/'E2-Temporal-Skill.pdf',PDF),(DL/'E2-Temporal-Skill-r13-20260824.pdf',DL/f'{PID}.pdf',PDF),(DL/'E2-Temporal-Skill-r13-20260824-source.zip',DL/f'{PID}-source.zip',SRC),(DL/'E2-Temporal-Skill-r13-20260824-supplement.zip',DL/f'{PID}-supplement.zip',SUP)]
 for s,d,h in srcs:
 ...
 if pub['title']!=TITLE or (pub.get('source_native_evidence') or {}).get('runtime_valid_rows')!=2056: raise RuntimeError('live E2 projection not R13')
 ...
 out={'schema_version':'1.0','status':'E2_R13_SELECTIVE_PROJECTION_PUBLISHED','paper_id':PID,'title':TITLE,'runtime_valid_rows':2056,'stable_hashes':{'pdf':sha(DL/'E2-Temporal-Skill.pdf'),'source_zip':sha(DL/f'{PID}-source.zip'),'supplement_zip':sha(DL/f'{PID}-supplement.zip')},'other_paper_rows_preserved':True,'scientific_authority':False,'submission_authority':False}
 out['receipt_sha256']=hashlib.sha256(json.dumps(out,ensure_ascii=False,sort_keys=True,separators=(',',':')).encode()).hexdigest(); (GEN/'temporal-skill-r13-publication-projection-20260824.json').write_text(json.dumps(out,ensure_ascii=False,indent=2)+'\n'); print(json.dumps(out,ensure_ascii=False,indent=2))
