...
EXPECTED_PREFLIGHT_STATUS = "PASS_ZERO_PROVIDER_SEMANTIC_TRANSFER_STAGE_A_R2_PREFLIGHT"
...
def main() -> int:
    ...
    req(not args.output.exists(), "Stage-A R2 authorization already exists")
    ...
    req(contract.get("status") == EXPECTED_CONTRACT_STATUS, "Stage-A R2 contract not frozen")
    req(preflight.get("status") == EXPECTED_PREFLIGHT_STATUS, "Stage-A R2 preflight not passing")
    ...
    req(len(all_tasks) == 96 and len(set(all_tasks)) == 96, "authorization Stage-A task shape drift")
    ...
    req(not lease_path.exists(), "global Stage-A R2 lineage lease already exists")
    req(not Path(contract["run_root"]).exists(), "Stage-A R2 run root already exists")
    ...
    payload = {
        "schema_version": "1.0",
        "artifact_type": "e2-r17-semantic-transfer-v1-stage-a-r2-authorization",
        "created_at_utc": datetime.now(timezone.utc).isoformat(timespec="seconds"),
        "status": "AUTHORIZED_SEMANTIC_TRANSFER_V1_STAGE_A_R2",
