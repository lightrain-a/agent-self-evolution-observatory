...
def validate_authority(
    *,
    ...
    if not contract_path.is_file() or sha256(contract_path) != payload.get("contract_sha256"):
        raise RuntimeError("continuation authorization/contract binding drift")
    if payload.get("status") != "AUTHORIZED_E2_R17_DEEPSEEK_V2_REPAIR2_CONTINUATION_V1":
        raise RuntimeError("authorization artifact is not a Repair2 Continuation V1 authorization")
    authority = payload.get("authority") or {}
    if authority.get("scientific_experiment") is not True or authority.get("repair2_continuation_v1") is not True:
        raise RuntimeError("authorization has no Repair2 Continuation V1 scientific authority")
    if authority.get("analyzer") is not False or authority.get("paper_promotion") is not False:
        raise RuntimeError("continuation execution authorization must forbid analyzer and paper authority")

    # The child is scope-bound and fails closed on every actor dimension.
    scope = payload.get("execution_scope")
    if not isinstance(scope, dict):
        raise RuntimeError("continuation authorization requires an execution_scope")
    if scope.get("continuation_version") != "repair2_continuation_v1":
        raise RuntimeError("continuation version drift")
    if scope.get("allow_noninitial_skill") is not True:
        raise RuntimeError("continuation authorization must explicitly allow bound learned skills")
...
async def main_async(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if authorization_payload is not None:
        ...
        if provider_budget_scope.get("required") is True:
            ...
            standard_total = int(provider_budget_scope.get("total_limit"))
            boundary_totals = {
                int(row["child_total_limit"])
                for row in scope.get("boundary_learned_states") or []
            }
            if int(args.provider_total_call_limit) not in {standard_total, *boundary_totals}:
    ...
    if skill_source != default_skill_source.resolve():
        ...
        same_child_provenance = (
            updater_receipt.get("contract_sha256") == contract_sha
            and updater_receipt.get("authorization_sha256") == authorization_sha
        )
        scope = authorization_payload.get("execution_scope") or {}
        boundary_matches = [
            row
            ...
            and Path(str(row.get("update_receipt_path") or "")).resolve() == args.updater_receipt.resolve()
        ]
        if same_child_provenance:
            if boundary_matches:
                raise RuntimeError("boundary state cannot masquerade as a child-produced learned state")
            if int(args.provider_total_call_limit) != int((scope.get("provider_budget") or {}).get("total_limit")):
                raise RuntimeError("new child state must use the full frozen state budget")
        else:
            if len(boundary_matches) != 1:
                raise RuntimeError("cross-authorization learned state is not an exact authorized boundary state")
            boundary = boundary_matches[0]
            if boundary.get("skill_post_sha256") != skill_sha:
                raise RuntimeError("boundary learned-state SHA drift")
            if boundary.get("update_receipt_sha256") != updater_receipt_sha:
                raise RuntimeError("boundary updater-receipt SHA drift")
            parent = authorization_payload.get("v3_parent_provenance") or {}
            if updater_receipt.get("contract_sha256") != parent.get("contract_sha256"):
                raise RuntimeError("boundary updater receipt V3 contract provenance drift")
            if updater_receipt.get("authorization_sha256") != parent.get("authorization_sha256"):
                raise RuntimeError("boundary updater receipt V3 authorization provenance drift")
            parent_contract = Path(str(parent.get("contract_path") or ""))
            parent_auth = Path(str(parent.get("authorization_path") or ""))
            if not parent_contract.is_file() or sha256(parent_contract) != parent.get("contract_sha256"):
                raise RuntimeError("V3 parent contract provenance drift")
            if not parent_auth.is_file() or sha256(parent_auth) != parent.get("authorization_sha256"):
                raise RuntimeError("V3 parent authorization provenance drift")
            if len(task_ids) != 1 or task_ids[0] not in boundary.get("remaining_task_ids", []):
                raise RuntimeError("boundary actor may evaluate only its exact remaining heldout task set")
            if int(args.provider_total_call_limit) != int(boundary.get("child_total_limit")):
                raise RuntimeError("boundary child ledger residual-budget drift")
    ...
    if args.stop_before_provider_io:
        ...
        if snapshot is None or snapshot.total_claimed != 0:
            raise RuntimeError("continuation pre-provider stop requires a bound zero-claim provider ledger")
        return {
            "schema_version": "1.0",
            "artifact_type": "e2-r17-repair2-continuation-v1-actual-actor-authorization-path-preflight-unit",
