...
EXPECTED_CONTRACT_STATUS = "FROZEN_SEMANTIC_TRANSFER_V1_STAGE_A_R2"
EXPECTED_AUTH_STATUS = "AUTHORIZED_SEMANTIC_TRANSFER_V1_STAGE_A_R2"
...
def validate_contract_auth(contract_path: Path, auth_path: Path) -> tuple[dict[str, Any], dict[str, Any], str, str]:
    ...
    require(contract.get("status") == EXPECTED_CONTRACT_STATUS, "Stage-A R2 contract status invalid")
    require(auth.get("status") == EXPECTED_AUTH_STATUS, "Stage-A R2 authorization status invalid")
    require(auth.get("contract_sha256") == contract_sha, "authorization does not bind exact Stage-A R2 contract")
...
def verify_authorization_scope(contract: dict[str, Any], auth: dict[str, Any], all_tasks: list[str], heldout: list[str]) -> None:
    ...
    require(scope.get("identity_artifact_sha256") == contract["model_identity"]["sha256"], "authorization identity SHA drift")
...
def main() -> int:
    ...
    require(list(streams) == stream_ids, "Stage-A R2 stream ordering drift")
    all_tasks = [task for stream_id in stream_ids for task in streams[stream_id]]
    require(len(all_tasks) == 96 and len(set(all_tasks)) == 96, "Stage-A R2 task set drift")
    ...
    require(identity_path.is_file() and sha_file(identity_path) == contract["model_identity"]["sha256"], "model identity binding drift")
    identity = load_json(identity_path)
    require(identity.get("status") == EXPECTED_IDENTITY_STATUS, "model identity adjudication not passing")
    ...
    require(not run_root.exists(), "Stage-A R2 run root already exists; resume requires separate adjudication")
    require(not summary_path.exists(), "Stage-A R2 terminal summary already exists")

    global_lease_path = Path(contract["global_lease_path"])
    lease_payload = {
        "schema_version": "1.0",
        "artifact_type": "e2-r17-semantic-transfer-v1-stage-a-global-lineage-lease",
        "status": "RUNNING_STAGE_A_R2",
    ...
    try:
        ...
        require(not manifest_path.exists(), "first-run Stage-A R2 unexpectedly has a completed-stream manifest")
        ...
        for stream_id in stream_ids:
            ...
            if result.returncode != 0:
                ...
                raise RuntimeError(f"Stage-A R2 stream failed: {stream_id}")
        ...
        # Only after all 12 streams have completed do we open/validate stream summaries.
        require(len(completed_rows) == 12, "Stage-A R2 did not complete all streams")
        total_rollouts = 0
        ...
        require(total_rollouts == 768, f"Stage-A R2 rollout count drift: {total_rollouts}")
        ...
        require(not touched_heldout, f"Stage-A R2 touched forbidden heldout tasks: {touched_heldout}")

        final = {
            "schema_version": "1.0",
            "artifact_type": "e2-r17-semantic-transfer-v1-stage-a-pool-freeze-summary",
            "created_at_utc": datetime.now(timezone.utc).isoformat(timespec="seconds"),
            "status": "COMPLETED_ALL_96_POOLS_PENDING_EQUAL_DOSE_ADJUDICATION",
            "contract_sha256": contract_sha,
            "authorization_sha256": auth_sha,
        ...
        terminal_lease = {
            **lease_payload,
            "status": "COMPLETED_STAGE_A_R2_PENDING_EQUAL_DOSE_ADJUDICATION",
