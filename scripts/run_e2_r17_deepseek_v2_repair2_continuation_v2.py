...
def validate_v2_inheritance(contract: dict[str, Any]) -> list[dict[str, Any]]:
    item = contract["lineage_inheritance_manifest"]
    path = ROOT / item["path"]
    require(path.is_file() and sha_file(path) == item["sha256"], "V2 lineage inheritance manifest drift")
    payload = load_json(path)
    require(payload.get("status") == "PASS_CONTINUATION_V2_CANONICAL_INHERITANCE_29_PAIRS", "V2 inheritance not passing")
    require(payload.get("partial_effect_read") is False and payload.get("scientific_scores_read") is False, "V2 inheritance opened outcome")
    rows = payload.get("valid_rows") or []
    require(len(rows) == 29 and len({row["unit_id"] for row in rows}) == 29, "V2 inheritance cardinality drift")
    tasks = list(map(str, contract["heldout"]["task_ids"]))
    for row in rows:
        require(set(row["arms"]) == set(ARMS), f"V2 inherited arm drift: {row['unit_id']}")
        require(Path(row["pair_summary_path"]).is_file() and sha_file(Path(row["pair_summary_path"])) == row["pair_summary_sha256"], f"V2 inherited pair summary drift: {row['unit_id']}")
        for arm in ARMS:
            state = row["arms"][arm]
            skill = Path(state["state_root"]) / "update/skill_post/SKILL.md"
            require(skill.is_file() and sha_file(skill) == state["skill_sha256"], f"V2 inherited skill drift: {row['unit_id']}/{arm}")
            manifest = Path(state["eval_manifest_path"])
            require(manifest.is_file() and sha_file(manifest) == state["eval_manifest_sha256"], f"V2 inherited eval manifest drift: {row['unit_id']}/{arm}")
            eval_rows = [json.loads(line) for line in manifest.read_text().splitlines() if line.strip()]
            require([str(value["task_id"]) for value in eval_rows] == tasks, f"V2 inherited heldout order drift: {row['unit_id']}/{arm}")
            for value in eval_rows:
                require(Path(value["summary_path"]).is_file() and sha_file(Path(value["summary_path"])) == value["summary_sha256"], f"V2 inherited summary drift: {row['unit_id']}/{arm}/{value['task_id']}")
                require(Path(value["trajectory_ref_path"]).is_file() and sha_file(Path(value["trajectory_ref_path"])) == value["trajectory_ref_sha256"], f"V2 inherited ref drift: {row['unit_id']}/{arm}/{value['task_id']}")
    return rows


def validate_valid_rows_v2(rows: list[dict[str, Any]], contract: dict[str, Any], require_complete: bool) -> None:
    streams = list(map(str, contract["streams"]))
    expected = [f"{stream}/rep{rep}" for stream in streams for rep in REPLICATES]
    seen = set()
    allowed = {"repair1_inherited", "repair2_m1_recovered", "repair2_v3_fresh", "repair2_v3_pair29_recovered", "repair2_continuation_v2_fresh"}
    for row in rows:
        unit = str(row["unit_id"])
        require(unit not in seen and unit in expected, f"V2 invalid/duplicate pair: {unit}")
        seen.add(unit)
        require(str(row.get("source")) in allowed, f"V2 source drift: {unit}")
        require(set(row.get("arms") or {}) == set(ARMS), f"V2 arm drift: {unit}")
    require(seen == set(expected[:len(rows)]), "V2 valid rows are not the frozen design prefix")
    if require_complete:
        require(len(rows) == 48 and seen == set(expected), "V2 final manifest incomplete")


def acquire_global_lease(path: Path, contract: dict[str, Any], contract_sha: str, auth_sha: str) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    payload = {
        "schema_version": "1.0",
        "artifact_type": "e2-r17-deepseek-v2-repair2-global-lineage-lease",
        "created_at_utc": datetime.now(timezone.utc).isoformat(timespec="seconds"),
        "status": "RUNNING_CONTINUATION_V2",
        "pid": os.getpid(),
        "pgid": os.getpgrp(),
        "contract_sha256": contract_sha,
        "authorization_sha256": auth_sha,
        "canonical_lineage_manifest_sha256": contract["pia1"]["canonical_lineage"]["sha256"],
        "pair29_recovery_pass_sha256": contract["pair29_recovery"]["pass_sha256"],
        "lineage_inheritance_manifest_sha256": contract["lineage_inheritance_manifest"]["sha256"],
        "remaining_set_manifest_sha256": contract["remaining_set_manifest"]["sha256"],
        "exactly_once": True,
...
def assert_global_lease(path: Path, contract: dict[str, Any], contract_sha: str, auth_sha: str) -> None:
    lease = load_json(path)
    require(lease.get("status") == "RUNNING_CONTINUATION_V2", "global lineage lease not running")
    require(int(lease.get("pid")) == os.getpid() and int(lease.get("pgid")) == os.getpgrp(), "global lineage lease owner drift")
    require(lease.get("contract_sha256") == contract_sha and lease.get("authorization_sha256") == auth_sha, "global lineage lease binding drift")
    require(lease.get("lineage_inheritance_manifest_sha256") == contract["lineage_inheritance_manifest"]["sha256"], "global lineage lease inheritance drift")
    require(lease.get("remaining_set_manifest_sha256") == contract["remaining_set_manifest"]["sha256"], "global lineage lease remaining-set drift")


def seal_global_lease(path: Path, status: str) -> None:
    payload = load_json(path)
    payload["status"] = status
    payload["sealed_at_utc"] = datetime.now(timezone.utc).isoformat(timespec="seconds")
...
def validate_contract_auth(contract_path: Path, auth_path: Path) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    require(contract.get("status") == "FROZEN_E2_R17_DEEPSEEK_V2_REPAIR2_CONTINUATION_V2", "DeepSeek V2 Repair2 V3 contract not frozen")
    require(auth.get("status") == "AUTHORIZED_E2_R17_DEEPSEEK_V2_REPAIR2_CONTINUATION_V2", "Repair2 V3 authorization invalid")
    ...
    require(authority.get("repair2_continuation_v2") is True, "Repair2 V3 continuation authority absent")
    require(authority.get("analyzer") is False, "V3 execution authorization must forbid analyzer")
    ...
    require(scope.get("continuation_version") == "repair2_continuation_v2", "V3 continuation version drift")
    ...
    for key in ("pia_adjudication", "canonical_lineage", "duplicate_quarantine"):
        item = contract["pia1"][key]
        path = ROOT / item["path"]
        require(path.is_file() and sha_file(path) == item["sha256"], f"PIA binding drift: {key}")
    recovery = contract["pair29_recovery"]
    recovery_path = ROOT / recovery["pass_path"]
    require(recovery_path.is_file() and sha_file(recovery_path) == recovery["pass_sha256"], "pair29 recovery PASS drift")
    recovery_payload = load_json(recovery_path)
    require(recovery_payload.get("status") == "PAIR29_MEASUREMENT_RECOVERY_PASS", "pair29 recovery not passing")
    require(recovery_payload.get("partial_effect_read") is False and recovery_payload.get("analyzer_run") is False, "pair29 recovery outcome boundary drift")
    for key in ("lineage_inheritance_manifest", "remaining_set_manifest"):
        item = contract[key]
        path = ROOT / item["path"]
        require(path.is_file() and sha_file(path) == item["sha256"], f"V2 lineage binding drift: {key}")
    require(scope.get("remaining_units") == contract["remaining_units"], "V2 authorized remaining set drift")
    require(len(contract["remaining_units"]) == 19 and int(contract["execution_plan"]["new_learned_states"]) == 38 and int(contract["execution_plan"]["new_heldout_units"]) == 684, "V2 execution cardinality drift")
    require(scope.get("global_lineage_lease_path") == contract["global_lineage_lease"]["path"], "global lineage lease scope drift")
...
async def main_async(args: argparse.Namespace) -> dict[str, Any]:
    ...
    validate_quarantine(quarantine_path, quarantine_item["sha256"])
    inherited_rows = validate_v2_inheritance(contract)
    require(len(inherited_rows) == 29, "Continuation V2 must freeze exactly 29 completed prefix pairs")
    remaining_item = contract["remaining_set_manifest"]
    remaining_path = ROOT / remaining_item["path"]
    require(remaining_path.is_file() and sha_file(remaining_path) == remaining_item["sha256"], "V2 remaining-set drift")
    remaining_payload = load_json(remaining_path)
    require(remaining_payload.get("status") == "PASS_CONTINUATION_V2_REMAINING_SET_19_PAIRS", "V2 remaining set not passing")
    require(remaining_payload.get("partial_effect_read") is False, "V2 remaining set opened outcome")
    remaining_units = list(map(str, remaining_payload["remaining_units"]))
    ...
    require(not completed_units and not valid_units, "Continuation V2 manifests must start empty")
    for inherited in inherited_rows:
        append_jsonl(valid_manifest, inherited)
        valid_units[inherited["unit_id"]] = inherited
        completed = {"unit_id":inherited["unit_id"],"stream_id":inherited["stream_id"],"replicate":inherited["replicate_id"],"summary_path":inherited["pair_summary_path"],"summary_sha256":inherited["pair_summary_sha256"],"source":inherited["source"],"completed_at_utc":datetime.now(timezone.utc).isoformat(timespec="seconds")}
        append_jsonl(unit_manifest, completed)
        completed_units[completed["unit_id"]] = completed
    inherited_sources = {"repair1_inherited", "repair2_m1_recovered", "repair2_v3_fresh", "repair2_v3_pair29_recovered"}
    require({row["unit_id"] for row in inherited_rows} == set(valid_units), "runtime inherited set differs from frozen V2 lineage")
    validate_valid_rows_v2(list(valid_units.values()), contract, require_complete=False)
    ...
    try:
        ...
        for stream_id in contract["streams"]:
            ...
            for replicate in REPLICATES:
                ...
                if unit_id in completed_units:
                    continue
                require(unit_id in remaining_units, f"V2 attempted out-of-scope unit: {unit_id}")
                ...
                manifest_row={"unit_id":unit_id,"stream_id":stream_id,"replicate":replicate,"summary_path":str(rep_summary_path),"summary_sha256":sha_file(rep_summary_path),"source":"repair2_continuation_v2_fresh","completed_at_utc":datetime.now(timezone.utc).isoformat(timespec="seconds")}
                valid_row={"unit_id":unit_id,"stream_id":stream_id,"replicate_id":replicate,"source":"repair2_continuation_v2_fresh","pair_summary_path":str(rep_summary_path),"pair_summary_sha256":sha_file(rep_summary_path),"arms":{}}
                for arm in ARMS:
                    state_root=rep_root/arm
                    update=updates[arm]
        ...
        require(set(completed_units)==expected, "DeepSeek V2 Repair2 Continuation V2 did not complete all 48 paired replicate units")
        validate_valid_rows_v2(list(valid_units.values()), contract, require_complete=True)
        ...
        final={"schema_version":"1.0","artifact_type":"e2-r17-deepseek-v2-repair2-continuation-v2-summary","created_at_utc":datetime.now(timezone.utc).isoformat(timespec="seconds"),"status":"COMPLETED_PENDING_SEPARATE_DEEPSEEK_V2_ADJUDICATION","contract_sha256":contract_sha,"authorization_sha256":auth_sha,"streams":len(contract["streams"]),"replicates_per_stream":len(REPLICATES),"paired_replicate_units":len(expected),"inherited_paired_units":sum(row.get("source") in inherited_sources for row in valid_units.values()),"repair1_inherited_paired_units":sum(row.get("source")=="repair1_inherited" for row in valid_units.values()),"repair2_m1_recovered_paired_units":sum(row.get("source")=="repair2_m1_recovered" for row in valid_units.values()),"fresh_paired_units":sum(row.get("source")=="repair2_continuation_v2_fresh" for row in valid_units.values()),"pair29_recovered_paired_units":sum(row.get("source")=="repair2_v3_pair29_recovered" for row in valid_units.values()),"arms":list(ARMS),"learned_states":len(expected)*2,"heldout_tasks_per_state":len(contract["heldout"]["task_ids"]),"heldout_rollout_units":len(expected)*2*len(contract["heldout"]["task_ids"]),"mrw_executed":True,"primary_control":"win_c","inference_performed":False,"paper_promotion_authority":False,"completed_replicate_manifest":str(unit_manifest),"completed_replicate_manifest_sha256":sha_file(unit_manifest),"valid_replicate_manifest":str(valid_manifest),"valid_replicate_manifest_sha256":sha_file(valid_manifest),"runtime_reliability":reliability,"repair1_quarantined_patch_apply_failures":{"win_c":0,"mrw":1}}
        atomic_json(run_root/"summary/deepseek_v2_repair2_continuation_v2_summary.json",final); success=True; return final
    finally:
        os.close(lock_fd)
        seal_global_lease(lease_path, "COMPLETED_CONTINUATION_V2" if success else "FAIL_CLOSED_CONTINUATION_V2")
