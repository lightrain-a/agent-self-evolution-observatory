...
PKG = Path("/data/wyt/agent-self-evolution-observatory/submission-packages/stri-e1-r16-experiment-breadth-20260824")
PDF = PKG / "E1-STRI-R16-experiment-breadth.pdf"
SOURCE = PKG / "E1-STRI-R16-experiment-breadth-source.zip"
SUPPLEMENT = PKG / "E1-STRI-R16-experiment-breadth-supplement.zip"
MANIFEST = PKG / "R16-MANIFEST.json"

PDF_SHA = "227c0e47bbb4cddd26bb5823f39d56b9978e2a134185a71ba1b3eb66338721c9"
SOURCE_SHA = "4ec82b977f391840fe5e5a7a88d14e94d79c3d8841d1955017da161a1ad24aae"
SUPPLEMENT_SHA = "cdd6272374d7acc37b3620c516fdc1800806ff26233797723fefdbf55a17c5c4"
SUPPLEMENT_MANIFEST_SHA = "07cc0b11420c2a7e518fdb23c493da0989203dd419801af54afcff4fbff22e25"
PACKAGE_METADATA_SHA = "f5b50473426063a7b5f258ecd981d2192c8e5ca116dfb9fece90298792e9e7bf"
MANIFEST_FILE_SHA = "82dc75096c1f6a59f35bd549d19d8373880dfc392558d8e17742c121572628f4"
MANIFEST_CANONICAL_SHA = "6a37bbf3a6884411435c6b2c895ba4f6306dda74a66e8590a09865d01afe6908"
CANONICAL_BASE = "622709f4f2a5c58740fc97dbd080c72aa632a988"
INTEGRATION_SOURCE_HEAD = "a6a7bae8b234902ddcf1f864bbee5391bd4b2a4b"
...
PROJECTION_RECEIPT = GEN / "asset-first-stri-r16-canonical-projection-20260824.json"
...
def verify_package() -> dict:
    ...
    for path, digest in expected.items():
        if not path.is_file() or sha(path) != digest:
            raise RuntimeError(f"R16 artifact mismatch: {path}")
    manifest = load(MANIFEST)
    if manifest.get("candidate") != "R16-experiment-breadth" or manifest.get("manifest_sha256") != MANIFEST_CANONICAL_SHA:
        raise RuntimeError("R16 manifest identity/digest mismatch")
    if ((manifest.get("artifacts") or {}).get("supplement_zip") or {}).get("unit_tests") != "29/29 PASS":
        raise RuntimeError("R16 supplement unit-test contract missing")
    if int(((manifest.get("qa") or {}).get("paper_quality_content_addressed_files") or 0)) != 58:
        raise RuntimeError("R16 paper-quality breadth contract missing")
    return manifest
...
def compile_and_refresh_qa() -> dict:
    ...
    with tempfile.TemporaryDirectory(prefix="stri-r16-source-binding-") as tmp:
        ...
        for source in (MAIN := PAPER / "stri-20260816-iclr2027-main.tex", BODY := PAPER / "stri-20260816-narrow-body.tex", TABLES := PAPER / "stri-20260816-tables.tex", BIB := PAPER / "stri-20260816-references.bib"):
            ...
            if not packaged.is_file() or packaged.read_bytes() != source.read_bytes():
                raise RuntimeError(f"R16 source binding mismatch: {source.name}")
    qa = current_qa()
...
def update_stri_states() -> None:
    ...
    supplement["isolated_verification"] = {
        ...
        "figure_regeneration": "R16_BOUNDARY_FIGURE_REGENERATED_AND_SOURCE_BOUND",
        "paper_quality_v2_reproduction": "PASS_MANUSCRIPT_EVIDENCE",
    }
    ...
    mv.update({
        ...
        "experimental_breadth": "10 package-weight baselines; no-refit heldout transfer; SkillRL 8-budget sweep x 12 clones x 223 tasks; SkillRouter 75-query relevance analogue; SkillsBench support qualification STOP",
        "r16_canonical_reconciliation": True,
        "claim_expansion_for_r16": False,
        "new_model_calls_for_r16": 0,
        "new_gpu_runs_for_r16": 0,
    })
...
def append_r16_ledger(root: Path) -> dict:
    ...
    existing = [e for e in before.get("events") or [] if isinstance(e, dict) and e.get("event_type") == "source-native-r16-finalization"]
    contract = contract_from_ledger(before)
    if existing:
        clarification = [e for e in before.get("events") or [] if isinstance(e, dict) and e.get("event_type") == "source-native-r16-metric-clarification"]
        if not clarification:
            event = {
                "event_type": "source-native-r16-metric-clarification",
                "schema_version": "1.0",
                "paper_id": PID,
                "revision": REVISION,
                "receipt_type": "metric-clarification",
                "paper_quality_referenced_files": 55,
                "paper_quality_registered_digests": 58,
                "clarification": "R16 release manifest used the label paper_quality_content_addressed_files for the 58 registered source digests; the canonical public completion metric counts 55 files actually referenced by the claim/evidence graph.",
            ...
            _append(root, contract, "stri-r16-canonical-publisher", event)
        after = load_paper_ledger(root, PID)
        errors = validate_paper_ledger(after)
        if errors:
            raise RuntimeError("existing R16 ledger is invalid: " + "; ".join(errors))
        return after
    ...
    append("paper-preparation-r16", {
        "receipt_type": "paper-preparation",
        "protocol_version": "1.0+r16-experiment-breadth",
        "pass": True,
        "required_gates": 8,
        "passed_gates": 8,
        "gate_pass": gates,
        ...
        "paper_quality_registered_digests": 58,
        "supplement_unit_tests": "29/29 PASS",
        "new_experiment_unit_tests": "17/17 PASS",
        "new_external_reviewer_run": False,
    })
    append("submission-readiness-r16", {
    ...
    append("submission-readiness-context-r16", {
    ...
    append("source-native-r16-finalization", {
    ...
    errors = validate_paper_ledger(after)
    if errors:
        raise RuntimeError("R16 ledger invalid: " + "; ".join(errors))
    return after
...
def selective_projection(root: Path) -> dict:
    ...
    if pub.get("gate_clean_submission_ready") is not True or pub.get("immediate_submission_hold") is True:
        raise RuntimeError("live STRI R16 readiness is not gate-clean")
    prep = pub.get("latest_paper_preparation") or {}
    if prep.get("protocol_version") != "1.0+r16-experiment-breadth":
        raise RuntimeError("live STRI paper-preparation is not R16")
    ...
    if errors:
        raise RuntimeError("R16 STRI public status invalid: " + "; ".join(errors))
    if status.get("status") != "READY_NARROW_ICLR":
        raise RuntimeError(f"R16 public status is not ready: {status.get('gates')}")
...
def main() -> None:
    ...
    receipt = {
        ...
        "revision": "R16",
        "status": "R16_CANONICAL_PROJECTION_PUBLISHED",
        "canonical_base": CANONICAL_BASE,
        "integration_source_head": INTEGRATION_SOURCE_HEAD,
        "release_manifest": str(MANIFEST),
        "release_manifest_sha256": MANIFEST_FILE_SHA,
