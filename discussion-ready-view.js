  ...
  function card(idea, index) {
    ...
    return `<details class="discussion-idea-card r3-card-${escText(finalVerdict)}" id="discussion-${escText(idea.id)}">
      ...
      <div class="discussion-idea-body">
        <section class="discussion-r3-gate r3-${escText(finalVerdict)}"><div class="discussion-r3-gate-head"><b>${language === "zh" ? "最终讨论门槛" : "Final pre-advisor gate"}</b><span>${escText(revision)} · ${escText(finalVerdict.toUpperCase())}</span></div><p>${escText(finalFinding)}</p><div><strong>${language === "zh" ? "独立复审：" : "Independent review:"}</strong> ${escText(reviewerLine)} · collision ${escText(String(finalAudit.collision_gate || "pending").toUpperCase())}</div>${remainingRisk ? `<div><strong>${language === "zh" ? "剩余风险：" : "Remaining risk:"}</strong> ${escText(remainingRisk)}</div>` : ""}</section>
