...
function renderProjectStatusStrip(){
  ...
  const registry=window.PAPER_REGISTRY_STATE||{}, registrySummary=registry.summary||{};
  const message=selectedPaper
    ? (language==="zh"?`PaperRegistry 当前有 ${registrySummary.papers||0} 篇 canonical paper：${registrySummary.submission_ready||0} 篇 SUBMISSION_READY，${registrySummary.targeted_repair||0} 篇 TARGETED_REPAIR；新 8 门 Paper Preparation 已完成 ${registrySummary.preparation_pass||0} 篇，另有 ${registrySummary.legacy_ready_needs_preparation_migration||0} 篇 legacy-ready 待迁移。真实 SUBMITTED 仍需要人工 authority。`:`PaperRegistry currently contains ${registrySummary.papers||0} canonical papers: ${registrySummary.submission_ready||0} SUBMISSION_READY and ${registrySummary.targeted_repair||0} in TARGETED_REPAIR. The new eight-gate Paper Preparation protocol passes for ${registrySummary.preparation_pass||0}; ${registrySummary.legacy_ready_needs_preparation_migration||0} legacy-ready papers still await migration. Actual SUBMITTED still requires human authority.`)
    : (language==="zh"?`截至 ${asOf||"当前"}：STRI 已达到投稿就绪；还缺的论文证据项=${h.paper_quality_evidence_debt||0}；通过正式问题检查、可以继续进入方法设计的新研究问题=${h.canonical_live_ideas||0}；现在允许正式启动的实验=${h.launchable_formal_experiments||0}。另有 ${h.fresh_active_f0||0} 个新现象正在做最小验证，${h.fresh_support_holds||0} 个因为缺关键证据暂缓。以前的记忆效应只保留为历史观察；已关闭的精确候选表述=${h.shadow_closed_basins||h.shadow_dead_ends||0}，其中真正关闭到核心原理层=${h.shadow_core_principle_stops||0}、整个基准或现象也被判定不成立=${h.shadow_broader_core_principle_falsifications||0}，等待具体证据=${h.shadow_holds||0}。`:`As of ${asOf||"now"}: STRI is submission-ready; unfinished paper-evidence items=${h.paper_quality_evidence_debt||0}; new ideas that passed the formal problem check=${h.canonical_live_ideas||0}; formal experiments allowed to launch now=${h.launchable_formal_experiments||0}. ${h.fresh_active_f0||0} fresh phenomena are in minimal validation and ${h.fresh_support_holds||0} are waiting for specific evidence. The earlier memory effect is historical only; closed exact candidate formulations=${h.shadow_closed_basins||h.shadow_dead_ends||0}, scientific closures at the core-principle layer=${h.shadow_core_principle_stops||0}, whole benchmark/phenomenon falsifications=${h.shadow_broader_core_principle_falsifications||0}, waiting for evidence=${h.shadow_holds||0}.`);
  const statusLabels = selectedPaper
    ? (language === "zh"
      ? [["canonical papers",registrySummary.papers||0],["SUBMISSION_READY",registrySummary.submission_ready||0],["TARGETED_REPAIR",registrySummary.targeted_repair||0],["Paper Preparation PASS",registrySummary.preparation_pass||0],["legacy-ready 待迁移",registrySummary.legacy_ready_needs_preparation_migration||0],["等待人工投稿签字",registrySummary.human_submission_signoff_pending||0]]
      : [["canonical papers",registrySummary.papers||0],["SUBMISSION_READY",registrySummary.submission_ready||0],["TARGETED_REPAIR",registrySummary.targeted_repair||0],["Paper Preparation PASS",registrySummary.preparation_pass||0],["legacy-ready awaiting migration",registrySummary.legacy_ready_needs_preparation_migration||0],["awaiting human submission signoff",registrySummary.human_submission_signoff_pending||0]])
    : (language === "zh"
  ...
  return `<section class="project-status-strip current"><div class="project-status-copy"><b>${selectedPaper?(language==="zh"?"PaperRegistry · canonical 论文状态":"PaperRegistry · canonical paper state"):(language==="zh"?`当前科研状态${asOf?` · ${asOf}`:""}`:`Current research state${asOf?` · ${asOf}`:""}`)}</b><span>${message}</span></div><dl class="project-status-metrics">${statusLabels.map(([label,value])=>`<div><dt>${label}</dt><dd>${value}</dd></div>`).join("")}</dl></section>`;
}
