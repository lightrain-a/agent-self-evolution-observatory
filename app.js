...
function renderIdeaPortfolio(config) {
  ...
  const pipeline = `${renderSemanticScholarStatus()}${renderIdeaBackendArchitecture()}${renderIclrReviewDimensions()}${renderIdeaOperators()}${(config.sections || []).map((section,index) => renderSectionForPage(section,index,pageId,"idea-selection-section",3)).join("")}`;
