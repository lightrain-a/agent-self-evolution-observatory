...
const pageDefaultLanguage = (pageId === "research-timeline" || pageId === "selected-paper" || /^paper-(?:e1|g1|c1|e2|b1|a|b|agent-constraint|3d)$/.test(pageId)) ? "zh" : "en";
...
function renderCanonicalCategoryIndex(groups,parents,independent,inventory=canonicalInventorySummary(groups,parents,independent)) {
  const researchItems=window.RESEARCH_ITEM_STATE?.research_items||[];
  return `<section class="panel canonical-category-index" data-research-inventory-total="${inventory.total}"><div class="idea-panel-heading"><div><div class="eyebrow">${language==="zh"?"A–G 研究组合 · 先选方向":"A–G PORTFOLIO · CHOOSE A DIRECTION"}</div><h2 id="canonical-category-index">${language==="zh"?"先用七类总览定位，再进入具体 ResearchItem":"Use the seven-category overview first, then open individual ResearchItems"}</h2><p class="section-intro">${language==="zh"?`默认只强调当前关注对象；${inventory.total} 个去重对象仍全部保留。每类卡片同时给总量、当前关注数和父级科学终态，避免先读完整历史再找当前结论。`:`The default view emphasizes current attention while preserving all ${inventory.total} deduplicated objects. Each category shows total volume, current attention, and parent scientific states.`}</p></div><strong>${inventory.total}<span>${language==="zh"?"完整对象":"objects"}</span></strong></div><nav class="canonical-category-nav" aria-label="${language==="zh"?"研究大类":"Research categories"}">${groups.map(group=>{
    const row=inventory.byGroup[group.id]||{parent:0,related:0,context:0,closed:0,total:0};
    const current=researchItems.filter(item=>item.category===group.id&&["HOLD","PAPER_READY"].includes(item.scientific_state)).length;
    const parentRows=parents.filter(item=>item.meta.group===group.id), holds=parentRows.filter(item=>item.meta.status==="hold").length, stops=parentRows.filter(item=>item.meta.status==="stop").length, merges=parentRows.filter(item=>item.meta.status==="merge").length;
    return `<a href="#canonical-group-${esc(group.id.toLowerCase())}" data-category-total="${row.total}" data-category-current="${current}"><span>${esc(group.id)}</span><div><b>${textOf(group.title)}</b><small><strong>${language==="zh"?`当前关注 ${current}`:`attention ${current}`}</strong> · ${language==="zh"?`总计 ${row.total}`:`${row.total} total`} · HOLD ${holds} · STOP ${stops} · MERGED ${merges}</small></div></a>`;
  }).join("")}</nav></section>`;
}
...
function renderCanonicalIdeaGroup(group,parents,independent) {
  ...
  const categoryCurrent=(window.RESEARCH_ITEM_STATE?.research_items||[]).filter(item=>item.category===group.id&&["HOLD","PAPER_READY"].includes(item.scientific_state)).length;
  const lane=(kind,titleZh,titleEn,noteZh,noteEn,body,count)=>{
    const title=language==="zh"?titleZh:titleEn, note=language==="zh"?noteZh:noteEn, number=kind==="current"?"01":kind==="concluded"?"02":"03";
    const head=`<div class="research-category-lane-head"><div><span>${number}</span><div><h3 data-toc="false">${title}</h3><p>${note}</p></div></div><strong>${count}</strong></div>`;
    return kind==="current"?`<section class="research-category-lane lane-${kind}">${head}<div class="research-category-lane-body">${body}</div></section>`:`<details class="research-category-lane lane-${kind}"><summary>${head}</summary><div class="research-category-lane-body">${body}</div></details>`;
  };
  return `<section class="canonical-idea-group" id="canonical-group-${esc(group.id.toLowerCase())}" data-canonical-group="${esc(group.id)}" data-category-total="${inventory.total}"><header class="canonical-group-header"><span>${esc(group.id)}</span><div><h2>${textOf(group.title)}</h2><p>${textOf(group.question)}</p><div class="canonical-group-counts"><b>${inventory.total} ${language==="zh"?"个去重研究对象":"deduplicated objects"}</b><small>${language==="zh"?`父级方向 ${inventory.parent} · 研究方向/方法 ${inventory.related} · 论文/证据 ${inventory.context} · 编号关闭研究方向 ${inventory.closed}`:`${inventory.parent} parents · ${inventory.related} directions/methods · ${inventory.context} paper/evidence · ${inventory.closed} numbered stopped ideas`}</small>${terminalLine?`<small>${terminalLine}</small>`:""}</div></div></header>${categoryBriefing}${lane("current","当前值得关注","Current attention","默认只展开真正还需要判断、等待重开或保持论文谱系可见的对象。","Only items that still need a decision, await reopening, or retain paper lineage are expanded by default.",currentBody,categoryCurrent)}${lane("concluded","已形成明确结论","Clear conclusions","STOP 与已关闭候选默认收起；需要核查停止理由或重开条件时再展开。","STOPPED and closed candidates stay collapsed until their stop reason or reopen condition is needed.",concludedBody,stopRows.length+inventory.closed)}${lane("assets","已合并 / 沉淀为资产","Merged / retained assets","MERGED 和方法资产默认收起；它们保留可复用机制、协议、基线与审计规则。","MERGED items and method assets stay collapsed while preserving reusable mechanisms, protocols, baselines, and audit rules.",assetBody,mergeRows.length+inventory.related)}</section>`;
}
...
function renderCanonicalIdeaLedger(groups=canonicalIdeaGroups(),parents=canonicalParentRows(),independent=canonicalIndependentRows(),inventory=canonicalInventorySummary(groups,parents,independent),auditContext="") {
  ...
  const audit=`<details class="panel portfolio-audit-fold" id="portfolio-audit"><summary><div><b>${language==="zh"?"审计与历史：完整对象账本、停止分类、P0 与技术证据":"Audit & history: object accounting, stop taxonomy, P0, and technical evidence"}</b><span>${language==="zh"?"默认阅读不需要展开；这里保留完整计数关系、1min 写作规范、历史阶段和审计规则。":"Not required for default reading; this preserves complete accounting, briefing standards, historical stages, and audit rules."}</span></div><strong>${inventory.total}</strong></summary><div class="portfolio-audit-body">${auditContext}${renderResearchBriefingGuide(inventory)}${terminalSummary}${appendix}</div></details>`;
  return `${renderCanonicalCategoryIndex(groups,parents,independent,inventory)}${canonicalStatusControls(parents)}${renderLiveMementoPaperDesignCandidate()}${groups.map(group=>renderCanonicalIdeaGroup(group,parents,independent)).join("")}${audit}`;
}
...
function initIdeaBriefingMode(){
  ...
  const setMode=(mode)=>{
    ...
    document.querySelectorAll(".human-technical-details,.terminal-technical-audit,.human-lineage-details,.human-complete-intro,.lane-concluded,.lane-assets,.agent-safety-program-fold,.live-paper-design-candidate,.canonical-related-bank,.canonical-closed-idea-bank,.categorized-context-bank").forEach(detail=>{detail.open=mode==="audit";});
  };
  buttons.forEach(button=>button.addEventListener("click",()=>setMode(button.dataset.briefingMode||"brief")));
  setMode("brief");
}
function renderPortfolioDecisionConsole(config,inventory) {
  const state=window.RESEARCH_ITEM_STATE||{}, summary=state.summary||{}, items=state.research_items||[];
  const current=items.filter(row=>["HOLD","PAPER_READY"].includes(row.scientific_state)).sort((a,b)=>{
    const rank=s=>s==="PAPER_READY"?0:s==="HOLD"?1:2;
    return rank(a.scientific_state)-rank(b.scientific_state)||String(a.code||"").localeCompare(String(b.code||""),undefined,{numeric:true});
  });
  const stateLabel=value=>language==="zh"?({PAPER_READY:"已进入论文",HOLD:"等待重开条件"}[value]||value):String(value||"").replaceAll("_"," ");
  const compact=(value,n=128)=>{const text=String(value||"");return text.length>n?`${text.slice(0,n-1)}…`:text;};
  const cards=current.map(row=>{
    const action=row.primary_next_action||{}, paperId=action.paper_id||"";
    const actionText=language==="zh"?(action.action_zh||row.reopen_condition_zh||row.reopen_condition?.zh||row.reopen_condition||""):(action.action||row.reopen_condition?.en||row.reopen_condition||"");
    const title=textOf(row.title||{})||row.code||"--";
    return `<a class="portfolio-attention-card state-${String(row.scientific_state||"").toLowerCase()}" href="paper-ideas.html?research=${encodeURIComponent(row.code||"")}"><header><span>${esc(row.code||"")}</span><em>${esc(stateLabel(row.scientific_state))}</em></header><b>${esc(title)}</b><p>${esc(compact(actionText))}</p>${paperId?`<small>PaperState → ${esc(paperId)}</small>`:""}</a>`;
  }).join("");
  const actionCounts=summary.primary_next_action_counts||{}, holds=summary.scientific_state_counts?.HOLD||0;
  const memento=window.MEMENTO_JOINT_IDENTIFIABILITY_PAPER_DESIGN||{};
  const mementoTitle=language==="zh"?(memento.title?.zh||memento.title?.en||""):(memento.title?.en||memento.title?.zh||"");
  return `<section class="portfolio-decision-console" id="portfolio-current"><header class="portfolio-decision-head"><div><div class="eyebrow">${language==="zh"?"当前科研 · 先决策后审计":"CURRENT RESEARCH · DECISIONS FIRST"}</div><h1>${language==="zh"?`先看 ${current.length} 个当前需要关注对象，再进入 A–G 全账本`:`Start with ${current.length} current attention items, then enter the full A–G ledger`}</h1><p>${language==="zh"?`这页默认回答“现在看什么、为什么停或继续、下一步是什么”。${inventory.total} 个完整对象仍全部保留，但 STOP / MERGED、历史 P0、技术证据和对象计数关系默认收进二级审计层。`:`The default view answers what needs attention, why a line stopped or continues, and what happens next. All ${inventory.total} objects remain available, while STOP/MERGED history, P0 lineage, technical evidence, and accounting stay in the audit layer.`}</p></div><nav><a href="#canonical-category-index">${language==="zh"?"看 A–G 总览 ↓":"A–G overview ↓"}</a><a href="#portfolio-audit">${language==="zh"?"审计与历史 ↓":"Audit & history ↓"}</a></nav></header><div class="portfolio-decision-metrics"><span><b>${current.length}</b>${language==="zh"?"当前关注对象":"current attention"}</span><span><b>${holds}</b>${language==="zh"?"等待重开条件":"waiting to reopen"}</span><span><b>${actionCounts.PAPERSTATE_HANDOFF||0}</b>PaperState handoff</span><span><b>${summary.machine_actionable_research_items||0}</b>${language==="zh"?"machine-actionable":"machine-actionable"}</span></div><div class="portfolio-attention-grid">${cards}</div>${mementoTitle?`<a class="portfolio-outside-candidate" href="#live-memento-paper-design"><span>${language==="zh"?"A–G 外 · Paper Design 候选":"OUTSIDE A–G · PAPER DESIGN CANDIDATE"}</span><b>${esc(mementoTitle)}</b><small>${language==="zh"?"已通过 ProblemGate，但新的 F0 尚未获得执行权限；点开查看冻结控制与 GO/STOP。":"ProblemGate passed, but the new F0 still lacks execution authority; open the frozen control and GO/STOP contract."}</small></a>`:""}<footer><span>${language==="zh"?`完整账本：${inventory.total} 个对象 · 26 个父级方向 · 正式实验权限=${summary.current_formal_experiment_authority||0}`:`Full ledger: ${inventory.total} objects · 26 parent directions · formal experiment authority=${summary.current_formal_experiment_authority||0}`}</span><a href="selected-paper.html">${language==="zh"?"打开当前论文合集 →":"Open current paper collection →"}</a></footer></section>`;
}
...
function buildToc() {
  ...
  if (!container) return;
  if (pageId === "paper-ideas") {
    const categories=[...document.querySelectorAll('.canonical-idea-group')].map(section=>({id:section.id,label:(section.querySelector('.canonical-group-header h2')?.textContent||section.dataset.canonicalGroup||'').trim()}));
    const rows=[
      {id:'portfolio-current',label:language==='zh'?'当前需要看什么':'Current attention',children:[]},
      {id:'canonical-category-index',label:language==='zh'?'A–G 研究组合':'A–G portfolio',children:categories},
      {id:'portfolio-audit',label:language==='zh'?'审计与历史':'Audit & history',children:[]},
    ];
    container.innerHTML=`<div class="toc-title">${language==='zh'?'本页阅读路径':'Reading path'}</div><nav class="toc-tree portfolio-toc-tree" aria-label="${language==='zh'?'研究组合阅读路径':'Research Portfolio reading path'}"><ul>${rows.map(row=>`<li class="toc-node toc-level-2"><a href="#${esc(row.id)}">${esc(row.label)}</a>${row.children.length?`<ul>${row.children.map(child=>`<li class="toc-node toc-level-3"><a href="#${esc(child.id)}">${esc(child.label)}</a></li>`).join('')}</ul>`:''}</li>`).join('')}</ul></nav>`;
    return;
  }
  if (pageId === "selected-paper") {
    ...
    container.innerHTML = `<div class="toc-title">${language === "zh" ? "论文合集" : "Paper collection"}</div><nav class="toc-tree paper-toc-tree" aria-label="${language === "zh" ? "论文合集目录" : "Paper collection contents"}"><ul>${rows.map(row=>`<li class="toc-node toc-level-2"><a href="#${row.id}">${esc(row.label)}</a></li>`).join("")}</ul></nav>`;
    return;
  }
...
function enhanceExperimentCostPriceColumns(root) {
  ...
  const price = (paper, model) => {
    ...
    if (model === "Hosted Qwen API") return {used:`<b>¥0</b>`, plan:`<b>${msg("具体模型未冻结 · 先做阶梯预算","Exact model not frozen · tier-aware budget first")}</b><small>${msg("P0 预计 52.8–105.6M 输入 Token + 0.27–0.55M 输出 Token。不能只报最低档。若仅把 qwen3.7-flash 当预检参考：≤32K 约 ¥10.8–21.6；32–256K 约 ¥32.3–64.7；256K–1M 约 ¥64.7–129.4。真正的 P0 成本在具体模型 / 服务商冻结后按每次请求的输入 Token 数分档求和；P1–P3 再用 P0 请求长度分布重估。","P0 is estimated at 52.8–105.6M input + 0.27–0.55M output. Do not report only the cheapest tier. Using qwen3.7-flash purely as a preflight reference: ≤32K ≈¥10.8–21.6; 32–256K ≈¥32.3–64.7; 256K–1M ≈¥64.7–129.4. Actual P0 cost will be summed call-by-call after the exact model/provider freeze; P1–P3 will be re-estimated from the P0 context histogram.")}</small>`};
...
function renderPage() {
  ...
  else if (pageId === "selected-paper") root.innerHTML = window.renderCurrentPaperCollection ? window.renderCurrentPaperCollection(config) : `${pageHeader(config)}<div class="empty">Paper collection unavailable.</div>`;
