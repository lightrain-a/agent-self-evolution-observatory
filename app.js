...
const PARENT_SIMPLE_COMPARISONS_ZH = {
  ...
  "A-5":{ours:"用 Prompt／Memory／Workflow 语义压缩 40 次连续更新的历史，并保持选择性回滚。",baseline:"方法一只保存通用状态差分，不理解 Agent 语义；方法二按固定间隔保存检查点。",matched:"同一 40 次更新、12 个冻结回滚查询；检查点方法使用与我们约 72 cells 相同的存储预算。",rows:[{metric:"回滚正确",ours:"12/12",baseline:"12/12",delta:"0"},{metric:"存储（状态差分）",ours:"73 cells",baseline:"38 cells",delta:"简单方法少 35 cells（47.9%）"},{metric:"平均回放（检查点）",ours:"6.5 段",baseline:"2.25 次更新",delta:"简单方法少 4.25（65.4%）"}],verdict:"两种通用版本管理方法都同样正确，而且一个更省存储、一个更少回放，所以不再主张 Agent 专用历史压缩。"},
...
const SUPPLEMENTAL_SIMPLE_COMPARISONS_ZH = {
  ...
  "active-causal-minimal-rollback":{ours:"主动选择干预组合，利用已经观察到的结果学习下一次测试，寻找最小故障更新集。",baseline:"不学习，只用标准 binary group testing 按稀疏故障假设二分测试。",matched:"24 个冻结的 4–8 update 稀疏故障案例；两边看到相同信息并按逐例测试次数比较。",rows:[{metric:"找对最小故障集",ours:"24/24",baseline:"24/24",delta:"0"},{metric:"平均测试次数",ours:"5.708",baseline:"5.708",delta:"0"},{metric:"配对检验",ours:"p=1.0",baseline:"逐例完全一致",delta:"无差异"}],verdict:"相对 ddmin 的 62% 节省确实存在，但它全部来自标准稀疏 group testing，不来自主动因果学习。"},
