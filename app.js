...
function renderP0ExperimentEntry() {
  ...
  const registry = experimentPilotRegistry();
  return `<section class="panel p0-entry-panel"><div><div class="eyebrow">P0 · ${language === "zh" ? "实验入口" : "EXPERIMENT TRACKER"}</div><h3 id="experiment-tracker-entry" data-toc="false">${language === "zh" ? "20 个活跃方向已进入 P0；执行门禁与真实进展在实验页统一维护" : "All 20 active directions are in P0; launch gates and real progress live on the experiment page"}</h3><p>${language === "zh" ? "Idea 页只保留科学问题与方法论证。P0 表示方法、最强对照、独立真值、最小实验、Stop 与资源合同已经冻结；是否真正开跑仍由 GPU-0、Pre-P0、8 Gate 和 runtime/throughput 决定。" : "The idea page focuses on the scientific problem and mechanism. P0 means the method, strongest baseline, independent truth, minimum experiment, stop rule, and resource contract are frozen; actual launch still depends on GPU-0, Pre-P0, the eight formal gates, and runtime/throughput."}</p></div><div class="p0-entry-stats"><span><b>${admission.active_p0 || 0}</b>P0</span><span><b>${admission.transitioned_from_p0_ready || 0}</b>${language === "zh" ? "本轮迁入" : "transitioned"}</span><span><b>${admission.settings_complete || 0}</b>${language === "zh" ? "设置完整" : "settings complete"}</span><span><b>${admission.execution_authorized || 0}</b>${language === "zh" ? "执行授权" : "execution authorized"}</span><span><b>${registry.summary?.p1_authorized || 0}</b>P1 ${language === "zh" ? "授权" : "authorized"}</span></div><a class="link-btn p0-entry-link" href="experiments.html">${language === "zh" ? "打开实验进展与启动门禁 →" : "Open Experiment Progress & Launch Gates →"}</a></section>`;
}
...
function renderExperimentDashboard(config) {
  ...
  const legacyQueue = `${renderPreP0IdentifiabilityPanel()}${renderP0ExperimentBoard()}`;
  const queue = `${renderTerminalExperimentPortfolio()}${renderP0AdmissionSettingsPanel()}${renderP0OfflineQualificationPanel()}${renderPreGpuCandidateGateBoard()}${renderP0RuntimeReadiness()}${renderLegacyExperimentArchive(legacyQueue,"历史 Pre-P0 / 旧 P0 计划","Historical Pre-P0 / legacy P0 plans","只作追溯：其中包含后来已合并、已 pivot 或不再代表当前 20 个终态方向的旧计划。","Traceability only: this includes plans that were later merged, pivoted, or no longer represent the current 20 terminal directions.")}`;
  const legacyResults = `${renderExperimentResourceLedger()}${renderExperimentResultsSnapshot()}`;
  const results = `${renderExperimentIterationPanel()}${renderLegacyExperimentArchive(legacyResults,"旧 Pilot Registry 结果与资源账本","Legacy Pilot Registry results and ledger","该账本只覆盖旧冻结计划的 registry，不代表 mem-xfer 等独立工作流的全部真实运行。","This ledger covers the legacy frozen-plan registry only; it is not the complete accounting for standalone workflows such as mem-xfer.")}`;
  const approvals = `${renderTerminalUnstartedAuditPanel()}${renderLegacyExperimentArchive(renderExperimentApprovalPanel(),"旧 P0→P1 审批锁","Legacy P0→P1 approval lock","保留旧 registry 的人工审批机制；当前首先审计上面的 16 个新 P0 execution gate。","Preserves the legacy registry approval mechanism; the immediate task is to clear execution gates for the 16 newly admitted P0s above.")}`;
