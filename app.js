...
function renderProjectStatusStrip(){
  ...
  if(pageId==="system-overview") {
    ...
    const labels=language==="zh"
      ? [[`STRI · ${stri.current_state||"--"}`,striHuman],[`Agent Safety · ${safety.current_state||"--"}`,safetyHuman],["真正投稿就绪",acceptance.summary?.submission_ready_papers??index.summary?.submission_ready??0],["科学暂缓论文",acceptance.summary?.scientific_holds??index.summary?.scientific_holds??0],["可启动正式实验",h.launchable_formal_experiments||0]]
      : [["STRI",stri.current_state||"--"],["Agent Safety",safety.current_state||"--"],["Submission-ready",acceptance.summary?.submission_ready_papers??index.summary?.submission_ready??0],["Scientific holds",acceptance.summary?.scientific_holds??index.summary?.scientific_holds??0],["Launchable experiments",h.launchable_formal_experiments||0]];
    const message=language==="zh"
      ? `STRI 与边界冻结的 Agent Safety R9 论文都已完成论文硬门并达到 SUBMISSION_READY。Agent Safety 的同 schedule no-update 对照关闭了原先的论文因果证据 HOLD；更宽的 G-1 复现/支持研究线仍保持 HOLD。真实 SUBMITTED 继续要求外部人工投稿权限。`
      : `Both STRI and the bounded Agent Safety R9 paper have closed their paper gates and reached SUBMISSION_READY. Agent Safety's same-schedule no-update control closed the paper's recorded causal-evidence hold; the broader G-1 replication/support research program remains HOLD. Real SUBMITTED still requires external human submission authority.`;
