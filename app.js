...
function renderProjectStatusStrip(){
  ...
  const message=selectedPaper
    ...
    : (language==="zh"?`截至 2026-08-16：STRI Paper Quality v2 已 PASS，evidence debt=${h.paper_quality_evidence_debt||0}，paper-ready=${h.paper_ready||0}；canonical live idea=${h.canonical_live_ideas||0}。memory positive residual 已归档，active mechanism seed=0、shadow qualification=${h.shadow_qualification_ready||0}；只有新的 primary evidence 满足 reopen receipt 才能重开。旧 P0 lifecycle 仍是终态历史账本。`:`As of 2026-08-16: STRI passes Paper Quality v2 with evidence debt=${h.paper_quality_evidence_debt||0} and paper-ready=${h.paper_ready||0}; canonical live ideas=${h.canonical_live_ideas||0}. The memory positive residual is archived with active mechanism seed=0 and shadow qualification=${h.shadow_qualification_ready||0}; reopening requires new primary evidence satisfying the receipt. Legacy P0 remains a terminal ledger.`);
  return `<section class="project-status-strip current"><div><b>${selectedPaper?(language==="zh"?"当前选中论文 · STRI":"Current selected paper · STRI"):(language==="zh"?"当前科研状态 · 2026-08-16":"Current research state · 2026-08-16")}</b><span>${message}</span></div><dl><div><dt>paper-ready</dt><dd>${h.paper_ready||0}</dd></div><div><dt>evidence debt</dt><dd>${h.paper_quality_evidence_debt||0}</dd></div><div><dt>canonical live</dt><dd>${h.canonical_live_ideas||0}</dd></div><div><dt>${language==="zh"?"formal 可启动":"formal launchable"}</dt><dd>${h.launchable_formal_experiments||0}</dd></div><div><dt>shadow dead-end</dt><dd>${h.shadow_dead_ends||0}</dd></div><div><dt>shadow HOLD</dt><dd>${h.shadow_holds||0}</dd></div><div><dt>${language==="zh"?"shadow 可资格化":"shadow qualification"}</dt><dd>${h.shadow_qualification_ready||0}</dd></div></dl></section>`;
}
