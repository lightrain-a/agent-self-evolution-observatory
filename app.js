...
function renderExperimentDashboard(config) {
  ...
  const approvals = `${renderTerminalUnstartedAuditPanel()}${renderLegacyExperimentArchive(renderExperimentApprovalPanel(),"旧 P0→P1 审批锁","Legacy P0→P1 approval lock","保留旧 registry 的人工审批机制；当前首先审计上面的 16 个未启动 P0-ready。","Preserves the legacy registry approval mechanism; the immediate task is to audit the 16 unstarted P0-ready directions above.")}`;
