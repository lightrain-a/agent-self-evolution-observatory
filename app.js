...
function renderProjectStatusStrip(){
  ...
  const message=selectedPaper
    ...
    : (language==="zh"?`截至 ${asOf||"当前"}：STRI 已达到投稿就绪；还缺的论文证据项=${h.paper_quality_evidence_debt||0}；通过正式问题检查、可以继续进入方法设计的新研究问题=${h.canonical_live_ideas||0}；现在允许正式启动的实验=${h.launchable_formal_experiments||0}。另有 ${h.fresh_active_f0||0} 个新现象正在做最小验证，${h.fresh_support_holds||0} 个因为缺关键证据暂缓。以前的记忆效应只保留为历史观察；暂定候选中已关闭=${h.shadow_dead_ends||0}，等待具体证据=${h.shadow_holds||0}。`:`As of ${asOf||"now"}: STRI is submission-ready; unfinished paper-evidence items=${h.paper_quality_evidence_debt||0}; new ideas that passed the formal problem check=${h.canonical_live_ideas||0}; formal experiments allowed to launch now=${h.launchable_formal_experiments||0}. ${h.fresh_active_f0||0} fresh phenomena are in minimal validation and ${h.fresh_support_holds||0} are waiting for specific evidence. The earlier memory effect is historical only; tentative candidates closed=${h.shadow_dead_ends||0}, waiting for evidence=${h.shadow_holds||0}.`);
  const statusLabels = language === "zh"
    ? [["投稿就绪论文",h.paper_ready||0],["还缺的论文证据",h.paper_quality_evidence_debt||0],["通过正式问题检查的新研究问题",h.canonical_live_ideas||0],["正在做最小验证的新现象",h.fresh_active_f0||0],["因缺证据暂缓的新现象",h.fresh_support_holds||0],["现在允许启动的正式实验",h.launchable_formal_experiments||0],["已关闭的暂定候选",h.shadow_dead_ends||0],["等待具体证据的暂定候选",h.shadow_holds||0]]
    : [["submission-ready papers",h.paper_ready||0],["unfinished paper evidence",h.paper_quality_evidence_debt||0],["new ideas past formal problem check",h.canonical_live_ideas||0],["fresh phenomena in minimal validation",h.fresh_active_f0||0],["fresh phenomena waiting for evidence",h.fresh_support_holds||0],["formal experiments launchable now",h.launchable_formal_experiments||0],["closed tentative candidates",h.shadow_dead_ends||0],["tentative candidates waiting for evidence",h.shadow_holds||0]];
