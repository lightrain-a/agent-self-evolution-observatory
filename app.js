...
function renderHomeResearchConsole() {
  ...
  const holdCards=holds.map(row=>`<article class="home-attention-row" data-dashboard-research="${esc(row.code)}"><header><span>${esc(row.code)}</span><b>${esc(title(row))}</b><em>${esc(dashboardStateLabel(row.scientific_state))}</em></header><p>${esc(brief(row))}</p><small><b>${language==="zh"?"重开前先做：":"Before reopening: "}${esc(nextClass(row))}</b> · ${esc(next(row))}</small><footer><a href="${esc(row.portfolio_href||"paper-ideas.html")}">${language==="zh"?"完整 ResearchItem":"Full ResearchItem"}</a><a href="${esc(row.timeline_href||"research-timeline.html")}">${language==="zh"?"时间线":"Timeline"}</a>${row.paper_href?`<a href="${esc(row.paper_href)}">PaperState</a>`:""}</footer></article>`).join("");
