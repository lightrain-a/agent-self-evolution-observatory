...
function renderHome(config) {
  ...
  const stats = `<div class="grid"><div class="stat"><b>${catalog.length || DATA.length}</b><span>${language === "zh" ? "篇去重研究条目" : "deduplicated research records"}</span></div><div class="stat"><b>${portfolioDirections().length || 10}</b><span>${language === "zh" ? "个历史问题方向" : "historical problem directions"}</span></div><div class="stat"><b>${homePaperSummary.gate_clean_submission_ready ?? homeDashboardSummary.submission_ready ?? homePaperSummary.submission_ready ?? 0}</b><span>${language === "zh" ? "篇最新门禁可投稿" : "latest gate-clean papers"}</span></div><div class="stat"><b>${homeStatus.canonical_live_ideas ?? 0}</b><span>${language === "zh" ? "个正式活跃 ResearchItem" : "active ResearchItems"}</span></div></div>`;
