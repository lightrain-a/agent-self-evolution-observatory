...
function renderResearchSystemState() {
  ...
  const componentRows = (state.components || []).map((item) => `<article class="automation-component tone-${esc(item.status)}"><header><b>${esc(item.source)}</b><span>${esc(item.status)}</span></header><h4 data-toc="false">${esc(item.component)}</h4><p>${esc(item.evidence)}</p></article>`).join("");
...
function renderIclrIdeaBank() {
  ...
  const topRows = ideas.slice(0,15).map((idea) => `<tr><td><strong>#${idea.rank}</strong></td><td><a href="#iclr-${esc(idea.id)}" class="iclr-jump" data-iclr-id="${esc(idea.id)}"><b>${textOf(idea.title)}</b></a><small>${textOf(idea.track)}</small></td><td>${textOf(idea.purpose)}</td><td>${textOf(idea.core_idea)}</td><td>${(idea.domains || []).map(esc).join(" · ")}</td><td>${idea.budget.max_gpus} GPU · ${idea.budget.gpu_hours}h</td><td>${idea.priority}</td></tr>`).join("");
