...
function renderProjectStatusStrip(){
  const state=projectStatusState(), h=state.headline||{};
  if(!Object.keys(h).length) return "";
  const selectedPaper=pageId==="selected-paper";
  const message=selectedPaper
    ? (language==="zh"?"当前选中论文是 STRI：Paper Quality v2 已 PASS、evidence debt=0、paper-ready=1；当前只剩 human author signoff 与 OpenReview handoff。旧 Regression-Gated formulation 已下沉为本页历史归档。":"The current selected paper is STRI: Paper Quality v2 passes with evidence debt=0 and paper-ready=1; only human author signoff and OpenReview handoff remain. The former Regression-Gated formulation is retained below as a historical archive.")
    : (language==="zh"?`截至 2026-08-17：STRI Paper Quality v2 已 PASS，evidence debt=${h.paper_quality_evidence_debt||0}，paper-ready=${h.paper_ready||0}；canonical live idea=${h.canonical_live_ideas||0}。新的 fresh-phenomenon portfolio 有 ${h.fresh_active_f0||0} 条 ACTIVE_F0、${h.fresh_support_holds||0} 条 support HOLD；ACTIVE_F0 只做 generic-reduction falsifier，尚未进入 Problem Gate。memory positive residual 已归档，shadow qualification=${h.shadow_qualification_ready||0}。`:`As of 2026-08-17: STRI passes Paper Quality v2 with evidence debt=${h.paper_quality_evidence_debt||0} and paper-ready=${h.paper_ready||0}; canonical live ideas=${h.canonical_live_ideas||0}. The fresh-phenomenon portfolio has ${h.fresh_active_f0||0} ACTIVE_F0 and ${h.fresh_support_holds||0} support holds; ACTIVE_F0 is only a generic-reduction falsifier and has not entered Problem Gate. The memory positive residual is archived; shadow qualification=${h.shadow_qualification_ready||0}.`);
  return `<section class="project-status-strip current"><div><b>${selectedPaper?(language==="zh"?"当前选中论文 · STRI":"Current selected paper · STRI"):(language==="zh"?"当前科研状态 · 2026-08-17":"Current research state · 2026-08-17")}</b><span>${message}</span></div><dl><div><dt>paper-ready</dt><dd>${h.paper_ready||0}</dd></div><div><dt>evidence debt</dt><dd>${h.paper_quality_evidence_debt||0}</dd></div><div><dt>canonical live</dt><dd>${h.canonical_live_ideas||0}</dd></div><div><dt>fresh ACTIVE_F0</dt><dd>${h.fresh_active_f0||0}</dd></div><div><dt>fresh HOLD</dt><dd>${h.fresh_support_holds||0}</dd></div><div><dt>${language==="zh"?"formal 可启动":"formal launchable"}</dt><dd>${h.launchable_formal_experiments||0}</dd></div><div><dt>shadow dead-end</dt><dd>${h.shadow_dead_ends||0}</dd></div><div><dt>shadow HOLD</dt><dd>${h.shadow_holds||0}</dd></div></dl></section>`;
}
...
function renderDiscussedIdeaBank() {
  ...
  return `<section class="panel human-review-overview"><div class="idea-panel-heading"><div><b class="human-overview-kicker">H1 · ${esc(canonicalDate)} · HISTORICAL LINEAGE</b><p class="section-intro">${language === "zh" ? "这一章保留 2026-08-11 人工终态 ledger，用于解释 26 个 parent 为什么曾进入 P0、P0-ready、merge 或 drop；它不是 2026-08-16 的活动 Idea 队列。原始人工意见、历史自动审查和实验 artifact 继续保留，当前是否可执行必须以上方 unified current-status ledger 为准。" : "This chapter preserves the 2026-08-11 human-terminal ledger to explain why 26 parents historically entered P0, P0-ready, merge, or drop states; it is not the active idea queue on 2026-08-16. Original opinions, automated reviews, and experiment artifacts remain traceable, while current executability comes only from the unified current-status ledger above."}</p></div><strong>${all.length} ${language === "zh" ? "个历史 parent" : "historical parents"}</strong></div><div class="human-review-stats">${statuses.map((status) => `<div class="human-stat human-stat-${humanReviewStatusTone(status)}"><b>${counts[status] || 0}</b><span>${esc(humanReviewStatusLabel(status))}</span></div>`).join("")}</div>${finalSummary}</section>${renderHumanReviewMethodology()}${groups}`;
}
...
function renderHome(config) {
  ...
  const stats = `<div class="grid"><div class="stat"><b>${catalog.length || DATA.length}</b><span>${language === "zh" ? "篇去重后的研究条目" : "deduplicated research records"}</span></div><div class="stat"><b>${portfolioDirections().length || 10}</b><span>${language === "zh" ? "个研究方向" : "research directions"}</span></div><div class="stat"><b>${homeStatus.paper_ready ?? 0}</b><span>${language === "zh" ? "篇当前 paper-ready 论文" : "current paper-ready paper"}</span></div><div class="stat"><b>${homeStatus.canonical_live_ideas ?? 0}</b><span>${language === "zh" ? "个 canonical live idea" : "canonical live ideas"}</span></div></div>`;
