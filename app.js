...
function renderProjectStatusStrip(){
  ...
  const message=historical
    ? (language==="zh"?"本页是历史 formulation / protocol 档案。最新状态中 STRI 已完成 Paper Quality v2 evidence loop 并恢复 paper-ready；canonical live queue 仍为 0。":"This is a historical formulation/protocol archive. In the latest state, STRI has closed the Paper Quality v2 evidence loop and is paper-ready again; the canonical live queue remains empty.")
    : (language==="zh"?`截至 2026-08-16：STRI Paper Quality v2 已 PASS，evidence debt=${h.paper_quality_evidence_debt||0}，paper-ready=${h.paper_ready||0}；canonical live idea=${h.canonical_live_ideas||0}。memory positive residual 只允许新的 shadow qualification，不能越过 Problem Gate；旧 P0 lifecycle 仍是终态历史账本。`:`As of 2026-08-16: STRI passes Paper Quality v2 with evidence debt=${h.paper_quality_evidence_debt||0} and paper-ready=${h.paper_ready||0}; canonical live ideas=${h.canonical_live_ideas||0}. The memory positive residual only permits a new shadow qualification and cannot bypass the Problem Gate; legacy P0 remains a terminal ledger.`);
  return `<section class="project-status-strip ${historical?"historical":"current"}"><div><b>${historical?(language==="zh"?"历史工作区 · 当前状态已同步":"Historical workspace · current status synced"):(language==="zh"?"当前科研状态 · 2026-08-16":"Current research state · 2026-08-16")}</b><span>${message}</span></div><dl><div><dt>paper-ready</dt><dd>${h.paper_ready||0}</dd></div><div><dt>evidence debt</dt><dd>${h.paper_quality_evidence_debt||0}</dd></div><div><dt>canonical live</dt><dd>${h.canonical_live_ideas||0}</dd></div><div><dt>${language==="zh"?"formal 可启动":"formal launchable"}</dt><dd>${h.launchable_formal_experiments||0}</dd></div><div><dt>shadow dead-end</dt><dd>${h.shadow_dead_ends||0}</dd></div><div><dt>shadow HOLD</dt><dd>${h.shadow_holds||0}</dd></div><div><dt>${language==="zh"?"shadow 可资格化":"shadow qualification"}</dt><dd>${h.shadow_qualification_ready||0}</dd></div></dl></section>`;
}
