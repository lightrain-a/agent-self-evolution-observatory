...
function renderProjectStatusStrip(){
  ...
  if(pageId==="system-overview") {
    ...
    const paperTotal=index.summary?.papers??entries.length, readyPapers=acceptance.summary?.submission_ready_papers??index.summary?.submission_ready??0, repairPapers=index.summary?.by_state?.TARGETED_REPAIR??0;
    const labels=language==="zh"
      ? [[`STRI · ${stri.current_state||"--"}`,striHuman],[`Agent Safety · ${safety.current_state||"--"}`,safetyHuman],["PaperState 总数",paperTotal],["真正投稿就绪",readyPapers],["Targeted Repair",repairPapers],["可启动正式实验",h.launchable_formal_experiments||0]]
      : [["STRI",stri.current_state||"--"],["Agent Safety",safety.current_state||"--"],["PaperStates",paperTotal],["Submission-ready",readyPapers],["Targeted Repair",repairPapers],["Launchable experiments",h.launchable_formal_experiments||0]];
    const message=language==="zh"
      ? `Canonical Paper Acceptance 当前共有 ${paperTotal} 篇论文：${readyPapers} 篇达到 SUBMISSION_READY，${repairPapers} 篇仍在 TARGETED_REPAIR。STRI 与 Agent Safety 保留 ResearchItem 父项；新增 D2 PaperState 直接保留 paper-first discovery provenance。论文就绪不等于外部 evidence/support debt 消失，真实 SUBMITTED 继续要求人工投稿权限。`
      : `Canonical Paper Acceptance currently contains ${paperTotal} papers: ${readyPapers} are at SUBMISSION_READY and ${repairPapers} remain in TARGETED_REPAIR. STRI and Agent Safety retain ResearchItem parents, while the D2 PaperStates preserve paper-first discovery provenance. Manuscript readiness does not erase external evidence/support debt, and real SUBMITTED still requires human submission authority.`;
  ...
  const selectedPaperCount=paperSummary.papers??selectedRegistryPapers.length, selectedReadyCount=paperSummary.submission_ready??0, selectedRepairCount=paperSummary.by_stage?.TARGETED_REPAIR??0;
  const selectedMessage=selectedPaper?(language==="zh"?`PaperRegistry 当前有 ${selectedPaperCount} 篇正式 PaperState：${selectedReadyCount} 篇达到 SUBMISSION_READY，${selectedRepairCount} 篇仍在 TARGETED_REPAIR。左侧一级目录按全部 canonical paper ledger 拆分；D2 论文保留 paper-first discovery candidate 来源，不伪造 A–G 父项。真实 SUBMITTED 仍需要人工 OpenReview 提交。`:`PaperRegistry currently contains ${selectedPaperCount} formal PaperStates: ${selectedReadyCount} are at SUBMISSION_READY and ${selectedRepairCount} remain in TARGETED_REPAIR. The left hierarchy follows every canonical paper ledger; D2 papers retain paper-first discovery-candidate provenance rather than fake A–G parents. Real SUBMITTED still requires human OpenReview action.`):message;
  const selectedStatusLabels=selectedPaper?(language==="zh"?[["PaperState 总数",selectedPaperCount],["真正 Submission Ready",selectedReadyCount],["Targeted Repair",selectedRepairCount],["STRI",paper.paper_stage||paper.current_state||"--"],["Agent Safety R9",selectedSafetyPaper.paper_stage||selectedSafetyPaper.current_state||"--"],["Invalid Ledger",acceptanceSummary.invalid_ledgers??acceptanceIndex.summary?.invalid_ledgers??0]]:[["PaperStates",selectedPaperCount],["Truly submission-ready",selectedReadyCount],["Targeted Repair",selectedRepairCount],["STRI",paper.paper_stage||paper.current_state||"--"],["Agent Safety R9",selectedSafetyPaper.paper_stage||selectedSafetyPaper.current_state||"--"],["Invalid ledgers",acceptanceSummary.invalid_ledgers??acceptanceIndex.summary?.invalid_ledgers??0]]):statusLabels;
...
function renderHumanReviewedIdeaCard(idea, meta, index) {
  ...
  const oneMinute=parentOneMinuteCopy({idea,current,terminal,currentStatus,disposition,lifecycleStage,plainReopen,decisiveEvidence});
...
function renderHomeResearchConsole() {
  ...
  const holdCards=holds.map(row=>`<article class="home-attention-row" data-dashboard-research="${esc(row.code)}"><header><span>${esc(row.code)}</span><b>${esc(title(row))}</b><em>${esc(dashboardStateLabel(row.scientific_state))}</em></header><p>${esc(brief(row))}</p><small><b>${language==="zh"?"重开前先做：":"Before reopening: "}</b>${esc(next(row))}</small><footer><a href="${esc(row.portfolio_href||"paper-ideas.html")}">${language==="zh"?"完整 ResearchItem":"Full ResearchItem"}</a><a href="${esc(row.timeline_href||"research-timeline.html")}">${language==="zh"?"时间线":"Timeline"}</a>${row.paper_href?`<a href="${esc(row.paper_href)}">PaperState</a>`:""}</footer></article>`).join("");
  ...
  const readyHeadline=language==="zh"?(s.submission_ready?`${s.submission_ready} 篇论文已投稿就绪`:`${s.paper_ready||0} 条论文线在推进`):(s.submission_ready?`${s.submission_ready} paper is submission-ready`:`${s.paper_ready||0} paper line advancing`);
  const readyKpiValue=s.submission_ready||s.paper_ready||0, readyKpiLabel=language==="zh"?(s.submission_ready?"篇真正投稿就绪":"条科学证据闭环论文线"):(s.submission_ready?"truly submission-ready":"paper-ready research line");
  return `<section class="home-research-console" id="current-research-console"><header class="home-console-head"><div><div class="eyebrow">${language==="zh"?"当前研究组合 · 30 秒控制台":"CURRENT PORTFOLIO · 30-SECOND CONSOLE"} · ${esc(d.as_of_date||"")}</div><h2 data-toc="false">${readyHeadline}${language==="zh"?`，${s.holds||0} 个问题等待明确条件，${s.launchable_formal_experiments||0} 个正式实验可启动`:`; ${s.holds||0} problems waiting on explicit conditions; ${s.launchable_formal_experiments||0} formal experiments launchable`}</h2><p>${language==="zh"?"先看真正需要行动的对象，不从 91 个历史/终态记录开始翻。这里是 ResearchItem、PaperRegistry 和 Timeline 的只读摘要；任何状态变化仍以原账本为准。":"Start with the objects that actually require action rather than browsing all 91 historical/terminal records. This is a read-only summary of ResearchItem, PaperRegistry, and Timeline; source ledgers remain authoritative."}</p></div><nav><a href="research-map.html">${language==="zh"?"当前全景":"Current map"}</a><a href="paper-ideas.html">${language==="zh"?"完整研究组合":"Full portfolio"}</a><a href="selected-paper.html">Papers</a></nav></header><div class="home-console-kpis"><span><b>${s.current_attention||attention.length}</b>${language==="zh"?"个当前关注对象":"current attention"}</span><span><b>${readyKpiValue}</b>${readyKpiLabel}</span><span><b>${s.holds||0}</b>${language==="zh"?"个可条件重开 HOLD":"conditional HOLDs"}</span><span><b>${s.launchable_formal_experiments||0}</b>${language==="zh"?"个可启动正式实验":"launchable formal experiments"}</span></div><div class="home-console-grid"><section class="home-console-current"><div class="home-console-section-title"><div><b>${language==="zh"?"现在真正推进什么":"What is actually advancing"}</b><span>${language==="zh"?"科学证据和论文接受流程分开看":"separate scientific evidence from paper acceptance"}</span></div><strong>${esc(primaryPaper)}</strong></div><article class="home-primary-paper" data-dashboard-research="${esc(primary.code)}"><header><span>${esc(primary.code)} → ${esc(primary.paper_id||"PaperState")}</span><em>${esc(dashboardPaperStageLabel(primary.paper_stage))}</em></header><h3 data-toc="false">${esc(title(primary))}</h3><p>${esc(brief(primary))}</p><div><b>${language==="zh"?"当前下一步":"Current next step"}</b><span>${esc(next(primary))}</span></div><footer><a class="link-btn" href="${esc(primary.paper_href||"selected-paper.html")}">${language==="zh"?"打开 PaperState →":"Open PaperState →"}</a><a class="link-btn" href="${esc(primary.timeline_href||"research-timeline.html")}">${language==="zh"?"看完整时间线 →":"Open timeline →"}</a></footer></article><div class="home-console-section-title home-hold-title"><div><b>${language==="zh"?"其余为什么没有继续跑":"Why the remaining lines are not running"}</b><span>${language==="zh"?"HOLD 不是原理失败；每条都必须先满足自己的重开条件":"HOLD is not a principle failure; each line has an explicit reopen condition"}</span></div><strong>${holds.length} HOLD</strong></div><div class="home-attention-list">${holdCards}</div></section><aside class="home-console-week"><header><div><b>${language==="zh"?"最近一周发生了什么":"What changed this week"}</b><span>${esc(weekRange)}</span></div><a href="${esc(week.timeline_href||"research-timeline.html")}">${language==="zh"?"完整时间线 →":"Full timeline →"}</a></header><div class="home-week-kpis"><span><b>${week.research_days||0}</b>${language==="zh"?"研究日":"research days"}</span><span><b>${week.substantive_events||0}</b>${language==="zh"?"科研相关事件":"research events"}</span><span><b>${week.key_changes||0}</b>${language==="zh"?"关键变化":"key changes"}</span></div><div class="home-week-highlights">${highlights||`<div class="empty">${language==="zh"?"暂无周级摘要":"No weekly summary yet"}</div>`}</div><div class="home-week-note"><b>${language==="zh"?"读法":"How to read this"}</b><p>${language==="zh"?"周摘要只用来回答“最近哪里发生了变化”；具体结果、失败层和科研权限仍回到对应 ResearchItem / PaperState 核查。":"The weekly strip only answers where change happened. Exact results, failure layers, and authority still resolve to the linked ResearchItem or PaperState."}</p></div></aside></div></section>`;
}
function renderHome(config) {
  ...
  const stats = `<div class="grid"><div class="stat"><b>${catalog.length || DATA.length}</b><span>${language === "zh" ? "篇去重研究条目" : "deduplicated research records"}</span></div><div class="stat"><b>${portfolioDirections().length || 10}</b><span>${language === "zh" ? "个历史问题方向" : "historical problem directions"}</span></div><div class="stat"><b>${homeDashboardSummary.submission_ready ?? homePaperSummary.submission_ready ?? 0}</b><span>${language === "zh" ? "篇真正投稿就绪" : "truly submission-ready papers"}</span></div><div class="stat"><b>${homeStatus.canonical_live_ideas ?? 0}</b><span>${language === "zh" ? "个正式活跃 ResearchItem" : "active ResearchItems"}</span></div></div>`;
  ...
  return `${pageHeader(config)}${renderHomeResearchConsole()}${stats}${renderArchitectureOverview(pageArchitecture("home"))}${renderCustomChapter(chapters[0],0,understand)}${renderCustomChapter(chapters[1],1,select)}${renderCustomChapter(chapters[2],2,execute)}`;
}
...
function renderBibliography(config) {
  ...
  const refreshLog = refreshMeta.verified_at ? `<section class="panel bibliography-refresh-log"><div><div class="eyebrow">${language === "zh" ? "最近一次增量核验" : "LATEST INCREMENTAL VERIFICATION"} · ${esc(refreshMeta.verified_at)}</div><h3 data-toc="false">${language === "zh" ? `本轮新增 ${(Number(openAlexDelta.added)||0)+(Number(s2Delta.added)||0)} 篇，并核对 ${Number(s2Delta.updated)||0} 个既有版本更新` : `${(Number(openAlexDelta.added)||0)+(Number(s2Delta.added)||0)} papers added, ${Number(s2Delta.updated)||0} existing record updated`}</h3><p>${esc(textOf(refreshMeta.note))}</p></div><div class="bibliography-refresh-methods"><article><b>OpenAlex + arXiv</b><strong>+${Number(openAlexDelta.added)||0}</strong><span>${esc(openAlexDelta.method||"")}</span></article><article><b>Semantic Scholar + arXiv</b><strong>+${Number(s2Delta.added)||0} / Δ${Number(s2Delta.updated)||0}</strong><span>${esc(s2Delta.method||"")}</span></article></div><div class="bibliography-refresh-chips">${(refreshMeta.key_deltas||[]).map(name=>`<span>${esc(name)}</span>`).join("")}</div></section>` : "";
