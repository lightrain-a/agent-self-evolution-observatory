(() => {
  ...
  window.renderAdvisorPriorityIdeas = function renderAdvisorPriorityIdeas() {
    ...
    return `<section class="panel advisor-priority-panel"><div class="idea-panel-heading"><div><h3 id="advisor-priority-shortlist">${language === "zh" ? "师兄正式讨论池：22 个严格 R2 PASS" : "Senior discussion pool: 22 strict R2 PASS ideas"}</h3><p class="section-intro">${language === "zh" ? "这 22 个方向全部达到此前约定的讨论门槛，都会交给师兄判断，不再由系统先删减。相对元审查只负责排序、标注重叠关系和 P0 优先级，帮助师兄更快浏览。" : "All 22 directions meet the agreed discussion threshold and are presented for senior review; the system does not remove any of them. The comparative meta-review only ranks them, marks overlap, and adds P0 priority to make review faster."}</p></div><strong>${pool.length}/${d.discussion_target||20} ${language === "zh" ? "正式讨论候选" : "discussion-ready"}</strong></div>
