(() => {
  ...
  const tone = status => {
    ...
    if(s.includes("INCONCLUSIVE") || s.includes("NEXT") || s.includes("NO_NEW_DATA") || s.includes("CONDITIONAL") || s.includes("WAIT_PROVIDER_RESET") || s.includes("REQUALIFICATION")) return "check";
  ...
  const statusLabel = status => {
    ...
    return ({
      ...
      FAIL_CLOSED_RECOVERY_REVIEW:"Fail-closed · 恢复审查中",
      FAIL_CLOSED_TECHNICAL_MISSING_RECOVERY_REVIEW:"Fail-closed · technical-missing 恢复审查中",
      R3_RECOVERY_READY_WAIT_PROVIDER_RESET:"R3 recovery 已就绪 · 等 provider reset",
      R3D_CONTROL_PLANE_PASS_WAIT_PROVIDER_RESET:"R3D control-plane 已 PASS · 等 provider reset",
      PASS_R3D_CONTROL_PLANE_AND_HOST69_REPLAY_WAIT_PROVIDER_RESET:"R3D code/replay 已 PASS · 等 provider reset",
      R3_REQUALIFICATION_WAIT_PROVIDER_RESET:"R3D fresh identity · 等 provider reset"
  ...
  const authorityBlock = s => {
    ...
    const identityState=(a.r3_fresh_identity_required_after_reset&&!a.r3_recovery_authorized)?pick("首次已执行；R3 fresh requalification 等 reset","initial executed; R3 fresh requalification waits for reset"):(a.fresh_identity_called?pick("已执行","executed"):(a.fresh_identity_qualification_permitted?pick("下一条可执行资格门","next executable qualification gate"):pick("关闭","closed")));
  ...
  const incidentPanel = s => {
    const x=s.stage_a_incident||{};
    if(!x.status)return "";
    return `<section class="e2r17-incident"><header><span>${pick("STAGE-A INCIDENT / R3 RECOVERY","STAGE-A INCIDENT / R3 RECOVERY")}</span>${badge(x.status,statusLabel(x.status))}</header><div class="e2r17-incident-grid"><article><b>${pick("原因 / 时间硬门","Cause / time gate")}</b><p>${esc(x.cause||"--")}</p><small>${pick(`provider reset: ${x.provider_reset_time||"--"}`,`provider reset: ${x.provider_reset_time||"--"}`)}</small></article><article><b>${pick("Fail-closed 冻结","Fail-closed freeze")}</b><p>${pick(`${x.attempted_task_markers||0} attempted · ${x.sealed_task_receipts||0} sealed · ${x.frozen_k8_pools||0} K8 pools · ${x.completed_streams||0} streams`,`${x.attempted_task_markers||0} attempted · ${x.sealed_task_receipts||0} sealed · ${x.frozen_k8_pools||0} K8 pools · ${x.completed_streams||0} streams`)}</p><small>${pick(`burned=${x.burned_task_id||"--"} · matched censor=${x.matched_no_provider_censor_task_id||"--"}`,`burned=${x.burned_task_id||"--"} · matched censor=${x.matched_no_provider_censor_task_id||"--"}`)}</small></article><article><b>${pick("已审 recovery 边界","Reviewed recovery boundary")}</b><p>${esc(language==="zh"?x.proposed_recovery_zh:x.proposed_recovery_en)}</p><small>${pick("replay=false · replacement=false · support unread · Stage B closed","replay=false · replacement=false · support unread · Stage B closed")}</small></article></div></section>`;
  };
  const scopePanel = s => {
    const q=s.premise_scope||{};
    if(!q.title_zh&&!q.title_en)return "";
    return `<section class="e2r17-scope-calibration"><header><span>${pick("PREMISE / SCOPE","PREMISE / SCOPE")}</span><strong>${esc(text(q,"title"))}</strong></header><p>${esc(text(q,"premise"))}</p><div class="e2r17-scope-grid"><article><b>${pick("T_K 到底是什么","What T_K means")}</b><p>${esc(text(q,"tk_definition"))}</p></article><article><b>${pick("不适用对象","Out of scope")}</b><p>${esc(text(q,"out_of_scope"))}</p></article><article><b>${pick("论文 claim 边界","Claim boundary")}</b><p>${esc(text(q,"why_it_matters"))}</p></article></div><div class="e2r17-scope-workflows">${(q.representative_workflows||[]).map(x=>`<span>${esc(x)}</span>`).join("")}</div></section>`;
  };
  const publicPanel = s => {
    const p=s.public_p1||{};
    const baselineRows=[...(p.anchors||[]).map(x=>["Anchor",x]),...(p.closest_baselines||[]).map(x=>["Closest",x])];
    return `<section class="e2r17-public-lane"><header><div><span>C · PUBLIC P1</span><h3 data-toc="false">${esc(p.substrate||"SpreadsheetBench Verified-400")}</h3><p>${esc(text(p,"purpose"))}</p></div>${badge(p.status,statusLabel(p.status))}</header><div class="e2r17-public-grid"><article><b>${pick("进入条件","Entry condition")}</b><p>${esc(text(p,"entry"))}</p><small>${pick("Exact public IDs 当前未冻结；任何 public outcome 前必须固定。","Exact public IDs are not yet frozen and must be pinned before any public outcome.")}</small></article><article><b>${pick("统一 split / 主模型","Unified split / primary model")}</b><p><strong>${esc(p.split_policy||"")}</strong></p><p>${esc(text(p,"primary_model"))}</p></article><article><b>${pick("一次完成两件事","One lane, two purposes")}</b><p>${pick("Natural transport + closest-method baseline comparison 共用同一 split、actor/updater role、harness、evaluator 与 heldout panel。","Natural transport and closest-method comparison share the same split, actor/updater role, harness, evaluator, and heldout panel.")}</p></article></div><aside class="e2r17-public-scope-warning"><b>${pick("Public scope 要求","Public scope requirement")}</b><span>${esc(text(p,"search_workflow_requirement"))}</span></aside><div class="advisor-table-scroll"><table class="matrix e2r17-baseline-table"><thead><tr><th>${pick("层级","Type")}</th><th>Baseline</th><th>${pick("为什么需要","Why it is there")}</th></tr></thead><tbody>${baselineRows.map(([kind,name])=>`<tr><td>${esc(kind)}</td><td><strong>${esc(name)}</strong></td><td>${esc(kind==="Anchor"?pick("能力 / parent / tied-projection / fixed-alternative 锚点","Capability / parent / tied-projection / fixed-alternative anchor"):pick("与当前 self-evolving skill 文献做统一 harness 公平比较","Fair unified-harness comparison against current self-evolving-skill methods"))}</td></tr>`).join("")}<tr><td>Contrastive</td><td><strong>${pick("Trace2Skill（优先）或 SkillCAT-style","Trace2Skill (preferred) or SkillCAT-style")}</strong></td><td>${esc(text(p,"contrastive_baseline"))}</td></tr></tbody></table></div><div class="e2r17-public-footer"><span><b>${pick("统一 heldout","Unified heldout")}</b>${esc(text(p,"evaluation"))}</span><span><b>${pick("Transport STOP","Transport STOP")}</b>${esc(text(p,"transport_stop"))}</span></div></section>`;
  };
  ...
  window.renderE2R17ExperimentPanel = function(){
    ...
    return `<section class="panel e2r17-project-panel" id="e2-r17-current-experiment"><header class="e2r17-project-head"><div><div class="eyebrow">${esc(s.project_track)} · ${pick("当前 Project Track · 实验执行图","CURRENT PROJECT TRACK · EXPERIMENT EXECUTION MAP")} · ${esc(s.as_of_date||"")}</div><h2 data-toc="false">${esc(text(s,"title"))}</h2><p>${esc(text(s,"scientific_object"))}</p></div>${badge(s.stage_a_incident?.status||"R3D_CONTROL_PLANE_PASS_WAIT_PROVIDER_RESET",pick("R3D code/replay PASS · 等 provider reset","R3D code/replay PASS · wait provider reset"))}</header>${authorityBlock(s)}${incidentPanel(s)}${scopePanel(s)}<div class="e2r17-next-gate"><b>${pick("下一条真正可执行门禁","Next executable gate")}</b><span>${esc(language==="zh"?s.next_gate?.zh:s.next_gate?.en)}</span></div>${rqStrip(s)}<h3 class="e2r17-section-title" data-toc="false">${pick("A · 已完成证据：不再重复跑","A · Completed evidence: do not rerun")}</h3><div class="e2r17-evidence-grid">${(s.completed_evidence||[]).map(evidenceCard).join("")}</div><h3 class="e2r17-section-title" data-toc="false">${pick("B · 必跑 Controlled V3：每一层都有硬门","B · Mandatory controlled V3: every stage has a hard gate")}</h3><div class="e2r17-stage-grid">${(s.mandatory_controlled||[]).map(stageCard).join("")}</div>${publicPanel(s)}${optionalPanel(s)}${workloadPanel(s)}<footer class="e2r17-provenance"><span>${pick("Frozen R2","Frozen R2")} · commit ${esc(hash12(s.frozen_scientific_r2?.commit))}… · contract ${esc(hash12(s.frozen_scientific_r2?.contract_sha256))}… · preflight ${esc(hash12(s.frozen_scientific_r2?.preflight_sha256))}…</span><span>${pick("前端更新不改变已冻结 R2 scientific object。R2 Stage-A 首次授权已被 fail-closed run 消费；当前禁止 replay，并且 Stage-B / Public-P1 科学权限仍为 false。","This frontend update does not change the frozen R2 scientific object. The first R2 Stage-A authorization was consumed by a fail-closed run; replay is forbidden and Stage-B / Public-P1 scientific authority remains false.")}</span></footer></section>`;
  };
