(() => {
  ...
  const incidentPanel = s => {
    ...
    if(!x.status)return "";
    return `<section class="e2r17-incident"><header><span>${pick("STAGE-A INCIDENT","STAGE-A INCIDENT")}</span>${badge(x.status,statusLabel(x.status))}</header><div class="e2r17-incident-grid"><article><b>${pick("原因","Cause")}</b><p>${esc(x.cause||"--")}</p><small>${pick(`provider reset: ${x.provider_reset_time||"--"}`,`provider reset: ${x.provider_reset_time||"--"}`)}</small></article><article><b>${pick("Fail-closed 状态","Fail-closed state")}</b><p>${pick(`${x.attempted_task_markers||0} attempted · ${x.sealed_task_receipts||0} sealed · ${x.frozen_k8_pools||0} K8 pools · ${x.completed_streams||0} streams`,`${x.attempted_task_markers||0} attempted · ${x.sealed_task_receipts||0} sealed · ${x.frozen_k8_pools||0} K8 pools · ${x.completed_streams||0} streams`)}</p><small>${pick(`burned task=${x.burned_task_id||"--"}`,`burned task=${x.burned_task_id||"--"}`)}</small></article><article><b>${pick("恢复边界","Recovery boundary")}</b><p>${esc(language==="zh"?x.proposed_recovery_zh:x.proposed_recovery_en)}</p><small>${pick("replay=false · replacement=false · support unread","replay=false · replacement=false · support unread")}</small></article></div></section>`;
  };
  const scopePanel = s => {
