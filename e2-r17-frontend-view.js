(() => {
  ...
  const authorityBlock = s => {
    ...
    return `<div class="e2r17-authority"><div><b>${pick("当前科研执行权限","Current scientific execution authority")}</b><strong>${x.enabled}/${x.total}</strong></div><span>${pick(`fresh identity=${a.fresh_identity_called?"已执行":"未执行"} · Stage A=${a.stage_a?"已授权":"未授权"} · Stage B=${a.stage_b?"已授权":"未授权"} · Public P1=${a.public_p1?"已授权":"未授权"} · baseline=${a.baseline_execution?"已执行":"未执行"}`,`fresh identity=${a.fresh_identity_called?"executed":"not executed"} · Stage A=${a.stage_a?"authorized":"not authorized"} · Stage B=${a.stage_b?"authorized":"not authorized"} · Public P1=${a.public_p1?"authorized":"not authorized"} · baseline=${a.baseline_execution?"executed":"not executed"}`)}</span></div>`;
  };
  const publicPanel = s => {
    const p=s.public_p1||{};
