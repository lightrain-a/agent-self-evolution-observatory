...
Manuscript strengthening status (2026-08-25): `STAGE_SIGNATURE_R4_EVALUATION_COARSENING_ANALYZED`. R1 separated the old one-step ``reward error -> future behavior'' interpretation into write/exposure/uptake/outcome. R2 added capacity-versus-native-transport control and a six-alternative explanation audit. R3 made the ordinal localization rule explicit; R3b stress-tested novelty and demoted generic retrieval-versus-use and lifecycle-stage claims already covered by QCR and memory-lifecycle work. R4 now explains **why stage resolution is scientifically necessary rather than merely more detailed reporting**: coarse projections of the same frozen system support incompatible summaries. Write-only evaluation says the update is strong; retrieval-only says reuse is active; native endpoint-only says the effect is almost absent; forced-only says memory has behavioral leverage. This is an **evaluation-coarsening / observational-aliasing** problem. The retained stage signature separates persistent-state construction, forced downstream capacity, native exposure, first-action uptake, and native outcome without scalarizing their incomparable units. On the frozen evidence, the first unsupported native stage remains `first-action uptake`, so the supported localization remains **after exposure and before stable first-action uptake**, explicitly not causal mediation. Machine-readable analyses now include `evaluation-coarsening-analysis-20260825.json` in addition to the R2/R3/R3b artifacts.
...
The remaining paper-development debt is therefore presentation/coverage debt, not an unfilled method slot. R4 makes the stage signature answer a sharper scientific question: which apparently conflicting conclusions are created by observing only a coarsened subset of the transport chain? Keep that diagnostic role, the alternative-explanation audit, and the negative boundaries visually/narratively central, and do not reopen CBRG without its four-part evidence contract.
