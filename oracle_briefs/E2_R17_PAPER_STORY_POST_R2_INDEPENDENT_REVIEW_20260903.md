# Independent adversarial paper-story review — E2-R17 after V3 Stage-A R2 PASS
...
Review the paper story below from first principles. This is NOT another Stage-A protocol review: the frozen V3 Stage-A R2 protocol already passed an independent pre-execution review. Your job is to decide whether the *paper-level scientific object and evidence arc* are actually strong, novel, and causally coherent enough to justify proceeding, or whether the authors are merely renaming known experience-selection / failure-learning ideas.

Be adversarial. Do not protect the current story because prior protocol review passed. Do not infer any V3 Stage-A or Stage-B outcome; none exists. The completed DeepSeek global MRW result is outcome-consumed and must remain exactly as reported. Do not propose broad benchmark/model expansion as a generic remedy.

Frozen anchors:
- paper-story source commit: `1eb852670551abb6649a796396c8ec93c1499860`
- paper-story SHA256: `f4d2e81a4f47895a490d69d3accf87c89b208cdc2a865d70b063a9d83f13ba5d`
...
- R2 pre-execution verdict: `PASS_TO_SEPARATE_STAGE_A_AUTHORIZATION`
- current provider calls for V3/R2: 0
- current Stage-A authority: false
- current Stage-B authority: false

Closest-work collision boundary already identified by the authors:
- ReasoningBank/MaTTS: reusable memory from successful and failed search trajectories; test-time scaling creates multiple experiences.
- SkillCAT: same-task multi-trajectory success/failure contrast distilled into textual skill updates.
- SkillRevise / Rethinking Self-Evolving Agent Skills / WikiSkill: trace-grounded revision, success/failure feedback studies, and persistent experience/knowledge/skill accumulation.
- Therefore novelty may NOT rely on “failures help,” rejected rollouts, sibling contrast, multi-trajectory learning, validated textual skill editing, or generic persistence.

The surviving proposed object is: a shared search object `T_K` feeds separate acting and persistent-learning projections `a(T_K)` and `g(T_K)`; serving-optimal selection can censor already-generated evidence from the learner-visible channel, and the same-pool causal intervention holds `T_K`, served winner, initial state, updater, budget, and order fixed while changing only `g(T_K)`.

Completed global causal evidence that MUST be treated as fixed:
...
This does NOT establish universal MRW benefit.

Prospective V3/R2 mechanism experiment:
- five independent matched skeletons; R=4 is measurement replication only
- within each skeleton, cross `PROCEDURAL_TRANSFORMATION` vs `INSTANCE_BINDING_LOCALIZATION` while holding structural nuisance factors fixed as specified
- same search-pool generation / acting; later compare WIN-C vs MRW4 learning projection
- primary scientific object is the skeleton-level interaction `I_h = effect(MRW4-WIN | procedural) - effect(MRW4-WIN | binding)`
- router is frozen, hand-engineered, actor-visible; it is NOT learned semantic discovery and cannot rescue a failed primary interaction
- finite-five-unit all-positive directional resolution is 1/32; no population-level generalization should be claimed from five units

Audit the following explicitly:

A. Is **Act–Learn Dual Projection / Search-Projection Censoring** genuinely a useful and distinct scientific object, or mainly new terminology for logging policy, experience selection, memory curation, selective labels, or offline-data selection? Identify the irreducible novelty, if any.

B. Does the exact-same-pool, acting-fixed intervention add causal identification that close works do not already supply? What exact claim can it identify, and what claim can it not identify?

C. Is the rescue/censoring formulation scientifically substantive enough to anchor a paper, or is it merely an obvious restatement of best-of-K selection? Distinguish a useful identity/prediction from decorative formalization.

D. Is it legitimate and reviewer-convincing to place the inconclusive global MRW result in the main empirical arc as evidence against a universal rule and motivation for a prospective moderator test? Or does that read as post-hoc story rescue? State what makes it acceptable or unacceptable.

E. Does V3's crossed procedural-vs-binding moderator test actually address the heterogeneity question, or is it too synthetic / author-constructed to be mechanistically persuasive? Separate internal causal validity from external generalization.

F. Is the hand-engineered observable router correctly demoted to a proof-of-implementability, or should it be removed from the main method entirely?

G. With only five independent skeleton interactions, is the proposed confirmatory claim meaningful? State the strongest permissible wording if all five are positive, and the proper interpretation if they are not.

H. Is “failure-specific diagnostic value” still under-identified by WIN-C vs failed-nonwinner MRW4? If yes, should a generic-alternative/nonwinner-success control be required BEFORE Stage A, only after a positive V3 result, or not at all? Choose one and justify scientifically.

I. Is one natural/out-of-family transport test necessary for the *controlled causal paper* itself, or only for a stronger deployable/general projection-policy claim? Again choose one.

J. Pressure-test the contribution hierarchy. Which contributions survive current closest-work collisions? Which should be removed or rewritten?

K. Give the strongest paper story if V3 interaction PASSES, and the strongest honest paper story if V3 interaction FAILS. Does a failed V3 still leave a publishable diagnostic/interface paper, or does it collapse the central thesis?

L. Decide the immediate next action. Choose exactly one:
1. proceed with the already-reviewed Stage A boundary (fresh identity qualification, then separately scoped Stage-A authorization if identity passes), without redesigning R2;
2. require a specific zero-provider paper/protocol fix before Stage A;
3. stop/pivot before Stage A because the scientific object is not strong enough.
Do not recommend extra experiments before Stage A unless they are genuinely verdict-changing.

Finally provide:
- one recommended one-sentence paper thesis;
- one recommended title direction;
- at most THREE verdict-changing fixes, if any;
- a compact machine-readable block with `paper_object`, `novelty_status`, `stage_a_recommendation`, `v3_pass_story`, `v3_fail_story`, `required_fixes`, and `verdict`.

End with EXACTLY one of:
`KEEP_DUAL_PROJECTION_STORY_AND_PROCEED_STAGE_A`
`REVISE_PAPER_OBJECT_BEFORE_STAGE_A`
`PIVOT_TO_DIAGNOSTIC_INTERFACE_PAPER_NOW`
`STOP_E2_R17`

---

## Bound paper-story document
