...
\title{After a Static Safety Pass: Controlled First-Violation Evidence in a Persistent Web Agent}
...
Longitudinal safety is not an empty comparison class. Most directly, \citet{altawaha2026remembering} evaluate memory-equipped agents across time using fixed trigger probes, memory snapshots, and a no-memory counterfactual. We adopt the corresponding control logic---a same-schedule base-workflow arm and read-only fixed probes across snapshots---but target a different object: an action-taking BrowserART/AWM trajectory and its branch-level first-violation time. Their study is broader across memory architectures; ours isolates a realized update contrast inside a frozen interactive web-agent protocol. The contribution is therefore not priority over longitudinal memory safety, but controlled evidence about when a current safety observation ceases to predict a declared future.
...
Figure~\ref{fig:protocol} separates the current observation from two longitudinal tests. Qualification is evaluated once at $s_0$. In the primary paired comparison, each state--branch unit executes the same behavior IDs, seeds, three-step held-out schedule, runtime, evaluator, guard, threshold, and call policy. The updated arm receives the frozen step-specific workflow snapshot; the base-workflow arm receives its step-0 snapshot at every step. In the secondary panel, the same qualification probes and seeds are evaluated at exposure snapshots 0--3, with probe writeback disabled. Both plans and decision rules were frozen before either new arm's outcomes were inspected.
...
\caption{Controlled longitudinal evidence. (a) Updated workflows produced events in 8/12 branches versus 4/12 under the same held-out schedule with the base workflow held fixed. (b) All paired first-event times; $>3$ denotes censoring. (c) Re-evaluating the same 12 qualification probes at updated snapshots produced 0, 1, 1, and 2 violation episodes at exposure steps 0--3. Counts describe the frozen design only.}
\label{fig:control}
\end{figure*}
