...
Figure~\ref{fig:g1-scene} makes the measurement problem explicit: the agent trajectories are held fixed, while evaluator identity changes the current-pass premise, future first-event set, and arm ordering. The scientific object is therefore not a scalar ``safety label'' attached to persistent state, but an evaluator-indexed temporal claim whose robustness must be audited before mechanism interpretation.

\begin{figure}[t]
\centering
\includegraphics[width=0.99\linewidth]{figures/g1-evaluator-relative-scene-handdrawn.pdf}
\caption{\textbf{Scene: evaluator identity can change a temporal safety conclusion without changing the trajectory.} The same frozen Qwen3-8B/AWM trajectories are evaluated by independently frozen HarmBench and DeepSeek judges. HarmBench reports a current pass and the branch ordering updated $>$ base $>$ NullMemory; DeepSeek changes one current label and reports NullMemory $>$ updated $=$ base. The lower timeline emphasizes that first-event interpretation can differ even when state, task, seed, and trace bytes are held fixed.}
\label{fig:g1-scene}
\end{figure}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.99\linewidth]{figures/g1-erta-method-handdrawn.pdf}
\caption{\textbf{Method: ERTA keeps evaluator uncertainty visible and fails closed.} We freeze the longitudinal trajectories and evaluator identities before interpretation. ERTA then checks current-premise stability, reports definite/possible first-event sets, tests evaluator contrast envelopes, and localizes disagreement by task. A deterministic compiler returns robust-supported, robust-refuted, or evaluator-unidentified without learning judge weights. The held-out PV1 panel is frozen after method derivation and before its new labels. The separate MCTA capability-matched extension remains stopped because no qualified pre-treatment capability variable was available; no MCTA safety outcomes enter the current paper.}
\label{fig:g1-method}
\end{figure}

\paragraph{Reader's map.} Figure~\ref{fig:g1-scene} motivates evaluator-relative temporal conclusions; Figure~\ref{fig:g1-method} summarizes the ERTA audit contract. The later longitudinal-protocol figure records the exact three-arm substrate. Results are then organized by the particular scientific predicates ERTA can preserve or withhold, rather than by choosing whichever evaluator keeps the original mechanism story intact.
