...
\subsection{Interpretation flips, mechanism placebo, and transport}

M0--M3 are evaluated on the same executed rows. We preregister an Interpretation Flip Matrix that records how M0/M1/M2 conclusions change under M3 for shared-capability, capability-divergent, capability-censored, asymmetric-execution, execution-invalid, and baseline-ineligible slots. The primary method-necessity diagnostic is the M2 graph-overadmission rate, defined as the fraction of M2 capability-positive slots for which M3 has $C=0$, reported with numerator/denominator and state stratification.

If P1 satisfies the predeclared positive shared-capability R1 rule, a pre-frozen 12-unit placebo matches update count, insertion cadence, wrapper/section structure, and tokenizer-length envelope while replacing executable workflow semantics with execution-neutral content. This placebo is mandatory before attributing a positive effect specifically to executable workflow semantics. If placebo follows Updated, any workflow-semantic mechanism claim is demoted to context/load/packaging.

A second-backbone transport subset is triggered only after a qualified main result when the manuscript seeks a claim beyond the exact primary substrate. A different model family is preferred; a same-family larger model supports scale robustness only. Without transport, positive claims remain explicitly scoped to the tested Qwen3.5-397B-A17B + AWM + BrowserART/BrowserGym setting.
...
\pending{P0 capability support: eligible pair count, M1/M2/M3 disagreements, M2 graph-overadmission rate, capability-censored and asymmetric-execution fractions.}

\pending{P1 main result: raw R1 contrast, shared-capability M3 contrast, support denominator, state/update-sequence localization, capability divergence, Interpretation Flip Matrix, and exact state-block sign-flip sensitivity.}
