...
\item \textbf{Released and dynamic evidence.} Skill-SP exhibits the controller failure and a Level-1 residual with Level-3/logical equalizable boundaries; AutoSkill provides a controlled one-substrate representation-to-retrieval-to-behavior consequence with ID-placebo and quotient restoration; SkillRL provides an independent identity-sensitivity boundary.
\end{enumerate}
...
AutoSkill~\citep{yang2026autoskill} exposes a first-party \texttt{sdk.search} seam. On a frozen five-skill P19 library with \texttt{top\_k=5}, the original representation retrieves five semantic skills including a post-checkout reset/clean skill. Splitting the highest-ranked Makefile skill into four exact-semantic IDs makes the identity-level top-$k$ contain four copies plus one Python-cleanup skill, deterministically crowding out the post-checkout skill. An ID-renaming placebo preserves the original semantic set, while exact-semantic quotienting before the same five-slot budget restores it.

With the same frozen prompt, Doubao executor, fresh Codex container, and zero judges, the mechanically defined destructive post-checkout signature occurs in 6/6 original, 0/6 split, 3/3 ID-placebo, and 3/3 quotient-control runs (one-sided Fisher $p=0.00108$). The signature is read from tool commands containing a post-checkout workflow with \texttt{git reset --hard} and forced cleanup. This establishes one representation-to-retrieval-to-behavior consequence under a common executor, not end-to-end AutoSkill utility or general safety.
...
STRI-Cert audits target realizability under a declared package-mass class. We claim representation-dependent released control, exact support-cone realizability, and one bounded AutoSkill behavior consequence with ID-placebo and quotient restoration. We \textbf{do not} generalize this to task utility, longitudinal regret, or system-wide safety, nor claim a new LP algorithm or broadly validated dynamic repair; Proposition~2 is only a constructive action-basis boundary.

\paragraph{Limitations.} Exact repair and $R^*$ require identified clone classes and independent support/target truth; robust bounds do not learn them. Dynamic evidence is one P19 substrate under a common executor. Neutral $q=\mathbf1$ is only the no-priority audit, so conclusions remain domain- and controller-class-specific.
