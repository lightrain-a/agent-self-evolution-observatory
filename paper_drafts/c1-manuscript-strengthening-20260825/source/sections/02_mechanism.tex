...
\subsection{Prospective implication: test selective post-exposure authorization}
The completed experiment does not directly observe latent state relevance or latent ``behavioral authority.'' It establishes an operational boundary: source-item exposure can be frequent while a stable branch-conditioned first-action distribution shift remains unsupported by the frozen test. This leaves several explanations open, including content redundancy, policy priors, prompt/context attenuation, later-action effects, action-serialization ambiguity, and stochastic heterogeneity.

We therefore treat selective post-exposure authorization as a \emph{prospective falsifiable hypothesis}, not as the mechanism already identified by C1. The clean future comparison asks whether a selector based on reproducible matched-state branch sensitivity outperforms an equally sparse outcome-blind random authorization rule. A stable branch contrast is not assumed to be correct, useful, or reward-improving; any future controller result must establish those properties separately. The full frozen PACTA-MSR protocol, including its matched-state reveal, developmental shadow gate, rate-matched random control, and unbiased exact-match-kernel MMD$^2$ endpoint, is retained in Appendix/Future Work so that a successor cannot move its goalposts. No PACTA-MSR method-effect result is reported in this paper.
