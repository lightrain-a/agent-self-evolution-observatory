...
ROOT = HERE.parents[1]
SRC = HERE / "source-r7"
OUT_PDF = HERE / "C1-stage-resolved-r7-review-repair.pdf"
OUT_ZIP = HERE / "C1-stage-resolved-r7-review-repair-source.zip"
OUT_MANIFEST = HERE / "c1-r7-package-manifest-20260829.json"
CLAIM_RUNNER = HERE / "run_claim_audit_r7.py"
CLAIM_AUDIT = HERE / "claim-audit-r7-provenance-seal-20260829.json"
SENSITIVITY = HERE / "stage-evidence-sensitivity-audit-20260826.json"
STAGE = HERE / "stage-evidence-ladder-analysis-20260825.json"
SOURCE_DATE_EPOCH = "1787961600"  # 2026-08-29T00:00:00Z; fixed for reproducible PDF metadata.
...
EXPECTED = {
    ...
    "claim_audit": "baad7e26fa297412c3959bb58d2df5bdd04d3de52d1175f6b0ea5e14d8caf4cf",
...
def main() -> None:
    for rel in SOURCE_FILES:
        if not (SRC / rel).is_file():
            raise RuntimeError(f"missing R7 source file: {rel}")
    for path in (CLAIM_RUNNER, CLAIM_AUDIT, SENSITIVITY, STAGE):
        if not path.is_file():
            raise RuntimeError(f"missing R7 evidence input: {path}")
    ...
    if '"status": "REPLAY_PASS"' not in replay:
        raise RuntimeError("35/35 claim audit is not replayable on the R7 source")
    ...
    payload = {
        "schema_version": "1.0",
        "artifact_type": "c1-r7-paper-only-package-manifest",
        "paper_id": "D2-PAPER-PROXY-REWARD-MEMORY-VARIANCE",
        "revision_id": "C1-STAGE-SIGNATURE-R7-STANFORD-REVIEW-REPAIR-20260829",
        "title": "Memory Divergence Is Not Behavioral Divergence: Stage-Resolved Transport in Self-Improving Agent Memory",
        "contract_sha256": EXPECTED["contract"],
        "status": "R7_PAPER_ONLY_REVIEW_REPAIR_PACKAGE_SEALED",
        ...
        "artifacts": {
            ...
            "claim_audit": {"path": str(CLAIM_AUDIT.relative_to(ROOT)), "sha256": EXPECTED["claim_audit"], "replay": "35/35 PASS", "revision": "R7"},
