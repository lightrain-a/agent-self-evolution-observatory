...
B11 reuses the 432 archived B10 outputs and introduces zero provider calls. The BrowserUse-style response schema emits a \texttt{current\_state.memory} field before the first structured action. This field is recoverable for all 432 outputs (405 strict JSON, 27 narrow string recoveries). For each future task, we embed this working-memory text and the exact success/failure input memories with the same cached all-MiniLM-L6-v2 encoder used by the released retriever. The pair-relative margin is $m(y)=\cos(E(y),E(M_S))-\cos(E(y),E(M_F))$; the task-level uptake statistic is the mean margin under success-conditioned outputs minus the mean margin under failure-conditioned outputs. Because this observable was introduced after B10, all tests here are post-hoc and have no confirmatory gate.

The mean task-level branch shift is 0.003347 (median 0.003835), with 21 positive and 15 negative tasks, paired $d_z=0.148$, and within-state label-permutation $p=0.2052$. Thus explicit branch-specific working-memory uptake is not established. A common-centroid diagnostic---similarity to the normalized midpoint of the two input memories under either memory condition relative to no-memory---is larger at 0.02233, but remains suggestive only ($d_z=0.258$, sign-flip $p=0.0664$). Branch-relative working-memory shift is descriptively associated with first-action TV (Pearson $r=0.464$, Spearman $\rho=0.419$; leave-one-out Pearson range 0.358--0.515) but not with terminal absolute effect ($r=-0.023$). Finally, input S/F memory cosine distance averages 0.131 (range 0.045--0.218) and correlates only 0.095, 0.115, and 0.137 with working-memory shift, first-action TV, and terminal effect, respectively; simple input-pair semantic distance therefore does not explain the attenuation monotonically. We use B11 only to localize the boundary earlier in the chain and do not select positive cells for further experiments.

\begin{table}[h]
\centering
\small
\caption{B11 zero-call working-memory localization. All quantities are post-hoc diagnostics.}
\label{tab:b11-working-memory}
\begin{tabular}{lr}
\toprule
Metric & Value \\
\midrule
