...
\subsection{Branch-specific uptake is the dominant native bottleneck}
...
A GLM-5.3 writer replication changes all four original paired memories (mean token-set Jaccard 0.737), but its matched 256-call terminal effect is 0.140625 ($p=0.00012$), below the frozen 0.15 floor, with two sign reversals. Upstream write divergence therefore transfers more readily than downstream magnitude or direction.

A preregistered DeepSeek-V4-Flash downstream-policy transfer on the same 36-task native support remains unresolved. Its first frozen unit length-censors without assistant text at both the 900-token parent and the sole allowed 2,200-token repair, after which further cap escalation and model substitution are forbidden. It contributes zero scientific units and is a provider-support stop, not a scientific null.

The resulting picture is deliberately bounded. Reward-conditioned writing is robust, direct injection can reveal latent leverage, and native retrieval can expose the state broadly. What fails to generalize broadly on this substrate is the success/failure distinction at policy uptake and terminal outcome. That stage-resolved boundary is the paper's scientific object; it is not a claim that memory is irrelevant or that evaluator errors can never matter.
