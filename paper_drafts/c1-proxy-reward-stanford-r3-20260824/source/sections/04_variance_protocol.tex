...
A 144-call raw writer-input trajectory arm on the same 36 tasks gives mean rewrite-versus-raw effect 0.04514 ($p=0.00775$), below the 0.15 floor, with 31/36 S/F/raw/no-memory point estimates identical. A post-hoc fractional-reference re-score leaves mean S/F separation at 0.01951, arguing against binary discretization as the main explanation. We therefore move earlier in the chain. B10 freezes each native-hit task at released trajectory step 1 and samples success-memory, failure-memory, and no-memory first structured actions. All 432 calls complete. Mean S/F first-action TV is 0.06944 ($p=0.5801$), below the inherited 0.20 process floor; 9/36 states have any S/F TV and 0/36 change their modal action. A descriptive memory-versus-no-memory TV of 0.17014 is not confirmatory. A zero-call post-hoc B11 audit moves one step earlier again: in the model-emitted pre-action \texttt{current\_state.memory}, pair-relative S/F semantic shift is only 0.00335 ($p=0.205$, $d_z=0.148$). This shift correlates descriptively with first-action TV ($r=0.464$; leave-one-out range 0.358--0.515) but not terminal effect ($r=-0.023$). Thus branch-specific information is already weakly internalized before the first action.
...
Overall, forced swaps establish conditional leverage, native retrieval establishes exposure, raw/endpoint controls reject two simple explanations for the broad terminal non-pass, and B10/B11 place the dominant attenuation at branch-specific internalization and first-action uptake. Appendix~\ref{sec:breadth-transport-details} gives cell-level results and bounded consequence analyses.
