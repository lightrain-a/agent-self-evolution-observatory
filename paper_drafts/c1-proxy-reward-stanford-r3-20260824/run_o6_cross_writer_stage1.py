...
def validate_contract(contract_path: Path, contract: dict[str, Any]) -> None:
    ...
    require(int(model.get("max_output_tokens")) == 2200, "writer token budget drift")
...
def run_one(*, client, error_type, contract: dict[str, Any], task: str, label: str, system_prompt: str, source: dict[str, Any], private_root: Path) -> dict[str, Any]:
    ...
    try:
        response = client.respond(prompt, model=EXPECTED_MODEL, max_output_tokens=2200, temperature=0.0, thinking=None, store=True, allow_thinking_compatibility_fallback=False)
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Run the frozen eight-call PROXY-O6 GLM-5.3 cross-writer Stage 1.")
