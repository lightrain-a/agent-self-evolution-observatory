...
MODEL='deepseek-v4-flash'; RESOLVED='deepseek-v4-flash-ga-260731'; BASE_URL='https://ark.cn-beijing.volces.com/api/plan/v3'; TOTAL=288; N=4
...
def validate(cp:Path,c:dict[str,Any]):
 req(c.get('status')=='FROZEN_BEFORE_PROVIDER_CALLS' and c.get('experiment_id')=='D2-PROXY-B6-CROSS-POLICY-RETRIEVAL-MATCHED-R1-2200','contract identity drift');req(c.get('future_task_count')==36 and len(c.get('task_units') or [])==36,'future support drift');req(c.get('expected_provider_calls')==TOTAL and c.get('rollouts_per_task_per_condition')==N and c.get('conditions')==['success','failure'],'geometry drift')
 m=c['model'];req(m['requested']==MODEL and m['expected_resolved']==RESOLVED and m['temperature']==0.2 and m['max_output_tokens']==2200 and m['thinking'] is None and m['provider_retries']==0 and m['substitution_allowed'] is False,'model drift');g=c['primary_gate'];req(g['min_mean_absolute_success_rate_difference']==0.15 and g['permutation_p_lt']==0.05 and g['permutation_repetitions']==100000 and g['permutation_seed']==20260824,'gate drift');miss=c['missingness_policy'];req(miss['provider_retries']==0 and miss['stop_after_first_no_text_provider_failure'] is True and miss['top_up_failed_units'] is False and miss['replace_future_tasks'] is False,'missingness drift');a=c['authority'];req(a['experiment_authority'] is True and a['provider_call_authority'] is True and a['claim_expansion_authority'] is False and a['submission_authority'] is False,'authority drift')
 for row in list(c['source_artifacts'].values())+[c['b4_contract'],c['b4_result'],c['cross_policy_program'],c['memory_manifest'],c['human_authority']]:
...
def one(client,error_type,u,cond,i,t,root):
 ...
 try:
  r=client.respond(pr,model=MODEL,max_output_tokens=2200,temperature=0.2,thinking=None,store=True,allow_thinking_compatibility_fallback=False);ans=str(r.get('text') or '').strip();writej(root/'provider-responses'/f'{st}.json',{**base,'response_id':r.get('response_id'),'provider_status':r.get('status'),'requested_model_returned':r.get('requested_model'),'resolved_model':r.get('resolved_model'),'usage':r.get('usage') or {},'answer':ans,'answer_sha256':tsha(ans) if ans else '','thinking_compatibility_fallback':r.get('thinking_compatibility_fallback')});req(str(r.get('requested_model'))==MODEL and str(r.get('resolved_model'))==RESOLVED,'model resolution drift');req(r.get('thinking_compatibility_fallback') is not True,'thinking fallback');req(bool(ans),'no assistant text');sc,checks=score(ans,t['refs']);row={**base,'status':'complete','provider_status':r.get('status'),'resolved_model':r.get('resolved_model'),'usage':r.get('usage') or {},'answer_sha256':tsha(ans),'benchmark_score':sc,'evaluator_checks':checks}
