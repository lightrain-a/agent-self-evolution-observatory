...
Beat neutral extra help? & T--G$_0$ & Executed & Operation-specific value after G$_0$ independently passes a fresh N-equivalence gate. \\
Does container matter? & T--R & Executed & Callable-skill value beyond the same frozen operation materialized in retrieved context. \\
\bottomrule
\end{tabular}
\caption{Completed identification ladder. The stronger G$_0$ and R treatments change the interpretation of the historical N/G/T results: G$_0$ validates operation specificity in surviving cells, whereas R removes any supported claim of callable-container superiority.}
\label{tab:identification}
\end{table}

\paragraph{G$_0$: prospectively validated neutral helper.}
G$_0$ executes exactly one assigned helper call through the same harness surface as T but is information-empty: the frozen function is \texttt{def skill(package, context): return \{\}}. It reads neither package nor context. On DeepSeek, the first 35-endpoint two-repeat tranche is deliberately not promoted because its G$_0$--N 90\% interval $[-14.3,2.9]$ points exceeds the predeclared $\pm10$-point neutrality margin. We do not pool that tranche or relax the margin. An independently frozen A2 replication with four fresh repeats yields $-2.1$ points, 90\% CI $[-9.3,5.0]$, and passes the standalone equivalence gate. Kimi independently gives $+2.2$ points, 90\% CI $[-4.3,8.7]$. Only after these gates do we inspect the pre-frozen T--G$_0$ load-bearing cells. DeepSeek C3 grounding, C4 grounding, and held-out EIA cutoff retain positive direction; Kimi alignment retains +50 points, while Kimi same-domain grounding is downgraded (T--G$_0=-25$ points, T--N=0).

\paragraph{R: operation-matched retrieval/context treatment.}
R runs the exact same targeted operation source as T on the same candidate evidence. Rather than exposing the output through \texttt{reusable\_helper\_output}, the harness adds the identical object as \texttt{retrieval\_operation\_output} inside the evidence package and exposes no callable helper. A zero-call parity audit verifies for every tested endpoint that (i) removing this one field recovers the original evidence package exactly, (ii) the operation-output content hashes are identical between T and R, and (iii) no candidate evidence is removed or reordered. On all 18 pre-frozen DeepSeek load-bearing endpoints, two fresh repeats per arm give T--R $=0.0$ points with 90\% CI $[-5.6,5.6]$; all three load-bearing cells have zero mean residual. A secondary Kimi alignment check uses the three pre-frozen held-out alignment endpoints and five fresh repeats per arm: both T and R succeed on every run. We therefore rule out a material $>10$-point callable-container advantage at the registered primary resolution. This is a same-information integration result, not an evaluation of every possible learned temporal retriever.
