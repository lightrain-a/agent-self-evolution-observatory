# E2-R17 manuscript R4 — M3R2 + V4-R1 protocol alignment — 2026-09-03
...
M3R2 freezes the same four already-existing states and preserves 72 new actor units with **zero updater calls**:
...
The unused M3R `D_U-D_A` statistic has now been explicitly superseded before execution by the M3R2 object at commit `f8fb39d2289fdc3af2baa7bec23fe9c12087c1d1`. The replacement uses `E_REAL = D_X-D_A`, with pairwise cross-state and within-state actor disagreement on the same scale and exhaustive zero-provider tests verifying `E[E_REAL]=(p_A-p_B)^2` under the declared Bernoulli/exchangeability assumptions. The repair changes no state, task, actor unit, model, or provider budget.

M3R2 still remains an outcome-selected development audit. A positive result does not estimate a population variance component; a nonpositive result downgrades regeneration-instability-beyond-actor-noise language.

## R4 manuscript alignment applied

R4 prospective text follows both M3R2 and V4-R1:
...
- both the repaired M3R2 localization and bridge realization localization use commensurate `D_X-D_A` / `E_REAL`, under their separately frozen task/authority objects;
...
## R4 static qualification

After M3R2 + V4-R1 alignment:

- combined adjacent E2 regression: 85/85 PASS;
- LaTeX compile: PASS;
- PDF pages: 16;
- overfull boxes: 0;
- undefined references/citations: 0;
- PDF SHA-256: `8558dfd6f5ceaaa6dcc1d0c05e5a2323d0ec1856158ccc777c9884debbe37b14`;
...
These checks qualify manuscript consistency and layout only; they do not create M3/M4 execution authority or a V4-R1 independent-review verdict.

## Current authority boundary

R4 is manuscript/design alignment only. It does not authorize:
