...
Generic identity fragmentation, generic path dependence, and the statement that reparameterization can alter learning are broader known phenomena. Our claim is not that fragmentation exists in the abstract. It is that a released self-evolving skill lifecycle instantiates a particular partition-before-gate mechanism whose intervention variable, persistent state, phase law, and repair can be isolated while semantic feedback is held fixed.
...
Consider additive evidence count and a per-identity gate that requires at least $M$ retirement-eligible observations. Let $N$ such observations belong to one semantic class and be balanced across $k$ exact identities.

\paragraph{Proposition 1 (fragmentation window).}
Canonical or quotient-credit evolution retires the class iff $N\ge M$. Native balanced $k$-identity evolution retires the full class iff $N\ge kM$. Therefore native and quotient decisions diverge exactly for
\begin{equation}
\label{eq:phase-window}
M\le N<kM,
\end{equation}
and the first full-retirement boundary is delayed by $(k-1)M$ observations.

\paragraph{Reason.}
The quotient has one class bucket containing $N$ observations. Native balanced refinement gives each identity at most the evenly assigned share, and all $k$ identities cross a threshold $M$ only once $N\ge kM$.

\paragraph{Corollary 1 (arbitrary partitions).}
...
\exists j:\ g(s_j)=0.
\]
Thus additive statistics alone do not guarantee invariance: nonlinearity enters through applying the lifecycle decision \emph{before} semantic aggregation. Proposition~1 is the balanced-count specialization that yields a closed-form boundary in the released Skill-SP gate.
...
We next sweep $k=1,\ldots,6$, $N=0,\ldots,48$, and $\hat p\in\{0.1,0.5,0.9\}$ through the released controller, producing 882 cells. The two lower-score regimes never retire unexpectedly. In the retirement-eligible $\hat p=0.9$ regime, the observed first full-retirement boundaries are
\[
8,16,24,32,40,48
\]
for $k=1,\ldots,6$, exactly matching $kM$. Across the entire grid the analytic prediction has zero mismatches. For $k=2$, native and quotient evolution disagree for $N=8,\ldots,15$; for $k=4$, they disagree for $N=8,\ldots,31$, as Eq.~\ref{eq:phase-window} predicts.

\begin{table}[t]
\centering
\caption{Released Skill-SP credit-fragmentation phase boundary at the default $M=8$ for retirement-eligible feedback. Quotient-credit remains at the canonical boundary $N=8$ for all $k$.}
\label{tab:phase}
\small
