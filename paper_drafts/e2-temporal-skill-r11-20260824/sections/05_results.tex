...
\subsection{Release alignment is selective rather than model-invariant}
Alignment is at ceiling on C3-R for every evaluated initial-replay model, so that split cannot identify a procedure effect. Held-out Kimi moves away from ceiling: T2 succeeds on 12/12 observations, versus 7/12 for N0 and 4/12 for G2, giving +41.7 and +66.7 points. Targeted-minus-generic is positive on all three held-out alignment endpoints, with endpoint-bootstrap interval [50,75] points. The exact sign test is nevertheless $p=0.25$ because there are only three independent endpoints. DeepSeek remains at ceiling. We therefore report a reusable alignment safeguard in one non-ceiling model regime, not a model-invariant alignment effect.

\subsection{Unchanged-code transfer has explicit positive and null boundaries}
The transfer evidence is strongest when interpreted together with the negative cells. T1/G1 are reused byte-identically in EIA; the grounding procedure retains direction on compatible future periods; and the Kimi alignment effect appears only when the held-out cell has headroom. Conversely, cross-domain grounding is null, several alignment cells are at ceiling, and the mini model is at no-skill ceiling on EIA. These outcomes are not discarded as failures of the story: they define where the proposed bottleneck is and is not identifiable.

Across the study, the supported conclusion is therefore conditional: a reusable temporal procedure can be a binding repair for a particular procedure$\times$model$\times$task regime. The experiments do not support a universal temporal-skill effect, a monotonic model-capacity law, or general cross-domain transfer.
