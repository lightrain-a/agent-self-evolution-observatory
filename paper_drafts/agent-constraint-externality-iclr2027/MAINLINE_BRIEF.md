...
- Provider-readiness R1 (2026-09-05): separately authorized as a **non-scientific synthetic check only**, but stopped before dispatch because the approved `AA_API_KEY` was not configured in the canonical execution worktree. `provider_request_count=0`, `scientific_provider_calls_created=0`, `scientific_outcomes_created=0`; therefore this is **not** evidence that credit remains exhausted or recovered. Gate 0 remains closed.
...
Execution sequencing is mechanically closed in `generated/agent-constraint-externality-confirmatory-execution-readiness-v2-20260905.json`. It adds no authority. Provider-readiness R1 is closed out separately in `consultations/agent-constraint-externality-provider-readiness-r1-closeout-20260905.md`: the approved credential was absent, so **no provider request was dispatched** and Gate 0 remains closed. After a future fresh readiness PASS and a separate Gate 0 authority, the confirmatory reserve must pass source/repair creation and topology-neutral `TARGET_ONLY_VERIFICATION` before the panel is frozen; SHAM artifacts/subset are frozen before collateral outcomes. RQ1/RQ2 outcomes are then **collected once on one locked panel**. RQ1 is adjudicated first; RQ2 is a conditional analysis of those same frozen outcomes, not a second adaptively chosen data collection.
