...
- Added OpenReview-safe metadata projection for title/abstract contracts. Math-mode TeX remains inside its source delimiters, supported presentation-only text macros are deterministically mapped to Markdown, escaped prose punctuation such as `\\%` is normalized for the form, and unknown LaTeX text-mode commands outside math fail closed. The form-contract schema is now v1.1 and binds both source-metadata hashes and projected OpenReview hashes.
