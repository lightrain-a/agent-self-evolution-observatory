## 2026-08-30 · Independent relational-constraint-capacity Pre-F0

- Registered `RELATIONAL-CONSTRAINT-CAPACITY-20260830` as a separate zero-authority scientific object rather than reopening or repurposing VWE-specific `PORT-010`. PORT-010 remains `HOLD_EVIDENCE_REVIEW_BLOCKED / BLOCK_BAKE_IN`; its remaining reopen component is still `per_case_outcomes`, and all offline-replay/provider/GPU/scientific-execution authorities remain false.
- Pinned the publication/source audit to ATISS (`0909ce00...`, NeurIPS 2021), DiffuScene (`d78a2890...`, CVPR 2024), and InstructScene (`a9097a62...`, ICLR 2024 Spotlight), including exact license hashes. The InstructScene Hugging Face surface is fixed at revision `c8cf0bd2...`.
- Recorded the real P0 asset boundary: the checked 69 paths contain source snapshots but no materialized 3D-FRONT/3D-FUTURE dataset, no verified terms acknowledgement, no official fVQ-VAE file, and no official end-to-end two-stage checkpoint. The InstructScene authors explicitly identify available two-stage checkpoints as unofficial and estimate 1-3 A40-days per graph prior or layout decoder when training from source.
- Preserved `relation_count = |R|` only as a preregistered candidate dose. Existing outcome-blind SceneEval evidence shows explicit-spec count is nearly collinear with instruction length (`rho=0.939932`) and strongly associated with authored difficulty (`rho=0.861922`), so raw cross-sectional count degradation cannot qualify the object. A paired nested construction must still pass prompt-length/content controls.
- Kept ATISS and DiffuScene out of an undifferentiated same-access table because they do not natively expose the same free-form relational-instruction interface. InstructScene already contains a semantic graph prior, so direct ground-truth graphs are an oracle structured-access upper bound unless a deterministic same-information transform or matched architecture ablation is frozen before outcomes.
- Current state is `PRE_F0_HOLD_ASSET_AND_CONSTRUCT_QUALIFICATION`: P0 executed `0` cases, P1-P3 are not authorized, no outcome was read, and no provider/GPU call occurred. This is an asset/formulation qualification hold, not a mechanism failure.

## 2026-08-30 · SceneEval uncertainty and practical-equivalence gate

- Froze the zero-authority confirmatory decision rule for the separate SceneEval-500 cross-substrate constraint-integration object without reading any generator scene, object-matching result, semantic metric output, or per-case baseline score. Every scene contributes exactly one candidate-minus-N2 held-out joint log-score under the frozen five-fold contract; uncertainty is a 10,000-resample paired scene-cluster bootstrap with fixed seed `2026083017`.
- Fixed the topology practical-equivalence region outcome-blind at `max_abs(pair_correlation - pair_mean) <= 0.10`. The threshold is derived from the already frozen design warning that one-pair residual-correlation increments of `0.10-0.15` are too small for independence claims: a one-pair increment of `0.15` maps to exchangeability deviation `2*0.15/3 = 0.10`. It cannot be re-estimated from HSM outcomes.
- Synthetic q7 mixed-model validation passes the joint rule. Under an exchangeable null, mean held-out candidate-minus-N2 ELPD is `-0.003721` per scene with 95% bootstrap CI `[-0.007360, -0.000038]` and topology deviation `0.028536`, correctly firing the N2-equivalent/better STOP. Under a strong nonexchangeable alternative, mean held-out gain is `+0.026741` per scene with 95% CI `[+0.003140, +0.050518]` and topology deviation `0.304006`, correctly triggering only the synthetic support branch. All authority fields remain false.
- The proposal therefore advances only from `HOLD_UNCERTAINTY_AND_ACCESS` to `HOLD_ACCESS_AND_MEASUREMENT`: legitimate HSM gated-dataset access, evaluator measurement-format smoke preserving raw matching/prerequisite state, and a second independently qualified generator for paper-level transport remain blockers. PORT-010 remains unchanged and cannot be reopened by this object.

## 2026-08-28 · Formal-goal coupling replication preflight and memory hardening

- Preserved PORT-010 and its EmbodiedBench/BEHAVIOR predecessor failures as immutable HOLD evidence, then promoted four reusable zero-authority lessons into canonical Research Memory: authoritative runtime semantics outrank surface-token heuristics; a development substrate cannot independently confirm its own repair; policy-family independence is a lineage/provenance property rather than a measure of downstream modification; and a reproducible training contract must freeze hidden source/library defaults plus checkpoint-save semantics, not only headline CLI hyperparameters. The incremental public projection preserves the 17 pre-existing canonical failure assets, adds exactly four new signatures, and validates with zero Research System errors.
- Audited the third-policy-family gate before outcomes. The BEHAVIOR 2025 winning PiBehavior implementation is source-complete but explicitly Pi0.5/OpenPI-derived, so it cannot satisfy the frozen independent-family minimum. SmolVLA and OpenVLA-OFT remain source-qualification HOLDs because no pinned public BEHAVIOR-2026 R1Pro training/serving/checkpoint contract was available. Kimi K3 and DeepSeek V4 Pro independently returned `CONFIRM-HOLD`; the three-family minimum remains unchanged and no adapter built later by this project may be relabeled as pre-existing qualification evidence.
- Froze source-faithful π0.5 and GR00T N1.7 task-specific training contracts down to exact repository revisions, dependency locks, optimizer/scheduler/precision/data-ordering semantics, and anti-checkpoint-shopping terminal units. The source audit resolves π0.5's zero-based terminal checkpoint label as step `49999` for 50,000 optimizer iterations and GR00T's terminal periodic unit as `checkpoint-150000`. The first draft was independently rejected as incomplete; v2 received `2/2 CONTRACT-PASS-PREFLIGHT-ONLY`, which still authorizes no model load, forward/backward pass, optimizer update, training, or rollout.
- Pinned the official `behavior-1k/2026-challenge-demos` `v3.0` tag to exact revision `4f50b44796641a4d526a19d9aeadc8aa51e2f2c2`. Phase-0 verified the complete 100-task metadata namespace, 20,000 episodes, R1Pro 61-D state / 23-D action schema, 100 contiguous episode-metadata chunks, and exact OpenPI/GR00T dependency locks without downloading the ~3.26 TB video corpus. Phase-1 then used only pre-frozen task 0 (`turning_on_radio`): 200 episode metadata rows and two content-verified frame Parquet shards contain exactly 429,928 contiguous frames, fixed 23-D actions and 61-D states. The reward column was existence-checked only; no reward/success value was read.
- Current successor state is `HOLD_THIRD_FAMILY_PREOUTCOME_QUALIFICATION_PHASE1_READY`. Large-dataset materialization, policy checkpoints, model load, scientific forward passes, training, rollouts, and outcome analysis remain forbidden. The next scientific gate is a genuinely independent third policy family with a pre-existing source-pinned BEHAVIOR interface/training/serving/checkpoint contract, followed by separate independent review and explicit human execution authority.

## 2026-08-28 · SceneEval prerequisite-aware constraint-coupling preflight

- Promoted SceneEval-500 to the primary zero-execution-authority substrate for the separate cross-substrate constraint-integration object while preserving VWE-specific PORT-010 in `HOLD_EVIDENCE_REVIEW_BLOCKED / BLOCK_BAKE_IN`. The pinned SceneEval-500 release contains 500 typed instructions (`150/200/150` easy/medium/hard); the outcome-blind audit rejects raw requirement-count/difficulty degradation as a contribution because total explicit specs are almost collinear with instruction length (`rho=0.939932`) and strongly track authored difficulty (`rho=0.861922`).
- Froze 52 disjoint outcome-blind SceneEval pairs matched on exact total spec count and authored difficulty, within 10 words and at least 0.35 bits of constraint-type-entropy contrast. Entropy remains a secondary moderator only; it cannot replace the primary full-population residual-coupling test.
- Audited the exact SceneEval v1.1.1 evaluator dependency DAG before reading any generator output. Object Count, Object Attribute, Object-Object Relationship, and Object-Architecture Relationship all share VLM-produced `ObjMatching`; missing matched objects deterministically force downstream attribute/relation failures, and Object Count is directly matching-derived. Naive four-way co-failure is therefore prohibited as generator-interaction evidence. Primary outcomes are now prerequisite-eligible `ObjAttr/OORel/OARel`, with matching/ObjCount preserved as a separate prerequisite/control stage.
- Two independent outcome-blind reviewers (DeepSeek v4 Pro and Kimi K3) both returned `REVISE_BEFORE_PREREGISTRATION` (`0.82-0.85` confidence). Their shared strongest null is prerequisite-aware conditional independence plus exchangeable scene-level frailty/overdispersion. Completed and failed/truncated review calls are recorded separately; advisory review created no experiment, P0, GPU, or scientific execution authority.
- Froze an N0/N1/N2 ladder and an identifiable nonexchangeable downstream candidate before outcomes. N2 absorbs ordinary load/composition and official matching coverage, then allows channel-specific latent scales plus one arbitrary-strength exchangeable residual correlation shared by all three downstream pairs. The candidate replaces that one-rho exchangeable subspace with an unstructured 3×3 correlation matrix, adding exactly two type-specific nonexchangeability degrees of freedom; the average/shared correlation is nuisance, not candidate evidence. No outcome from one semantic channel may be used to predict another, and the same metadata-composition vocabulary is shared by null and candidate.
- Ran a standard-library-only synthetic power/design preflight. SceneEval metadata contain 402 instructions annotated in all three downstream channels; a conservative complete-case topology proxy needs at least 350 jointly prerequisite-eligible scenes for >=80% worst-case sensitivity to a frozen 0.20 one-pair residual-correlation increment. Below that availability, failure to reject cannot be interpreted as independence.
- Implemented and synthetic-tested the core three-channel binomial logistic-normal topology likelihood without reading SceneEval outcomes. Seven-point 3D Gauss-Hermite integration gives exact N2→candidate nesting at the same covariance; on an exchangeable synthetic null the candidate has no held-out gain (`-0.53`) and only `0.031` nonexchangeability deviation, while a strong nonexchangeable alternative yields `+24.40` held-out log-likelihood and `0.443` recovered deviation. The covariance engine therefore passes, while uncertainty/bootstrap, evaluator smoke, gated HSM access, and second-generator qualification remain separate blockers.
- Froze and synthetic-tested the high-dimensional marginal metadata calibration used by both N2 and the candidate. Five outer instruction-hash folds and three inner folds select ridge regularization using training data only; every outer-training `eta` entering covariance fitting is itself inner-cross-fitted, and the held-out `eta` is produced only from the corresponding outer training set. With the frozen 244/200/192-feature ObjAttr/OORel/OARel designs, all synthetic held-out channels beat an intercept predictor (`+381.71/+257.52/+150.49` binomial log-likelihood); N2 and the candidate are required to share these exact offsets, so topology gains cannot come from differential marginal fitting.
- Content-addressed the HSM authors' complete 500-scene SceneEval release manifest, but the current server identity receives HTTP 403 because the Hugging Face dataset requires ordinary gated-license acknowledgement. The gate is respected rather than bypassed. HSM can become only a bounded single-generator P0 after legitimate access and evaluator smoke validation; paper-level transport/generalization remains HOLD until a second independently qualified generator/external scene lane exists.
...
- Completed a zero-authority executability preflight without reading per-case evaluator outcomes: all 130 pinned LEGO-Bench manually aligned scene JSONs were materialized and content-addressed (6.87 MB total), and a complete local Qwen3-VL-8B checkpoint is already cached. LEGO-Eval can in principle use a local OpenAI-compatible backend, but the checked environments lack vLLM/AI2-THOR and the required Objathor assets plus 48 GB object-image release are not materialized. Holodeck, LayoutGPT, I-Design, and LayoutVLM remain source-faithful provider-bound baselines, so no generator experiment is execution-ready or authorized.
...
## 2026-08-28 · PORT-010 outcome-blind closure and formal-goal successor

- Ran a strictly outcome-blind EmbodiedBench E0 for PORT-010 before any `success` / `action_success` access. Two lexical BindingLoad proxies and the frozen `ConstraintGraph-v2` failed independent construct-transfer gates; the resulting `ROBUST_HOLD_AFTER_E0_CONSTRUCT_FAILURE` remains zero-authority and cannot be rescued by later switching from the frozen edge/depth endpoint to post-gate prevalence signals.
- Distilled the E0 failure into typed institutional memory: independent proxy-transfer failure, source-schema/identifier-semantics preflight, and proxy length/capability nonspecificity. These assets explicitly do not close the underlying scientific principle; they prohibit reusing a development-fit proxy as confirmatory mechanism evidence without an untouched outcome-blind construct surface.
- Opened a fresh successor object around BEHAVIOR-1K's author-defined BDDL goal logic instead of another natural-language complexity proxy. `shared_argument_edge_count` is frozen as the formal coupling variable with mandatory `atomic_goal_count` size adjustment; synthetic invariants and official-parser integration pass. A single independently reviewed v1.1 repair corrected BEHAVIOR's `?`-prefixed concrete-object grounding semantics without changing the frozen edge/leaf/depth/branch definitions or thresholds, after which all seven 1016-task structure gates pass.
- Audited historical 2025 public challenge outcomes without scientific statistics. The 50-task formal-goal snapshot is provenance-stable across the 2025 evaluation window, but 0/15 standard-public submission artifacts satisfy the preregistered complete 50×10 coverage; missing-as-zero and attempted-only rescues are therefore forbidden. The public 2025 confirmatory outcome lane is closed on evidence completeness rather than interpreted as a scientific null.
- Froze the 2026 public-outcome contract while the official leaderboard's `submissions.jsonl`, `results.jsonl`, and `per_task_results.jsonl` are all empty. The release watch keys only on those 2026 target OID/size changes plus schema drift; 2025 history, portal activity, and running-agent counts cannot trigger scientific admission. Current local replication also remains HOLD: the official challenge provides two starter baseline families and only task-local ready-checkpoint examples, so no 3-policy × 100-task scientific rollout is authorized.
