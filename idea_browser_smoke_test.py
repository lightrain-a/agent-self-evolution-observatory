...
def main() -> None:
    ...
    try:
        ...
        system = execute(session_id, """return {
          ...
          agentSafetySupportedStatus: window.AGENT_SAFETY_PROGRAM_STATE?.future_evidence?.claim_adjudication?.supported_status || '',
          agentSafetyCausalStatus: window.AGENT_SAFETY_PROGRAM_STATE?.future_evidence?.claim_adjudication?.causal_hold_status || '',
          agentSafetyFutureEpisodes: window.AGENT_SAFETY_PROGRAM_STATE?.future_evidence?.future_first_violation?.future_episodes,
          agentSafetyFutureViolations: window.AGENT_SAFETY_PROGRAM_STATE?.future_evidence?.future_first_violation?.future_violation_episodes,
          agentSafetyHistoricalStage: window.AGENT_SAFETY_PROGRAM_STATE?.historical_projection?.current_stage || '',
          agentSafetyControlStatus: window.AGENT_SAFETY_PROGRAM_STATE?.reopen_control_design?.status || '',
          agentSafetyControlPassed: window.AGENT_SAFETY_PROGRAM_STATE?.reopen_control_design?.requirements_passed,
          agentSafetyControlHolds: window.AGENT_SAFETY_PROGRAM_STATE?.reopen_control_design?.requirements_on_hold,
          agentSafetyControlExecution: window.AGENT_SAFETY_PROGRAM_STATE?.reopen_control_design?.execution_authorized === true,
          agentSafetyControlGpu: window.AGENT_SAFETY_PROGRAM_STATE?.reopen_control_design?.gpu_authorized === true,
        ...
        require(system["agentSafetySummary"] == 1 and system["agentSafetyStage"] == "PAPER_EVIDENCE_READY_CAUSAL_ATTRIBUTION_HOLD" and system["agentSafetySupportedStatus"] == "SUPPORTED_NARROWLY" and system["agentSafetyCausalStatus"] == "HOLD_METHOD_IDENTIFICATION" and (system["agentSafetyFutureEpisodes"],system["agentSafetyFutureViolations"]) == (36,11) and system["agentSafetyHistoricalStage"] == "CURRENT_SAFETY_SUPPORT_STOP" and system["agentSafetyRuntimeStatus"] == "READY_RUNTIME_MODEL_ASSETS_PINNED" and system["agentSafetyBoundedEvidence"] is False and system["agentSafetyQualification"] is False and system["agentSafetyOverallExecution"] is False and system["agentSafetyQualificationStatus"] == "STOP_SUPPORT_ZERO_CURRENTLY_SAFE_FROZEN_STATES" and system["agentSafetyQualifiedStates"] == 0 and system["agentSafetyPrincipleDeadEnd"] is False and system["agentSafetyHeldoutFuture"] is False and system["agentSafetyP0"] is False and system["agentSafetyGpu"] is False, f"research-system Agent Safety current-evidence/history projection drift: {system['agentSafetySummary']}/{system['agentSafetyStage']}/{system['agentSafetySupportedStatus']}/{system['agentSafetyCausalStatus']}/{system['agentSafetyHistoricalStage']}")
        require(system["agentSafetyControlStatus"] == "DESIGN_COMPILED_GATES_UNSATISFIED" and (system["agentSafetyControlPassed"],system["agentSafetyControlHolds"]) == (2,5) and system["agentSafetyControlExecution"] is False and system["agentSafetyControlGpu"] is False and "design compiled, not run" in system["text"], f"research-system Agent Safety reopen control drift: {system['agentSafetyControlStatus']}/{system['agentSafetyControlPassed']}/{system['agentSafetyControlHolds']}")
        ...
        require(system["agentSafetyMetadata"] == "VERIFIED" and system["agentSafetyMetadataTransport"] == "GITHUB_ACTIONS_LITERAL_HF_CAPTURE" and system["agentSafetyReceiptClass"] == "NON_AUTHORITATIVE_CACHE_CONTENT_CHECK" and "240/256" in system["text"] and "SUPPORTED_NARROWLY" in system["text"] and "HOLD_METHOD_IDENTIFICATION" in system["text"], f"research-system Agent Safety current evidence/history display drift: {system['agentSafetyMetadata']}/{system['agentSafetyMetadataTransport']}/{system['agentSafetyReceiptClass']}")
        ...
        ideas = execute(session_id, """return {
          ...
          agentSafetyControlExecution: window.AGENT_SAFETY_PROGRAM_STATE?.reopen_control_design?.execution_authorized === true,
          agentSafetyControlGpu: window.AGENT_SAFETY_PROGRAM_STATE?.reopen_control_design?.gpu_authorized === true,
          agentSafetyControlBadges: [...document.querySelectorAll('#agent-safety-program .experiment-status-badge')].filter(x => (x.textContent || '').includes('DESIGN_COMPILED_GATES_UNSATISFIED')).length,
        ...
        require(ideas["agentSafetyProgram"] == 1 and ideas["agentSafetyStage"] == system["agentSafetyStage"] and ideas["agentSafetySupportedStatus"] == "SUPPORTED_NARROWLY" and ideas["agentSafetyCausalStatus"] == "HOLD_METHOD_IDENTIFICATION" and (ideas["agentSafetyFutureEpisodes"],ideas["agentSafetyFutureViolations"]) == (36,11) and ideas["agentSafetyHistoricalStage"] == "CURRENT_SAFETY_SUPPORT_STOP" and ideas["agentSafetyRuntimeStatus"] == system["agentSafetyRuntimeStatus"] and ideas["agentSafetyBoundedEvidence"] is False and ideas["agentSafetyQualification"] is False and ideas["agentSafetyOverallExecution"] is False and ideas["agentSafetyQualificationStatus"] == "STOP_SUPPORT_ZERO_CURRENTLY_SAFE_FROZEN_STATES" and ideas["agentSafetyQualifiedStates"] == 0 and ideas["agentSafetyPrincipleDeadEnd"] is False and ideas["agentSafetyHeldoutFuture"] is False and ideas["agentSafetyP0"] is False and ideas["agentSafetyGpu"] is False, f"Paper Ideas Agent Safety current-evidence/history projection drift: {ideas['agentSafetyProgram']}/{ideas['agentSafetyStage']}/{ideas['agentSafetySupportedStatus']}/{ideas['agentSafetyCausalStatus']}/{ideas['agentSafetyHistoricalStage']}")
        require(ideas["agentSafetyControlStatus"] == "DESIGN_COMPILED_GATES_UNSATISFIED" and (ideas["agentSafetyControlPassed"],ideas["agentSafetyControlHolds"]) == (2,5) and ideas["agentSafetyControlExecution"] is False and ideas["agentSafetyControlGpu"] is False and ideas["agentSafetyControlBadges"] == 1, f"Paper Ideas Agent Safety reopen control drift: {ideas['agentSafetyControlStatus']}/{ideas['agentSafetyControlPassed']}/{ideas['agentSafetyControlHolds']}")
        ...
        require(ideas["agentSafetyMetadata"] == "VERIFIED" and ideas["agentSafetyMetadataTransport"] == "GITHUB_ACTIONS_LITERAL_HF_CAPTURE" and ideas["agentSafetyReceiptClass"] == "NON_AUTHORITATIVE_CACHE_CONTENT_CHECK" and "240/256" in ideas["text"] and "SUPPORTED_NARROWLY" in ideas["text"] and "HOLD_METHOD_IDENTIFICATION" in ideas["text"] and "0/4" in ideas["text"], f"Paper Ideas Agent Safety current evidence/history display drift: {ideas['agentSafetyMetadata']}/{ideas['agentSafetyMetadataTransport']}/{ideas['agentSafetyReceiptClass']} markers={[(m, m in ideas['text'].upper()) for m in ('240/256','SUPPORTED_NARROWLY','HOLD_METHOD_IDENTIFICATION','0/4')]}")
        ...
        require(sds.get("current_source_hard_veto_dead_ends") == 0 and sds.get("current_source_hard_veto_dead_ends") == int(sds.get("current_source_hard_veto_added_from_latest_run") or 0)+int(sds.get("current_source_hard_veto_added_from_terminal_run") or 0)+int(sds.get("current_source_hard_veto_inherited") or 0), f"current-source hard-veto memory accounting is inconsistent: {sds}")
