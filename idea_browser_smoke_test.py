...
def main() -> None:
    ...
    try:
        ...
        require((selected["chapters"],selected["currentSTRI"],selected["archive"],selected["archiveOpen"],selected["acceptancePanels"],selected["acceptanceStages"]) == (5,1,1,False,1,12), f"PaperRegistry must keep STRI current, expose the 12-stage canonical acceptance workflow, and keep the former project inside one collapsed historical archive: {selected}")
        require(selected["paperRegistrySummary"].get("papers") == 2 and selected["paperRegistrySummary"].get("submission_ready") == 1 and selected["paperRegistrySummary"].get("scientific_holds") == 1 and selected["paperRegistryPanel"] == 1 and selected["paperRegistryCards"] == 2 and set(selected["paperRegistryIds"]) == {"STRI","AGENT-SAFETY-R9"} and selected["paperRegistryStages"] == {"STRI":"SUBMISSION_READY","AGENT-SAFETY-R9":"PAPER_EVIDENCE"}, f"PaperRegistry overview is missing or stale: {selected}")
        ...
        require("PaperRegistry" in selected["text"] and "SUBMISSION_READY" in selected["text"] and "AGENT-SAFETY-R9" in selected["text"] and "CAUSAL_HOLD" in selected["text"] and "非当前 PaperState · 历史归档" in selected["text"] and "真正 Submission Ready=1" in selected["text"] and "Manuscript CI=9/9 PASS" in selected["text"] and "Prebuttal=PASS" in selected["text"] and "论文侧闭环已经完成" in selected["text"], "PaperRegistry / STRI submission-ready boundary or historical archive is missing")
        require(set(selected["submissionDownloads"]) == {"downloads/STRI-ICLR2027-submission-ready-20260821.pdf","downloads/STRI-ICLR2027-submission-ready-20260821.tex","downloads/STRI-ICLR2027-submission-ready-20260821-source.zip"}, f"PaperState submission-ready download set is stale: {selected['submissionDownloads']}")
