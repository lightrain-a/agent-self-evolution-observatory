...
def main() -> None:
    ...
    try:
        ...
        require(stri.get("status")=="READY_NARROW_ICLR" and stri_summary.get("paper_ready")==1 and (stri_summary.get("claims_supported"),stri_summary.get("claims_total"))==(3,3) and (stri_summary.get("qa_checks_passed"),stri_summary.get("qa_checks_total"))==(59,59), f"asset-first STRI paper-ready projection is stale: {stri}")
        ...
        require("ASSET-FIRST PAPER-READY" in system["text"] and "READY_NARROW_ICLR" in system["text"] and "59/59 PASS" in system["text"] and f"Problem-Gate PASS={int(canonical_queue_summary.get('passed_problem_gate') or 0)}" in system["text"], "asset-first STRI/canonical dual-track status is not rendered")
        ...
        require(sds.get("current_source_hard_veto_dead_ends") == 2 and sds.get("current_source_hard_veto_dead_ends") == int(sds.get("current_source_hard_veto_added_from_latest_run") or 0)+int(sds.get("current_source_hard_veto_added_from_terminal_run") or 0)+int(sds.get("current_source_hard_veto_inherited") or 0), f"current-source hard-veto memory accounting is inconsistent: {sds}")
        require(int(sds.get("semantic_blocker_dead_ends") or 0)+int(sds.get("semantic_hold_objects") or 0) == 4 and int(sds.get("semantic_hold_objects") or 0) == int(sds.get("semantic_hold_added_from_latest_run") or 0)+int(sds.get("semantic_hold_added_from_terminal_run") or 0)+int(sds.get("semantic_hold_inherited") or 0), f"semantic dead-end/HOLD memory accounting is inconsistent: {sds}")
        require(int(sds.get("near_miss_preflight_dead_ends") or 0)+int(sds.get("near_miss_holds") or 0) == 17 and (sds.get("near_miss_support_holds"),sds.get("near_miss_terminal_support_holds"),sds.get("near_miss_current_primary_stops"),sds.get("near_miss_mature_theory_stops")) == (1,13,2,1), f"near-miss dead-end/HOLD memory accounting is inconsistent: {sds}")
        ...
        require(ideas["shadowLatestRun"].get("run_id") == "shadow-auto-a0ea279fac5b4997" and (latest.get("requested_raw_seeds"),latest.get("expansion_successful_shards"),latest.get("expansion_execution_failures"),latest.get("raw_seeds"),latest.get("semantic_unique"),latest.get("evolved_branches"),latest.get("evolution_g1_requested"),latest.get("evolution_g1_valid"),latest.get("evolution_g2_requested"),latest.get("evolution_g2_valid"),latest.get("formulation_successful_shards"),latest.get("formulation_provider_failures"),latest.get("formulation_parse_failures"),latest.get("formulation_successful_branches"),latest.get("formulation_execution_censored_branches"),latest.get("formulated_candidates"),latest.get("formulation_reduction_pending",0),latest.get("machine_reviewable"),latest.get("machine_reduction_pending",0),latest.get("problem_falsifier_eligible"),latest.get("problem_falsifier_inventory_requested"),latest.get("problem_falsifier_support_qualified"),latest.get("problem_falsifier_hold_support_unavailable"),latest.get("problem_falsifier_executed"),latest.get("semantic_clear"),latest.get("terminal_shadow_survivors")) == (120,20,0,103,36,35,24,23,12,12,11,1,0,22,2,0,9,0,9,9,9,0,9,0,0,0), f"latest shadow r6 funnel/accounting is stale: {ideas['shadowLatestRun']}")
        require((latest.get("current_source_missing"),latest.get("current_source_reviewed"),latest.get("live_paper_design_eligible"),ideas["shadowLatestPanels"]) == (0,0,0,1), f"latest shadow r6 must be terminal-complete, zero-live-authority, and rendered once: {latest}/{ideas['shadowLatestPanels']}")
        shadow_admission=ideas["shadowAdmission"];shadow_admission_summary=shadow_admission.get("summary") or {}
        require(shadow_admission.get("status") == "SKIPPED_SHADOW_SOURCE_TRANSACTION_ALREADY_TERMINAL" and shadow_admission_summary.get("canonical_transaction_closed") is True and shadow_admission_summary.get("same_source_transaction") is True and shadow_admission_summary.get("qualification_allowed") is False and shadow_admission_summary.get("automatic_provider_calls_authorized") == 0 and ideas["shadowAdmissionPanels"] == 1, f"shadow next-run admission must skip the terminal same-source transaction with zero provider authority: {shadow_admission}/{ideas['shadowAdmissionPanels']}")
        ...
        require("Shadow Search Portfolio · retrospective Paper Design audit" in ideas["text"] and "Shadow Search Portfolio · latest current-source terminal" in ideas["text"] and "Shadow Search · next-run admission" in ideas["text"] and "SKIPPED_SHADOW_SOURCE_TRANSACTION_ALREADY_TERMINAL" in ideas["text"] and "provider-calls=0" in ideas["text"] and "qualifying query-level units=0" in ideas["text"] and "current-source hard-veto memory=2" in ideas["text"] and "semantic reduction/lane memory=4" in ideas["text"] and "near-miss memory=17" in ideas["text"] and "terminal support HOLD=13" in ideas["text"] and "last terminal memory=shadow-auto-a0ea279fac5b4997" in ideas["text"] and "mature-theory STOP=1" in ideas["text"] and "control=1.4/a2cad5d626dd" in ideas["text"] and "G1=23/24" in ideas["text"] and "G2=12/12" in ideas["text"] and "execution-censored=2" in ideas["text"] and "reduction-pending=9" in ideas["text"] and "machine-pending=9" in ideas["text"] and "problem-falsifier=9" in ideas["text"] and "inventory-request=9" in ideas["text"] and "support-qualified=0" in ideas["text"] and "HOLD=9" in ideas["text"] and "executed=0" in ideas["text"], "shadow Paper Design/latest terminal/admission/execution/dead-end/SP-15 results are not rendered")
