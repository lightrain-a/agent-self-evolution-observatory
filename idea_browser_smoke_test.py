...
def main() -> None:
    ...
    expected_registry_papers = {str(row.get("paper_id") or ""): row for row in (expected_registry.get("papers") or [])}
    expected_registry_stages = {paper_id: str(row.get("paper_stage") or "") for paper_id,row in expected_registry_papers.items()}
    expected_stri_prebuttal = (expected_registry_papers.get("STRI") or {}).get("latest_prebuttal") or {}
    expected_safety_prebuttal = (expected_registry_papers.get("AGENT-SAFETY-R9") or {}).get("latest_prebuttal") or {}
    expected_stri_downloads = set(((expected_registry_papers.get("STRI") or {}).get("downloads") or {}).values())
    expected_safety_downloads = set(((expected_registry_papers.get("AGENT-SAFETY-R9") or {}).get("downloads") or {}).values())
    ...
    try:
        ...
        system = execute(session_id, """return {
          ...
          memorySummary: window.RESEARCH_MEMORY_WIKI?.summary || {},
        ...
        require(system["memorySummary"].get("paper_development_guidance") == 1 and len(system["paperDevelopmentGuidance"]) == 1 and len((system["paperDevelopmentGuidance"][0] or {}).get("paper_development_backlog") or []) == 5, f"paper-development guidance/backlog is missing from Research Memory: {system['memorySummary']}/{system['paperDevelopmentGuidance']}")
        tpl=system["iclrTemplate"] or {};lanes=tpl.get("experiment_lanes") or []
        require(system["iclrTemplatePanels"] == 1 and len(tpl.get("derived_from") or []) == 8 and len(lanes) == 7 and sum(row.get("required") is True for row in lanes) == 6 and all(value is False for value in (tpl.get("authority") or {}).values()), f"ICLR paper template must expose 8 exemplars / 7 lanes / zero authority: {tpl}")
        require(all(marker in system["text"] for marker in ("Scientific closure is not the same as manuscript maturity","Problem necessity, challenge, and related work","Method intuition, design principles, and load-bearing details","Plain, direct, reader-comprehensible writing","ICLR PAPER TEMPLATE V1","E1 · Main comparison","E6 · Efficiency / cost / scale")), "system-overview must render the senior paper-development guidance and ICLR template in English")
        ...
        require(system["lessons"] == 6 + int(system["memorySummary"].get("discovery_lessons") or 0) and system["failureLayers"] == 7 and system["repairLoops"] == 1, f"learning/diagnosis visualization is incomplete: {system['lessons']}/{system['failureLayers']}/{system['repairLoops']}")
        ...
        require(all(marker in zh_system_text for marker in ("科学闭环不等于论文已经成熟","问题必要性、Challenge 与 Related Work","方法核心 Intuition、设计理念与全部关键细节","更完整的实验 Program，而不是只有一张主表","讲人话：清晰、直白、易懂的论文写作","固定论文论证与实验骨架","不授权实际运行")), "system-overview Chinese paper-development/ICLR-template guidance is incomplete")
        ensure_language("en")
        stri=system["assetFirstSTRI"] or {};stri_summary=stri.get("summary") or {};stri_authority=stri.get("authority") or {};canonical_queue_summary=(system["problemQueue"] or {}).get("summary") or {};legacy_download_bytes_current=(stri.get("gates") or {}).get("public_download_assets") is True
        legacy_state_ok=(legacy_download_bytes_current and stri.get("status")=="READY_NARROW_ICLR" and stri.get("submission_status")=="READY_TO_SUBMIT_PENDING_HUMAN_AUTHOR_SIGNOFF_AND_OPENREVIEW" and stri_summary.get("paper_ready")==1) or ((not legacy_download_bytes_current) and stri.get("status")=="HOLD_ASSET_FIRST_PAPER_NOT_READY" and stri.get("submission_status")=="HOLD_PAPER_QUALITY_V2" and stri_summary.get("paper_ready")==0)
        require(legacy_state_ok and stri_summary.get("paper_quality_v2_passed")==1 and stri_summary.get("paper_quality_content_addressed_completion")==1 and stri_summary.get("paper_quality_content_addressed_files")==29 and stri_summary.get("paper_quality_evidence_debt")==0 and stri_summary.get("paper_quality_main_visualizations")==4 and "failure" in (stri_summary.get("paper_quality_main_visual_roles") or []) and (stri_summary.get("paper_quality_missing_ids") or [])==[] and (stri_summary.get("claims_supported"),stri_summary.get("claims_total"))==(3,3) and int(stri_summary.get("qa_checks_total") or 0)>0 and stri_summary.get("qa_checks_passed")==stri_summary.get("qa_checks_total") and int(stri_summary.get("official_qa_checks_total") or 0)>0 and stri_summary.get("official_qa_checks_passed")==stri_summary.get("official_qa_checks_total") and (stri_summary.get("main_text_pages"),stri_summary.get("main_text_page_limit"))==(9,9) and stri_summary.get("supplement_ready")==1 and stri_summary.get("supplement_unit_tests")=="29/29 PASS" and stri_summary.get("human_signoff_pending")==1 and int(stri_summary.get("new_gpu_evidence_required") or 0)==0, f"historical asset-first STRI projection must fail safe only when its old download hashes are superseded: {stri}")
        ...
        ideas = execute(session_id, """return {
          ...
          decisionConsoles: document.querySelectorAll('#portfolio-current.portfolio-decision-console').length,
          decisionMetrics: document.querySelectorAll('#portfolio-current .portfolio-decision-metrics > span').length,
          attentionCards: document.querySelectorAll('#portfolio-current .portfolio-attention-card').length,
          outsideCandidates: document.querySelectorAll('#portfolio-current .portfolio-outside-candidate').length,
          auditFolds: document.querySelectorAll('#portfolio-audit.portfolio-audit-fold').length,
          openAuditFolds: document.querySelectorAll('#portfolio-audit.portfolio-audit-fold[open]').length,
          decisionConsoleText: document.querySelector('#portfolio-current')?.textContent || '',
          ...
          closedTechnicalReasonsPreserved: document.querySelectorAll('.closed-idea-stop p').length,
          closedReaderJudgmentsAllZh: [...document.querySelectorAll('.closed-one-minute-briefing [data-briefing-part="judgment"] p')].every(node => /[\u3400-\u9fff]/.test(node.textContent || '')),
          closedAuditReopensAllZh: [...document.querySelectorAll('.closed-one-minute-briefing [data-briefing-part="next"] p')].every(node => /[\u3400-\u9fff]/.test(node.textContent || '')),
          statusOuterTracks: (()=>{const x=document.querySelector('.project-status-strip.current');return x?getComputedStyle(x).gridTemplateColumns.trim().split(/\\s+/).length:0;})(),
          statusCopyTracks: (()=>{const x=document.querySelector('.project-status-copy');return x?getComputedStyle(x).gridTemplateColumns.trim().split(/\\s+/).length:0;})(),
          statusMetricTracks: (()=>{const x=document.querySelector('.project-status-metrics');return x?getComputedStyle(x).gridTemplateColumns.trim().split(/\\s+/).length:0;})(),
        ...
        require(ideas["categorizedContextBanks"] == 1 and ideas["openCategorizedContextBanks"] == 0 and ideas["categorizedContextCards"] == 2 and set(ideas["categorizedContextIds"]) == {"MEM-HISTORY","SP-15"}, f"B-category evidence context must stay complete and default-collapsed without splitting the STRI paper chain into peer cards: {ideas['categorizedContextBanks']}/{ideas['openCategorizedContextBanks']}/{ideas['categorizedContextCards']}/{ideas['categorizedContextIds']}")
        ...
        require((ideas["briefingHeroes"],ideas["briefingMetrics"],ideas["briefingDecisions"]) == (0,0,0), f"legacy briefing hero/metrics/decision blocks must stay retired from the default paper-ideas control plane: {ideas['briefingHeroes']}/{ideas['briefingMetrics']}/{ideas['briefingDecisions']}")
        require((ideas["decisionConsoles"],ideas["decisionMetrics"],ideas["attentionCards"],ideas["outsideCandidates"],ideas["auditFolds"],ideas["openAuditFolds"]) == (1,4,6,1,1,0) and all(marker in ideas["decisionConsoleText"] for marker in ("当前科研", "先决策后审计", "当前关注对象", "等待重开条件", "PaperState handoff", "machine-actionable", "完整账本", "正式实验权限=0")), f"decision-first control plane is incomplete or the audit layer is not default-collapsed: {ideas}")
        require((ideas["briefingGuides"],ideas["briefingLessons"],ideas["briefingTaxonomyCards"],ideas["briefingModeButtons"],ideas["categoryBriefings"]) == (1,3,6,2,7), f"audit-layer briefing guide/taxonomy/category summaries are incomplete: {ideas}")
        ...
        require((ideas["newGroups"],ideas["openRelatedBanks"],ideas["categoryRelatedBanks"],ideas["openCategoryRelatedBanks"],ideas["newCards"]) == (11,0,10,0,7), f"all related-direction and B-context banks must remain present but default-collapsed in the decision-first view, while the E-7 paper chain renders as one ResearchItem handoff: {ideas['newGroups']}/{ideas['openRelatedBanks']}/{ideas['categoryRelatedBanks']}/{ideas['openCategoryRelatedBanks']}/{ideas['newCards']}")
        ...
        require((ideas["closedIdeaBanks"],ideas["openClosedIdeaBanks"]) == (6,0) and ideas["closedIdeaCards"] == expected_shadow_closed and ideas["mergedClosureRows"] == 3 and ideas["closedIdeaCards"] + ideas["mergedClosureRows"] == expected_closed == len(closed_rows), f"all closure decisions must remain represented as default-collapsed canonical numbered banks plus three merges: banks={ideas['closedIdeaBanks']} open={ideas['openClosedIdeaBanks']} cards={ideas['closedIdeaCards']} expected_cards={expected_shadow_closed} merged={ideas['mergedClosureRows']} expected={expected_closed}")
        ...
        require(ideas["closedTechnicalReasonsPreserved"] == expected_shadow_closed and ideas["closedReaderJudgmentsAllZh"] and ideas["closedAuditReopensAllZh"], f"all numbered closed ideas must retain exact technical stop reasons while reader-layer judgments and next steps render in Chinese: technical={ideas['closedTechnicalReasonsPreserved']} expected={expected_shadow_closed} judgment_zh={ideas['closedReaderJudgmentsAllZh']} next_zh={ideas['closedAuditReopensAllZh']}")
        status_layout=(ideas["statusOuterTracks"],ideas["statusCopyTracks"],ideas["statusMetricTracks"],ideas["statusMetricCount"])
        require(status_layout in {(1,2,5,10),(0,0,0,0)}, f"when the paper-ideas page renders the project status strip it must retain the title/message + five-column metric layout; pages without that strip must return zero layout metrics: {status_layout}")
        ...
        require(len(ideas["closedIdeaBankLabels"]) == 6 and all(label == "已停止的编号研究方向" for label in ideas["closedIdeaBankLabels"]), f"all six numbered stopped-idea banks must retain the unified label: {ideas['closedIdeaBankLabels']}")
        ...
        toc_roots=selected["paperTocRoots"]
        require(len(toc_roots) == 5 and all(marker in toc_roots[index] for index,marker in enumerate(("E1","G1","C1","E2","B1"))) and "STRI" in toc_roots[0] and "R9" in toc_roots[1] and "Proxy Reward" in toc_roots[2] and "Temporal Skill" in toc_roots[3] and "Provenance Ladder" in toc_roots[4] and all(count >= 3 for count in selected["paperTocChildCounts"]), f"selected-paper left hierarchy must expose publication-facing E1/G1/C1/E2/B1 paper labels with their second-level sections: {toc_roots}/{selected['paperTocChildCounts']}")
        ...
        temporal=selected["temporalPaper"]
        ...
        require(selected["paperRegistrySummary"] == expected_registry_summary and selected["paperRegistryPanel"] == 1 and selected["paperRegistryCards"] == 5 and set(selected["paperRegistryIds"]) == expected_registry_ids and selected["paperRegistryStages"].get("D2-PAPER-TEMPORAL-SKILL-CAUSAL-BOTTLENECK") == "SUBMISSION_READY" and selected["paperRegistryStages"].get("D2-PAPER-FAILURE-MEMORY-PROVENANCE") == "SUBMISSION_READY" and selected["paperRegistryNextActions"].get("D2-PAPER-TEMPORAL-SKILL-CAUSAL-BOTTLENECK") == temporal_action and selected["paperRegistryNextActions"].get("D2-PAPER-FAILURE-MEMORY-PROVENANCE") == "NO_INTERNAL_ACTION" and temporal.get("gate_clean_submission_ready") is temporal_clean and int(temporal_prep.get("required_gates") or 0) == 8 and (temporal_clean or bool(temporal_prep.get("blockers"))) and ((temporal.get("latest_mock_review") or {}).get("summary") or {}).get("scores") == [8,8,7] and ((temporal.get("source_native_evidence") or {}).get("runtime_valid_rows"),(temporal.get("source_native_evidence") or {}).get("distinct_endpoints"),(temporal.get("source_native_evidence") or {}).get("institutional_systems")) == (1326,35,3), f"PaperRegistry must follow the latest Temporal paper-quality receipt without losing source-native evidence: {selected}")
        paper=selected["currentPaper"]
        require(paper.get("paper_id") == "STRI" and paper.get("source_research_item") == "E-7" and paper.get("paper_stage") == "SUBMISSION_READY" and paper.get("scientific_status") == "READY" and paper.get("submission_ready") is True and paper.get("paper_quality_v2_passed") is True and paper.get("paper_quality_content_addressed_completion") is True and paper.get("paper_quality_content_addressed_files") == 29 and paper.get("paper_quality_evidence_debt") == 0 and (paper.get("qa_passed"),paper.get("qa_total")) == (60,60) and (paper.get("official_qa_passed"),paper.get("official_qa_total")) == (60,60) and paper.get("paper_quality_schema_version") == "2.1" and paper.get("paper_quality_main_visualizations") == 4 and paper.get("paper_visual_figure_qa") == "PASS" and paper.get("supplement_unit_tests") == "29/29 PASS" and paper.get("official_source_conflict") is False and paper.get("deadline_status") == "AUTHOR_SUBMISSION_SOURCES_ALIGNED" and paper.get("operational_safe_abstract_deadline_aoe") == "2026-09-18" and paper.get("operational_safe_full_paper_deadline_aoe") == "2026-09-25" and paper.get("recorded_author_guide_abstract_deadline_aoe") == "2026-09-18" and paper.get("recorded_author_guide_full_paper_deadline_aoe") == "2026-09-25" and paper.get("author_membership_freezes_at_abstract_deadline") is True and paper.get("title_freezes_at_full_paper_deadline") is True and (paper.get("latest_story_search") or {}).get("pass") is True and bool((paper.get("mock_pc_modes") or {}).get("BLIND_MANUSCRIPT")) and bool((paper.get("mock_pc_modes") or {}).get("ARTIFACT_AWARE")) and (paper.get("latest_claim_audit") or {}).get("pass") is True and (paper.get("latest_manuscript_ci") or {}).get("pass") is True and ((paper.get("latest_manuscript_ci") or {}).get("passed"),(paper.get("latest_manuscript_ci") or {}).get("required")) == (9,9) and (paper.get("latest_prebuttal") or {}).get("pass") is True and (paper.get("latest_prebuttal") or {}).get("decision_critical") == expected_stri_prebuttal.get("decision_critical") and (paper.get("latest_prebuttal") or {}).get("unresolved_decision_critical") == expected_stri_prebuttal.get("unresolved_decision_critical") and (paper.get("latest_submission_readiness") or {}).get("submission_ready") is True and (paper.get("latest_transition") or {}).get("from") == "PREBUTTAL" and (paper.get("latest_transition") or {}).get("to") == "SUBMISSION_READY" and (paper.get("latest_transition") or {}).get("allowed") is True and (paper.get("authority") or {}).get("submission") is False and selected["currentStatus"].get("paper_ready") == 1, f"STRI PaperState projection is stale: {paper}")
        safety_paper=selected["agentSafetyPaper"]
        require(safety_paper.get("source_research_item") == "G-1" and safety_paper.get("paper_stage") == "SUBMISSION_READY" and safety_paper.get("scientific_status") == "READY" and safety_paper.get("submission_ready") is True and (safety_paper.get("latest_story_search") or {}).get("selected_story_id") == "S1-TEMPORAL-CERTIFICATE-CONTROL" and all((safety_paper.get("mock_pc_modes") or {}).get(mode) for mode in ("BLIND_MANUSCRIPT","ARTIFACT_AWARE")) and (safety_paper.get("latest_claim_audit") or {}).get("pass") is True and ((safety_paper.get("latest_manuscript_ci") or {}).get("passed"),(safety_paper.get("latest_manuscript_ci") or {}).get("required")) == (9,9) and (safety_paper.get("latest_prebuttal") or {}).get("decision_critical") == expected_safety_prebuttal.get("decision_critical") and (safety_paper.get("latest_prebuttal") or {}).get("unresolved_decision_critical") == expected_safety_prebuttal.get("unresolved_decision_critical") and (safety_paper.get("latest_submission_readiness") or {}).get("submission_ready") is True and (safety_paper.get("authority") or {}).get("submission") is False, f"Agent Safety bounded R9 PaperState projection is stale: {safety_paper}")
        safety_acceptance=selected["agentSafetyAcceptance"]
        require(safety_acceptance.get("current_state") == "SUBMISSION_READY" and safety_acceptance.get("scientific_status") == "READY" and (safety_acceptance.get("latest_story_search") or {}).get("selected_story_id") == "S1-TEMPORAL-CERTIFICATE-CONTROL" and (safety_acceptance.get("latest_claim_audit") or {}).get("pass") is True and ((safety_acceptance.get("latest_manuscript_ci") or {}).get("passed"),(safety_acceptance.get("latest_manuscript_ci") or {}).get("required")) == (9,9) and (safety_acceptance.get("latest_prebuttal") or {}).get("decision_critical") == expected_safety_prebuttal.get("decision_critical") and (safety_acceptance.get("latest_prebuttal") or {}).get("unresolved_decision_critical") == expected_safety_prebuttal.get("unresolved_decision_critical") and (safety_acceptance.get("latest_submission_readiness") or {}).get("submission_ready") is True and safety_acceptance.get("authority") == {"scientific":False,"experiment":False,"gpu":False,"submission":False}, f"canonical Agent Safety Paper Acceptance projection is stale or unsafe: {selected}")
        acceptance=selected["paperAcceptance"]
        require(acceptance.get("current_state") == "SUBMISSION_READY" and acceptance.get("scientific_status") == "READY" and (acceptance.get("latest_story_search") or {}).get("pass") is True and (acceptance.get("latest_story_search") or {}).get("selected_story_id") == "S1-INVARIANCE-BOUNDARY" and all((acceptance.get("mock_pc_modes") or {}).get(mode) for mode in ("BLIND_MANUSCRIPT","ARTIFACT_AWARE")) and (acceptance.get("latest_claim_audit") or {}).get("pass") is True and (acceptance.get("latest_manuscript_ci") or {}).get("pass") is True and ((acceptance.get("latest_manuscript_ci") or {}).get("passed"),(acceptance.get("latest_manuscript_ci") or {}).get("required")) == (9,9) and (acceptance.get("latest_prebuttal") or {}).get("pass") is True and (acceptance.get("latest_prebuttal") or {}).get("decision_critical") == expected_stri_prebuttal.get("decision_critical") and (acceptance.get("latest_prebuttal") or {}).get("unresolved_decision_critical") == expected_stri_prebuttal.get("unresolved_decision_critical") and (acceptance.get("latest_submission_readiness") or {}).get("submission_ready") is True and acceptance.get("authority") == {"scientific":False,"experiment":False,"gpu":False,"submission":False} and selected["paperAcceptanceSummary"].get("invalid_ledgers") == 0 and selected["paperAcceptanceSummary"].get("scientific_holds") == 0 and selected["paperAcceptanceSummary"].get("registered_papers") == 5 and selected["paperAcceptanceSummary"].get("ledger_submission_ready_papers") == expected_registry_summary.get("submission_ready") and selected["paperAcceptanceSummary"].get("gate_clean_submission_ready_papers") == expected_registry_summary.get("gate_clean_submission_ready") and selected["paperAcceptanceSummary"].get("internal_action_required_papers") == expected_registry_summary.get("internal_action_required") and selected["paperAcceptanceSummary"].get("no_internal_action_papers") == expected_registry_summary.get("no_internal_action"), f"canonical STRI Paper Acceptance projection is stale or unsafe: {selected}")
        ...
        dual_ready_markers=("PaperRegistry","SUBMISSION_READY","AGENT-SAFETY-R9","4 update-only / 0 control-only","非当前 PaperState · 历史归档","Research OS 下一步","When Reusable Temporal Skills Become Causal Bottlenecks","Does Memory Provenance Matter?","中央机制证据","1,326 rows","35 endpoints","Mock-PC=8/8/7","TimeSage-EV 精确复现","Manuscript CI=9/9 PASS","Prebuttal=PASS")
