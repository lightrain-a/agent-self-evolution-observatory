...
def main() -> None:
    ...
    try:
        ...
        require(stri.get("status")=="READY_NARROW_ICLR" and stri.get("submission_status")=="READY_TO_SUBMIT_PENDING_HUMAN_AUTHOR_SIGNOFF_AND_OPENREVIEW" and stri_summary.get("paper_ready")==1 and stri_summary.get("paper_quality_v2_passed")==1 and stri_summary.get("paper_quality_evidence_debt")==0 and (stri_summary.get("paper_quality_missing_ids") or [])==[] and (stri_summary.get("claims_supported"),stri_summary.get("claims_total"))==(3,3) and (stri_summary.get("qa_checks_passed"),stri_summary.get("qa_checks_total"))==(59,59) and (stri_summary.get("official_qa_checks_passed"),stri_summary.get("official_qa_checks_total"))==(50,50) and (stri_summary.get("main_text_pages"),stri_summary.get("main_text_page_limit"))==(9,9) and stri_summary.get("supplement_ready")==1 and stri_summary.get("supplement_unit_tests")=="11/11 PASS" and stri_summary.get("human_signoff_pending")==1 and int(stri_summary.get("new_gpu_evidence_required") or 0)==0, f"asset-first STRI Paper Quality v2 ready projection is stale: {stri}")
        ...
        require("ASSET-FIRST ICLR 2027 · PAPER QUALITY V2" in system["text"] and "Paper Quality v2" in system["text"] and "PASS · receipt binding=PASS · evidence debt=0" in system["text"] and "59/59 PASS" in system["text"] and "50/50 PASS" in system["text"] and "main=9/9 pages" in system["text"] and "tests=11/11 PASS" in system["text"] and f"Problem-Gate PASS={int(canonical_queue_summary.get('passed_problem_gate') or 0)}" in system["text"], "asset-first STRI Paper Quality v2/canonical dual-track status is not rendered")
