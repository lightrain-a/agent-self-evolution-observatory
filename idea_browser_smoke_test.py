...
def main() -> None:
    ...
    try:
        ...
        require(ideas["shadowLatestRun"].get("run_id") == "shadow-20260814-r4b" and (latest.get("requested_raw_seeds"),latest.get("expansion_successful_shards"),latest.get("expansion_execution_failures"),latest.get("raw_seeds"),latest.get("semantic_unique"),latest.get("evolved_branches"),latest.get("formulation_successful_shards"),latest.get("formulation_provider_failures"),latest.get("formulation_successful_branches"),latest.get("formulation_execution_censored_branches"),latest.get("formulated_candidates"),latest.get("machine_reviewable"),latest.get("problem_falsifier_eligible"),latest.get("problem_falsifier_hold_support_unavailable"),latest.get("semantic_clear"),latest.get("current_source_blocked"),latest.get("terminal_shadow_survivors")) == (120,19,1,95,45,36,10,2,20,4,12,1,4,4,1,1,0), f"latest shadow r4b funnel/accounting is stale: {ideas['shadowLatestRun']}")
        require((latest.get("current_source_missing"),latest.get("current_source_reviewed"),latest.get("live_paper_design_eligible"),ideas["shadowLatestPanels"]) == (0,1,0,1), f"latest shadow r4b must be current-source complete, zero-live-authority, and rendered once: {latest}/{ideas['shadowLatestPanels']}")
        ...
        require("Shadow Search Portfolio · retrospective Paper Design audit" in ideas["text"] and "Shadow Search Portfolio · latest current-source terminal" in ideas["text"] and "qualifying query-level units=0" in ideas["text"] and "current-source hard-veto memory=2" in ideas["text"] and "semantic reduction/lane memory=4" in ideas["text"] and "near-miss memory=4" in ideas["text"] and "mature-theory STOP=1" in ideas["text"] and "execution-censored=4" in ideas["text"] and "problem-falsifier=4" in ideas["text"] and "HOLD=4" in ideas["text"], "shadow Paper Design/latest terminal/execution/falsifier/dead-end/SP-15 results are not rendered")
