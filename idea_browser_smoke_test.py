...
def main() -> None:
    ...
    try:
        ...
        require(ideas["inventoryTotals"] == [91,91] and ideas["categoryRecordTotals"] == [12,20,10,3,27,6,13] and sum(ideas["categoryRecordTotals"]) == 91, f"full A-G research inventory must expose 91 deduplicated portfolio objects: {ideas['inventoryTotals']}/{ideas['categoryRecordTotals']}")
        ...
        require(ideas["oneMinuteSixPartCards"] == 81 and ideas["oneMinuteStandardCards"] == 6, f"all 81 one-minute briefings must implement the six-part decision-memory standard: cards={ideas['oneMinuteSixPartCards']} standard={ideas['oneMinuteStandardCards']}")
        ...
        require(all(marker in ideas["text"] for marker in ("8 个候选 patch 只有 1 个真正让目标任务变好","标准二元组测试在相同信息和预算下取得完全相同结果","简单的来源降权已经取得相同效果","保持当前能力不等于保留未来学习能力","真正的未来对照从未打开")), "representative A-3/A-6/C-1/PF-1/G-1 one-minute briefing copy is missing")
        ...
        require(ideas["closedIdeaBanks"] == ideas["openClosedIdeaBanks"] == 5 and ideas["closedIdeaCards"] == 38 and ideas["mergedClosureRows"] == 3 and ideas["closedIdeaCards"] + ideas["mergedClosureRows"] == expected_closed == len(closed_rows), f"all 41 closure decisions must be represented as 38 numbered cards plus three merges: banks={ideas['closedIdeaBanks']} cards={ideas['closedIdeaCards']} merged={ideas['mergedClosureRows']} expected={expected_closed}")
        require(len(ideas["closedIdeaCodes"]) == len(set(ideas["closedIdeaCodes"])) == 38 and set(ideas["closedIdeaCodes"]) == ({f"B-{n}" for n in range(14,21)} | {f"C-{n}" for n in range(8,11)} | {f"E-{n}" for n in range(8,25)} | {f"F-{n}" for n in range(4,7)} | {f"G-{n}" for n in range(6,14)}), f"numbered stopped-idea codes are incomplete, unstable, or duplicated: {ideas['closedIdeaCodes']}")
        require(len(ideas["allResearchObjectCodes"]) == len(set(ideas["allResearchObjectCodes"])) == 91, f"every deduplicated research object must have one unique A-G code: {ideas['allResearchObjectCodes']}")
        ...
        require(selected["paperRegistrySummary"].get("papers") == 5 and selected["paperRegistrySummary"].get("submission_ready") == 4 and (selected["paperRegistrySummary"].get("by_stage") or {}).get("TARGETED_REPAIR") == 1 and selected["paperRegistrySummary"].get("scientific_holds") == 0 and selected["paperRegistryPanel"] == 1 and selected["paperRegistryCards"] == 5 and set(selected["paperRegistryIds"]) == expected_registry_ids and selected["paperRegistryStages"].get("D2-PAPER-TEMPORAL-SKILL-CAUSAL-BOTTLENECK") == "SUBMISSION_READY" and selected["paperRegistryStages"].get("D2-PAPER-FAILURE-MEMORY-PROVENANCE") == "TARGETED_REPAIR", f"PaperRegistry 5/4/1 overview is missing or stale: {selected}")
        ...
        dual_ready_markers=("PaperRegistry","SUBMISSION_READY","AGENT-SAFETY-R9","4 update-only / 0 control-only","非当前 PaperState · 历史归档","真正 Submission Ready=4","Temporal Skills Are a Causal Bottleneck","Failure Memories Are Not Neutral","TARGETED_REPAIR","Manuscript CI=9/9 PASS","Prebuttal=PASS","论文侧闭环已经完成")
