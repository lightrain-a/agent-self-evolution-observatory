...
def main() -> None:
    ...
    try:
        ...
        require(int(sds.get("near_miss_preflight_dead_ends") or 0) == int(sds.get("near_miss_current_primary_stops") or 0)+int(sds.get("near_miss_mature_theory_stops") or 0) and int(sds.get("near_miss_holds") or 0) == int(sds.get("near_miss_support_holds") or 0)+int(sds.get("near_miss_terminal_support_holds") or 0) and int(sds.get("shadow_dead_end_objects") or 0) >= int(sds.get("principle_readjudication_dead_ends") or 0) >= 1 and int(sds.get("shadow_hold_objects") or 0) >= int(sds.get("near_miss_holds") or 0), f"near-miss/principle dead-end/HOLD memory accounting is inconsistent: {sds}")
        ...
        require((ecs.get("paper_ready"),ecs.get("paper_quality_hold"),ecs.get("paper_quality_evidence_debt"),ecs.get("canonical_live_ideas"),ecs.get("launchable_formal_experiments"),ecs.get("shadow_qualification_ready"),ecs.get("fresh_active_f0"),ecs.get("fresh_support_holds"),ecs.get("fresh_ready_problem_review"),ecs.get("method_authorized"),ecs.get("gpu_authorized")) == (1,0,0,0,0,0,1,3,0,0,0) and int(ecs.get("shadow_dead_ends") or 0)>=1 and int(ecs.get("shadow_holds") or 0)>=0, f"experiments current-status snapshot is stale: {ecs}")
