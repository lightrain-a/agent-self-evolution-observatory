...
def main() -> None:
    ...
    try:
        ...
        require(ideas["oneMinuteTemplateScenes"] == 0 and not any(token in ideas["oneMinuteReaderText"] for token in reader_jargon), f"one-minute reader layer still contains template prose or backend jargon: templates={ideas['oneMinuteTemplateScenes']} hits={[token for token in reader_jargon if token in ideas['oneMinuteReaderText']]}")
