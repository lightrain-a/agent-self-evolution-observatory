...
def main() -> None:
    ...
    try:
        ...
        require(stri.get("status")=="HOLD_ASSET_FIRST_PAPER_NOT_READY" and stri.get("submission_status")=="HOLD_PAPER_QUALITY_V2" and stri_summary.get("paper_ready")==0 and stri_summary.get("paper_quality_v2_passed")==0 and stri_summary.get("paper_quality_evidence_debt")==8 and "O-ABLATION" in (stri_summary.get("paper_quality_missing_ids") or []) and (stri_summary.get("claims_supported"),stri_summary.get("claims_total"))==(3,3) and (stri_summary.get("qa_checks_passed"),stri_summary.get("qa_checks_total"))==(59,59) and (stri_summary.get("official_qa_checks_passed"),stri_summary.get("official_qa_checks_total"))==(50,50) and (stri_summary.get("main_text_pages"),stri_summary.get("main_text_page_limit"))==(9,9) and stri_summary.get("supplement_ready")==1 and stri_summary.get("supplement_unit_tests")=="11/11 PASS" and stri_summary.get("human_signoff_pending")==1 and int(stri_summary.get("new_gpu_evidence_required") or 0)==0, f"asset-first STRI Paper Quality v2 hold projection is stale: {stri}")
        ...
        require("ASSET-FIRST ICLR 2027 · PAPER QUALITY V2" in system["text"] and "HOLD_PAPER_QUALITY_V2" in system["text"] and "evidence debt=8" in system["text"] and "59/59 PASS" in system["text"] and "50/50 PASS" in system["text"] and "main=9/9 pages" in system["text"] and "tests=11/11 PASS" in system["text"] and f"Problem-Gate PASS={int(canonical_queue_summary.get('passed_problem_gate') or 0)}" in system["text"], "asset-first STRI Paper Quality v2/canonical dual-track status is not rendered")
