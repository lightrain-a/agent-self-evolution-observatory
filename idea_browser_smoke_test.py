...
def main() -> None:
    ...
    try:
        ...
        require((ideas["shadowDesignSummary"].get("reviewed"),ideas["shadowDesignSummary"].get("advance_to_method_design"),ideas["shadowDesignSummary"].get("revise_paper_problem"),ideas["shadowDesignSummary"].get("stop_standalone")) == (2,0,1,1), f"shadow Search Portfolio design routing is stale: {ideas['shadowDesignSummary']}")
        ...
        require(ideas["shadowLatestRun"].get("run_id") == "shadow-20260814-r2" and (latest.get("raw_seeds"),latest.get("semantic_unique"),latest.get("evolved_branches"),latest.get("formulated_candidates"),latest.get("machine_reviewable"),latest.get("semantic_clear"),latest.get("current_source_blocked"),latest.get("terminal_shadow_survivors")) == (101,54,36,15,3,1,1,0), f"latest shadow r2 funnel is stale: {ideas['shadowLatestRun']}")
        require((latest.get("current_source_missing"),latest.get("live_paper_design_eligible"),ideas["shadowLatestPanels"]) == (0,0,1), f"latest shadow r2 must be complete, zero-live-authority, and rendered once: {latest}/{ideas['shadowLatestPanels']}")
        ...
        require("Shadow Search Portfolio · retrospective Paper Design audit" in ideas["text"] and "Shadow Search Portfolio r2 · current-source terminal" in ideas["text"] and "qualifying query-level units=0" in ideas["text"], "shadow Paper Design/r2 terminal/SP-15 support results are not rendered")
