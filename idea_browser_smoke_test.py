...
def main() -> None:
    ...
    try:
        ...
        if str(primary.get("schema_version") or "0") >= "1.1":
            ...
            require(all(row.get("scientific_authority") is False and len(str(row.get("primary_sha256") or ""))==64 and len(str(row.get("fulltext_sha256") or ""))==64 and all(str(value) in allowed for value in row.get("live_rescue_eligible_lanes") or []) for row in carrier.get("portable_receipts") or []), f"browser carrier receipts are invalid: {carrier.get('portable_receipts')}")
        ...
        require((ideas["shadowDesignSummary"].get("reviewed"),ideas["shadowDesignSummary"].get("advance_to_method_design"),ideas["shadowDesignSummary"].get("revise_paper_problem"),ideas["shadowDesignSummary"].get("stop_standalone"),ideas["shadowDesignSummary"].get("current_source_hard_veto_dead_ends"),ideas["shadowDesignSummary"].get("current_source_hard_veto_added_from_latest_run"),ideas["shadowDesignSummary"].get("current_source_hard_veto_inherited"),ideas["shadowDesignSummary"].get("near_miss_preflight_dead_ends"),ideas["shadowDesignSummary"].get("near_miss_support_holds"),ideas["shadowDesignSummary"].get("near_miss_current_primary_stops"),ideas["shadowDesignSummary"].get("near_miss_mature_theory_stops")) == (2,0,1,1,1,0,1,4,1,2,1), f"shadow Search Portfolio design/dead-end routing is stale: {ideas['shadowDesignSummary']}")
        ...
        require(ideas["shadowLatestRun"].get("run_id") == "shadow-20260814-r3" and (latest.get("requested_raw_seeds"),latest.get("expansion_successful_shards"),latest.get("expansion_parse_failures"),latest.get("raw_seeds"),latest.get("semantic_unique"),latest.get("evolved_branches"),latest.get("formulated_candidates"),latest.get("machine_reviewable"),latest.get("semantic_clear"),latest.get("terminal_shadow_survivors")) == (120,18,2,88,47,36,15,2,0,0), f"latest shadow r3 funnel is stale: {ideas['shadowLatestRun']}")
        require((latest.get("current_source_missing"),latest.get("current_source_reviewed"),latest.get("live_paper_design_eligible"),ideas["shadowLatestPanels"]) == (0,0,0,1), f"latest shadow r3 must require no web receipt after zero semantic CLEAR, remain zero-live-authority, and render once: {latest}/{ideas['shadowLatestPanels']}")
        ...
        require("Shadow Search Portfolio · retrospective Paper Design audit" in ideas["text"] and "Shadow Search Portfolio · latest current-source terminal" in ideas["text"] and "qualifying query-level units=0" in ideas["text"] and "current-source hard-veto memory=1" in ideas["text"] and "near-miss memory=4" in ideas["text"] and "mature-theory STOP=1" in ideas["text"], "shadow Paper Design/latest terminal/dead-end/SP-15 support results are not rendered")
