...
def main() -> None:
    ...
    try:
        ...
        require(system["chapters"] == 10 and system["readerChapters"] == 10 and system["readerPhases"] == 9 and system["deepDives"] == 4 and system["authorityCards"] == 3, f"research-system reading framework is incomplete: chapters={system['chapters']} roadmap={system['readerChapters']} phases={system['readerPhases']} deep={system['deepDives']} authority={system['authorityCards']}")
        ...
        require((selected["chapters"],selected["currentSTRI"],selected["archive"],selected["archiveOpen"],selected["acceptancePanels"],selected["acceptanceStages"]) == (5,1,1,False,1,12), f"PaperState must keep STRI current, expose the 12-stage canonical acceptance workflow, and keep the former project inside one collapsed historical archive: {selected}")
        ...
        require(acceptance.get("current_state") == "TARGETED_REPAIR" and acceptance.get("scientific_status") == "READY" and (acceptance.get("latest_story_search") or {}).get("selected_story_id") == "S1-INVARIANCE-BOUNDARY" and (acceptance.get("latest_story_search") or {}).get("pass") is True and all((acceptance.get("mock_pc_modes") or {}).get(mode) for mode in ("BLIND_MANUSCRIPT","ARTIFACT_AWARE")) and (acceptance.get("latest_claim_audit") or {}).get("pass") is False and acceptance.get("authority") == {"scientific":False,"experiment":False,"gpu":False,"submission":False} and selected["paperAcceptanceSummary"].get("invalid_ledgers") == 0, f"STRI canonical Paper Acceptance projection is stale or unsafe: {acceptance}")
        ...
        require("论文 · STRI" in selected["text"] and "非当前 PaperState · 历史归档" in selected["text"] and "CANONICAL PAPER LEDGER · 投稿闭环真值" in selected["text"] and "S1-INVARIANCE-BOUNDARY" in selected["text"] and "PDF QA 仍被硬锁" in selected["text"] and "已有投稿工件 · 不是当前终态" in selected["text"], "PaperState reader boundary or canonical acceptance workflow is missing")
