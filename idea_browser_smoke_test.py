...
def main() -> None:
    ...
    try:
        ...
        require(system["agentSafetySummary"] == 1 and system["agentSafetyStage"] == "EVIDENCE_EXECUTION_READY" and system["agentSafetyRuntimeStatus"] == "READY_RUNTIME_MODEL_ASSETS_PINNED" and system["agentSafetyBoundedEvidence"] is True and system["agentSafetyQualification"] is True and system["agentSafetyHeldoutFuture"] is False and system["agentSafetyP0"] is False and system["agentSafetyGpu"] is False, f"research-system Agent Safety qualification-only state drift: {system['agentSafetySummary']}/{system['agentSafetyStage']}/{system['agentSafetyRuntimeStatus']}/{system['agentSafetyBoundedEvidence']}/{system['agentSafetyQualification']}/{system['agentSafetyHeldoutFuture']}/{system['agentSafetyP0']}/{system['agentSafetyGpu']}")
        ...
        require(system["agentSafetyMetadata"] == "VERIFIED" and system["agentSafetyMetadataTransport"] == "GITHUB_ACTIONS_LITERAL_HF_CAPTURE" and system["agentSafetyReceiptClass"] == "NON_AUTHORITATIVE_CACHE_CONTENT_CHECK" and "240/256" in system["text"] and "QUALIFICATION" in system["text"].upper(), f"research-system Agent Safety provenance/display drift: {system['agentSafetyMetadata']}/{system['agentSafetyMetadataTransport']}/{system['agentSafetyReceiptClass']}")
        ...
        require("ASSET-FIRST ICLR 2027 · PAPER QUALITY V2.1" in system["text"] and "Paper Quality v2.1" in system["text"] and "PASS · receipt binding=PASS · content-addressed=PASS (15 files) · evidence debt=0" in system["text"] and "60/60 PASS" in system["text"] and "51/51 PASS" in system["text"] and "main=9/9 pages" in system["text"] and "tests=19/19 PASS" in system["text"] and f"Problem-Gate PASS={int(canonical_queue_summary.get('passed_problem_gate') or 0)}" in system["text"], "asset-first STRI Paper Quality v2/canonical dual-track status is not rendered")
        ...
        require(ideas["agentSafetyProgram"] == 1 and ideas["agentSafetyStage"] == system["agentSafetyStage"] and ideas["agentSafetyRuntimeStatus"] == system["agentSafetyRuntimeStatus"] and ideas["agentSafetyBoundedEvidence"] is True and ideas["agentSafetyQualification"] is True and ideas["agentSafetyHeldoutFuture"] is False and ideas["agentSafetyP0"] is False and ideas["agentSafetyGpu"] is False, f"Paper Ideas Agent Safety qualification-only state drift: {ideas['agentSafetyProgram']}/{ideas['agentSafetyStage']}/{ideas['agentSafetyRuntimeStatus']}/{ideas['agentSafetyBoundedEvidence']}/{ideas['agentSafetyQualification']}/{ideas['agentSafetyHeldoutFuture']}/{ideas['agentSafetyP0']}/{ideas['agentSafetyGpu']}")
        ...
        require(ideas["agentSafetyMetadata"] == "VERIFIED" and ideas["agentSafetyMetadataTransport"] == "GITHUB_ACTIONS_LITERAL_HF_CAPTURE" and ideas["agentSafetyReceiptClass"] == "NON_AUTHORITATIVE_CACHE_CONTENT_CHECK" and "240/256" in ideas["text"] and "QUALIFICATION" in ideas["text"].upper(), f"Paper Ideas Agent Safety provenance/display drift: {ideas['agentSafetyMetadata']}/{ideas['agentSafetyMetadataTransport']}/{ideas['agentSafetyReceiptClass']}")
        ...
        require(int(sds.get("near_miss_preflight_dead_ends") or 0) == int(sds.get("near_miss_current_primary_stops") or 0)+int(sds.get("near_miss_mature_theory_stops") or 0) and int(sds.get("near_miss_holds") or 0) == int(sds.get("near_miss_support_holds") or 0)+int(sds.get("near_miss_terminal_support_holds") or 0) and int(sds.get("shadow_closed_basins") or 0) == 40 and int(sds.get("shadow_dead_end_objects") or 0) == 1 and (int(sds.get("problem_novelty_stops") or 0),int(sds.get("execution_stops") or 0),int(sds.get("experiment_identifiability_stops") or 0),int(sds.get("optimization_stops") or 0),int(sds.get("operationalization_stops") or 0),int(sds.get("method_realization_stops") or 0),int(sds.get("assumption_scope_stops") or 0),int(sds.get("core_principle_stops") or 0),int(sds.get("broader_core_principle_falsifications") or 0)) == (4,0,2,0,3,28,2,1,0) and int(sds.get("shadow_hold_objects") or 0) == 9, f"near-miss/canonical-failure-layer/search-closure/dead-end/HOLD accounting is inconsistent: {sds}")
        closed_rows=(ideas["currentShadowSearch"].get("closed_rows") or [])
        require(ideas["closedAuditPanels"] == 1 and ideas["closedAuditRows"] == 40 and len(closed_rows) == 40, f"closed-basin per-candidate audit table is incomplete: panels={ideas['closedAuditPanels']} rows={ideas['closedAuditRows']} state={len(closed_rows)}")
        ...
        require(all(marker in ideas["text"] for marker in ("逐条查看 40 个关闭候选","负实验是否决定关闭","实验前问题/新颖性","实验可辨识性","方法实现/独立机制")), "closed-basin failure-layer explanations are not visible in the Chinese Paper Ideas view")
