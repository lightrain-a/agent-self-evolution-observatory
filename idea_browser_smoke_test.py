...
def main() -> None:
    ...
    try:
        ...
        require(ideas["p0AdmissionSummary"].get("active_p0") == 31 and ideas["p0AdmissionSummary"].get("transitioned_from_p0_ready") == 16 and ideas["p0AdmissionSummary"].get("revived_from_drop") == 7 and ideas["p0AdmissionSummary"].get("settings_complete") == 31 and ideas["p0AdmissionSummary"].get("execution_authorized") == 0, f"paper-ideas unified P0 admission state is stale: {ideas['p0AdmissionSummary']}")
        ...
        require(ideas["newGroups"] == 4 and ideas["newCards"] == 11, f"standalone/Paper-first P0 area is incomplete: {ideas['newGroups']}/{ideas['newCards']}")
        require(set(ideas["standaloneCodes"]) == {"A-6","A-7","A-8","A-9","B-8","B-9","B-10","C-7","E-3","E-4","E-5"}, f"standalone methods must have stable scientific-group codes: {ideas['standaloneCodes']}")
        ...
        require((ideas["incubationCards"],ideas["incubationP0"],ideas["incubationRevise"],ideas["incubationBlock"],ideas["incubationOpen"]) == (8,4,3,1,0), f"Paper-first new-problem rendering is wrong: {ideas['incubationCards']}/{ideas['incubationP0']}/{ideas['incubationRevise']}/{ideas['incubationBlock']}/{ideas['incubationOpen']}")
        require((ideas["incubationSummary"].get("p0_authorized"),ideas["incubationSummary"].get("gpu_authorized")) == (4,0), f"four Paper-first problems should be in P0 lifecycle with zero direct GPU authority: {ideas['incubationSummary']}")
        ...
        require("Human terminal ledger" in ideas["text"] and ideas["newCards"] == 11 and ideas["absorbedChildCount"] == 17, "terminal/current idea summary or standalone-method rendering is missing")
        ...
        require((experiments["terminalPortfolio"],experiments["terminalRows"],experiments["terminalP0"],experiments["terminalP0Ready"]) == (1,31,31,0), f"terminal experiment portfolio is not aligned with Paper Ideas: {experiments}")
        require((experiments["terminalStarted"],experiments["terminalPending"],experiments["auditQueue"],experiments["auditItems"]) == (27,4,0,0), f"started/pending audit split is wrong: {experiments}")
        ...
        require(experiments["decisionLedgerSummary"].get("active_p0") == 31 and experiments["decisionLedgerSummary"].get("experiment_stopped") == 24 and experiments["decisionLedgerSummary"].get("upstream_hold") == 3 and experiments["decisionLedgerSummary"].get("economy_blocked") == 0 and experiments["decisionLedgerSummary"].get("method_admission_blocked") == 2 and experiments["decisionLedgerSummary"].get("launchable") == 0, f"unified Decision Ledger must cover 31 P0 with two Support HOLDs, two method-admission-blocked promotions, and 0 launchable: {experiments['decisionLedgerSummary']}")
