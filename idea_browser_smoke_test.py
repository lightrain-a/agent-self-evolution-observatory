...
def main() -> None:
    ...
    try:
        ...
        require((experiments["terminalPortfolio"],experiments["terminalRows"],experiments["terminalP0"],experiments["terminalP0Ready"]) == (1,27,27,0), f"terminal experiment portfolio is not aligned with Paper Ideas: {experiments}")
        require((experiments["terminalStarted"],experiments["terminalPending"],experiments["auditQueue"],experiments["auditItems"]) == (24,3,0,0), f"started/pending audit split is wrong: {experiments}")
        require(experiments["batchPanel"] == 1 and (experiments["batchSummary"].get("parent_p0"),experiments["batchSummary"].get("reused_existing_p0"),experiments["batchSummary"].get("fresh_cpu_f0"),experiments["batchSummary"].get("fresh_matched_simplification_stop"),experiments["batchSummary"].get("fresh_upstream_hold"),experiments["batchSummary"].get("gpu_queue_candidates_before_economy")) == (20,13,7,4,3,0), f"20-Idea P0 batch is missing or stale: {experiments['batchSummary']}")
        ...
        require(experiments["decisionLedgerSummary"].get("active_p0") == 27 and experiments["decisionLedgerSummary"].get("experiment_stopped") == 21 and experiments["decisionLedgerSummary"].get("upstream_hold") == 4 and experiments["decisionLedgerSummary"].get("economy_blocked") == 0 and experiments["decisionLedgerSummary"].get("launchable") == 0, f"unified Decision Ledger must cover 27 P0 with 0 launchable: {experiments['decisionLedgerSummary']}")
