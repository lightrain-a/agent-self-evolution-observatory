...
def main() -> None:
    ...
    try:
        ...
        require(stri.get("status")=="READY_NARROW_ICLR" and stri.get("submission_status")=="READY_TO_SUBMIT_PENDING_HUMAN_AUTHOR_SIGNOFF_AND_OPENREVIEW" and stri_summary.get("paper_ready")==1 and stri_summary.get("paper_quality_v2_passed")==1 and stri_summary.get("paper_quality_evidence_debt")==0 and stri_summary.get("paper_quality_main_visualizations")==4 and "failure" in (stri_summary.get("paper_quality_main_visual_roles") or []) and (stri_summary.get("paper_quality_missing_ids") or [])==[] and (stri_summary.get("claims_supported"),stri_summary.get("claims_total"))==(3,3) and (stri_summary.get("qa_checks_passed"),stri_summary.get("qa_checks_total"))==(60,60) and (stri_summary.get("official_qa_checks_passed"),stri_summary.get("official_qa_checks_total"))==(50,50) and (stri_summary.get("main_text_pages"),stri_summary.get("main_text_page_limit"))==(9,9) and stri_summary.get("supplement_ready")==1 and stri_summary.get("supplement_unit_tests")=="13/13 PASS" and stri_summary.get("human_signoff_pending")==1 and int(stri_summary.get("new_gpu_evidence_required") or 0)==0, f"asset-first STRI Paper Quality v2 ready projection is stale: {stri}")
        ...
        require("ASSET-FIRST ICLR 2027 · PAPER QUALITY V2.1" in system["text"] and "Paper Quality v2.1" in system["text"] and "PASS · receipt binding=PASS · evidence debt=0" in system["text"] and "60/60 PASS" in system["text"] and "50/50 PASS" in system["text"] and "main=9/9 pages" in system["text"] and "tests=13/13 PASS" in system["text"] and f"Problem-Gate PASS={int(canonical_queue_summary.get('passed_problem_gate') or 0)}" in system["text"], "asset-first STRI Paper Quality v2/canonical dual-track status is not rendered")
        ...
        require((primary.get("policy") or {}).get("scientific_object_lanes") == ["skill_harness","memory_continual","knowledge_retrieval_state","world_model","parametric_model_state"] and (primary.get("policy") or {}).get("source_coverage_exploration_prefers_scientific_objects") is True and (primary.get("policy") or {}).get("context_and_property_tags_have_zero_scientific_authority") is True, f"browser scientific-object policy is stale: {primary.get('policy')}")
        ...
        require(shadow_system_admission.get("scientific_authority") is False and shadow_system_summary.get("same_source_transaction") is True and int(shadow_system_summary.get("automatic_provider_calls_authorized") or 0)==0 and "SHADOW RUN ADMISSION" in system["text"], f"system overview shadow-run admission authority/accounting is invalid: {shadow_system_admission}")
        if shadow_system_summary.get("operator_upgrade_recompile") is True:
            require(shadow_system_summary.get("qualification_allowed") is True and str(shadow_system_admission.get("status") or "").startswith("READY_FOR_"), f"operator upgrade must allow only a new qualification transaction: {shadow_system_admission}")
        else:
            require(shadow_system_admission.get("status") == "SKIPPED_SHADOW_SOURCE_TRANSACTION_ALREADY_TERMINAL" and shadow_system_summary.get("qualification_allowed") is False, f"same-source terminal shadow without an operator-upgrade receipt must zero-call skip: {shadow_system_admission}")
        ...
        require(int(sds.get("semantic_blocker_dead_ends") or 0)+int(sds.get("semantic_hold_objects") or 0) == 0 and int(sds.get("semantic_hold_objects") or 0) == int(sds.get("semantic_hold_added_from_latest_run") or 0)+int(sds.get("semantic_hold_added_from_terminal_run") or 0)+int(sds.get("semantic_hold_inherited") or 0), f"semantic dead-end/HOLD memory accounting is inconsistent: {sds}")
        require(int(sds.get("near_miss_preflight_dead_ends") or 0)+int(sds.get("near_miss_holds") or 0) == 4 and (sds.get("near_miss_support_holds"),sds.get("near_miss_terminal_support_holds"),sds.get("near_miss_current_primary_stops"),sds.get("near_miss_mature_theory_stops")) == (1,0,2,1) and (sds.get("shadow_dead_end_objects"),sds.get("principle_readjudication_dead_ends"),sds.get("shadow_hold_objects")) == (12,8,2), f"near-miss/principle dead-end/HOLD memory accounting is inconsistent: {sds}")
        ...
        require(shadow_admission.get("scientific_authority") is False and shadow_admission_summary.get("canonical_transaction_closed") is True and shadow_admission_summary.get("same_source_transaction") is True and int(shadow_admission_summary.get("automatic_provider_calls_authorized") or 0)==0 and ideas["shadowAdmissionPanels"] == 1, f"shadow next-run admission authority/accounting is invalid: {shadow_admission}/{ideas['shadowAdmissionPanels']}")
        if shadow_admission_summary.get("operator_upgrade_recompile") is True:
            require(shadow_admission_summary.get("qualification_allowed") is True and str(shadow_admission.get("status") or "").startswith("READY_FOR_"), f"operator upgrade should reopen only qualification: {shadow_admission}")
        else:
        ...
        require((ecs.get("paper_ready"),ecs.get("paper_quality_hold"),ecs.get("paper_quality_evidence_debt"),ecs.get("canonical_live_ideas"),ecs.get("launchable_formal_experiments"),ecs.get("shadow_dead_ends"),ecs.get("shadow_holds"),ecs.get("shadow_qualification_ready")) == (1,0,0,0,0,12,2,0), f"experiments current-status snapshot is stale: {ecs}")
        require((ep.get("paper_id"),ep.get("status"),ep.get("submission_status"),ep.get("claims_supported"),ep.get("claims_total"),ep.get("paper_quality_v2_passed"),ep.get("paper_quality_evidence_debt"),ep.get("new_gpu_evidence_required")) == ("STRI","READY_NARROW_ICLR","READY_TO_SUBMIT_PENDING_HUMAN_AUTHOR_SIGNOFF_AND_OPENREVIEW",3,3,True,0,False), f"STRI paper-quality projection is stale: {ep}")
